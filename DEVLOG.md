...
call, it finds all `[data-countup]` elements, checks if their current `textContent` differs from
`data-countup-last` (the last-animated value), and if so runs an rAF ease-out-cubic tween from 0 to
the target number over `--wonder-dur-slow` (read from the CSS custom property at call time). The
final frame always restores the exact original formatted string so currency symbols, separators, and
accounting parentheses are pixel-perfect. Double-guarded: exits early if `--wonder-on` is 0 or
`prefers-reduced-motion: reduce` matches. KPI `fig` divs in `dashboard.go` carry `data-countup=""`
so they are picked up automatically on each render.

**W-18 (chart draw-in):** Pure CSS using `pathLength="1"` on the SVG `<path>` — normalises dash
coordinate space to 0..1 regardless of actual path length. The line path gets `stroke-dasharray: 1`
and `@keyframes wonder-chart-draw` animates `stroke-dashoffset` from 1 to 0 (left-to-right draw-in).
The area fill gets `@keyframes wonder-chart-fade` (opacity 0 to 1). Both gated by
`[data-wonder="off"]` (animation:none) and `prefers-reduced-motion: reduce` (same). D3 bars and
donut arcs deferred (GO-STRUCTURAL — those render inside chart.js outside the Go SVG path).

**sw.js:** Bumped cache to `cashflux-v249`, added `./countup.js` to CORE list for offline caching.


## 2026-06-23 â€” feat: WONDER W-9 page-enter + W-21 scroll-reveal (fail-safe, reduced-motion safe)
...
calls `triggerScrollReveal()` â†’ `cashfluxWonder.observe()`. The JS stamps `.wonder-reveal` on
...
reveals all immediately or no class is ever added â€” content stays visible. CSS covers it twice:
...
## 2026-06-23 â€” feat: GLAMOR quick-wins + GX1 shell fixes + Widget Builder build-fix

Cleared the design-system tail of the GLAMOR backlog. Landed all 10 CSS-only Quick-wins
(QW-1..10) and the nine GX1 app-shell fixes (F1â€“F9) as one consolidated last-wins style block
appended to `web/index.html` â€” radius/weight/semantic-color/spacing polish plus the critical
...
never defined and `uiw` was imported-unused. Finished the publishâ†’dashboard path â€” added the
...
Next: GLAMOR TIER 1â€“3 cross-cutting items and the deep-analysis Â§1â€“10 / R-redesign backlog.

## 2026-06-23 â€” feat: WONDER W-11..W-20 â€” entrance/reveal/polish flourishes (token-driven, reduced-motion safe)

Batch 2 of the WONDER series â€” all CSS-only, no JS.

**W-11 List stagger:** `@keyframes wonder-row-enter` (opacity 0â†’1 + `translateY(6px * --wonder-on)`) applied to `.rows .row` / `.list-rows .row`; `:nth-child(1..8)` get increasing delays (0..210ms Ã— `--wonder-on`) capping total cascade at ~320ms; items 9+ enter together. Table rows excluded (`.txn-table .row`) to preserve column alignment.

**W-12 Bento entrance:** `@keyframes wonder-bento-enter` (scale `1 - 0.02 * --wonder-on` â†’ 1, opacity 0â†’1) on `.bento .w`; plays once per page load over `--wonder-dur-slow`.

**W-13 Modal backdrop blur:** `.flip-backdrop` now transitions `backdrop-filter` (blur 0 â†’ `3px * --wonder-on`) alongside the existing opacity, giving a clean blur-in as the settings panel opens. The existing `transition:none` in the reduce block covers it.

**W-14 Toast spring:** redefined `toast-in` with a mid-frame bounce (-0.15rem Ã— `--wonder-on`) and spring cubic-bezier `(.34,1.56,.64,1)`; added `.toast.hide` class for smooth exit (opacity+transform over `--wonder-dur-fast`). Duration uses `--wonder-dur-slow` so it's 0ms when off.

**W-16 Progress-bar ease:** verified pass-through â€” all bar colour variants share `.bar-fill` which already carries `width 0.45s cubic-bezier(.2,.75,.2,1)` under `no-preference`; no new CSS needed.

**W-19 Skeleton shimmer:** `.skeleton` + `.shimmer` reusable classes; `@keyframes wonder-shimmer` sweeps a 3-stop gradient at `4 Ã— --wonder-dur-slow` (â‰ˆ1.2s). Under `[data-wonder="off"]` and `prefers-reduced-motion: reduce` â†’ static `--bg-elev` block, no animation.
...
## 2026-06-23 â€” feat: WONDER W-3..W-8 â€” interaction-feedback flourishes (token-driven, reduced-motion safe)
...
## 2026-06-23 â€” C74 final 3 sub-bullets complete
...
â†’ `draft.Set` â€” same pipeline as image import. Gated on key (sets `needsKey` like image path). Button rendered
...
**e2e:** `e2e/c74_ai_extract_check.mjs` gates UI affordances â€” both buttons present, no crash on key-absent
click, deterministic categorization path runs clean.

## 2026-06-23 â€” refactor: C73 Phase 2 COMPLETE â€” every screen card on the primitive

Finished the C73 component-ization epic: zero hand-rolled Section(css.Class("card")) remain in internal/screens â€” every card renders through Card/EntityListSection. Drove raw scaffolds 68 -> 38. Unlock was extending the primitive (Header slot for verbatim bespoke headers â€” H3 titles, flex headers, .card-head/.budget-head, card-alert banner, receipt overlay; Rows slot wrapping Div(.rows); TestID; ClassParts merging extra classes into ONE css.Class â€” which caught a latent bug where a second css.Class prop overrides the first, silently dropping the base class). Ported screen-by-screen with build + per-screen gates; every timing-out gate confirmed to fail identically at HEAD (pre-existing +Add-menu + bulk-delete). Phase-5 ratchet rewritten: TestNoBespokeCardScaffold is a hard zero invariant, TestRowsContainerRatchet caps the 38 Div(.rows) list containers. C73 fully checked.

## 2026-06-23 â€” feat: G9.1a Reports charts â€” ranked bars + donuts from existing aggregates
...
`chartspec.Spec`. The donut helper uses a single series with one point per slice â€” matching the
...
## 2026-06-23 â€” refactor: C73 Phase 3 row extraction + honest phase reconciliation

Extracted `AccountRow` (455 lines), `BudgetRow`, and `GoalRow` each into a self-contained `*_row.go`,
mirroring the earlier `transactions_row.go` split â€” pure relocation via `sed` line-range extraction
(no hand-retyping), imports trimmed by usage-grep + compiler, gofmt'd. Build clean, `go test ./...`
green, and every per-screen gate (accounts Ã—9, budgets, goals Ã—5+) passes. The two "failing" goal
...
(lists â†’ DataTable/EntityListSection) is the sole remainder** and is deliberately left as
ratchet-governed incremental work, NOT fake-ticked: the primitives + the Phase-5 scaffold ratchet
(baseline 165) are in place, but converting the ~117 card / 48 list sites is not a mechanical sweep â€”
...
## 2026-06-23 â€” refactor: C73 super-screen decomposition + primitive completion (6-agent wave)
...
Bâ€“F each decomposed one super-screen (Customize, Documents, Planning, Allocate, Settings) plus the
Transactions row. **Decomposition rule that kept it safe:** every agent left all `UseState`/`UseEvent` hooks
in the parent shell and made the extracted sub-components *hook-free*, receiving state as props â€” so hook
...
clean â€” the agents' *final* states compose. Removed a stray orphan worktree (`agent-af0bceâ€¦`) that was
...
"Keep back" placeholder that's been "Emergency buffer (%s)" for a while. `bulk_ops` (L25) fails â€” but it
...
**Still open in C73 (next wave):** the inline-`Style{}`â†’utility-class sweep (39Ã—), splitting
AccountRow/BudgetRow/GoalRow into Display+Edit (TransactionRow done), full Phase-1 form coverage on the
remaining screens, Phase 2 (listsâ†’DataTable/EntityListSection), and Phase 5 (cleanup + a lint banning raw
`Div(.rows)`/`Section(.card)` in screens).

## 2026-06-23 â€” feat: GLAMOR G9.1 Reports redesign â€” hero zone, type hierarchy, heads-up alert, Sankey-up, advanced collapse

Six G9.1 items shipped in one pass. **Hero zone (Item 1):** promoted Net / Income / Spend out of the flat stat-grid into a dedicated `.reports-hero` strip above all cards â€” Net at 2.5rem/800, Income+Spend flankers at 1.75rem/700, savings rate / runway / no-spend days in a secondary row below a hairline divider. The old `stat-grid` with six equal-weight tiles is gone; the headline number now lands first. **Card-title weight (Item 2):** added `font-weight: 600` to `.card-title` â€” one CSS change, lifts every card app-wide. **Alert urgency (Item 3):** new `.card-alert` class (`border-left: 4px solid var(--danger)` + 6% danger tint via `color-mix`) applied to the "Heads up" anomaly card so it reads as an urgent callout, not a passive list. **Tabular amounts (Item 4):** `.budget-amount` already had `font-variant-numeric: tabular-nums`; switched color to `var(--fg, var(--text))` so amounts are full-contrast rather than dim. **Sankey up (Item 5):** reordered the return block â€” Sankey moved from the Income section to directly after the Spending-by-category card, giving the group: category â†’ Sankey â†’ top payees â†’ biggest expenses. Income section (biggest deposits + income by source) follows. **Advanced collapse (Item 6):** wrapped "Custom field spend" and "Deductible totals" cards in a `ui.UseState`-powered disclosure toggle ("Advanced â–¾/â–²"), collapsed by default. The deductible card compiled cleanly so both cards are included. Build: `GOOS=js GOARCH=wasm go build` â€” exit 0, no errors.

## 2026-06-23 â€” feat: GLAMOR GM4 palette/gear â€” a11y, keyboard hints, entity cap, backdrop close
...
**Palette (shortcuts.go):** Added `role="dialog"` + `aria-modal` + `aria-label` to the card div (GM4-1), `role="listbox"` on `#cf-cmd-list` with live `aria-selected` per row (GM4-2). Added keyboard-hint footer `â†‘â†“ navigate Â· â†µ select Â· Esc close` to `buildCommandPalette` (GM4-11). Capped `entityJumpCommands` at 8 via `entityJumpMaxUnfiltered` const â€” the unfiltered list was 58+ rows with 10 accounts; typing still fuzzy-matches all entities (GM4-12). Also fixed a latent bug in `movePaletteSel`: the code compared `i == cmdPaletteSel` against the DOM child index but group-header `<div>` elements were interleaved, so the index was wrong whenever headers were present. Introduced a separate `rowPos` counter that only increments on `[data-cmd-row]` children.

**Gear/FlipPanel (flippanel.go):** Close (Ã—) button given `tabindex="-1"` so `focusables()` in UseEffect skips it and focus lands on the first toggle/input (GM4-17). Backdrop click-to-close added via a second `js.FuncOf` click listener on `document` in the same UseEffect, checking `event.target == .flip-backdrop`; listener is released in the cleanup func (GM4-19).
...
**Deferred:** GM4-4 (768 card width), GM4-14 (number-input label wrap), GM4-15 (post-save toast), GM4-18 (mutual-exclusion), GM4-20 (focus-visible ring) â€” low priority or requires separate probing setup.

## 2026-06-23 â€” feat: GLAMOR GM3 confirm dialogs â€” bulk-delete gate, safe focus, aria
...
**D1 â€” Bulk-delete had no confirmation (CRITICAL / L50):** `bulkDelete` in `screens/transactions.go`
previously executed immediately after "Delete selected" â€” no dialog, no count, one mis-click nukes
...
**D2 â€” Wrong default focus on destructive confirms:** All 4 audit runs showed `focusedId:
"cf-dialog-confirm"` â€” the red danger button was receiving default focus. Added a third branch to
...
**D3 â€” No title on destructive confirm dialogs:** `ConfirmModal` has no `title` parameter (no
...
**D4 â€” No `aria-labelledby`:** `<h3>` gets `id="cf-dialog-title"`, backdrop gets
...
**D7 â€” Cramped dialog at 110px:** CSS override block updated with `padding:1.5rem 1.5rem 1.25rem`
...
## 2026-06-23 - feat: GLAMOR GM1 Settings modal â€” 768px collapse, h4 headings, password aria-labels
...
**S4 â€” 768px grid collapse (was CRITICAL):** The existing media query `@media (max-width:768px)`
targeted `.flip-wrap .grid-cols-2` as a class, but `tw.GridCols2` in the Go tw package emits an
inline `style="grid-template-columns: repeat(2, â€¦)"` on the element â€” no class name. Fixed selector
...
**S10 â€” Heading hierarchy:** All 22 `set-label` section headers in settings.go changed from `<div>`
...
**S11 â€” Password aria-labels:** The AI key input was using the section heading ("AI (OpenAI Â· bring
your own key)") as its aria-label â€” usable but imprecise. Both the AI key and web-search key inputs
now use their placeholder text as the aria-label ("OpenAI API key (sk-â€¦)" and "Web search API key
...
this change â€” it exists in both the worktree and main at the same commit.

## 2026-06-23 - feat: GLAMOR GI2 Categories â€” map-first, clickable usage, nesting cue, dim zero-usage, sort-by-usage

GI2 polish pass on the Categories screen. Five items shipped (one was already done in G17):
(1) Category map moved first â€” the Mermaid hierarchy card now leads the page instead of being buried
below 22+ expense rows; users see the tree structure immediately on arrival. (2) Usage count afforded
as a link â€” added `.btn-link` CSS class (`color: var(--accent); text-decoration: underline;
cursor: pointer`) so "N transactions" reads as a drill-through, not inert metadata. (3) Sub-category
nesting cue â€” `.cat-child-row` class on rows with `Depth > 0` adds a subtle background fill
...
â€” `.cat-zero-usage` (opacity 0.55) applied to rows with `TxnCount == 0` so safe-to-delete categories
stand out during cleanup audits. (5) Sort-by-usage toggle â€” `sortByUsage ui.UseState(false)` +
...
## 2026-06-23 - feat: GLAMOR GI1 Rules â€” collapse suggestions, demote add buttons, fit-content save, drag hint

GI1 polish pass on the Rules screen. Four items shipped: (1) Suggested rules now caps at 5 visible rows
with a `showAllSuggestions` `ui.UseState(false)` + "Show all (N)" / "Show fewer" toggle button â€” avoids
...
width instead of stretching full card width. (4) Drag hint "Drag â ¿ to reorder â€” first match wins" shown
as a `.muted` `<p>` under the heading when â‰¥2 rules exist â€” makes the C64 grips readable as interactive.
...
## 2026-06-23 - feat: C74 Tier 2/3 â€” OFX parser, import mapping, categorizer, ParseAny

Three new pure-logic packages land the Tier 2/3 import engine infrastructure.

**`internal/ofx`** â€” The key design decision was dual-path parsing: OFX 1.x is SGML (not XML), so `encoding/xml` would choke on it. The SGML path is a simple line scanner that tracks `<STMTTRN>` / `</STMTTRN>` aggregate boundaries and harvests leaf values. OFX 2.x goes through `encoding/xml` with a struct tree covering both `BANKMSGSRSV1` and `CREDITCARDMSGSRSV1` message sets. Date parsing strips the `[tz:NAME]` annotation before trying `20060102150405` then `20060102` as UTC. Amount is `ParseFloat â†’ Round(f * 10^decimals)`.

**`internal/importmap`** â€” `Profile` captures the full column mapping for a given bank's CSV export format, plus `SkipRows`, `DateLayout`, and `Decimals`. `Apply` is deliberately lenient: bad-date or bad-amount rows are silently skipped so a partially-bad file still imports cleanly. `DefaultProfile` does case-insensitive header-name matching â€” handles the common "Date", "Transaction Date", "Posted Date" variants. The debit/credit split path subtracts debit and adds credit, treating both as positive raw values.

**`internal/statement` extensions** â€” `Category string` added to `Row` with zero cost to existing consumers (zero value = uncategorized). `DefaultCategorizer` is a keyword-table scan (O(n Ã— k), fine for statement sizes). `ParseAny` peeks at the byte prefix after stripping BOM; the four OFX-prefix strings cover all realistic real-world variants, everything else falls through to the existing CSV `Parse`.

No external packages used. All 20 tests pass; `go vet` clean.

## 2026-06-23 - feat: GLAMOR series wave 4 â€” G15 Customize â€¦ G19 Workflows

Cam kept adding glamor tickets (G15â€“G19) as I worked. Pattern across this wave: the deepest light-mode
issues are all variations of one root cause I'd already cracked in G9/G14 â€” legacy components reading
...
card; G19 fixed the one real non-systemic item â€” the primary button's dark-green-on-green text failed
...
"black band" background bleed in light mode â€” the shell/bento read tokens the theme engine doesn't
switch; that's a dedicated shell pass, and Cam's actively in the shell. (2) The general accent-button
contrast for arbitrary custom accents â€” the theme engine should compute contrast-aware button text;
I only nudged the default-accent light case. Series now spans G1â€“G19, all reviewed.

## 2026-06-23 - feat: GLAMOR series wave 3 â€” G12 Split, G13 Insights, G14 Documents

Cam added five more glamor tickets (G12â€“G14 plus the earlier G7â€“G11). Worked through G12â€“G14.

G12 Split and G13 Insights were mostly already-working pages â€” their CRITICAL light-mode title issues
...
*above* the statement card that produced it â€” a simple render-order move of draftBody/summaryBody
...
a `[data-theme="light"] .amount:not(.amount-income):not(.amount-expense)` pin â€” the `:not()` scoping
...
dark nav rail in light mode) are *shell-level* â€” the theme engine emits `--bg-base`/`--bg-card` but
...
That brings the GLAMOR per-page review series to G1â€“G14, all reviewed and the high-value/CRITICAL
items shipped.

## 2026-06-23 - feat: GLAMOR series wave 2 â€” G11 Bills (series G1â€“G11 complete)
...
## 2026-06-23 - feat: GLAMOR series wave 2 â€” G10 Subscriptions "The Subscription Audit"

The audit flagged a genuinely-broken CRITICAL: subscription names invisible at 1280/1440 (visible only
at 768 after the row reflows). Root cause was `.row-main { min-width: 0 }` â€” it lets the name column
...
## 2026-06-23 - feat: GLAMOR series wave 2 â€” G9 Reports + the real light-mode root cause
...
override â€” so a legacy component reading `var(--text)` keeps the dark value even with the light
palette active, while the light palette's `--bg-card: #fff` (applied via a different path) does take.
White text on white card.

Fix: stop routing those through the var in light mode â€” pin the `color` *directly* under
...
This is the definitive fix and retroactively repairs G4â€“G8 light mode too.
...
## 2026-06-23 - feat: GLAMOR series wave 2 â€” G8 Allocate "Every Dollar a Job"
...
reliability) so the common path is profile â†’ amount â†’ list. Labelled the two unlabelled selects,
...
## 2026-06-23 - feat: GLAMOR series wave 2 â€” G7 Planning "The What-If Sunday"

Cam extended the glamor sweep to G7â€“G11 (Planning, Allocate, Reports, Subscriptions, Bills). Starting
G7. Two genuinely high-value fixes: a headline projection stat (the forecast card had Dev's primary
answer â€” projected 12-month net worth â€” buried in a muted hint sentence; promoted it to a display
stat strip), and calendar X-axis labels (the chart read "0Â·2Â·4Â·6"; chart.js already renders
`Point.Label` as X ticks when no `x.format` is set, so attaching "Jul 2026"-style labels to the
baseline points was a clean one-loop fix â€” no renderer change).
...
## 2026-06-23 - feat: GLAMOR series â€” G6 To-do "The Money To-Do List" (series complete)

Final screen. Small, contained pass: a header "+ Add task" button (the global "+" was ~700px from
the list), a "N open Â· N overdue Â· N done" summary strip so the To-do page opens with portfolio
...
Note: couldn't run a full wasm build to verify at commit time â€” there were concurrent in-progress
...
the G5/G4 commits that built cleanly, so they're committed independently. The series (G1â€“G6) is done.

## 2026-06-23 - feat: GLAMOR series â€” G1 Dashboard "The 7am Glance"

Auditing G1, several "defects" were already fixed in the code since the screenshots were taken: the
attention chips already carry severity classes + a colored left-border + â–²/â—/â—‹ glyph (the triage
...
"Start fresh" and "Dismiss" rendered as "Start freshDismiss" â€” added a styled banner. Added a signed
cash-flow sub-line to the Income tile (income âˆ’ spending), and made the net-worth trend sub-line
...
enlarged hero figure â€” reflowing the packed default risks overlaps and test breakage for marginal
glance benefit. Logged the reasoning in TODOS rather than silently skipping.

## 2026-06-23 - feat: GLAMOR series â€” G2 Transactions "The Reconciler"
...
categorize like this", "Attach receipt"). Collapsed them to an icon strip (`.btn-icon`, label â†’
title/aria) â€” that single change reclaims most of the width and lets Description breathe.
...
inline-edit row since the Tags column is now conditional â€” it only renders when something on the
page is actually tagged (otherwise tags fold into a `#chip` on the Description cell). Added zebra +
themed hover, a real âœ“/â—‹ cleared toggle (cleared rows dim), tighter row padding + single-line
...
## 2026-06-23 - feat: GLAMOR series â€” G3 Accounts "The Net-Worth Check"
...
proxy (income âˆ’ expense ignores transfers and debt paydown), I used `ledger.NetWorthSeries` with
cutoffs [first-of-month, tomorrow] and subtracted the two snapshots â€” that's the *actual* net-worth
change, transfers and liability movements included. Rendered as a colored â†‘/â†“ caption.
...
"Update balance" on stale rows, and a `.row` flex-wrap at â‰¤760px (shared across list screens) so long
names stop colliding with buttons.

Next: G2 Transactions â€” the dense one (Actions column eats 44% of width, no zebra, amount buried).

## 2026-06-23 - feat: GLAMOR series â€” G4 Budgets "The Mid-Month Pulse"

Second glamor screen. Headline gap was ordering: budget rows came out in insertion order, so Renu
scanned past healthy budgets to reach the over-budget ones. Added a health-first stable sort (Over â†’
Near/At-risk â†’ On-track, then %-used desc) where "At risk" (the pace projection flags a trending
...
that **had no CSS at all** â€” they rendered as bare colored text, which is exactly what the audit
...
## 2026-06-23 - feat: GLAMOR series â€” G5 Goals "Are We There Yet?"
...
Key decision â€” **pace is honestly underivable** here. Goals carry only a target date + current/target
amounts; there is no start date or contribution history, so a true on-pace/behind calc would be a
fabrication. So I classified pace by what the app *does* know â€” completion, percent, and the target
date vs now â€” into `goals.ClassifyPace`: complete â†’ overdue â†’ final-stretch (â‰¥90%) â†’ due-soon (â‰¤60d)
â†’ on-track, with undated goals having no pace. Put it (and `LessForList` for the new sort) in the
...
Next: G4 Budgets (same shape â€” sort by health, near-limit pill, row separators, contrast).

## 2026-06-22 - fix: palette/keyboard direct actions crashed the whole app

Cam asked me to e2e the Ctrl+K toggle AND the direct actions â€” and that immediately surfaced a serious latent
...
these "direct actions" â€” from the palette OR the keyboard â€” has been killing the app, undetected, because no
test ever executed them. Classic gap: the happy-path nav commands route via the router (no hook) and worked, so
it looked fine.

Fix: gave QuickAdd, RailCollapsed, and Prefs the same captured-atom seam the toast notice already uses â€” the
...
gaps were already shipped â€” verified via `palette_fuzzy_check`). The dream-big gap was making the user's own
data searchable: added `entityJumpCommands()` that appends one palette command per non-archived account / goal
/ budget ("<name> Â· Account/Goal/Budget", with the type word as a keyword) routing to that screen. Boot-safe
(nil app â†’ none). Known limitation: the palette builds its command list once on first open, so entities added
later need a reopen/reload to appear â€” acceptable, matches existing behavior. e2e `palette_entities_check.mjs`
(polls for the seeded account, searches it, Enter â†’ /accounts).

Deferring the L11 mobile/responsive pass â€” tap-target sizing, a real mobile nav pattern, touch affordances â€”
as a dedicated effort: it's a broad visual change that needs real device/viewport review rather than headless
e2e, so it's the wrong thing to grind blind at the tail of this run. Everything else across L1â€“L30 is now
...
resolution â€” pure, just `yearStart` + the Truncate/Step/Label arms, table-tested (and flipped the old test that
asserted "year" was invalid). Wired "Year" into the top-bar segmented control, and added `/reports` to the
period-aware route set so the resolution control actually appears there (it wasn't â€” Reports read the shared
...
probes â€” reload picks up the rebuild.
...
silent. Added a top-bar "Offline" pill (hidden when online) backed by a shared `uistate` online atom â€” same
captured-atom seam as dialog/notice â€” kept in sync by `wireOnlineStatus()` (boot): seeds from `navigator.onLine`
...
also found several "gaps" already shipped â€” the account-currency control is already a validated `<select>` (not
...
row action (`pickFile("image/*")` â†’ `PutArtifact` â†’ append `AttachmentRef` â†’ `PutTransaction`), a paperclip
...
attachment ref already round-trip in the dataset export/import, so backup survival was free â€” locked it in with
`store.TestAttachmentRoundTrip` (PNG bytes + ref survive exportâ†’import).

Process note: this one's agent **timed out mid-stream** (API stream idle timeout) after only adding the
Paperclip icon + i18n keys â€” the Go UI/store/e2e weren't done. Rather than re-dispatch (risking duplicate
...
addInitScript pattern and filters the ledger to that txn (so its row is on page 1), then asserts marker â†’
preview img â†’ artifacts linkage. Builds + all regressions green. This was the last fully-new story; remaining
...
affected rows' prior full state into a `lastBulk` UseState *before* mutating, and an inline "<Op> N Â· Undo"
banner restores them via new `appstate.RestoreTransactions` â€” which just upserts each snapshot, so a deleted row
...
Integration: the agent's two gates both failed initially â€” not feature bugs. bulk_ops waited for the bulk
...
changed" was unprovable from the DOM. Added `data-id` (the txn ID) to the row â€” a small, justified testability
win â€” then rewrote both gates to map selected rows â†’ IDs precisely and poll with autosave flush. Now bulk_ops
proves exactly-the-selected for recategorize and delete, and bulk_undo proves deleteâ†’restore round-trips the
same IDs (604â†’602â†’604). Sub-agent built the feature; integrated + regressed (add/bulk-clear/filter green).
Next: L29.
...
`txn_abs > 200`, `ActionFlagReview`). So the genuine net-new work was two things. (1) A CI gate â€” `rules_check.mjs`
â€” for the core create-rule â†’ matching-txn â†’ auto-file â†’ reload round-trip that nothing covered (wrote it myself).
...
localStorage once right after submit (no autosave flush) and looked at slice(0,5) â€” so it intermittently missed
the rule. A probe proved the feature works (count 5â†’6, "Northside Goods" at index 0); fixed the gate to poll with
...
TODOS had the Logic and State/UI sub-items checked, but a grep proved none of it existed â€” the marks were
...
`appstate.MarkSubscriptionCancelled/Unmark/Cancellations` (dedup by name); and the UI â€” a per-row "Mark as
cancelled" + Undo, and a top `role="alert"` banner listing late charges (the headline value).

e2e notes: the agent's first gate imported `@playwright/test` (wrong â€” this repo drives raw `playwright` via
createRequire off `.tools/package.json`) and injected the cancellation via a plain localStorage write before a
reload, which the pagehideâ†’autosave clobbers. Rewrote it to the project pattern + the one-shot addInitScript
injection (document-start, post-pagehide). Subtlety worth recording: the UI "Mark as cancelled" stamps *today*,
and the sample's last gym charge is Jun 3 (past), so a UI-driven cancel can't itself produce a late charge â€” the
...
"Mark as cancelled" â€” select rows by the sub name. Sub-agent built; integrated + regressed (drill/price-tone
green). Next: L15.

## 2026-06-22 - feat: runway indicator on what-if plans (L27)

The what-if planner projected a sabbatical to âˆ’$25,500 at 12 months but never stated the number that matters:
when does the money run out? Added pure `planning.RunwayMonths(p) (months float64, depletes bool)` â€” walks the
existing `Project` curve, finds the first month the balance goes negative, and linearly interpolates the
fractional crossing (prev/(prevâˆ’cur)); one-time dips land at month boundaries so mid-month interpolation across
one is approximate but consistent with the displayed curve (documented). PlanCard now shows "Money lasts ~5.6
months" + a âš  marker in the danger tone, or a calm "Stays positive through N months". L13's cash runway card
already existed; this is the per-plan readout.

Integration note: the logic was right first try â€” the e2e was the problem. Two bugs in the gate, both
instructive: (1) the /planning page has multiple plans (the sample seeds its own) so a global `.plan-runway`
`.first()` read a SEEDED plan, not the new one â€” scoped every assertion to the card containing the unique plan
name; (2) more subtly, the page's "Can I afford it?" section has its own `step="0.01"` number inputs earlier in
the DOM, so `input[step=0.01].nth(0/1)` filled THOSE, leaving the plan's start balance at 0 â†’ it "depleted" at
...
The completion *moment* was already handled (capped bar + "Complete ðŸŽ‰", pace nag removed); this is what
happens after. Added `Goal.Archived` (JSON round-trips, no store change), pure `goals.Overfund(goal)` (surplus
over target, same-currency) and `goals.OverallProgress(goals, includeArchived)` â€” pulled the headline %
...
redirect action, the "what next â†’ next goal" prompt, and the one-time celebration toast. e2e
`goal_lifecycle_check.mjs` (creates a fresh over-funded goal â€” no seeded one is over target). Next: L13/L27.

## 2026-06-22 - feat: spending report by custom field (L18, also L16)

Custom fields could be defined, filled, and filtered, but the data was a dead end â€” no way to total by them.
Added `reports.ByCustomField(txns, fieldKey, start, end, rates)` mirroring `SpendingByMember` (expenses only,
in-range, FXâ†’base, largest-first deterministic) + `reports.CustomFieldCSV`, and a "Spending by <field>" section
on Reports with a selector across the transaction custom fields. Value normalization is the fiddly bit: boolâ†’
Yes/No, floatâ†’trailing-zeros stripped, json.Number via Float64, missingâ†’"" shown as "(no value)". Notably this
also closes L16's tax-tagging ask â€” a bool "Deductible" field + this roll-up gives the deductibles total with no
...
the one-off now and also `PutRecurring`s an autopost schedule with NextDue = `cadence.Next(enteredDate)` â€” i.e.
...
Sub-agent built; integrated + regressed (add/transfer/member stories green). Cluster done â€” next: L18.
...
zero new cadence math) and pure `internal/taskrecur.NextOccurrence(done, newID, now)` â€” base date = the done
...
openâ†’done transition is the only thing that spawns (re-opening doesn't), which the e2e pins by re-completing
and asserting no runaway multiplication. UI: a "Repeat" select on add + inline edit, a "â†» cadence" row badge.
Integration snag: the agent added a package-level `cadenceLabel` that collided with the existing one in
planning.go â€” renamed the task one to `taskCadenceLabel`. Sub-agent built; integrated + regressed here. This
...
enum had `.Valid()`/`.String()` â€” but nothing ever set or read them. Added the whole UX on top of the existing
model: a "Link to" type picker + a conditional entity sub-select on both the add form and the inline editor,
and a clickable "â†’ <name>" deep-link in the row that `nav.Navigate`s to the entity's screen. Pulled the pure
...
Found L24's recurring engine already substantially built â€” `domain.Recurring` (cadence/NextDue/account/
...
- **Ordering matters.** It runs *after* `startDatasetAutosave()` so `resaveDataset` is non-nil â€” I force an
  immediate re-save once anything posts, otherwise the advanced NextDue lives only in memory and a crash before
  the next autosave tick would re-post (new IDs â†’ duplicates) on reopen. With the immediate save it's idempotent.
- **e2e gotcha (worth remembering).** Backdating the seeded `rec-salary` NextDue via a plain localStorage write
  then reload does NOT work: the reloading page fires pagehideâ†’autosave and clobbers the edit with the in-memory
  (future) value before the new page boots. Fix: a one-shot Playwright `addInitScript` that backdates at
  document-start â€” after that pagehide save, before wasm reads localStorage â€” and consumes its sentinel so the
...
was `memberFor(acc)` â€” the account owner â€” so on a joint account every purchase looked like the same person's.
...
- **Design decision (asked the user).** The money-movement semantics were a real fork â€” earmark-only vs real
  ...
  DestinationKind, Amount, â€¦}` records â€” a job assigned to dollars you already hold.
- **Bottom-up.** `allocate.PlanActions(plans, isLiability) []Action` (pure, sum-invariant tested) â†’ new
  `domain.Earmark` wired through the store exactly like Goals (`earmarks` table, Dataset field, Snapshot/Load,
  Export/Import round-trip test) â†’ `appstate.ApplyAllocation` (snapshot first; restore on any error = atomic)
  + `UndoLastAllocation` (single-slot pre-apply snapshot) â†’ `allocate.go` Apply button + confirm summary +
...
- **e2e gotchas fixed during integration.** `money.Money` has no json tags so it serializes `{Amount,Currency}`
  (capital A) â€” the check read `.amount` and saw nothing; and it read localStorage before the autosave flushed.
...
- **E2E.** `import_resilience_check.mjs` pastes 3 valid + 2 malformed rows and asserts exactly the 3 land and a
  ...
  column or the validated write path rejects them (imported 0) â€” the check now discovers a live account name
...
Retired `cdn.tailwindcss.com` â€” the last styling dependency â€” by migrating every utility class to a typed Go
vocabulary backed by the new gwc `css`/`css/u` engine.

- **gwc upgrade.** Pulled gwc v3.2.0 (@master) and migrated its one breaking change (`shorthand.Class`â†’
  `ClassStr`, ~1,526 sites). Confirmed regression-free by diffing the e2e baseline against the old gwc (the 4
  failing tests â€” muzak Ã—2, restore_backup, dashboard drag â€” fail identically on both, so they're pre-existing
...
- **Foundation.** Built `internal/ui/tw`: a typed Tailwind-compatible vocabulary where each utility emits the
  exact Tailwind-default CSS via the css registry/Sink (spacing nÃ—0.25rem, the type scale's size+line-height
  ...
  computed styles match â€” all while the CDN was still present (parity-safe parallel).
- **Conversion.** A regex converter mechanically turned ~1,450 literal `ClassStr("â€¦")` sites into
  `css.Class("semantic", tw.Utilâ€¦)` â€” semantic app classes (which live in the design-system stylesheet) stay
  strings, utilities become typed. Then hand-typed the ~40 dynamically composed strings (rail items, KPI tiles,
  menus, chips, progress bar) via `tw.Fold` (folds typed rules â€” incl. multi-decl/variant `[]Rule` â€” into a
  hashed class string) and `tw.ColorClass` (maps conditional tone tokens). Audits caught utilities the converter
  kept as strings (`shrink-0`, `line-through`) â€” added and relocated.
...
- **C78 undo/redo.** Pure `internal/undosnap` (dataset JSON â†” `history.Snapshot`) + `internal/app/undo.go`
  capturing an undo point on every autosave write (diff vs last snapshot â€” captures every mutation regardless of
  code path, no per-write instrumentation). Wired Ctrl+Z / Ctrl+Shift+Z + palette. Two real bugs surfaced via
  e2e: (1) `paletteNotify`â†’`UseNotice()` panicked because the `UseAtom` hook can't run in a global keyboard
  callback â€” fixed with captured-atom helpers `PostNotice`/`BumpDataRevision`; (2) data reverted in-store but the
  To-do list didn't re-render â€” it (and the Shell) didn't subscribe to the data-revision atom; added the read.
- **C45 encrypt-at-rest.** Rides the passcode lock. Pure `internal/cryptobox` envelope + `datasetcrypto.go`
  (Web Crypto PBKDF2â†’AES-GCM). Key design choices for safety: encryption tied to an **active** lock (suspended â†’
  ...
  re-save (the autosave's `s == last` dedup otherwise left data plaintext until the next edit â€” a real gap caught
...
- **C74 statement import.** Pure `internal/statement` (delimiter/column auto-detect, signed-or-debit/credit,
  ...
  draft review â†’ dedupe â†’ import pipeline, so column-mapping is automatic and dedupe/edit come for free.
...
## 2026-06-21 - feat: muzak â€” 8 tracks, mute icon, Settings volume, resume
...
- Top bar: replaced the â™ª glyph with new icon.Volume / icon.VolumeMute (Lucide, path-only â€” the renderer rejects
...
- Note on the DB-persistence request: the store autosaves by serializing the WHOLE dataset to a localStorage blob
  ...
  be checkpoint-only â€” pending decision.
- Follow-up (user chose checkpoint-only DB persistence): added `store.MusicState` to `Settings` (additive, no
  schema bump â€” Settings is a JSON blob) + `appstate.MusicState/PutMusicState`. A Goâ†”JS bridge
  (`window.cashfluxMusicSave` â†’ `checkpointMusic`) reads the live localStorage music keys and writes them to the
  dataset only at coarse moments: muzak.js calls it on force-saves (track change / pause / pagehide), and the Go
  toggle + volume-slider-release call it directly. `seedMusicFromDataset` (run after hydrate, BEFORE Mount â€” order
...
- `web/muzak.js`: a tiny ambient player â€” Audio element, looping playlist (DEFAULT_TRACKS = calm-01..03.mp3 in
...
- Go side: `uistate.UseMuzakEnabled` (localStorage `cashflux:muzak`, default ON) + `MuzakToggle` â™ª button in the
...
- Follow-up: rebuilt muzak.js around three pieces â€” a `Playlist` data structure (list + cursor, advance/shuffle),
...
- Pure `widgetstyle` package: reserved config keys (_bg/_text/_border/_borderW/_radius/_font/_weight/_shadow plus
  the existing _accent), `Effective(global, perWidget)` merge, and `InlineStyle(cfg)` â†’ inline CSS (validates hex,
...
- Editor on /widget-manager: target dropdown (All widgets + each widget), color pickers + selects, a Reset, and a
  live preview tile (a real `.w` shell with the effective style) â€” two-column layout, sticky preview.
- Two renderer gotchas hit + handled:
  1. CSS custom properties don't survive the inline `Style()` path â€” `--accent` silently dropped. So accent is a
  ...
  2. The renderer doesn't reset an *omitted* style key in place, so a cleared accent lingered. widget.go always
     writes box-shadow (`none` when unset) to guarantee revert â€” restoring the original B20 behavior.
- e2e `widget_style_check` (preview updates â†’ dashboard tiles override â†’ reset clears); updated `widget_color_check`
...
- Tied back into the dashboard: refactored `Dashboard()` to build an idâ†’render map and render from the
...
- Brought the previously-unmanaged "highlight" tile into `DefaultItems` (added to pack_test). Improved
  ...
  present default) instead of forcing all newcomers to the top â€” so highlight lands at the bottom, attention at
  the top.
- UI: per the feedback ("looks janky"), rebuilt the manager on the reusable sortable `DataTable` instead of a
  hand-rolled flex list â€” columns Widget/Visible/Size/Order (sortable; default sorts by layout order), bare
...
- e2e `widget_manager_check`: hide â†’ tile leaves the dashboard; resize + reorder persist to `cashflux:layout`.
  Updated `dashboard_attention_check` (layout controls now in the manager) and `widget_pages_check` (manager no
  longer blank). SW â†’ v227. Next phase: full per-widget styling + custom titles + lock.
...
- Registry-driven wiring: two new `screens.All()` routes in Tools â€º Build (beside Customize/Workflows), rail
  icons in `app.railMeta` (PlusCircle / Dashboard), and i18n keys (nav + screen subtitle + placeholder). No other
  changes â€” the screens registry is the single source of truth, so the rail picks them up automatically.
- e2e `widget_pages_check`: both rail entries present, navigate to their routes, render their placeholders.

## 2026-06-21 - feat: dashboard "Needs attention" digest â€” pure ranking package

- Approved plan: replace the dead full-width header cell (title + layout-mode select + reset) with a configurable,
  responsive, resizable "Needs attention" widget (default 4Ã—1) that surfaces the urgent/spicy signals; move the
...
- Step 1 (this commit): `internal/attention` pure package. `Rank(Inputs, Config) []Item` pulls from enabled
  ...
  model: over-budget / bill due â‰¤2d / overdue task = Critical; near-budget / bill in-window / stale / high-priority
...
- Steps 2â€“4 (this commit): the widget + integration. Registered a widgetcfg "attention" schema (5 source toggles,
  ...
  bills/budgeting/freshness/anomaly logic), ranks them, and renders responsive-by-span (compact 1Ã—1 â†’ chips when
  wide-and-short â†’ list when taller); each row is its own `attentionRow` component (stable nav hook) that
  deep-links + scrolls. Default placement 4Ã—1 at the top of `DefaultItems`.
...
- Verified by a new e2e (`dashboard_attention_check`): the widget is the top tile at grid-row 1, shows urgent
  ...
  icon curated-set test and a11y_check (direct deep-link goto 404s on the gwc dev server) â€” neither touched here.
...
- Reported: clicking a chat deep link worked but did a full page reload. Root cause: the interceptor bailed
  ...
  never reproduced â€” but a real model paraphrases the link and can emit an absolute same-origin URL
...
- Extended the e2e to actually detect a reload: set a `window.__cfSentinel` before the click and assert it
  survives (a reload wipes it), and run the whole flow three times â€” relative, absolute-same-origin, and
...
- Follow-up (user reported it still reloaded): verified the dev server serves the fixed wasm (hash matched the
  local build) and isolated the interceptor against the real wasm by injecting a bubble + clicking â€” no reload.
  ...
  SW serves the app shell â€” so the interceptor must catch *every* in-app link shape.

## 2026-06-21 - feat: creation tools return deep links + dedupe before creating

- Each creating tool now returns the new entity's id wrapped in a Markdown deep link (`openLink(route, id)` â†’
...
- Dedupe guard added to every creating tool before the write: `normText`/`similarText` (Jaccard â‰¥ 0.6, subset,
...
- C90.1 read tools: list_budgets, list_goals, list_tasks, list_recurring (bills), spending_breakdown â€” all
...
- Painful debugging: the write e2e's same-session two-run (approve then decline) reliably hung at the SECOND
  approval â€” the Decline handler never fired (confirmed via a JS-global flag) and no follow-up request was made,
  ...
  when a second run's goroutine blocks on the channel after a first completed run. KNOWN ISSUE â€” logged in
...
- Mapped the whole app surface (rails/pages/settings + ~45 appstate Put/Delete/action methods) and wrote
  **TODOS C90 â€” agentic tool coverage**: foundations (write-tool seam through C78 audit/undo, in-chat approval
  ...
  MCP-server stretch. One tool group per commit, each with an e2e â€” the model that's working for the read tools.
...
- Cam: "why can't the model search the web for the brackets and use the calculator?" Added a `web_search` tool:
  the handler does a blocking `fetch` (in the loop goroutine â€” Go wasm resumes it on the JS callback) to the
...
- e2e `insights_chat_websearch_check.mjs`: model asks web_search(tax brackets)+calculator(income*12); the loop
  ...
  â€” the test runs in a context with `serviceWorkers:'block'`. (Confirms the product fetch works: against real
...
- e2e `insights_chat_prompt_check.mjs`: open editor â†’ prefilled with default â†’ replace with a marker â†’ Save â†’
...
- "how much do I spend on groceries?" used to get "I don't have that info" â€” the chat only sent 4 aggregates.
...
- wasm layer (`chat_agent.go`): `buildChatTools` exposes read-only tools bound to live data â€”
  spending_by_category (nameâ†’ID resolution, since txnfilter matches by ID), list_transactions, list_members,
...
- The composer scrolled off the bottom as the conversation grew (the whole page grew with the thread). Made the
  ...
  (`scrollTo {top: scrollHeight, behavior:smooth}`) instead of `scrollIntoView` on a bottom anchor â€” only the
  thread scrolls, the page doesn't jump. Removed the now-unused anchor. e2e still green.

## 2026-06-20 - feat: Insights chat UX â€” hover actions, delete-unravel, auto-scroll
...
- Three asks: (1) per-message action icons hidden until the bubble is hovered/focused (`opacity-0
  ...
  assistant container â€” user already had it). (2) Deleting a message now unravels the thread from that index:
...
- Cam: "initial chats fail, retries work â€” did you e2e test it properly?" My e2e tested a fresh send (passed),
  but missed the real flow: reopening Insights resumes the last conversation (init effect â†’ switchTo), and the
  FIRST send into a resumed chat silently produced no reply. Reproduced by extending the e2e to reload â†’ resume
  â†’ send (it failed: "got 1 answer bubbles"), then instrumented with logging: `onResult` fired (request made,
  callsDelta=1, no pageerror) and saw turns=3, but after `turns.Update(append)` turns was STILL 3 â€” the
...
- The e2e now permanently covers reloadâ†’resumeâ†’send (3 OpenAI calls) + the 401 visible-error path. All green.
...
## 2026-06-20 - feat: Insights chat â€” actions below the bubble + retry on the last user message
...
- Moved Copy/Pin/Retry/Delete out of the message bubble into a row UNDER it (both bubbles became flex-col, user
  ...
  OR the user's own message (`retryFor(lastID)` â†’ `resendLast`, which re-answers the last user prompt regardless
  of trailing role) â€” so a turn that errored with no reply can still be re-sent. `UserBubble` gained `OnRetry`.
- e2e `insights_chat_check.mjs` re-run after the layout change: still green (selectors are role/text-based).
  (Note to self: gwc -root web is a file-watcher dev server â€” rebuild + refresh, no restart needed.)
...
- Cam reported the chat "doesn't work". Wrote `e2e/insights_chat_check.mjs` (Playwright; installed playwright +
  ...
  deep-link fallback â€” the rail item is `a[title="Insights"]`, no href), then drives a real send. Asserts: the
...
- Conclusion: the sendâ†’providerâ†’renderâ†’persist pipeline is correct with a valid key. So a real-world failure is
  the OpenAI call itself â€” most likely a reasoning model (fixed: temperature now omitted) or backend routing
  (new in-chat toggle) or a bad/absent key (which shows a red error under the composer). The error text is the
  diagnostic.

## 2026-06-20 - fix/feat: Insights chat â€” reasoning-model temp, backend toggle, pins-above-chat
...
- Cam: "chats don't seem to be working / is it using the openai provider?". Diagnosed the dual path: the chat
  ...
  offers `o4-mini (reasoning)`, and reasoning models reject a non-default temperature on /chat/completions â€”
  `reasoningModel()` now detects o1/o3/o4/gpt-5* and sends temperature 0 (dropped by omitempty â†’ API default),
...
- Commit C of the chat-persistence work. The screen now tracks `convID`/`convCreated`/`inited`. A `persist`
  ...
  fires exactly when the thread's shape changes â€” add/delete/retry â€” not on every keystroke. A second one-time
  ...
  Ã— to `deleteConv`); deleting the open chat calls `newChat`. New i18n `newChat`/`deleteChat`.
...
- Bottom-up groundwork for Cam's "store the chats, switch between them, delete them". Domain: `Conversation`
  ...
  `PutConversation()` (id required, blank title â†’ "Untitled chat"), `DeleteConversation()` (via the audited
  `del` helper). Tests: store CRUD round-trip (incl. embedded messages + token count) and the dataset
  exportâ†’import losslessness test extended with a conversation. `go test ./internal/store/...` +
...
- Per direction on the chat bubbles: (1) Save-as-task button removed â€” it becomes an agent tool later. (2) Pin
  ...
  **DOMPurify** (sanitized innerHTML set by a `UseEffect` keyed on text+toggles so a self re-render doesn't
  blank it) instead of the framework renderer â€” `web/marked.min.js`/`web/purify.min.js` vendored (no CDN, C44),
...
- Per direction, the Insights screen is now a chat, not Explain/Q&A buttons. The two-card layout (just shipped
  for C59) is superseded â€” both answers now coexist naturally as messages in a thread, so the C59 "shared slot"
...
## 2026-06-20 - feat: Insights â€” separate Explain/Q&A answer cards + expandable pins (C59)
...
- Reconciliation note: while scoping this I found the agentic AI stack (C81/C82/C89 â€” `internal/agent`,
  ...
  the wasm/screen layer + a Responses transport â€” separate from these two self-contained UI fixes.
...
- Another C56 item: price-change rows conveyed up vs down by wording only (priceUp/priceDown). Added color-plus-
  shape, mirroring the Reports trend markers exactly: increase â†’ "text-down" (red) + icon.ArrowUp, decrease â†’
...
- Gate: screens build+vet + wasm clean; new `subscriptions_price_tone_check` e2e (every toned row has an arrow
  <svg>; no-op-but-visible if the sample has no changes) â€” PASS (6 rows). subscriptions_drill_check still PASS.
...
## 2026-06-20 - feat: C89 prompts â€” system-prompt assembler (pure)
...
- 3 tests (full prompt rules-first+context+tools; emptyâ†’rules only; tool-without-descriptionâ†’name only). go test
...
- Insights/AI pure scaffolding now COMPLETE: aiprovider (gpt-5.5/profiles/anthropic caps), internal/anthropic
  ...
  truncation, L8 chips, polish. Next: take internal/ai (C81-p2) when clear â€” it's the keystone the UI needs.
...
- Closed the C63 labelling item: the category type + parent selects (add form and inline-edit form) had no
  accessible name â€” only the parent select had a hover title. Added `aria-label`s to all four (literals; en.go is
...
- Gate: screens build+vet + wasm clean; new `categories_labels_check` e2e asserts every select in the add form
  carries an aria-label (2 selects) â€” PASS. categories.go uncontested (parallel is on internal/aiprovider). Committed
  by pathspec; TODOS.md untouched. C63 is now down to its last item (icon-only edit on narrow widths, C10/C19).

## 2026-06-20 - feat: C81 phase 3 â€” Anthropic Messages dialect (pure)

- Continued the insights/AI objective with C81-p3, the one non-OpenAI dialect, as a new pure package
  internal/anthropic (no I/O â†’ native tests; transport wires it later in the contended internal/ai).
- Modelled the dialect's real differences vs OpenAI: system is a TOP-LEVEL field not a message; max_tokens required
  ...
  system, messages, tools, maxTokens, stream) + ParseResponse â†’ text (concatenated blocks) + tool_use calls +
  usage(input/output) + stop_reason; {"type":"error"} envelope â†’ Go error. Types mirror internal/ai's OpenAI
...
- Insights/AI done so far (all pure): C89 p1 aicontext, gpt-5.5+profiles, C89 p2 read-tools, C81 p3 anthropic. The
  ...
  tools via appstate+C78; C59 insights.go result-slots/streaming; L8 chips; settings UI) â€” take as files come free.

## 2026-06-20 - fix: clear backend on/off switch in Settings (user-reported)

- User hit a websocket connection error on load ("WASM: WebSocket error during connection setup â€¦ online,
  ...
  auto-dials + registers visibility/focus/online listeners that re-dial â€” with no off switch.
- Added an inverted, default-on `BackendDisabled` pref (omitempty bool â†’ missing = enabled, so existing prefs are
...
- This is the parallel session's subsystem (C81/C89 AI transport), but settings.go/prefs.go/sync_client.go were all
  ...
  wasm â€” excluded, it's not in the app's import graph). New `settings_backend_toggle_check` e2e PASS (toggle hides
...
## 2026-06-20 - feat: C89 phase 2 â€” Insights agent read-tools (pure)
...
- Design: a small DataSource interface (transactions/accounts/balance/liquid/monthly-net/format+parse money) that
  ...
  account_balances, affordability (reuses afford.CanAfford/L8). Each agent.Tool parses JSON args â†’ concise text;
  bad args error â†’ the loop turns it into a recoverable ToolResult. Currency formatting stays at the impl so the
...
- Insights/AI progress: C89 p1 (aicontext) + gpt-5.5/profiles + C89 p2 (read-tools) done â€” all pure. Next un-taken
  slices: C89 p3 write-tools (need appstate+C78 â€” contended), C81 p3 anthropic dialect (could be pure shaping in a
  new file), C59 result-slot/streaming/truncation + L8 chips (insights.go â€” contended, other agent active there).
...
- Closed another C63 item: nested category rows rendered with literal "â€” " prefixes (`indentLabel` repeating
  em-dashes). Row labels now indent with depth-proportional CSS padding (16px/level) plus a subtle left guide line;
  `indentLabel` is repurposed for the parent-picker <option> lists only â€” switched from em-dashes to non-breaking
  spaces (regular leading spaces collapse inside an <option>, nbsp don't).
- Subtlety: the Edit tool rendered my `"Â ..."` input as literal nbsp bytes (0xC2 0xA0) in source â€” confirmed via
  od -c; that's exactly what's wanted, so left as-is. Added `strconv` for the px padding value.
- e2e `categories_nesting_check`: no row label starts with "â€”" and â‰¥1 nested row carries real left padding â€” PASS
...
- Capabilities gained Reasoning; OpenAI registry leads with gpt-5.5 (reasoning/vision/tools/json_schema) and
  ...
  target (Anthropic â†’ chat_completions/https; non-reasoning â†’ no effort; non-streaming â†’ no stream). 3 new test
...
- These are MODELLING the intent (pure/testable). The actual websocket+streaming Responses transport is C81-p2 in
  the contended internal/ai â€” to be wired carefully next, checking the other agent isn't mid-file. Objective stands:
  complete every insights/AI todo.

## 2026-06-20 - feat: C89 phase 1 â€” Insights-agent context builder (pure)

- User added C89 ("Insights as an agent") and redirected me to it, then asked to complete ALL insights/AI todos â€”
...
- New internal/aicontext: Build(Inputs, Opts) â†’ bounded, privacy-tiered Context; Context.Prompt() renders a compact
  Markdown block for the system prompt. Kept PURE by taking pre-formatted strings (caller does appstate/money
  formatting) â€” no appstate/money import, mirroring the history.Snapshot decoupling. Tiers gate sections (the opt-in
  ...
  Committed e4ac1af. (One snag: an inline here-string commit msg with quotes/arrows mangled `-m` arg parsing â†’
  switched back to the reliable -F .tmp/msg.txt.)
- NEW OBJECTIVE: complete every insights/AI todo. Inventory â€” C89 (p2 read-tools/p3 write-tools/p4 UI), C59
...
- The last open C63 item: category rows didn't show how many transactions used them (the count only surfaced on
  delete), and â€” unlike Accounts/Members â€” there was no drill into the filtered ledger. Added both. A single pass
  ...
  and navigates to /transactions â€” the same pattern accounts.go uses.
- The row meta now reads "Expense Â· 25 transactions" where the count is a `btn-link` drill button (its OnClick hook
  is stable â€” CategoryRow is its own component, so no loop-hook hazard); zero-use categories show a muted
...
- e2e `categories_usage_drill_check`: asserts the badge text matches "N transaction(s)", that clicking drills to
  /transactions, and that `cashflux:tx-filter` persists a category â€” PASS ("25 transactions"). Regression: the
...
- Now that the framework's Markdown is in use (C59), the custom-page Text widget was the next free win: its
  `textBody` rendered `P(Class("muted"), t)` â€” one flat paragraph. Swapped it for `Div(Class("md md-widget muted"),
...
- Same security posture as Insights: GFM, raw HTML escaped, `javascript:`/`data:` schemes dropped â€” matters more
...
- custompage.go + widgetspec.go were both clean (not mid-edit by the parallel session). Gate: widgetspec test +
  ...
  page to render and the dataset is a SQLite blob (not trivially seedable headlessly), so â€” as with the C59 wire â€”
...
- Going to wire the renderer for the markdown parser I'd just built, I found the framework already ships a
  ...
  my hand-rolled parser â€” tested upstream and security-hardened â€” so the right call was to use it and delete mine.
- Wired it into Insights: the answer card's `P(result.Get())` became `Div(Class("md insights-answer"),
  ...
  (parser + tests, added earlier today) and the unfinished `internal/ui/markdownview.go` â€” no dead code.
- Lesson logged: should have grepped the framework's exported surface before building a parser (the dot-import even
  ...
  still loads (insights_keyhint_check PASS â€” confirms no render regression). A real AI answer needs a key so the
...
- Insights AI answers render as a flat paragraph today even though the model emits lists/bold/headings (C59). Built
  the bottom-up half: a dependency-free `internal/markdown.Parse` â†’ `[]Block` (Paragraph/Heading/List/Code) with
...
- Design choice: the package returns an AST, not framework nodes â€” a pure logic package can't import the wasm `ui`
  layer, and keeping the tree pure means it unit-tests natively (12 block cases + 10 inline cases, all green) and
  the renderer stays the single security boundary (it will emit only known elements, never raw HTML â€” C59 a11y
...
- Deferred the screen wiring deliberately: insights.go is actively churned by the parallel session (recent C59/L8
  commits), so wiring the block-treeâ†’nodes renderer is its own later slice once this lands. Committed the package
...
- The account `Select` on the Documents importer (both the CSV-draft footer and the receipt-import footer) had
  no accessible name â€” the systemic placeholder/label gap C49/B15 track. Added `aria-label="Import into account"`
...
- The footer selects only render once a draft/receipt exists, which isn't trivially reachable in headless e2e, so
  the gate is build+vet plus the existing `story_documents_csv` end-to-end import (still PASS â€” no regression). The
...
- The asset add-form showed four optional scoring fields (Return %, Liquidity, Stability, Locked-until) inline,
  ...
  fields" toggle: new `advOpen` UseState + `onToggleAdv` UseEvent, the four `If(!isLiab, â€¦)` gates now also
...
- e2e: new `accounts_advanced_disclosure_check` asserts collapsed (0 fields, aria-expanded=false) â†’ expand
  (4 fields, true) â†’ re-collapse (0). The pre-existing `accounts_field_constraints_check` relied on the now-hidden
  Liquidity/Stability inputs, so it now clicks the toggle first â€” caught and fixed before commit. Used literal
  button labels (en.go is contested by the parallel session); the field labels themselves stay i18n'd.
- Built screens for wasm + full wasm to web/bin, ran all three accounts checks â†’ PASS. Committed via
...
- The mermaid shim hardcoded theme:'dark'; now that C69 lights the shell for light themes, dark diagrams looked
  wrong. web/mermaid.js now reads document.documentElement data-theme â€” "light" â†’ mermaid "default", else "dark" â€”
  and re-initialises when the theme changes (tracks lastTheme), so a diagram rendered after a theme switch picks up
  the new palette. web/mermaid.js is my own file (uncontested); bumped sw v219â†’v220 so clients re-fetch it.
...
- Fifth wired Mermaid case + a 5th generator. mermaid.FromRules(rs, catName): a flowchart TD of "match â†’ category"
...
- TestFromRules (native): shadowed rule + empty-match rule â†’ asserts labels (incl. Escape trimming the empty
...
- Took C88's pure Gravatar half (internal/gravatar free + un-taken; other agent on bills_screen.go). Tiny complete
  ...
  crypto/md5 is fine under js/wasm. Tested against the canonical Gravatar doc vector (0bc83cb5â€¦1346). Committed 43c0e64.
- STOCK-TAKE: the non-contended pure-logic C backlog is now effectively exhausted. Seven pure foundations landed
  this stretch â€” C69 tokens, C83 filter, C81-p1 providers, C82 agent, C78-p1 history, C74 statement, C88 gravatar â€”
...
- C57 item 3. BillRow's "due â€¦" meta now carries a tone via a new billUrgencyTone(daysUntil): text-down when n<=0
...
- Took C74's pure extraction/mapping slice (internal/statement area free + un-taken; other agent on Mermaid).
  First confirmed internal/extract does NOT cover this â€” it parses AI-vision JSON replies, not delimited statements.
- New internal/statement: DetectDelimiter + MapColumns (header heuristics) + ParseAmount (currency/commas/parens/
  DR-CR/sign â†’ signed minor units, delegating the final numeric to money.ParseMinor) + ParseDate (multi-layout,
  MM/DD-first then DD/MM fallback) + Parse â†’ []Row, amount = Amount col or Creditâˆ’Debit, bad rows skipped with line
  numbers, unmappable header rejected. Reused money.ParseMinor + kept date layouts in-package (dateutil is ISO-only).
- Two snags fixed: (1) I literally typed a UTF-8 BOM into a Go string literal â†’ "illegal byte order mark"; the Edit
  tool kept re-inserting the BOM when I tried to type the ï»¿ escape, so I built the backslash from [char]92 in
  PowerShell to write the escape as ASCII. (2) A debit/credit test used DD/MM dates the MM/DD-first parser rejected
  â†’ added DD/MM fallback layouts (strictly more lenient; day>12 resolves unambiguously). go test green. ae1a4f2.
- Documents-screen wiring (mapping UI, AI categorization, dedupe) is the contended later step. Pure-foundation stack
  now: C69, C83, C81-p1, C82, C78-p1, C74 â€” six tested foundations awaiting their contended wiring.

## 2026-06-20 - feat: wire the money-flow Sankey into Reports (C70)

- Fourth (and "highest wow") wired Mermaid case. Reports builds moneyFlows []mermaid.SankeyFlow: one Incomeâ†’category
  link per spending row (absI64(r.Amount)) plus an Incomeâ†’Savings link from flow.Net() when positive, and renders
  uiw.Mermaid(mermaid.Sankey(moneyFlows)) in a "Money flow" card (guarded len>1). Values are minor units â€” only
  relative widths matter. reports_screen.go uncontested; added the mermaid import.
- New reports_sankey_check.mjs: /reports â†’ asserts .cf-mermaid svg renders (confirms the vendored mermaid 11 bundle
  includes sankey-beta). PASS. App wasm builds clean; gofmt clean. Committed via git commit -- <paths>; TODOS.md
  untouched.
- C70 essentially complete: all 4 generators wired & visible â€” /workflows (flowchart), /categories (tree), /split
...
- Third wired Mermaid case. Split's settle-up card now renders uiw.Mermaid(mermaid.FromSettleUp(transfers, name,
  amount)) â€” the active split's debtorâ†’payer digraph, beside the existing "X owes Y" rows. transfers is already
...
## 2026-06-20 - feat: C78 phase 1 â€” diff-based change history (pure)
...
- Key purity call: the ticket frames the differ over store.Dataset, but to stay non-contended I made it GENERIC â€”
  Snapshot = map[collection]map[id]json.RawMessage. So it diffs all ~20 collections without importing store/appstate
  (phase 2's commit seam adapts Datasetâ†”Snapshot â€” that's the contended step). encoding/json only.
- history.go: Diff (deterministic, sorted; unchanged rows omitted) + Invert (addâ†”delete, update swap) + Apply
  (returns a fresh clone, never mutates input â€” verified by a test). stack.go: bounded Stack with cursor-based
...
- 11 test funcs incl. forward+inverse round-trip restore and coalesce-same-row-vs-different. go test green.
  ...
  persistence, UI: toast-undo / âŒ˜Z / Activity timeline) all touch contended files â€” deferred.
- Pure-foundation stack now: C69 tokens, C83 filter, C81-p1 providers, C82 agent loop, C78-p1 history â€” all
...
- Fourth C70 generator. mermaid.Sankey([]SankeyFlow{From,To,Value}) emits sankey-beta source (header + blank line +
  ...
  positive). Foundation for the incomeâ†’categoriesâ†’savings/debt money-flow chart (highest "wow" per the spec).
...
- Took C82's pure core (internal/agent area clean + un-taken; other agent on Mermaid). The ticket's finding: no Go
  agent framework fits wasm + local-first, and the loop is ~a few hundred lines of pure Go â€” so build in-house.
- New internal/agent: Tool/ToolSpec/ToolCall/ToolResult + a name-keyed Registry (register-replaces; Specs() is the
  model offer) + Run(ctx, model, reg, history, opts): a bounded modelâ†’tool-callsâ†’executeâ†’repeat loop. Model is an
...
- Key design calls: every tool failure (unknown tool / invalid JSON args / handler error) degrades to a ToolResult
  the model can react to â€” the loop NEVER aborts on a tool error (the realistic agent behavior); only a model error
...
- Second wired Mermaid case (renderer landed last iteration). Categories() now renders a "Category map" card after
  the expense/income lists: uiw.Mermaid(mermaid.FromCategories(cats)) â€” the hierarchy as a graph (also satisfies
  C63's "tree view" want). categories.go was uncontested; added the mermaid import; literal card title/label
  (en.go parallel-dirty).
- New categories_diagram_check.mjs: /categories â†’ asserts .cf-mermaid svg renders. PASS. App wasm builds clean;
  gofmt clean. Committed via git commit -- <paths>; TODOS.md untouched. index.html already loads mermaid (from the
  renderer commit), so no web-file edits this time â€” clean of the parallel index.html churn.
- Two diagrams now visible: /workflows (flowcharts) + /categories (tree). Next: sankey generator or another slice.

## 2026-06-20 - feat: C81 phase 1 â€” AI provider registry (pure package)
...
- Took C81 phase 1 (aiprovider area clean + un-taken; other agent on Mermaid C70). The ticket's key finding is that
  ...
  DATA model â€” perfect for a new pure package that can't collide with the contended internal/ai/settings/store.
- New internal/aiprovider: Provider/Model/Capabilities + Dialect (openai covers 6/7; anthropic the lone special),
  AuthStyle, Structured enum (json_schema/json_object/none â€” modelling the cross-provider structured-output gotcha
...
- Deliberately NOTED pricing + caps are indicative seeds that drift upstream (per the ticket) â€” not asserted as
  fact. 7 test funcs incl. dialectâ†”auth pairing and free-text-only-OpenRouter invariants. Fixed one self-inflicted
  test-math slip (4000 tok Ã—250c/M = exactly 1.0c, not 0). go test green. Committed 2d00b4e.
- Phases 2-6 (generalize transport, AIConfig + key migration + redact-all on export, anthropic dialect, settings UI,
  capability gating, backend proxy) all touch internal/ai + settings.go + store â€” contended; deferred. Next:
...
- User asked to actually SEE a diagram. Built the renderer over the C70 generators: vendored mermaid@11 locally
  (web/mermaid.min.js, 3.3MB, no CDN â€” C44; it ends with globalThis.mermaid = â€¦default so window.mermaid is the
  clean global), a web/mermaid.js shim (cashfluxRenderMermaid: securityLevel 'strict', startOnLoad off, async
  mermaid.render â†’ inline svg, clears box on parse error â€” C45), and uiw.Mermaid (internal/ui/mermaidview.go),
  ...
  the two scripts (after chart.js/flip.js); sw.js caches them + bumped v218â†’v219.
- New e2e mermaid_render_check.mjs: /workflows â†’ asserts real <svg> inside .cf-mermaid â€” PASS, 4 diagrams rendered.
...
- Moved to C83 (txnfilter area clean + un-taken; other agent on workflows.go). The ticket frames the "real work" as
  flipping Criteria.Account/Category/Member from stringâ†’[]string â€” but those fields are consumed by the CONTENDED
...
- 6 test funcs (OR/AND across 4 dims, tag intersection, order-preserving Filter, Normalize, Equal, Add/Toggle/
  ...
  tolerant JSON migration) is the contended transactions.go/txfilter.go step â€” deferred.
- Next non-contended C slice: C81 AI provider abstraction as a NEW package, or another pure gap. Check un-taken first.

## 2026-06-20 - feat: C69 step 2 â€” derived shell tokens (pure)
...
- Chose DERIVATION over new struct fields: rather than add BgElev/TextFaint/etc. to Theme (which would touch presets,
  ...
  lerp, clamps t, degrades to a real color on bad input) + derivedVars â†’ --bg-elev (card 6% toward text),
...
- Validate() gained a text-on-bg-elev AA pair. Risk: theme_test asserts Default + ALL presets validate with 0
  issues â€” a borderline preset could break. Verified empirically by running the full suite: all green (the elevated
  surface keeps text high-contrast on both dark and Paper). 7 mixHex cases + derived-token assertions added.
- Committed 159502f. Deeper C69 (rewire Tailwind/#design-system literals to var(--â€¦), kill dual --accent, retire the
  override block) CONSUMES these tokens but needs index.html â€” still deferred/contended. Next: another non-contended
...
- Third C70 generator. mermaid.FromSettleUp(transfers, name, amount): a flowchart LR where each person is a round
  node and each split.Transfer is a debtor(From)â†’creditor(To) edge labelled with the amount. Takes name(id) and
  amount(int64) closures so internal/mermaid stays currency-free (the screen passes its fmtMoney); per-person node
  ids generated (m0,m1,â€¦) so member ids can't break syntax; labels escaped.
- TestFromSettleUp (native): two transfers to one creditor â†’ asserts round nodes in first-seen order and both
  labelled debtorâ†’creditor edges. go test ./internal/mermaid green; vet clean; app wasm builds; gofmt clean.
...
## 2026-06-20 - fix: C69 immediate Paper unblock â€” light themes light the shell
...
- Root cause (per the ticket): two appearance systems. ApplyTheme writes content CSS vars; a separate ApplyPrefs
  ...
  Paper is the only light preset â†’ light vars but no data-theme â†’ light cards in a dark shell.
...
- e2e theme_shell_skin_check.mjs: default boots data-theme=dark; inject a light-bgBase theme into localStorage,
  ...
  var(--â€¦), kill the dual --accent writer, retire the override block) remain â€” they need index.html (protected/
...
- Continued C70's pure generator layer (C83 multi-select was the alternative but it's a breaking txnfilter change
  that ripples into the contested transactions.go and can't compile in isolation â€” deferred). Added
  mermaid.FromCategories(cats): a flowchart LR with one node per category and a parentâ†’child edge for each category
  whose ParentID resolves in the set. Node ids are generated (c0,c1,â€¦) so a Mermaid-unsafe category ID can't break
  syntax; labels escaped; an orphan parent reference yields a root node, not a dangling edge.
- TestFromCategories (native): root + two children + an orphan-parent â†’ asserts the nodes, the two parentâ†’child
...
- Scanned the full L-series (L1â€“L39, more than the original L1â€“L17). The named feature/UI work is done (L8/L9/L13/
  L14/L15/L16 wired + all pure-logic foundations). The five remaining unchecked "Pure" checkboxes (internal/lock,
  pwcheck, theme, notify.CatchUp, reports) are STALE â€” those packages already exist in internal/. L18â€“L39 are
  story / core-flow e2e-VALIDATION tickets whose loopstory_*.mjs drive scripts are the parallel session's untracked
  work (32 untracked, 0 tracked) â€” not mine to author without colliding.
...
## 2026-06-20 - feat: "Restore from a backup file" â€” L9 import half

- Completed L9 with the restore inverse. internal/app/backupall.go: restoreFromBackup (pickFile â†’ UnmarshalEnvelope
  â†’ confirmAction â†’ applyBackup â†’ reloadPage) + applyBackup, which writes registry + appearance (clearing omitted
  keys so it replaces, not merges) + each dataset by registry order (active â†’ canonical, rest â†’ blobs). Suspends
  autosave first so the dying page can't clobber the restore. One palette command + 3 i18n keys. All same-package
  workspace helpers â€” no protected files touched.
- e2e snag + fix: backup-then-restore in one page failed because the backup DOWNLOAD tears down the wasm runtime
  (the same "Go program has already exited" artifact) â€” the second Ctrl+K hit a dead handler. Fixed by reloading
...
- Last named deferred UI item. Took the export half of L9 (restore/import deferred â€” file-picker is harder to e2e).
...
- Two real findings while verifying:
  1) First run serialized datasets as JSON null â€” the active dataset wasn't flushed to localStorage yet (autosave is
     a 4s ticker). Fixed by taking the active dataset LIVE from the store (appstate.ExportJSONRedacted, same source
     the autosave uses) instead of reading the lagging localStorage key â€” more correct, not just a test fix. Also
...
- C70 (Mermaid support) is a JS-lib-behind-a-Go-interface feature like D3 charts; per "build bottom-up" this is the
  ...
  internal/mermaid (no syscall/js): Escape (whitespace-collapse, " â†’ ', and entity-escape < > so condition
  operators read correctly yet no raw tag forms â€” XSS defense paired with the renderer's strict mode, C45); a
  Flowchart builder (ShapeBox/Round/Diamond nodes, plain + labelled edges, "flowchart TD" output); and FromWorkflow
  (trigger terminal â†’ optional condition diamond â†’ ordered action boxes, condition's outgoing edge labelled "yes" =
  the dry-run path, C65). Imports internal/workflow (both pure).
- Table tests (native â€” both packages are pure Go, so this runs under plain go test unlike the wasm-tagged screen
...
- New version_rail_check.mjs: asserts .app-version reads like v0.1.0 at the rail foot â€” got "v0.1.0". go test
...
- C80 secondary (Settings About footer) + export/log stamping are nice follow-ups. Next: another C68â€“C90 ticket.
...
- Completed L13 by wiring the runway bridge (built last tick) into Planning. Targets clean (planning.go + en.go),
  UI quiet. New runwayCard: runway.Project(assets, app.Recurring(), now, 60, buffer, rates) â†’ starting balance +
  projected-low stats and a verdict (holds 60 days / "dips below your buffer on <date> â€” short $X" + low point).
- Start balance = ledger.NetWorth's ASSETS figure (sum of asset accounts), not net worth â€” a runway is about cash
...
- e2e runway_check.mjs: add a large recurring outflow via the recurring form (amount field has no min, so negatives
  work), assert the breach warning. The recurring NextDue defaults to now â†’ event on day 0 â†’ guaranteed breach with
...
- L13 now fully done (bridge 69602ba + card daf868f). Three of the deferred UI items now landed (L15/L14/L8/L13).
  Remaining deferred: L9 backup-everything (download/file-picker â€” still hard to e2e). Next: reassess for any
...
- C67 is a large rail-nav feature whose UI lives in internal/app/shell.go (heavily contested by the parallel
  ...
  build = customize/workflows. Membership stays registry-driven (B7) â€” shell renders by SubGroup later, no hardcoded
  path lists. No app behavior change yet (shell still groups by Group).
- Test: added TestToolsSubGroups to registry_test.go (every Tools route â†’ exactly one known sub-group; non-Tools
  carry none; the four partition all 11). The screens package is //go:build js && wasm (View funcs are wasm), so
  the test is wasm-only. Verification limitation (honest): this Windows env has no working js/wasm test exec â€” native
...
- C66 item 1 (flagged reliability), and the final original C-series ticket. uploadImage and importCSV both did
  `if err == nil { refresh() }`, swallowing PutArtifact failures â€” most plausibly a localStorage-quota overflow
...
- New artifacts_error_check.mjs: intercepts pickFile's native chooser (page.once filechooser), feeds an empty CSV,
  and asserts the app toast (.toast-msg) shows the error â€” got "artifacts: empty csv". App wasm builds clean; gofmt
...
- Scope note: the user extended the loop to C47â€“C90; the C-series has grown to C80 (parallel session added
  C67â€“C80). C47â€“C66 each now have shipped improvements. Next: C67 (Rail navigation v2).

## 2026-06-20 - feat: recurring â†’ cash-flow runway bridge (L13, logic)
...
- Also noted: the parallel session built C64 (live authoring match-count) ON TOP of my L15 rules preview â€” the
  shared work is compounding, not colliding.
- Next: wire the Planning runway card on top of internal/runway (planning.go is mine + clean) â€” short-term daily
...
- C65 item 2. The workflow action builder only appended; the staged list was plain text with no remove, so a
  ...
  stagedActionRow component (label + a âœ• remove button) â€” its own component so the per-row OnClick hook sits at a
  stable position (the staged list is variable-length, framework loop-hook gotcha). aria-label literal (en.go
  parallel-dirty).
- Chose this over C65's "top gap" inline workflow edit (a larger panel/form) and over the trivial H3â†’H2 heading
  fix â€” staged-remove is the most self-contained concrete win.
...
- C65 remaining: inline edit, staged reorder, condition formula help/validation, H3â†’H2 headings, labels, history
  paging. Next: C66 (Artifacts) â€” the final C-series ticket.
...
- Chose this over C64's "top gap" rule reordering: the Rule struct has no order field, so reorder needs a
  schema+store+state change (bottom-up, larger) â€” deferred.
- New rules_live_count_check.mjs: types "a" in #rule-add, asserts a [role=status] "Matches N transactions" with
  N>0 appears (and nothing before typing) â€” got "Matches 42 transactions". App wasm builds clean; gofmt clean.
...
- Third UI wiring tick. Targets check: en.go + internal/app + planning.go clean (only categories.go dirty â€”
  isolated). Considered L9 backup-everything but it's download + file-picker heavy â†’ hard to e2e reliably under the
...
- internal/screens/planning.go: new affordCard â€” amount + optional months + optional buffer â†’ afford.CanAfford(amt,
...
- Test snag worth noting: first e2e run failed asserting "Projected balance" via innerText â€” the stat label is
  CSS-uppercased so innerText returns "PROJECTED BALANCE"; fixed by matching case-insensitively. The wiring itself
  was correct from the start (debug showed the affordYes verdict rendering). afford_check.mjs (token amount â†’ stat
  breakdown; huge amount â†’ shortfall) PASSED standalone twice. Committed by pathspec (4e8813c).
- Next: remaining deferred UI is L9 backup-everything (palette action, accept harder verification) + L13 runway card
  (bills is the other session's C57 â€” verify before touching); else pure reports. Gate each on clean targets.
...
- New categories_reassign_kind_check.mjs: deletes the in-use sample expense "Groceries" â†’ reassign panel; asserts
  the picker does NOT offer "Income" (the only income category) and still offers other expense categories. PASS
  (offered Dining/Housing/Utilities/â€¦ not Income). App wasm builds clean; gofmt clean. Committed via
...
- C63 remaining: real indentation vs em-dash nesting, per-row usage count + drill-down, name/kind/parent labels,
  icon-only rows. Next: C64 (Rules) â€” flags a missing precedence-reorder.
...
- C62 remaining: name visible label + reassign-select label/focus-on-open, icon-only rows on narrow widths. Next:
  C63 (Categories) â€” note it flags a reassign-kind correctness bug worth prioritising.
...
- C61 item 2 (the C2-parallel formatting win; pure, no i18n/CSS). The formula result (formatFormulaValue) and the
  available-variables reference both printed strconv.FormatFloat raw (354070). Added groupThousands(f) â€” up to 2
  decimals, trailing zeros trimmed, integer part comma-grouped, sign preserved â€” and applied it to both the result
...
- Build note: the full app build was blocked twice this turn by the parallel session's internal/app/shortcuts.go
  ...
  built green. E2E limitation (honest): the draft/review list is populated ONLY by the key-gated vision flow â€” the
  CSV path imports directly with no draft rows â€” so the DraftRow category select isn't reachable in offline e2e.
...
- C60 remaining: image preview beside drafts (highest value; needs fileâ†’dataURL), CSV file-picker/drag-drop, the
  key-hintâ†’Settings link (same as C59), account-select aria-label. Next: C61 (Customize).

## 2026-06-20 - feat: fuzzy keyword matching in the command palette (L14)

- Second UI wiring tick. Targets check: en.go + shortcuts.go clean (only documents.go dirty â€” isolated parallel
...
- internal/app/shortcuts.go: paletteCmd gains an optional keywords []string (search-only synonyms, never rendered);
  ...
  the direct actions (add/new/expense â†’ New transaction, export/backup/download â†’ Export, lock/security â†’ passcode).
- KEY WIN: no en.go change at all â€” keywords are internal English search terms, not displayed â€” so the commit is
  just shortcuts.go + the e2e, zero i18n contention. Even safer than the rules-preview wiring.
- Verification: gofmt clean; wasm built; new palette_fuzzy_check.mjs (open palette, "add" â†’ "New transaction" via
  alias-only, "export" â†’ Export) PASSED standalone twice on 8099. Committed by pathspec (6ecf5f7).
- Next: keep going down the deferred UI backlog while quiet â€” L9 backup-everything action (internal/backup
  envelope), L8 affordability hook (internal/afford) â€” each gated on re-checking its target + en.go clean; pure
  reports as fallback.
...
- C59 item 2 (bounded, uncontested; the headline result-collision split ripples into pin/save/usage so it's a
  larger follow-up). The "Add your OpenAI key in Settingsâ€¦" hint appeared as a dead-end P in two places (the
  Explain action + the disabled Q&A box). Added a keyHintNode() closure (hint text + a "Settings" button that
  navigates to /settings via the router) and used it in both spots â€” built fresh per use so each placement gets an
...
- C58 item 3 (the no-persistence, no-CSS slice â€” the ephemeral/persist item is a bigger bottom-up feature). Added
  selectAll/clearAll handlers (set/clear the `selected` member map) surfaced as buttons when the household has 2+
  members, and a summary line: "$X split among N â†’ $Y each" with the even per-person figure and any rounding
...
- New split_summary_check.mjs adds a 2nd member via /members (sample seeds only "You"), then on /split fills $100,
  clicks Select all â†’ asserts "split among 2 â†’ $50.00 each", and Clear removes the summary. PASS. App wasm builds
...
- C57 item 5. The bills MapKeyed keyed rows by r.Bill.AccountID. Switched to a composite (AccountID + DueDate +
  ...
  one bill each, so keys don't actually collide yet â€” but AccountID-only is a latent footgun (a future where a
...
- First wasm-UI wiring since the contention pivot. Checked the targets specifically: en.go/rules.go/bills_screen.go/
  shortcuts.go all clean, parallel session had moved to docs/markdown (README/.github/CONTRIBUTING) â€” UI layer
...
- Verification: gofmt clean; built wasm (GOOS=js GOARCH=wasm -o web/bin/main.wasm) â€” compiled; new
  ...
  a5375b2. en.go edit landed without clobbering â€” the surgical-Edit + immediate-atomic-commit discipline held.
...
- C57 item 2 (flagged correctness). bills_screen showed annual = totalÃ—12, where total is the sum of the current
  upcoming occurrences across mixed cadences â€” multiplying that by 12 is wrong (e.g. a yearly bill counted 12Ã—).
  Added pure bills.AnnualAmounts(accounts, recurring) []money.Money: each qualifying liability's min payment Ã—12,
  plus each negative recurring annualized via MonthlyEquivalent()Ã—12 (weekly Ã—52 / monthly Ã—12 / quarterly Ã—4 /
  yearly Ã—1), in native currency. The screen FX-converts each (mirroring how it totals the upcoming list) and sums.
- Table test TestAnnualAmounts: a liability + 4 negative recurring of each cadence + skipped (asset/no-due/zero-min/
  archived/income) â†’ asserts count=5, all positive USD, total = 120,000+1,440,000+62,400+120,000+9,000. go test
...
- C57 remaining: mark-paid (needs persistence), urgency tone, countable/tappable calendar days, composite row key
  (also flagged â€” quick follow-up), guided empty state. Next: the row-key fix, then C58 (Split).

## 2026-06-20 - feat: subscription â†’ charges drill-down (C56)
...
- C56 item 2: a detected subscription should let you verify it against the real charges. Made the subscription
  ...
  "highest-value" confirm/ignore/add item because that needs new persistence (domain+store+state) â€” a bigger
...
- C55 item 3 (the "god-tier" win, no-i18n/no-CSS so uncontested). The spending-by-category, top-payees and
  ...
  sized amount/max of the list's largest (abs; sorted-desc lists â†’ max from a quick pre-loop), inserted into each
  row-main. Inline-styled (4px, accent fill on a border track) with a .share-bar class for selectability â€” no
...
- New reports_sharebars_check.mjs: on /reports asserts >=3 .share-bar, some inner fill reaches width:100% (the list
  max) and some don't (proportional, not decorative) â€” got 25 bars. PASS. App wasm builds clean; gofmt clean.
  Committed via git commit -- <paths>; sw.js parallel-dirty, left out; TODOS.md untouched.
- C55 remaining: section grouping/jump-nav, "Showing: <period>" header, consistent CSV + print/PDF, whole-screen
  empty state â€” mostly IA/CSS/i18n. Next: C56 (Subscriptions).

## 2026-06-20 - feat: per-category spend trend series (L16, logic)

- UI layer went quiet (parallel session committed its C48/C52/C53 burst â€” no tracked UI files dirty), so a wasm-UI
  wiring was in theory open. But the obvious next wiring (L15 rule match-count preview) needs new user-facing
  strings in i18n/en.go, the explicitly-contended file â€” risky to edit even while momentarily clean. Chose the
  lower-risk, equal-value move: another pure report from the L16 catalog.
- Picked "Category trends (sparklines + biggest movers %)" â€” a genuine gap. The reports package had TopMovers
  (two-period change) and IncomeExpenseSeries (whole-ledger buckets) but no per-category multi-bucket spend series.
  New internal/reports/trends.go: CategoryTrends(txns, bounds, rates) â†’ []CategoryTrend{CategoryID, Spend []int64,
  Total, DeltaPct, HasDelta}, reusing the unexported categoryTotals + the shared bounds convention; sorted by Total
  desc; firstâ†’last % via ledger.PercentChange. Table-tested (3 buckets, +100%/-50%/no-delta, exclusions, too-few-
...
- C54 item 4 (chosen as the clean no-i18n win; the labels/IA items need en.go/index.html which are parallel-dirty).
  AllocRow printed the score twice â€” head "60%" and a "Score 60%" .budget-sub â€” joined to the breakdown by a literal
  " Â· " span. Removed the duplicate sub-line and the separator; the score stays in the head + the role=progressbar
...
- New allocate_score_check.mjs: on /allocate asserts no .budget-sub is a "Score â€¦" duplicate or a bare "Â·", the
  "returns â€¦" breakdown sub-line is present, and the head still shows a "%" score (got "60%"). PASS. App wasm builds
  clean; gofmt clean. Committed via git commit -- <paths>; sw.js parallel-dirty, left out; TODOS.md untouched.
- C54 remaining: label the amount/reserve/max-per inputs + profile select (needs en.go), advanced-options
  disclosure (index.html), surface allocate-amount, AI-errorâ†’Settings link. Next: C55 (Reports).

## 2026-06-20 - feat: number-field constraints on the Planning screen (C53)

- C53 item 4 (chosen because it needs no new i18n â€” en.go is parallel-dirty, so the labels item is deferred). Added
  min/max to planning.go's number inputs: plans horizon min=1, one-time month min=1 max=plHorizon.Get() (dynamic;
  empty horizon â†’ max="" which the browser ignores), and the payoff calculator (balance/APR/payment/extra) + debt-
...
- C53 remaining: the big IA restructure (sub-nav/tabs, reunite payoff inputs+result, item 2 "most impactful"),
  labels (needs en.go), and guided empty states â€” the IA/tabs likely want index.html (contended). Deferring those
...
- C52 item 1: the priority Select and due-date Input in both the add and inline-edit forms had no aria-label or
  ...
  matching aria-label. Labels are literals ("Priority", "Due date") since en.go is parallel-dirty â€” committing it by
...
- C52 headline item: open tasks past their due date now render the due meta in the danger tone (.text-down) with
  an explicit " Â· overdue" suffix (colour + text, B15). overdue = !done && !Due.IsZero() && FormatDate(Due) <
  FormatDate(now) â€” ISO string compare sidesteps time-of-day/timezone edge cases. todo.go only.
...
- New todo_overdue_check.mjs adds a task due 2020-01-01 and asserts its .row-meta.text-down contains "overdue".
  PASS ("due 2020-01-01 Â· overdue"). App wasm builds clean; gofmt clean.
- COMMIT METHOD FIX: applying my own memory now â€” git add only the new e2e file, then git commit -F msg -- <paths>
...
- Stock-take after L15 conditions: with the parallel session mid-burst on the whole UI layer (dashboard/goals/
  ...
  unverifiable. So I looked for a clean PURE-LOGIC gap instead â€” found one in L16's Reports catalog: "Year-end /
...
- New internal/reports/yeartax.go: YearTax(txns, year, start, end, rates) â†’ YearTaxSummary{Year, Rows[]
...
- Table-tested (mixed income/expense/transfer/out-of-range â†’ per-row + totals + sort; empty â†’ zero summary, label
...
- Next: pure-logic for L8/L9/L13/L14/L15/L16 now all landed. Remaining L-series work is deferred wasm-UI wiring
  ...
  affordability hook) â€” paused until the parallel C-series burst settles. Slowing the loop a tick.

## 2026-06-20 - feat: linked-goal â†’ account transactions drill-down (C51)

- C51 item 6: a goal linked to an account should drill to that account's activity. Added Goals() nav + txFilter +
  viewAccountTxns(accountID) (TxFilter{Account}.Normalize â†’ atom.Set + PersistTxFilter â†’ nav to /transactions),
  wired via a new goalRowProps.OnDrillAccount (added router import). In GoalRow, split the linked account out of
  the run-on progress sub-line into its own clickable element (also a partial fix for item 3's run-on) â€” inline-
...
- C51 now: bar tone done, labels done, linked drill-down done. Remaining (row sub-line badges, contribute audit
  note, icon-only narrow rows) are CSS/behavior-heavy and overlap the contended index.html â€” deferred. Next: C52.
...
- Last clean pure-logic gap for L15: a rule today matches one case-insensitive description substring; the
  ...
  file (internal/rules/conditions.go) WITHOUT touching the shared Rule type â€” lowest-risk move under the parallel
...
- Next (C51): linked-goal â†’ account drill-down (mirror the budget drill via nav), then C52 (To-do).
...
- The L-series pure-logic seam is now nearly mined out. The remaining clean pure-logic candidates are richer
  rule conditions (amount/account/AND-OR â€” but that needs a Rule type extension in the shared rules.go, so a
...
- C51 headline item: goal bars were always the flat accent fill, so a reached goal looked the same as one barely
  started. Added barFillStyle(pct, complete) â€” width plus, when complete, an inline background:var(--up) (the
...
- Next (C51): visible labels on the goal add/edit/contribute forms (reuse labeledField) and the linked-goalâ†’account
  drill-down â€” both uncontested in goals.go.
...
- L14 gap 1: the palette labels commands by noun ("New transaction"), so a verb query ("add") misses them.
  ...
  total gaps (lower better â†’ prefix/contiguous beats scattered); keyword matches get a +1000 penalty so a title
  match always wins; empty query â†’ all in input order; sort.SliceStable with an order tiebreak.
...
- STOCK-TAKE â€” the L-series PURE-LOGIC foundations are now largely in place (L1-L5 full; L6 core; L7; L8 starter+
...
## 2026-06-20 - feat: budget â†’ filtered-transactions drill-down (C50)

- C50 item 4: a budget row should answer "why am I over?". Made the budget title a button that sets the persisted
  txfilter to the budget's CategoryID and navigates to /transactions â€” same pattern as Accountsâ†’Transactions
  (uistate.TxFilter{Category}.Normalize() â†’ atom.Set + PersistTxFilter â†’ nav.Navigate). Wired via a new
  ...
  (dotted underline affordance) â€” index.html is parallel-dirty, so no stylesheet hook.
...
- sw.js is parallel-dirty (they bumped to v215), so I left it out of this commit â€” their bump covers the wasm
...
- Next (C50): consolidate the up-to-4 stacked row sub-lines into tone badges + give the over/near summary the same
  treatment (uncontested budgets.go, but the badge styling may want index.html â€” will inline if needed).
...
- Next (C50): consolidate the up-to-4 stacked row sub-lines into tone badges, give the over/near summary the same
  badge treatment, and add budgetâ†’filtered-transactions drill-down (mirrors C30 + the new uiw.FilterToolbar/txfilter
...
- C49 status: currency-select (pre-existing) done, number constraints done, visible labels (add + edit + set-balance)
  ...
  narrow widths (item 5) â€” both likely want CSS in index.html (still parallel-dirty), so deferred until it frees up.
...
- C49 item 1: the add form was placeholder-only, so labels vanished on input (and APR/Liquidity/Stability/Due day
  ...
  with visible text stacked above it â€” styled inline (flex column) so it needs no stylesheet (web/index.html is
  ...
  fields). Kept existing aria-labels/placeholders (same text) â€” harmless, and placeholders still give example hints.
- Mid-iteration the package didn't compile: the parallel session's uncommitted insights.go referenced an
  undefined suggestChip (their WIP). I did NOT commit unverified â€” waited; they finished the symbol and the build
...
- Added min/max/step to the score + day inputs in BOTH the add form and the inline-edit row: Liquidity/Stability
  min=1 max=5 step=1, Due day min=1 max=28 step=1 (28 to match the field's existing "(1â€“28)" copy â€” safe for every
  month; the spec said 31 but I kept it internally consistent with the visible hint). Money fields (credit limit,
  APR, min payment) get min=0 so negatives can't be entered. Added the "(1â€“5)" hint to the Liquidity/Stability
  i18n strings (APR/Return already carried "%", Due day already "(1â€“28)").
- New e2e accounts_field_constraints_check.mjs: asserts Liquidity/Stability are 1/5/1 with the (1â€“5) hint on the
...
- Deferred for C49: visible labels for the placeholder-only fields (item 1), an "Advanced" disclosure to split the
  flat grid (item 4), icon-only row actions on narrow widths (item 5) â€” next. Then the remaining C48 polish once
...
- C48: the dashboard scattered text-[11px]/[12px]/[13px]/[22px]/[24px]/[34px] with no shared scale (inconsistent
  tile-to-tile). Note the ticket's "bypasses the text-size setting" framing is off â€” there's one Scale pref applied
  via #app{zoom}, and zoom scales px and rem alike; the real defect is inconsistency + no figure hierarchy.
- Added four rem-based semantic tokens to index.html â€” .t-caption (12px), .t-body (13px), .t-figure (24px primary),
  .t-figure-lg (34px hero); font-size only, so explicit leading-* utilities still own line-height. Swapped all 27
  ad-hoc sites in dashboard.go: 11+12px â†’ caption, 13px â†’ body, 22+24px â†’ the single primary figure, 34px â†’ hero.
  The KPI/net-worth-trend/goal figures (were 22/24 ad hoc) now share one primary size â€” one clear hierarchy.
...
- New e2e dashboard_typescale_check.mjs: asserts .t-figure-lg â‰ˆ34px, .t-figure â‰ˆ24px (found 5 primary figures
  sharing the size), caption/body resolve, and NO text-[Npx] remain inside .bento. PASS. App wasm builds clean;
  gofmt clean. The app shell (shell.go/custompagesnav.go/wsswitcher.go) still has a few text-[13px] â€” out of C48's
...
- Real finding: the Settings panel has 6 unlabeled .rate-in number inputs (no label/aria-label/placeholder).
  settings.go is owned by the parallel work stream (flagged), so I did NOT edit it â€” scoped the gate to the
...
- Extracted internal/ui/filtertoolbar.go: a screen-agnostic uiw.FilterToolbar(FilterToolbarProps) â€” always-visible
  ...
  for the loop). Badge is aria-hidden â€” the chips convey the active filters to SR users.
- transactions.go now just builds []uiw.Chip from f.ActiveFilters() (chipLabel resolves IDsâ†’names) and passes
  ...
  story_txn_bulk_clear all PASS â€” behavior preserved.
...
- The UI half over yesterday's active-filter logic. transactions.go's filter `form-grid` (10 controls in one
  ...
  `f.ActiveCount()`, then Clear + Export CSV. The trigger opens a `uiw.FlipPanel` (CloseOnly â€” filters apply live
...
- Active filters render as removable chips below the toolbar via `MapKeyed(f.ActiveFilters(), â€¦)` over a new
  `FilterChip` component (its own component so the per-chip remove-button OnClick hook sits at a stable position â€”
  the framework loop-hook gotcha). `chipLabel` resolves IDsâ†’names (account/category/member maps) and clearedâ†’its
  word; the âœ• calls `removeFilter(field)` â†’ `Criteria.Without(field)` (a scope change, so the page resets). A
...
- App wasm builds clean (go build -o static/bin/main.wasm . green); go vet + gofmt clean; ./internal/txnfilter
  tests still green. Note: `internal/server` is currently broken on a grpctunnel API drift â€” pre-existing and
...
- E2E-gated before pushing: new story_txn_filter_toolbar.test.mjs drives the real toolbar â€” a search term raises
  the badge to "1" + a chip; the Filters popover opens (FlipPanel) and adding cleared raises it to "2"; Escape
  closes it; the chip âœ• removes just that filter (search box clears, badge â†’ "1"); "Clear all filters" empties
...
- Picking up C47 (Transactions: paginated, sortable table + cleaner filter UI). The table half is already shipped:
  ...
  remaining piece is the **cleaner filter interface** â€” transactions.go still crams 10 controls into one `form-grid`
...
- Table-tested (TestActiveFiltersAndCount, TestWithoutClearsOneFilterKeepingSortAndPaging) â€” order, whitespace,
  scope-change and unknown-field no-op all covered. go test ./internal/txnfilter green; go vet clean.
- Note: a parallel session owns the uncommitted TODOS.md (the C47â€“C66 spec block); committing strictly by pathspec
  (logic + tests + journals only), never TODOS.md.
- Next: the toolbar UI in transactions.go â€” always-visible search + a FlipPanel "Filters" popover holding
...
- Persistence (additive, store was clean): store.Settings gains *PayoffBaseline{TotalOwed, Currency, StartedAt}
  (json omitempty pointer; dataset.go gained a time import) â€” round-trip tested. appstate seam (payoff_progress.go):
  StartPayoffTracking(total, currency) snapshots into Settings via PutSettings; ClearPayoffTracking nils it;
  PayoffProgress(currentOwed) returns (Progress, startedAt, tracking) â€” unit-tested.
...
- L6 core bug: hydrateDataset (persist.go) re-seeded the sample whenever the dataset key was empty/missing, so a
  wipe (or externally-cleared store) brought the stranger's household back on reload â€” a clean slate was
...
- Last tick's screenshot confirmed the lingering-splash bug visibly (translucent #boot over /planning), so I
  pulled the L12 root-cause forward â€” it's a fix-once that clears the splash complaint in L1/L2/L3/L6/L11.
...
- Fix (index.html boot script): a hideBoot() that adds .hidden AND removes the element from the layer
  (display:none) once faded â€” via transitionend plus a 700ms fallback (covers reduced-motion / no-transition).
...
- UI (planning.go): the debt card now prints "Debt-free by <Mon Year> (snowball) Â· <Mon Year> (avalanche)" and a
...
- Category mapping (the "map free text -> real category + run Rules" item): mapReceiptCategory prefers the
  ...
  (line description + merchant) against the user's rules â€” so the per-line vision categories win, but a
...
- L3 core gap: vision extraction yields N independent rows, so importing a grocery receipt creates many
  ...
  mode" â€” one charge split across categories. Started bottom-up with the pure logic.
...
- Started L3 ("The Receipt Snap") with its smallest, highest-value isolated fix. pickImageDataURL in
  ...
  there's no e2e surface to assert against â€” noted in the L3 probe-hardening item.)
- Next L3 (bottom-up): receipt mode â€” a receipt is ONE charge split across categories (not N transactions); pure
...
- L2 step 4 (UI). split_screen.go: after the forward split, a "Save split" button writes the current split as a
  ...
  ledger from app.SettleUp(base) â€” per-member net ("is owed"/"owes", negative tinted text-down), the minimal
...
- Deferred: seeding 2-3 sample members + shared expenses out of the box â€” that changes the seeded dataset and
...
- Wired both into internal/store the same way every other entity is: two new tables (sharedexpenses,
  ...
  omitempty), and full CRUD in crud.go (Put/Get/Delete/List for each). No schema-version bump needed â€” the new
...
- Added internal/settle: Expense{Payer, Shares}, Settlement{From,To,Amount}, Transfer{From,To,Amount}.
  ...
  Minimize(net) is the classic greedy debt-simplification â€” pay the largest debtor's debt to the largest
...
## 2026-06-20 - feat: L1 "Coverâ€¦" overspent budget (state + UI + e2e)
...
- Finished the L1 cover-overspending ritual on top of the pure budgeting.Transfer. Added
  ...
  PutBudget. The appstate layer enforces a stricter rule than the primitive â€” ValidateBudget requires limit > 0,
...
- UI (budgets.go): an over-budget row now shows a "Coverâ€¦" button (only when there's another budget to pull
  from). It toggles an inline .cover-form: a "Cover from" source select (each option labelled "Name Â· $X left"),
...
- E2E (story_budget_cover.test.mjs): seed a $1 Groceries budget (over, since Groceries has sample spend) + a
  $500 Shopping source, open Coverâ€¦, move $50, and assert the rendered rows re-balance ($1->$51, $500->$450) and
...
- The independent UI defect from L1: budget rows render the status line, the pace heads-up, the rollover-carry
  line, and the envelope line as adjacent `Span.budget-sub`s, but `.budget-sub` was inline â€” so they read as
  "â€¦$61.00 leftAt this pace, projected to go overâ€¦". Made `.budget-sub` block-level (display:block + a small
...
- Screenshot-confirmed via a new one-off check (e2e/budget_shot.mjs, like theme_shot.mjs): each Dining/Groceries/
  Shopping/etc. row now shows "Monthly Â· On track Â· 79% Â· $61.00 left" and "At this pace, projected to go over
...
- User: "when you are done with the table start the l-series todos." Started L1 ("The Sunday Budget Reset"),
  whose core ritual is *covering an overspend by moving money between budgets* â€” which the app didn't support
...
- Added `CoverAmount(Status)` â€” the shortfall (overspend) to clear, read off Status.Remaining so it's correct
...
- Next L1: persist the transfer (store + export round-trip), an appstate covering action, the budgets.go
  "Coverâ€¦" form on over-budget rows + e2e; plus the independent quick fix for the glued budget sub-lines.

## 2026-06-20 - feat: C47 reusable DataTable + ledger pagination bar

- Built the pagination bar (prev/next + "1â€“50 of N" + page-size select replacing the old visN "Show more"), and
...
- DataTable owns the *chrome*, the caller owns the *rows*: props are `Columns []Column{Label, SortKey, Class,
  Head}`, a `Body any` (the caller's MapKeyed <tr> result â€” typed `any` so the []ui.Node slice flattens through
  ...
  disabled at the ends, "fromâ€“to of total" via `pagination.Window`, and a Rows-per-page select of
...
- Verified: gofmt clean, wasm build green, full e2e suite 29/0 green, and a Playwright screenshot confirms the
  table renders with the pager footer "Prev Â· 1â€“50 of 57 Â· Next Â· Rows per page Â· 25/50/100/All" and no console
...
## 2026-06-20 - feat: C47 ledger table UI â€” semantic table + click-to-sort headers
...
- Preserved every behavior + the per-row button titles/classes the e2e relies on (#txn-add, input[type=search],
  ...
  failing run was banner_check flaking on a remove-timing race; it passes in isolation â€” unrelated to this
...
## 2026-06-20 - feat: C47 ledger redesign â€” sort direction + keys (pure, bottom-up)
...
- Extended internal/txnfilter: added an explicit Dir (asc/desc) field to Criteria; SortKeys =
  ...
  (idâ†’name maps) so category/account sort by display name, not opaque IDs; Apply delegates with empty labels
...
- Pivoted off B16 to fix the real bug the documents-CSV story surfaced. Checked first: clean tree, csv.go last
  touched 2 days ago by "fix: CSV import accepts its documented date,payee,amount,account format (C27)" â€” so
  ...
  requires Desc â†’ every row silently rejected. The existing test asserted Payee but not Desc, so it slipped
  through.
...
## 2026-06-20 - test: B16 story â€” documents CSV import (+ a real finding)
...
- FINDING worth a follow-up (not fixed here â€” it's an app-behavior/spec decision in internal/store/csv.go +
  ...
  REQUIRES Desc â€” so a payee-only row is silently rejected. The story therefore uses a `desc` column (which
...
## 2026-06-20 - test: B16 story â€” customize formula (evaluate + save)
...
## 2026-06-20 - test: B16 story â€” planning recurring item

- Eighteenth journey story (e2e/story_planning_recurring.test.mjs): on /planning, scope to the recurring form
  (the form.form-grid containing the "How often" cadence select â€” the screen has several forms), fill the label
...
## 2026-06-20 - test: B16 story â€” allocate exclude/restore
...
## 2026-06-20 - test: B16 story â€” bulk clear
...
## 2026-06-20 - test: B16 story â€” set the default member
...
## 2026-06-20 - test: B16 story â€” duplicate a transaction
...
- Fourteenth journey story (e2e/story_txn_duplicate.test.mjs): seed a txn, filter the ledger to it, click the
  ...
  standalone copy, not a transfer leg â€” the code clears TransferAccountID on duplicate). Suite now 23 green.
  Test-only, no sw bump.

## 2026-06-20 - test: B16 story â€” sub-category nesting
...
- Added e2e/run-stories.mjs: a Node, no-PowerShell runner so the whole browser suite can gate CI on Linux. It
  ...
  killed cleanly cross-platform â€” `go run` would leave an orphan child), starts it on :8099, polls until ready,
...
- CI wiring (NOT done here â€” ci.yml is the parallel session's territory; left untouched to avoid collision).
...
## 2026-06-20 - test: B16 story â€” account archive + restore
...
## 2026-06-20 - test: B16 story â€” transfer excluded from totals
...
- Eleventh journey story (e2e/story_txn_transfer.test.mjs), a correctness gem: add two same-currency accounts
  ...
  selects), and assert (a) two transfer legs are created (txns with transferAccountId set â€” IsTransfer) and
...
## 2026-06-20 - test: B16 story â€” member reassign-on-delete (no orphan)

- Tenth journey story (e2e/story_member_reassign.test.mjs): mirror of the category one for members. Add a
  member (#member-add), give them an account (owner select aria-label "Owner"), delete the member â€” which
  opens the reassign panel because they own an entity â€” confirm with the default target (the household /
...
## 2026-06-20 - test: B16 story â€” category reassign-on-delete (no orphan)

- Ninth, and trickiest, journey story (e2e/story_category_reassign.test.mjs): add a category (#cat-add), assign
  a transaction to it (txn add-form category select aria-label "Category"), then delete the category â€” which,
  because it's in use, opens the reassign panel â€” pick a target, and click "Move and delete". Asserts the
  category is gone and the transaction's categoryId moved to the CHOSEN target (captured from the select's
  value), is not the deleted id, and points to a live category â€” i.e. no orphan.
...
## 2026-06-19 - test: B16 story â€” to-do complete-toggle

- Eighth journey story (e2e/story_todo_toggle.test.mjs): add a task via #task-add, mark it complete via the
  row's .check toggle, and assert the UI shows the done state (.row.done) and the task's status flips openâ†’done
...
- Next: members and categories reassign-on-delete (the trickier flow â€” delete with a reassignment target).

## 2026-06-19 - test: B16 story â€” reconcile (toggle cleared)
...
- Seventh journey story (e2e/story_txn_cleared.test.mjs): seed a txn, toggle its cleared status via the per-row
  ...
  and a cleared one has `cleared:true` â€” the assertions handle both.
- Robustness fix worth noting: the dataset autosave is a short ticker, and a fixed 2.5s wait raced it (the
  ...
  store (waitForTxn: re-read up to ~7s until the predicate holds) â€” the right pattern for any
  "assert via persisted dataset after a mutation" story. Suite now 16 green. Test-only, no sw bump.

## 2026-06-19 - test: B16 story â€” transactions filter persistence
...
- Sixth journey story (e2e/story_txn_filter.test.mjs): seed a uniquely described transaction, type it into the
  ...
  still holds the term AND the view stays narrowed to 1 â€” covering the B16 backlog's "filter ... persist across
...
- Tied the suite together: e2e/run-stories.ps1 builds wasm once, serves web/ on :8099, then runs every
  *.test.mjs (nav, rerender, the 5 B16 stories) and *_check.mjs (theme/fontÃ—3/banner/unify/icon-weight/
  widget-color feature checks) â€” each in its own fresh browser so they're isolated â€” and exits non-zero if any
...
- Test infra only, no sw bump. B16 now has a real, runnable suite covering the core CRUD journeys + all the
  ...
  filter persistence, reconcile/cleared â€” or wire this runner into CI.

## 2026-06-19 - test: B16 story â€” settings exportâ†’import round-trip (lossless)

- Fifth B16 story (e2e/story_settings_roundtrip.test.mjs): the data-integrity teeth. Click Export JSON (capture
  via page.waitForEvent('download') + saveAs), import that exact file back (Importâ€¦ opens a pickFile chooser â†’
  page.once('filechooser', fc=>fc.setFiles(path))), export again, and assert the two exports carry the same
  entities â€” a structure-agnostic count of every object-with-an-id, plus that a known id survives. Proves
...
## 2026-06-19 - test: B16 story â€” create a goal + contribute
...
- Fourth B16 story (e2e/story_add_goal.test.mjs): create a savings goal (name #goal-add + target aria-required
  ...
  â€” click the row's Contribute button, fill the revealed `#goal-contrib-<id>` form, save â€” and assert the saved
...
- Test-only, no sw bump. Next: settings exportâ†’import round-trip, then an aggregating story runner.

## 2026-06-19 - test: B16 story â€” create a budget + pick its period
...
- Third B16 story (e2e/story_add_budget.test.mjs): create a Weekly budget via the Budgets add form (add()
  ...
  period via a recursive walk of the persisted dataset JSON for the object with our name â€” structure-agnostic.
- Test-only, no sw bump. Next: goal create + contribute, then settings exportâ†’import round-trip.

## 2026-06-19 - test: B16 story â€” add an account (net-worth correctness)
...
- Second B16 story (e2e/story_add_account.test.mjs): add an asset account with a $5000 opening balance via the
  ...
  balance (354070 â†’ 359070), it autosaves, and survives reload. The net-worth delta is the correctness teeth â€”
...
- Confirms the ledger.NetWorth path end-to-end through the UI. Test-only, no sw bump. Next stories: budget
  create + period switch, goal contribute, settings exportâ†’import round-trip.

## 2026-06-19 - test: first B16 E2E story â€” add a transaction

- B20 done, so moved to the next high-value, in-my-lane item: B16 (end-to-end stories). Its TODO assumed
  Playwright wasn't installed, but I installed it earlier this session â€” so B16 is now actually buildable.
...
- Gotcha found while authoring: opening the +Add menu's quick-add panel adds a SECOND amount input (the
  ...
  no overlay) â€” cleaner and unambiguous. Found this via a throwaway debug script (now removed).
- Test-only change (no app code), so no sw bump. Next B16 stories: account add (asset/liability), budget
  create + period switch, goal contribute, settings persist + export/import round-trip â€” each its own story
  file + commit.

## 2026-06-19 - feat: per-widget colors â€” the last B20 sub-item
...
- Two gotchas hit and fixed: (1) the shorthand Style() map does NOT apply CSS custom properties (only direct
  setProperty does), so the first `--w-accent`/data-attr approach left the strip transparent â€” switched to an
...
- Verified via Playwright (e2e/widget_color_check.mjs): opening a tile's gear and picking red paints an inset
  red strip and persists _accent=#ff0000; Clear removes the strip and the stored key â€” no page errors. Bumped
  sw cache v189â†’v190.
...
- Verified via Playwright (e2e/font_remove_check.mjs): upload a dummy font â†’ row + Remove appear, store +
  theme.fontUi set; Remove â†’ cashflux:fonts cleared, @font-face gone, theme.fontUi falls back to Inter, row
  disappears â€” no page errors. Bumped sw cache v188â†’v189.
...
- Verified via Playwright (e2e/icon_weight_check.mjs): switching to Bold takes a rail icon's computed
  stroke-width from 1.6px to 2.2px, sets --icon-stroke=2.2, and persists theme.iconStroke=2.2 â€” no page errors.
...
- Started B13's "icon pack" as a feasible, offline icon STYLE: a selectable line weight. Pure layer first â€”
  added `IconStroke float64` to theme.Theme (default 1.6, the current weight), threaded through Default, all
  presets, and the FromPrefs dark/light bases, with a Validate bound (1.0â€“3.0), a `--icon-stroke` CSSVar, and
...
- Confirmed no ui.Icon call site passes its own Style, so the renderer can drive stroke-width via an inline
  ...
  the attribute) â€” that wiring + the editor weight control come next. No rendered change yet â†’ no sw bump.

## 2026-06-19 - refactor: unify density/scale into the theme engine (B20)

- Diagnosed the duplication: density CSS keys off `[data-density]`, which only ApplyPrefs set â€” so the theme
...
- Removed the duplicate legacy controls from settings.go (the Compact ToggleRow, the Display-scale Select, the
  ...
  density+scale. (Left the now-unused settings.compact/displayScale/scaleDefault i18n keys in en.go alone â€”
...
- Verified via Playwright (e2e/unify_check.mjs): editor Compact sets data-density=compact, text-size 130 drives
  ...
  remains â€” no page errors. Bumped sw cache v186â†’v187.
...
## 2026-06-19 - feat: dashboard banner â€” store, band, editor controls (B20)

- Wasm half of the banner: `internal/uistate/banner.go` â€” a `cashflux:banner` localStorage slot
  ...
  return in a Fragment), driven purely by CSS â€” so changing the banner in settings repaints with NO dashboard
...
- Editor controls: gradient preset buttons (from theme.BannerPresets), Upload-image (pickFileNamed â†’ MIME
  fallback â†’ ValidateImageUpload â†’ artifacts.DataURL â†’ setBanner), and Remove. Boot wiring in app.go after
  ApplyFonts.
- Verified with a new Playwright check (e2e/banner_check.mjs): picking the Aurora preset sets data-banner=on,
  a gradient --banner-bg, a visible band, and persists; Remove turns it off â€” no page errors. Screenshot
  (banner-dashboard.png) confirms the band renders cleanly atop the dashboard. Bumped sw cache v185â†’v186.
...
- Started the header/banner image feature SDLC-first. New `internal/theme/image.go`: image-upload validation
  (`ValidateImageUpload` PNG/JPEG/WebP/GIF â‰¤2 MB, `ValidImageMIME`, `ImageMIMEForName` ext fallback) mirroring
  the font helpers, plus a `Banner` model â€” Kind none/gradient/image, `CSS()` returning the background-image
...
- A11y by construction: the banner is a purely decorative band with no essential text on it, so a busy image
  can't hurt content legibility â€” the guardrail is "don't put text over it" rather than a runtime contrast
...
- Storage will mirror fonts: a separate `cashflux:banner` slot, NOT embedded in the theme JSON. Table tests
  ...
  rendered change yet â†’ no sw bump.
...
- Completed the user's font-upload ask. Wasm pieces: `internal/uistate/fonts.go` â€” a `cashflux:fonts`
...
- The file picker dropped the name/MIME, which I need for the family + format fallback, so I added
  ...
  handler: pickFileNamed â†’ MIME fallback via theme.FontMIMEForName(name) â†’ theme.ValidateFontUpload â†’
  artifacts.DataURL â†’ uistate.AddFont â†’ set theme.FontUI to the new family and apply. The family is derived
...
- Verified end-to-end with a new Playwright check (e2e/font_upload_check.mjs) that drives the file chooser with
  ...
  select is set to it, and theme.fontUi updates â€” all green, no page errors. Bumped sw cache v184â†’v185.
...
## 2026-06-19 - feat: app consumes the font tokens â€” fonts are themeable (B20)

- Found the gap: ApplyTheme was already setting `--font-ui`/`--font-display`, but nothing read them â€” the
...
- Note: body text now resolves to Inter by default (the documented design-system UI font â€” Tailwind's `sans`
  was already Inter; only the raw body element lagged on the system stack). Left the boot-loader splash
  hardcoded to Fraunces â€” it's brand, shown before the engine runs, and shouldn't be themed.
- Verified live with a new Playwright check (e2e/font_check.mjs): switching the Interface font to the serif
  option flips `getComputedStyle(document.body).fontFamily` from the Inter stack to the serif stack â€” the
  token is live, no console errors. Bumped sw cache v183â†’v184.
...
- Started the user's "artifact uploads for fonts/icons/images" ask, SDLC-first: pure tested logic before any
  UI. New `internal/theme/font.go` â€” `FontAsset{Family,MIME,DataURL}`, `FontFaceCSS` (an @font-face rule with
  the right `format()` hint + `font-display: swap`), `ValidateFontUpload` (WOFF2/WOFF/TTF/OTF, â‰¤1 MiB), and
  `FontMIMEForName` (extensionâ†’MIME fallback, since browsers often report no type for .ttf/.otf).
- Storage decision: the font bytes will live in a SEPARATE durable slot (e.g. `cashflux:fonts`), NOT embedded
  ...
  the user's "artifact" as "uploaded asset" generically â€” storing fonts/images with appearance (localStorage),
...
- Rounded out the editor with shareable themes: Export writes the active theme via the pure `Theme.ToJSON`
  ...
  component and the JSON round-trip already unit-tested in internal/theme. Bumped sw cache v182â†’v183.
- Next (the user's explicit ask): artifact uploads â€” custom font-file upload (@font-face from a size-capped
  stored asset) and header/banner images, then the unify of the legacy density/scale controls, then icon packs.

## 2026-06-19 - feat: live theme editor in Settings â†’ Appearance (B20)

- Third B20 rung (UI): `internal/app/theme_editor.go` â€” a self-contained `themeEditor` component mounted with
...
- Kept English strings inline rather than threading new keys through the shared i18n en.go (parallel session
  territory) â€” a deliberate decoupling, can be migrated to i18n later.
- Verified in a real browser: a new Playwright one-off (e2e/theme_shot.mjs) opens Settings via the household
  button, waits for `.theme-editor`, screenshots it and clicks a preset â€” renders clean, live-applies, zero
...
- Known follow-up (the "unify" debt): the editor's density + text-size now duplicate the legacy prefs controls
  ...
  artifact uploads, icon packs. Bumped sw cache v181â†’v182.

## 2026-06-19 - feat: state bridge wires the theme engine to the DOM (B20)

- Second B20 rung: `internal/uistate/theme.go` (js+wasm) â€” `ApplyTheme` sets a theme's `CSSVars()` on the
...
- Wired at boot in app.go right after `ApplyPrefs`. Deliberately kept `ApplyPrefs` in place (it still sets
  ...
  rather than ripping out the legacy path in one commit â€” the full unify/migration of the Settings UI comes
...
- Added a `--bg` alias (legacy stylesheet paints the body from `--bg`; the engine names it `--bg-base`) so a
  future background edit actually takes effect. Bumped sw cache v180â†’v181. Wasm builds clean.
- Next: the Settingsâ†’Appearance editor (preset pick + token pickers + live preview + save/reset + import/
...
- New focus per the user: branding/theming + artifact uploads for fonts/icons/images. Grepped first and
  ...
  contrast, CSSVars, JSON round-trip) and is fully tested â€” but nothing imports it yet. So B20's foundation
...
- Built the migration rung first as pure tested logic (zero DOM risk): `theme.FromPrefs(prefs.Prefs)` maps
  ...
  the migrated theme is a *visual no-op* â€” switching the app onto the engine changes nothing until a token
  is edited. "system" can't be resolved in pure Go, so it falls back to dark; the wasm layer will resolve it
  before calling.
- Aligned `theme.Validate`'s scale floor 0.75 â†’ 0.70 to match prefs' 70% display-scale minimum, so a user at
  minimum zoom migrates to a valid theme. No existing test regressed.
- Next: the state bridge â€” `uistate.ApplyTheme/LoadTheme/PersistTheme` (set CSSVars on :root, alias index's
  `--bg`/`--accent-dim`/`--danger`, resolve systemâ†’light/dark), then the Settingsâ†’Appearance editor and
...
- User reported most left-rail items were not navigable. Root cause: the B3 Layout/outlet restructure in
  app.Run â€” `/` registered as `router.Options{Layout:true}` rendering the Shell around `router.GetOutlet()`,
...
- Confirmed against the framework router source that the flat structure never double-rendered (the history
  router only stacks a parent when it's registered as a layout, and the pre-B3 code registered none) â€” so B3
...
- Regression guards: `internal/screens/registry_test.go` (wasm lane) asserts the registry that drives both
  routing and the rail stays sound â€” unique rooted paths, non-nil views, known rail groups, "/" at the head, no
  collision with the /p/ pattern â€” so a route can't silently fall through to the "*" catch-all again. Plus a
...
- Continued the C46 iconography pass through the chrome/control layer. Swapped every ad-hoc Unicode glyph for a
  typed `ui.Icon`: the shared `internal/ui` components (period stepper â€¹ â€º â†’ chevrons, FlipPanel âœ• â†’ x, widget
  gear âš™ â†’ settings, including the hidden width-balancing spacer so the header stays centered) â€” which lands
  app-wide â€” plus the workspace switcher â–¾ â†’ chevron-down and the "My pages" row â‹¯ â†’ more-horizontal.
- Process note: two of these commits accidentally swept the parallel session's pre-staged files into my commit
  (the shared `.git` index â€” plain `git add; git commit` commits the whole index, not just my paths). Main still
...
- Stopping the icon effort at this milestone (registry + quick-add + all chrome/control glyphs). Remaining C46
  ...
  per-row âœ•) and the B14 chart migration to D3 are the next slice â€” they need an in-browser visual pass.
...
- Remaining in the group (handed back, since they need a browser or a decision): the actual placement of these
  ...
  B14 still has an open renderer decision (D3 â€” already implemented â€” vs. keep pure-Go SVG) plus an in-browser
...
- The user reported in-app nav drops the `/CashFlux/` base on the Pages site. Root cause confirmed against the
  framework router source: `RouterOptions` has no basename â€” the history router reads the raw `location.pathname`
...
- Chose the app-side base-prefix approach (B30 option B) over a framework change. Pure, table-tested
  ...
  the empty-base local case alone), `Strip` (raw pathname â†’ logical route). The wasm `uistate` reads the live
  `<base href>` at boot â€” using the same source as the assets, so they can't disagree â€” and exposes
...
- `ReassignOwner` already moves a member's transactions (via their MemberID), but the Members screen's
  `ownedCount` â€” which decides whether to open the reassign panel â€” only counted accounts, budgets, and
...
- The Goals screen's combined stat cards summed `g.CurrentAmount.Amount` / `g.TargetAmount.Amount` directly,
  ignoring per-goal currency â€” the lone screen not routing aggregation through the FX table (CLAUDE.md rule 6).
...
- Unlike members (which had an existing `ReassignOwner` cleanup path) and categories (whose tree defensively
  ...
  and steer the user to Archive â€” implemented at the screen level so the store/import API is untouched.
...
- The library fix alone wasn't enough: `enableAppLock` seeded from a fresh `applock.Config{}`, so the
  preserved-receiver prefs never reached it. Changed it to seed from `loadAppLock()` â€” the current config â€”
...
## 2026-06-19 â€” feat: paginate Transactions list (C39)
...
- Long ledgers built every row at once. Added a `visN` state (default `txnPageSize` = 50); the default render
  ...
  `OnClick` is a prop (not a hook), so it's safe in the conditional branch â€” same pattern the CSV buttons use.
...
## 2026-06-19 â€” fix: label Allocate weight inputs (C6)
...
## 2026-06-19 â€” fix: Budgets Quarter<Month anomaly (C40)

- Root cause (confirmed against the C40 narrowing notes): `budgets.go` evaluated each budget over
  `PeriodRange(b.Period, periodWin.From, â€¦)` where `periodWin.From` is the viewed window's START. Under a
  Quarter view that's the quarter start (Apr 1), so a Monthly budget's window resolved to the quarter's FIRST
  month (April), not June â€” making the SPENT summary smaller for Quarter than for Month. The shared period
...
## 2026-06-19 â€” revert: remove internal/quotes (duplicate of internal/lockquotes)

- `internal/quotes` (c4ef85c) duplicated the pre-existing `internal/lockquotes`, which the parallel session
  already wired into the lock screen (`applockgate.go` â†’ `lockquotes.ForIndex`). Removed the redundant package
  and its changelog/devlog entries. Fourth time a "new" package collided with existing/in-flight work
  (insights, rulesuggest, quotes) â€” confirming the solo-accessible scope is built out; stop minting packages.
...
## 2026-06-19 â€” feat: ledger.LiquidBalance + runway de-dup (docs)

- Catch-up doc for 483c388 (pure `ledger.LiquidBalance` â€” spendable cash across non-archived cash-type
  accounts, FX-converted, table-tested) and e11cf80 (Reports runway refactored to call it, âˆ’14 lines of inline
...
## 2026-06-19 â€” feat: spending stats + renewing-soon (catch-up docs)

- Batched docs for four commits shipped code-first while the parallel session held en.go/docs:
  `reports.SpendingStats` (26ccbe2 core, 9ce8e48 surface) â€” count/total/mean/median, median resists
  big-purchase skew, shown as a Reports line; and `subscriptions.UpcomingRenewals` (dee330c core, 841fad2
  surface) â€” subs renewing within 7 days, shown as a "Renewing soon" card. Both pure + table-tested.
...
## 2026-06-19 â€” feat: backup-reminder cadence selector (B28, docs for f9ac390)

- Code shipped in f9ac390 (code-only, docs deferred because the parallel session held CHANGELOG/DEVLOG
  mid-commit). Settings â†’ Data "Backup reminders" select (Monthly/Weekly/Off) persisted to localStorage
...
## 2026-06-19 â€” feat: backup reminders live end-to-end (B28)
...
- Added notify.backupTitle/backupBody/backupBodyNever i18n keys. Deferred only the per-cadence Settings
  selector (Off/Weekly/Monthly) â€” a refinement; the reminder works on the monthly default now.
...
## 2026-06-19 â€” refactor: unify anomaly detection (remove duplicate)

- While exploring the Insights screen I found `internal/insights.Detect` â€” the canonical category-spend anomaly
...
- Lesson: grep for existing packages before adding a new detector â€” full-repo knowledge isn't a given in a
...
## 2026-06-19 â€” feat: net-worth change on Reports (B21)

- Added a "Change this period" stat to the Reports net-worth card: the last step of the existing `nwSeries`
  (nwSeries[n-1] âˆ’ nwSeries[n-2]), color-cued via accentFor. Shown when the series has â‰¥2 points. UI-only,
...
## 2026-06-19 â€” feat: one-click duplicate cleanup (imports)
...
- Made the duplicate-transaction notice actionable: added a `selectDuplicates` UseEvent that builds a selection
  ...
  bulk-action bar's delete handles removal â€” no new delete plumbing needed. The notice is now a flex row with a
...
## 2026-06-19 â€” feat: duplicate-transaction heads-up (imports)
...
- New pure `internal/dedupe` package: `FindDuplicates` groups non-transfer transactions sharing the same
  ...
  accidental double entry); `Count` returns removable extras (sum of group size âˆ’ 1). Exact amount match only
...
- Table tests: normalized-desc match, date/amount discrimination, transfer exclusion, triples (3â†’2 removable),
...
## 2026-06-19 â€” feat: spending anomaly heads-up (B21)
...
- Added pure `reports.SpendingAnomalies(txns, now, months, overPct, minMinor, rates)`: per category, compares
  ...
  young category isn't diluted) and flags those over by â‰¥ overPct AND â‰¥ minMinor absolute, biggest overage
...
- Surfaced as a "Heads up" card on Reports (top 3): "%s is %d%% above its usual." Placed the computation after
  `nameOf` is in scope (first attempt referenced it too early â€” moved it). Added reports.headsUp/anomaly keys.
...
## 2026-06-19 â€” feat: minimum-payment guidance on Planning (D9)
...
- Aside: confirmed backup-reminder wiring stays deferred â€” recording lastBackupAt lives in app/settings.go
...
## 2026-06-19 â€” feat: no-spend days on Reports (B21)
...
## 2026-06-19 â€” feat: wire large-transaction alerts into catch-up (B19)
...
- Backup reminders remain deferred: they need a persisted `lastBackupAt` (updated by the export flow) that
  doesn't exist yet â€” without it there's nothing to judge the cadence against.
...
## 2026-06-19 â€” feat: large-transaction notifications (B19)
...
- Completed the notify event coverage for `EventLargeTransaction` (the constant existed but had no generator or
  ...
  base-currency expense magnitude â‰¥ threshold, on/after `since` (so the caller scopes it to the gap since last
...
- Added a `default-large` rule (threshold `defaultLargeTxnMinor` = 50000 = $500) and updated defaults_test
  (5â†’6 rules, added the event to the coverage list + a threshold assertion).
...
## 2026-06-19 â€” feat: settle-up CSV export (B24)
...
- Added pure `split.CSV(transfers, name, amount)` (From,To,Amount header; callback-based name/amount, FX/format
  ...
  `[]split.Transfer` alongside the on-screen owes list (sharerâ†’payer, share) and feeds it to split.CSV.
...
## 2026-06-19 â€” feat: budget limit suggestions from history (D6)
...
## 2026-06-19 â€” feat: headline spending trend on Reports (B21)
...
## 2026-06-19 â€” feat: subscriptions' share of spending (B25)
...
## 2026-06-19 â€” feat: biggest deposits (B21)
...
## 2026-06-19 â€” feat: annual bill cost on the Bills screen (B22)

- Added a "Per year" stat to the Bills grid: the already-computed (FX-converted) monthly bill total Ã— 12.
  Bills are monthly recurring (Upcoming derives one occurrence per liability account), so Ã—12 is the right
  annualization. UI-only â€” kept the sum screen-side because each bill's amount is FX-converted before summing
  (the bills package stays currency-agnostic via callbacks). Added the bills.annualCost i18n key. gofmt clean,
  i18n tests + wasm build green. Restored docs from HEAD first.

## 2026-06-19 â€” feat: net-worth breakdown on Reports (B21)
...
- Added a "Net worth" stat card (assets / liabilities / net, toned pos/neg/accent) to the Reports screen above
  ...
  are any accounts. UI-only â€” no new pure logic or i18n keys (reused dashboard.netWorth, accounts.assets,
  dashboard.liabilities). gofmt clean, wasm build green. Restored docs from HEAD first.

## 2026-06-19 â€” feat: goal on-track / pace check (D12)
...
- Added pure `goals.OnTrack(goal, monthly, from) (onTrack, known, err)` plus `OnTrack`/`PaceKnown` fields on
  ...
  complete â†’ on track; no target date or no usable projection â†’ known=false. Evaluate derives the fields from
...
- Did NOT surface on the Goals screen this pass: that screen computes progress directly (not via Evaluate) and
  has no stored per-goal contribution rate to judge against â€” wiring it needs a contribution input / product
...
## 2026-06-19 â€” feat: proportional mode on the Split screen (B24)

- Surfaced `split.ByWeights` on the Split screen behind a "Split by weight" toggle. Added `weighted` +
  `weights` (per-member string map) state; in weighted mode shares come from ByWeights (blank weight â†’ 1, so a
...
## 2026-06-19 â€” feat: weighted expense split (B24)
...
- Kept int64 throughout (documented the `total*weight` overflow assumption â€” fine for household sums). Table
...
## 2026-06-19 â€” feat: downloadable income & member breakdowns (B21)

- Added Download CSV buttons to the Reports "Income by source" and "Spending by member" cards. Income reuses
  the existing `reports.CategoryCSV` (income rows are CategorySpend with Amount only â†’ Prior/Change blank). For
...
## 2026-06-19 â€” feat: income by source (B21)
...
- Renamed the test helper `income`â†’`incomeTxn` (collided with an existing `income` in the package tests).
...
## 2026-06-19 â€” feat: debt payoff strategy comparison on Planning (D9)
...
## 2026-06-19 â€” feat: debt snowball / avalanche planner (D9)
...
- Viability: rejects negative extra / non-positive budget, and returns ok=false the first month total balance
  fails to fall (interest outpaces the budget) â€” plus the existing maxMonths cap. Tracks payoff Order, and a
  conservation invariant (TotalPaid == principal + TotalInterest) is asserted in tests.
- Table tests: snowball vs avalanche pick different focuses (and avalanche pays â‰¤ interest â€” the optimality
...
## 2026-06-19 â€” feat: spending-by-weekday insight (B21)
...
## 2026-06-19 â€” feat: spending by member (B21)
...
## 2026-06-19 â€” feat: cash runway on the Reports screen (B21)
...
- Gated the stat on `burn > 0` so a no-spend dataset doesn't render a misleading "0 months". Added
  `accentForRunway` (under 3 mo â†’ warning, 6+ â†’ positive) and the `reports.runway`/`reports.runwayMonths`
  i18n keys.
...
## 2026-06-19 â€” feat: financial runway estimator (B21)
...
- Edge handling: non-positive burn â†’ Sustainable (balance never depletes); non-positive balance with burn â†’
  zero runway; leftover prorated into days against a 30-day month (always 0â€“29, no overflow at realistic
  amounts). Table tests cover all branches + an end-to-end averageâ†’runway.
...
## 2026-06-19 â€” feat: budget pace warning on screen (D2)
...
- Surfaced `budgeting.ProjectPace` on the Budgets screen. In the per-budget evaluation loop I now also call
  ...
  â€” but only when `!OnTrack && 0 < Elapsed < 1 && state != StateOver`, so a finished period (Elapsed clamps to
  1 â†’ projected = actual) or an already-over budget doesn't show a redundant warning. Using real now (not the
...
- Restored CHANGELOG/DEVLOG from HEAD again first â€” the parallel session had re-truncated both to 0 bytes
  (now a recurring pattern; see the hazard memory). Verified size before committing. gofmt clean, i18n tests +
  wasm build green.

## 2026-06-19 â€” feat: show subscription price changes on screen (B25)
...
## 2026-06-19 â€” feat: subscription price-change detection (B25)
...
- Added `subscriptions.DetectPriceChanges` to catch when a recurring charge's price changes. The existing
  ...
  subscriptions â€” no good for this. The new detector groups by name only, sorts each series by date, confirms
...
- Documented the heuristic limit: it reports the latest distinct-amount transition, so usage-based/fluctuating
  charges can be noisy â€” but the cadence gate filters most of those out.
- Fixed a builtin shadow (`new` â†’ `cur` param) and a missing `domain` import in the new test file. Table
...
- Logic-first per the SDLC; surfacing it on the Subscriptions screen ("â–² $2/mo since April") is a follow-up.
  Path-scoped commit per the parallel-tree hazard memory.

## 2026-06-19 â€” feat: budget pace projection (D2)

- Added `budgeting.ProjectPace(status, start, end, now) Pace` â€” forecasts end-of-period spend by linear
  extrapolation (spent Ã· elapsed-fraction), the forward-looking complement to Status. Returns Elapsed,
...
- Design choices: recover the limit as `Spent + Remaining` so no `currency.Rates` is needed and the currency
  ...
  is noisy (one big day-one purchase projects huge) â†’ present as a gentle heads-up, not a hard prediction.
...
## 2026-06-19 â€” feat: wire backup reminders into notifications (B28)

- Bridged the B28 backup cadence core into the B19 notification system. Added `notify.EventBackupDue` plus a
  `default-backup` rule in `DefaultRules` (enabled, in-app), and updated `defaults_test.go` (4â†’5 rules,
...
## 2026-06-19 â€” feat: budget rollover & sinking-fund math (B26)
...
- Added pure `internal/budgeting/rollover.go`: `Carryover(prevRemaining, limit)` (advance one period â€” feed
  ...
  rather than a few cents short), `SinkingFundAccrued` (contribution Ã— made, capped at target so the rounded
  final period doesn't overshoot, with currency + overflow guards), and `SinkingFundProgress` (capped 0â€“100).
- Did the int64 arithmetic directly on `Money.Amount` and re-wrapped via `money.New` â€” the money package has
...
## 2026-06-18 â€” feat: Split rail icon (B24)
...
- Added a Lucide "split" `icon.Split` and wired `/split` into `railMeta` (`nav.split`), so the Split screen
  ...
  (19â†’20). Completes the dedicated rail icons for all four new Tools screens (Reports, Subscriptions,
...
## 2026-06-18 â€” feat: Split-a-shared-expense calculator screen (B24)
...
- Surfaced the pure `internal/split` core as a self-contained `screens.Split()` (`/split`, Tools group):
  ...
  owes their portion). No persistence or domain Split model needed â€” a handy household calculator that also
...
## 2026-06-18 â€” feat: Reports savings-rate trend (B21)

- Added pure `reports.SavingsRateSeries(txns, bounds, rates)`: the whole-percent savings rate per bucket
  (reuses `IncomeExpenseSeries` + `PeriodFlow.SavingsRate`, so it matches the dashboard â€” 0 with no income,
  negative when overspending). Table-tested incl. the +50% / âˆ’50% / no-income cases.
- Wired a "Savings-rate trend" `ui.AreaChart` on the Reports screen over the same six-period `bounds` used
  by the cash-flow/net-worth charts (ints â†’ float64; the chart handles negatives). New
...
## 2026-06-18 â€” feat: Reports largest-expenses report (B21)

- Added pure `reports.LargestExpenses(txns, start, end, rates, n)`: the period's biggest individual
  expenses, largest first (ties â†’ most recent date, then description). Same conventions as the other
...
## 2026-06-18 â€” feat: weekly digest reminder â€” all four events live (B19, step 10)
...
- **All four recommended Phase-A notification events now fire end-to-end**: stale-balance, bill-due,
  budget-threshold, and the weekly digest â€” each idempotent via the delivered log, surfaced as the single
...
## 2026-06-18 â€” feat: budget-threshold reminders in the catch-up (B19, step 9)
...
- Extended `runNotifyCatchUp` with the budget-threshold event. New `currentBudgetStatuses(app, now)` helper
  ...
  the statuses to `notifyfeed.BudgetCandidates` â†’ near (warning) / over (critical) candidates.
...
## 2026-06-18 â€” feat: wire notify catch-up on load (B19, step 8 â€” live!)
...
- Wired the pure notify pipeline into the wasm shell: new `internal/app/notifyrun.go` (`runNotifyCatchUp`)
  ...
  `notify.CatchUp(notify.DefaultRules(), â€¦)` against a delivered log persisted in localStorage
...
- Scope of this first wiring: stale-balance + bill-due (need only accounts). Budget-threshold needs the
  per-period budget-status assembly and the digest needs the period summary â€” both follow, as does the
...
## 2026-06-18 â€” feat: notify.DefaultRules â€” recommended rule set (B19, step 7)

- Added `notify.DefaultRules()` â€” one Rule per supported event (bill-due, budget-threshold, stale-balance,
...
- This is the last pre-wiring pure piece: the wasm shell can now seed rules from `DefaultRules()`, persist
  them + lastSeenAt + the delivered log, run the notifyfeed generators â†’ `notify.CatchUp` on open, and
...
## 2026-06-18 â€” feat: notifyfeed â€” digest event evaluator; event set complete (B19, step 6)
...
- That completes the four recommended Phase-A events â€” stale-balance, budget-threshold, bill-due, digest â€”
  ...
  center + browser Notifications â€” collision-prone appstate/UI work I'm leaving for a focused pass.
- SW cache rolls to v100. wasm build green, gofmt + go vet clean; full native suite green this iteration.

## 2026-06-18 â€” feat: notifyfeed â€” bill-due event evaluator (B19, step 5)

- Added `notifyfeed.BillDueCandidates(ruleID, upcoming, withinDays, now, text)`: from `bills.Upcoming`
  output, emits a `notify.Candidate` per bill due within `withinDays` (non-positive â†’ 7-day default).
  Keyed `<accountID>@<due-date>` so each due occurrence fires once (idempotent across opens) and the next
  cycle's date is a fresh key. Due today/tomorrow â†’ critical, else warning. Title/body via the `text`
...
## 2026-06-18 â€” feat: notifyfeed â€” budget-threshold event evaluator (B19, step 4)

- Added `notifyfeed.BudgetCandidates(ruleID, statuses, now, text)`: maps `budgeting.Status` values whose
  `State` is Near or Over into `notify.Candidate`s (Over â†’ critical, Near â†’ warning). Keyed
  `<budgetID>:<state>@<month>` so near and over are *distinct* occurrences â€” a budget that worsens from
...
- Takes pre-computed statuses (not budgets+txns) so notifyfeed needn't pull the whole ledger â€” the caller
...
## 2026-06-18 â€” feat: notifyfeed â€” stale-balance event evaluator (B19, step 3)
...
## 2026-06-18 â€” feat: Bills CSV export (B22)
...
- Wired a "Download CSV" button into the Bills list card (shown when there are bills) via `downloadBytes`
  â†’ `bills.csv`, with a per-bill base-currency conversion in the formatter. Completes CSV export across all
  three new Tools screens (Reports, Subscriptions, Bills). New `bills.downloadCsv*` keys; wasm build green.

## 2026-06-18 â€” feat: Subscriptions CSV export (B25)
...
- Added pure `subscriptions.CSV(subs, amount)` (encoding/csv): header + name, cadence, charge, normalized
  ...
  Table-tested incl. the monthlyâ†’annual (Ã—12) and yearlyâ†’monthly (Ã·12) normalizations.
- Wired a "Download CSV" button into the Subscriptions list card (shown when there are subscriptions) via
  the existing `downloadBytes` â†’ `subscriptions.csv`. New `subs.downloadCsv*` keys. Mirrors the Reports
  CSV export for consistency. wasm build green, gofmt clean.

## 2026-06-18 â€” feat: Reports CSV export (B21)

- Added pure `reports.CategoryCSV(rows, name, amount)` â€” builds the spending-by-category report as CSV
...
## 2026-06-18 â€” feat: Reports top-payees report (B21)
...
## 2026-06-18 â€” refactor: dashboard bills widget reuses bills.Upcoming (B22)

- The dashboard "Upcoming bills" widget had its own inline liabilityâ†’bill derivation (a local struct +
...
## 2026-06-18 â€” feat: split / settle-up pure core (B24, step 1)

- New pure `internal/split` (no syscall/js, table-tested): `Equal(total, members)` even split with exact
  remainder distribution (1000Â¢/3 â†’ 334/333/333, shares always sum to total); `Expense` + `NetBalances`
  (credit the payer the total, debit each participant their share â†’ per-member net, sums to zero);
  `SettleUp(balances)` greedy debtorâ†”creditor matching (descending, ties by id) yielding a small,
...
- Tests: even-split rounding (incl. exact-sum invariant + nil for no members), net-balance computation
  and zero-sum, settle-up correctness (apply transfers â†’ everyone at zero) and the simple two-party chain.
...
## 2026-06-18 â€” feat: Bills month-calendar view (B22)
...
## 2026-06-18 â€” feat: bills month-calendar layout helper (B22)

- Added `bills.MonthCalendar(bills, year, month, weekStart) [][]CalendarDay` â€” the pure month-grid layout
...
## 2026-06-18 â€” feat: bill payment reminder â†’ to-do (B22)
...
- Deferred mark-paid: per the spec it "creates/links a transaction" + persists paid-this-cycle status â€”
...
## 2026-06-18 â€” feat: Bills rail icon (B22)
...
- Added a Lucide calendar `icon.Bills` and wired `/bills` into `railMeta` with the `nav.bills` key, so the
  ...
  (18â†’19). Native icon test green, wasm build green.
...
## 2026-06-18 â€” feat: Bills screen â€” wire the B22 core into the UI (B22, step 2)
...
## 2026-06-18 â€” feat: bills tracker â€” pure core (B22, step 1)
...
- New pure `internal/bills` (no syscall/js, table-tested), derived-first per the spec's recommendation:
  ...
  `DueDayOfMonth` and a non-zero `MinPayment` â€” amount, next due date, and days-until â€” soonest first.
...
- Tests: NextDue (this-month / today / passedâ†’next / Feb clamp non-leap+leap / 31-day month / year
...
## 2026-06-18 â€” feat: subscription cancel-reminder â†’ to-do (B25)

- Added the B25 "cancel reminder â†’ task" action. Extracted a `SubscriptionRow` component (owns its click
  ...
  toasts a confirmation through the shared `uistate.UseNotice()` atom. No B19 dependency â€” reuses the
  existing to-do system.
- New `subs.{remind,remindTitle,reminderTitle,reminderNote,reminderAdded}` i18n keys. wasm build green,
  gofmt clean.

## 2026-06-18 â€” feat: Reports net-worth trend chart (B21)
...
- Added a second sparkline to the Reports screen: net worth as of each of the same `bounds` boundaries
  ...
  its own gradient id, shown only when there are â‰¥2 points. Reused the `dashboard.netWorthTrend` label.
  No new logic â€” pure reuse of the ledger series + the chart shim. wasm build green, gofmt clean.

## 2026-06-18 â€” feat: Subscriptions rail icon (B25, step 3)
...
- Added a Lucide "repeat" `icon.Subscriptions` and wired `/subscriptions` into `railMeta` with the
  ...
  Updated the icon curated-set test (17â†’18). gofmt realigned the (now wider) `railMeta` and icon maps.
  Native icon test green, wasm build green.

## 2026-06-18 â€” chore: route screen registry through i18n (copy pass, file 4)

- The deferred `internal/screens/screens.go` registry: every Route hardcoded Label, Title, and Subtitle in
  English, displayed raw by the shell (page heading + subline) â€” the last real i18n gap in the UI.
...
- This closes the UI i18n sweep: remaining hardcoded strings are AI system prompts (English by design) and the
  "CashFlux" brand wordmark (handled by the rebrand backlog Â§7, not i18n).

## 2026-06-18 â€” chore: route dashboard empty states through i18n (copy pass, file 3)
...
- Scan note: insights.go / allocate.go "hardcoded" strings are AI **system prompts** (prompt engineering), not
  display copy â€” intentionally left in English; localizing them would degrade AI output. The rest of the UI is
  already i18n'd. Remaining genuine gap is the `screens.go` route registry (deferred, needs a key-based refactor).

## 2026-06-18 â€” chore: route lock-screen greeting through i18n (copy pass, file 2)
...
- Deferred `internal/screens/screens.go`: its route registry hardcodes every screen's Label/Title/Subtitle and
  is consumed in several places (app.go Shell props, shell.go rail) â€” needs a registry-level i18n refactor
  (store keys, resolve centrally), not a quick literal swap. Flagged for a dedicated pass.

## 2026-06-18 â€” chore: route settings.go strings through i18n (copy pass, file 1)
...
- Added `settings.*` keys to `internal/i18n/en.go` and replaced the literals with `uistate.T(...)`. Copy
  ...
  freshness hint reads "days Â· 0 means never" (clearer than the parenthetical). Error toasts keep the existing
  "Couldn't â€¦ : %s" voice for consistency.
...
## 2026-06-18 â€” feat: Subscriptions screen â€” wire the B25 core into the UI (B25, step 2)
...
- Gotcha: `cadenceLabel` already exists in planning.go (for `domain.RecurringCadence`) â€” renamed mine to
...
## 2026-06-18 â€” feat: subscriptions detection â€” pure core (B25, step 1)
...
- New pure `internal/subscriptions` (no syscall/js, table-tested): `Detect(txns, rates, minCount)` finds
  ...
  (weekly 6â€“8d, monthly 26â€“33d, yearly 350â€“380d; anything else is not a subscription). Each result carries
...
- Used the median (not mean) gap so a single off-cycle charge doesn't break detection, and required â‰¥2
...
## 2026-06-18 â€” feat: Reports rail icon (B21, step 7)
...
- Reports already auto-appeared in the rail (navGroup shows any registered Tools screen, B7) but with the
  ...
  `railMeta` with the `nav.reports` key. Updated the icon package's curated-set test (16â†’17). Native icon
  test green, wasm build green, gofmt clean.

## 2026-06-18 â€” feat: Reports cash-flow trend chart (B21, step 6)
...
- Added a cash-flow trend to the Reports screen using the existing pure `ui.AreaChart` sparkline (no D3
  ...
  `reports.IncomeExpenseSeries`, and plots each bucket's `Net()`. Shown only when there are â‰¥2 points.
...
## 2026-06-18 â€” feat: Reports screen â€” wire the reports core into the UI (B21, step 5)
...
- The UI-last step for the first reports: a new `screens.Reports()` (`reports_screen.go`) registered as
  ...
  green â–¼ / red â–² percent-change badge. Category rows are plain text (no On* hooks), so the loop is safe.
...
## 2026-06-18 â€” feat: reports â€” top movers + narrative DRY (B21, step 4)

- Added `reports.TopMovers(rows, n)`: ranks compared categories by absolute change vs the prior period
  (largest first, ties by id, n<=0 = all), excluding unchanged/no-delta rows â€” the "top movers" catalog
...
## 2026-06-18 â€” feat: reports â€” deterministic spending narrative (B21, step 3)

- Added `reports.SpendingNarrative(rows, compared, format, name)` â€” the "narrative descriptions" piece of
  B21, template-based (no AI) so identical numbers always yield identical text. Headline (total + category
  count with correct singular/plural), biggest expense, and â€” when comparing â€” the single biggest mover
  vs the prior period (`topMover` = largest absolute change, with rose/fell + signed %). Stays decoupled
  via `format`/`name` callbacks (empty/unknown name â†’ "uncategorized"). Zero-spend (incl. all-dropped-to-
...
## 2026-06-18 â€” feat: reports â€” income-vs-expense / cash-flow (B21, step 2)
...
## 2026-06-18 â€” feat: reports engine â€” pure core (B21, step 1)
...
- Started the approved B21 reports engine bottom-up per the SDLC: a pure `internal/reports` (no
  ...
  priorEnd, rates)` â†’ `[]CategorySpend` sorted largest-first, with optional prior-period comparison
...
- Tests cover sorting, range/income/transfer exclusion, the headline total, and the comparison cases
  (increase %, drop-to-zero -100%, new category with no baseline â†’ HasDelta false). `go vet` clean. Pure
  package, not yet imported â€” no behavior change; the Reports nav/screen + richer chart kinds come next.
...
## 2026-06-18 â€” feat: notify catch-up engine (B19 Phase A, step 2)

- Added `notify.CatchUp(rules, candidates, now, log) â†’ []Notification` â€” the event-agnostic heart of
  catch-up-on-wake. A `Candidate` is a potential occurrence the (future) per-event evaluators emit;
  CatchUp gates by rule (exists + enabled + â‰¥1 channel), drops already-delivered occurrences (the
  DeliveredLog), and applies each rule's `FrequencyCap` (keep the most recent N, collapse the rest), then
  marks *every* fresh occurrence considered â€” including capped-out ones â€” as delivered so a long gap
  collapses and never replays. Results are newest-first with a key tiebreaker for determinism.
- Design call: quiet hours are NOT applied in CatchUp â€” a summary shown when the user actively opens the
...
- Keeping the engine candidate-driven means the still-open "which events ship first" decision lives in
  the per-event candidate generators, not here â€” so this layer is buildable and testable now. Table tests
  cover gating, idempotency (re-run â†’ empty), frequency-cap collapse, newest-first order, and nil-log
  safety. `go vet` clean. Still no UI / not yet wired into the shell.

## 2026-06-18 â€” feat: notify package â€” pure notification core (B19 Phase A, step 1)
...
- Started the approved B19 Phase A (client-only notifications) bottom-up per the SDLC: model + logic +
  ...
  - Types: `Channel` (in-app/browser; email/SMS are deferred Phase B and absent here), `Event` (the
    recommended first slice â€” bill-due, budget-threshold, goal-milestone, stale-balance, large-txn,
  ...
  - Logic: `Rule.HasChannel`, `Rule.InQuietHours` (half-open window, handles past-midnight wrap),
    `Rule.CanFireAt` (enabled âˆ§ has-channel âˆ§ not-quiet), `DedupeKey` + `Day/Week/MonthKey` occurrence
    keys, and a `DeliveredLog` map for idempotent catch-up. All table-tested; `go vet` clean.
- Deliberately did NOT build event-specific evaluation or `CatchUp` yet â€” the TODO leaves "which events
...
## 2026-06-18 â€” a11y: label the top-bar "Jump toâ€¦" select (C47 tail)

- Closed the last concrete C47 gap in the app chrome: the resolution control's "Jump toâ€¦" `<select>`
  (shell.go) had only a `title`, no programmatic name. Added `aria-label`. Audited the rest of the top
  bar â€” the segmented Week/Month/Quarter control and the from/to steppers are labeled buttons, and the
  jump-to select was the only bare control â€” so the dashboard chrome is now clean.
- While here, confirmed C41's "one-tap This period reset" (asked for in #64) already exists in the
  ...
  already un-clips the "+ Add" menu (C43's remaining z-index portal is a separate, larger refactor â€”
  left for now). Build green.

## 2026-06-18 â€” fix: resolution switch re-anchors to now, not the window start (C41)

- `Window.SetResolution(r)` re-snapped `w.From`/`w.To` (the existing window start, which is â‰¤ now) to the
  new unit, so each switch drifted backward: Month "Jun" â†’ Week gave June's first week (May 31â€“Jun 6),
  Week â†’ Month gave May, Week â†’ Quarter gave Q1 â€” compounding into the past.
- Changed it to `SetResolution(r, now)` returning `NewWindow(r, now, weekStart)` â€” the single period that
  contains now (matching the "This period" preset that #64 confirmed works). Only caller is shell.go's
  resolution control (passes `time.Now()`). Rewrote `TestSetResolutionResnaps` â†’ `â€¦ReanchorsToNow`,
...
- The period engine itself was already correct (per C40/#61); this was purely the re-anchor policy on a
  resolution change. Native period tests pass; wasm build green. (C40 â€” Budgets-screen SPENT under
  Quarter â€” is a separate, still-open bug.)

## 2026-06-18 â€” fix: workflow Save folds in a pending action (C37)
...
- Root cause of the "Save workflow is a no-op" report: the action builder stages an action only on *Add
  ...
  existed via the `msg`/`role=alert` line â€” the report predated it â€” but the silent loss was the real bug.)
- Extracted `buildDraft()` (draft fields â†’ Action + ok), used by both *Add action* (now refuses an empty
...
## 2026-06-18 â€” a11y: label planning + settings controls â€” C47 complete
...
- Final C47 batch: the planning recurring-item selects (cadence/account/category) and all six settings
  ...
  visible text â€” added `aria-label` from the same key (kept the visible div as the visual label; no double
...
- Dashboard's audit-reported "2" controls: the dashboard's interactive chrome (resolution control) is
  ...
  dashboard.go â€” treating C47 as complete for the form-control gap. Build green, gofmt clean.

## 2026-06-18 â€” a11y: label budgets/goals/accounts form controls (C47, batch 2)
...
## 2026-06-18 â€” a11y: label transactions form controls (C47, batch 1)
...
## 2026-06-18 â€” feat: dashboard tiles drill into their data screen (C30)
...
- Implemented centrally in the `ui.widget` shell via a `widgetRoute(id)` map + a `viewTitle` sub-component
  (its own click hook, like `gearButton`, per the On*-hooks-in-loops rule) â€” one file, instead of
...
- Deferred (noted in C30): deep-linking with a pre-applied filter (e.g. Spending â†’ Transactions filtered
  to expenses for the period) â€” needs the transactions screen to accept filter query params.

## 2026-06-18 â€” fix: top bar wraps instead of scrolling (C34)

- `.topbar` had `overflow-x: auto`, so at the awkward ~1000â€“1100px band (esp. Custom-range mode, which
...
## 2026-06-18 â€” feat: empty-state CTAs for transactions + categories (Â§6.5, batch 2)

- Extended `EmptyStateCTA` to transactions (truly-empty case only â€” the filtered no-match case keeps a
...
- That covers the meaningful "add your first" empty states. Remaining bare empties (artifacts, custom
  ...
  add-form to jump to â€” left as plain lines intentionally. Build green, gofmt clean.

## 2026-06-18 â€” feat: empty-state call-to-action blocks (Â§6.5, batch 1)

- Empty lists were a bare `P(Class("empty"), â€¦)` line. Added a reusable `EmptyStateCTA` component
  (`internal/screens/emptystate.go`): centered message + a primary button that calls `focusByID` on the
  screen's add form (reusing the Â§6.7 plumbing). Add-form first inputs got stable ids (`goal-add`,
...
- Made it a component (not a helper returning a node with an inline OnClick) so the click-handler hook
  lives inside its own lifecycle â€” safe to mount/unmount as the list toggles emptyâ†”non-empty, rather than
...
## 2026-06-18 â€” feat: focus-on-edit for todo/rules/documents/custom pages â€” Â§6.7 complete

- Final batch of the Â§6.7 focus-on-edit feature: `TaskRow`, `RuleRow`, `DraftRow`, and the custom-page
  `editWidgetForm`. Same open/closed-keyed `ui.UseEffect` â†’ `focusByID` on the first input.
...
- Â§6.7 now covers every inline editor in the app. Build green, gofmt clean.

## 2026-06-18 â€” feat: focus-on-edit for categories + members (Â§6.7 cont.)

- Same pattern applied to `CategoryRow` and `MemberRow`: open/closed-keyed `ui.UseEffect` â†’ `focusByID`
...
## 2026-06-18 â€” feat: focus-on-edit for transactions + budgets (Â§6.7 cont.)

- Extended the Â§6.7 focus-on-edit pattern to `TransactionRow` and `BudgetRow`: a `ui.UseEffect` keyed on
...
## 2026-06-18 â€” feat: focus the first field when an inline editor opens (Â§6.7, goals + accounts)
...
- Gotcha: in the `screens` package `ui` is the framework `GoWebComponents/ui` (hooks), not `internal/ui`.
  First put the helper in `internal/ui` (`ui.FocusByID`) â€” undefined at the call sites. Moved it to a
...
## 2026-06-18 â€” feat: complete custom-page widget management (reorder/resize/edit)
...
- Closed the last custom-page gaps from the plan: widgets were add/delete only. Added, in `custompage.go`:
  ...
  - **Resize:** â†” / â†• header buttons cycle width (max 4) / height (max 3) via `dashlayout.CycleSpan` +
    `ResizeItem`. `ensureLayout` synthesizes default 1Ã—1 entries for widgets missing a layout row so
    reorder/resize always have something to operate on.
  - **Edit:** an âœŽ button toggles an inline `editWidgetForm` in the tile body â€” edit title + the type's
...
- All pure layout ops were already tested (`dashlayout`); browser-verified the tile chrome (grip/â†”/â†•/âœŽ/âœ•)
...
## 2026-06-18 â€” fix: hide the left-rail scrollbar (C31)
...
- The rail's `nav` (`overflow-y-auto`) had no scrollbar styling, so it showed the default OS bar once it
  ...
  â€” hidden but still scrollable by wheel/trackpad/keyboard. CSS-only (web/index.html). (Skipped the
...
## 2026-06-18 â€” fix: in-app "Set balance" form; remove the last native prompt (Â§6.8 complete)
...
- Converted Accounts "Set balance" to an inline form in `AccountRow` (settingBal state + amount input +
  ...
  so removed `promptText` + the now-unused `syscall/js` import from goals.go. Â§6.8 "replace native dialogs"
  is done â€” no `window.prompt` left in the screens. 1 new i18n key.
- gofmt clean, disk wasm build green (served wasm rebuilt). Committed by pathspec.

## 2026-06-18 â€” fix: in-app goal-contribute form (Â§6.8, branched off lock-screen)
...
- Gotcha: `promptText` was a SHARED helper â€” accounts.go's "Set balance" also calls it â€” so removing it broke
  the build (`accounts.go:435 undefined: promptText`). Restored `promptText` + the `js` import; accounts
  keeps it for now (its in-app form is the next Â§6.8 follow-up). 1 new i18n key.
...
## 2026-06-18 â€” feat: suspend/resume the lock without wiping creds (B17)
...
- Implemented the spec's "Lock screen toggle that flips Enabled without touching the credentials."
  ...
  lock keeps its passcode but never appears. `setLockSuspended` flips it; Settings â†’ App lock gains a "Lock
  screen" toggle + a "Paused" status, and hides "Lock now" while paused. Remove still fully clears.
- Table test covers activeâ†’suspend (not active, no auto-lock, still verifies passcode) and disabled.
  applock tests green, gofmt clean, disk wasm build green (served wasm rebuilt). Committed by pathspec.

## 2026-06-18 â€” feat: lock-screen unlock animation (B17.1)
...
## 2026-06-18 â€” feat: toggle lock-screen quote/metadata in Settings (B17.1)
...
## 2026-06-18 â€” feat: passcode hint (revealed after failed attempts) (B17)
...
## 2026-06-18 â€” feat: lock-screen greeting + date + daily quote (B17.1)

- Started the B17.1 lock-screen experience: new pure `internal/lockquotes` package (12 curated finance/
  motivation lines; `ForIndex` wraps deterministically â€” no randomness, table-tested for wrap/negative/
  ...
  privacy-safe â€” nothing financial. Hardcoded greeting strings for now (English; the quotes are English
  anyway) â€” i18n + Settings toggles + opt-in glanceable data are follow-ups.
...
- Re-engaged the broader backlog (454 open items) after the user flagged it â€” dropping the
  collision-avoidance over-caution; using pathspec commits to coexist with the parallel session.

## 2026-06-18 â€” fix: focus-trap the lock gate (B17 modality complete)

- Closed the residual from the previous commit: the unlock gate now traps Tab within its controls
  (input â†’ Unlock â†’ Forgot, wrapping at both ends via a keydown listener on the gate, mirroring
...
## 2026-06-18 â€” fix: lock gate suppresses global keyboard shortcuts (B17 hardening)

- Spotted a bypass: the unlock gate is a visual/click overlay, but the keyboard shortcuts are document-level
  listeners, so while "locked" you could still Alt+1â€“9 navigate or Cmd+K the palette (behind the gate).
  Added `appLockActive()` (gate exists + display != none) and made `wireKeyboardShortcuts` bail when it's
  true â€” the gate's own input listeners are on the gate elements, so passcode entry still works.
...
## 2026-06-18 â€” test: D13 forecast net-worth feed
...
## 2026-06-18 â€” test: ReassignOwner entity coverage
...
## 2026-06-18 â€” test: recurring cadence catch-up
...
## 2026-06-18 â€” test: rules retroactive apply path
...
## 2026-06-18 â€” test: formula/custom-field round trip
...
## 2026-06-18 â€” test: extract/CSV column mapping
...
## 2026-06-18 â€” test: config layering current contract
...
## 2026-06-18 â€” test: D15 cleared balance adjustment math
...
## 2026-06-18 â€” test: D18 net-worth rollups
...
## 2026-06-18 â€” test: D9 payoff final-month boundary
...
## 2026-06-18 â€” test: D12 goal pace projection
...
## 2026-06-18 â€” fix: make the multi-currency (FX) editor functional (D16)
...
- Discovered the Settings FX editor was a dead stub: base-currency `<select>` had no `OnChange`, `rateRow`'s
  ...
  no way to add one). The whole pipeline below it already worked â€” `store.Settings{BaseCurrency,FXRates}`
  exists, every screen builds `currency.Rates` from it, and `ledger.*` converts â€” so this was purely a
  broken editor UI.
- Fix (settings.go): `onBase` saves the base via `PutSettings`+bump; `setRate` writes/clears a rate;
  `rateRow` â†’ `fxRateRow` component (stable change hook) with an editable number input that commits on
...
## 2026-06-18 â€” test: D11 planning one-time end balance
...
## 2026-06-18 â€” test: D4 budget scope aggregation
...
## 2026-06-18 â€” feat: app-lock "Forgot passcode?" recovery (close lockout gap)

- A forgotten passcode previously meant permanent lockout (no recovery â€” the B17 spec called for one).
  Added a "Forgot passcode?" link on the unlock gate â†’ `confirmAction` â†’ `wipeAllLocalData()` (removes every
  `cashflux:*` localStorage key) â†’ reload (boot re-seeds a fresh sample). For a soft, unencrypted gate,
  wipe-to-reset is the honest recovery â€” "just remove the lock" would let anyone bypass it. 2 new
...
## 2026-06-18 â€” fix: surface app lock in Settings (user couldn't find it)

- User feedback: no way to set up / toggle the lock from Settings â€” it was only in the Cmd+K palette, which
  they didn't discover. Added a **Settings â†’ App lock** section (`appLockSection` in new
...
- Made the setup form refresh-aware: `showAppLockSetup(onDone func())` stores a package-level callback run
  ...
  (palette) passes nil. 5 new `applock.*` keys (section/hint/statusÃ—3, statusOnAuto uses %d).
- Had to touch shared `settings.go` (3 lines) â€” the collision I'd been avoiding, but the user explicitly
  wants it there. gofmt clean, worktree build exit 0, committed by pathspec.

## 2026-06-18 â€” test: D16 currency conversion edge coverage
...
## 2026-06-18 â€” fix: UX polish Â§6.6 â€” segmented control arrow keys
...
## 2026-06-18 â€” fix: UX polish Â§6.4 â€” workspace switcher divider spacing
...
## 2026-06-18 â€” fix: UX polish Â§6.9 â€” collapsed rail flyout clicks
...
## 2026-06-18 â€” feat: export-data commands in the command palette

- Added "Export JSON" / "Export CSV" to the Cmd+K palette (reusing the existing `exportJSON`/`exportCSV`
  helpers + the `settings.exportJSON/CSV` i18n keys â€” no new keys). New `paletteNotify` posts the
  helpers' success/error via the global Notice/toast atom (works outside a render). Export is read-only
  (download), so no refresh/reload plumbing needed â€” kept import/sample/wipe out of the palette since those
...
## 2026-06-18 â€” feat: in-app passcode setup form (B17 final; UX audit Â§6.8)
...
- Replaced the set-passcode native `prompt()`s with an in-app modal form (`showAppLockSetup`/
  ...
  keys (escaped via a small `escT` helper). This closes the Â§6.8 "replace native dialogs" item for app-lock
  and finishes B17 (the old `applock.setPrompt/confirmPrompt/autoPrompt/enabled` keys are now unused data â€”
  harmless, left in place).
- gofmt clean, worktree build at HEAD exit 0, committed by pathspec. B17 done: core+tests â†’ gate â†’ palette
  â†’ auto-lock â†’ i18n â†’ in-app form.

## 2026-06-18 â€” refactor: i18n the app-lock strings (B17 polish)
...
## 2026-06-18 â€” fix: UX polish Â§6.1 â€” delete button hit area
...
- Still in my/new files only (`applockgate.go` + one `app.go` line) â€” collision-free. gofmt clean, worktree
  build exit 0. The B17 lock is now functional end-to-end: set (with optional auto-lock) â†’ boots/locks/idles
  behind the gate â†’ unlock. Remaining: in-app passcode form (vs prompts) + i18n.

## 2026-06-18 â€” feat: passcode lock MVP (B17) â€” gate + palette management, collision-free
...
- Built the functional lock on top of the pure `applock` core, deliberately keeping it out of the parallel
  ...
  `internal/app/applockgate.go` (full-screen `var(--bg)` cover, password input, Enter/Unlock â†’ constant-time
  Verify â†’ hide); and management via the Cmd+K palette (adaptive: Set / Change / Lock now / Remove) rather
  than settings.go. Boot wiring is a single `maybeLockOnBoot()` line in `app.go`.
- MVP scope: lock-on-reload + manual "Lock now"; passcode set via native promptÃ—2 (confirm). Deferred:
  idle auto-lock (the pure `ShouldAutoLock` is ready), an in-app passcode form, focus-trap, and i18n of the
  app-lock strings (hardcoded for now â€” flagged).
...
## 2026-06-18 â€” feat: make workflows genuinely valuable (acted on a value critique)
...
- A product critic judged the workflow engine "a demo with one real button": txn-added conditions/actions
  ...
  - **formula engine:** `Env` gained `Strs` (string variables) and a `contains()` (case-insensitive) +
    `lower()` function â€” so conditions can match text, not just numbers.
  ...
  - Tests: per-txn Eval (amount + contains), txn-mutating Plan, and an appstate routing test (bistroâ†’
    Dining; big spendâ†’needs-review) + a notify-hook test. All green.
- **Verification rabbit hole worth recording:** the browser showed "no workflows" even with a fresh
  ...
  **JS backtick template literal**, which un-escaped the `\"` inside a condition string
  ...
  to the sample (a 1-member household â€” which mimicked "members load, workflows don't"). Switched the
...
## 2026-06-18 â€” feat: app-lock pure-logic core (B17 groundwork)

- The user kept re-invoking /loop (wants continuous output), and the isolated app/keyboard/workspace work
  is done â€” so I started B17 from the bottom with a NEW, zero-collision package `internal/applock`.
...
- Pure native package (no wasm, no served-asset change â†’ no SW bump); `go test ./internal/applock` green,
...
## 2026-06-18 â€” fix: UX polish Â§6.4 â€” selected transaction checkbox state
...
## 2026-06-18 â€” fix: UX polish Â§6.11 â€” light-theme soon badge
...
## 2026-06-18 â€” fix: UX polish Â§6.1 â€” form field target height
...
## 2026-06-18 â€” fix: UX polish Â§6.2 â€” segmented button label size
...
## 2026-06-18 â€” fix: UX polish Â§6.11 â€” accent swatch hit area

- Closed the settings accent-swatch target-size item in `web/index.html`: `.swatch` moved from 22Ã—22px to
  24Ã—24px, preserving the existing 6px radius and selection border.
- This is CSS-only because the shared `ui.Swatch` component already emits the `.swatch` class for accent
  choices.

## 2026-06-18 â€” refactor: i18n the command palette (Â§6.6, finishes the keyboard-UI pass)
...
## 2026-06-18 â€” refactor: i18n the keyboard help overlay (Â§6.6)
...
- Routed the `?` cheat-sheet through the i18n catalog per the engineering standards: `helpHTML` went from a
  ...
  `shortcuts.title`. (Command-palette chrome strings â€” search placeholder, no-match, action labels â€” remain
  a follow-up.)
- en.go is the hottest shared file; took a couple of Readâ†’Edit retries as the parallel session wrote it.
...
## 2026-06-18 â€” fix: UX polish Â§6.2 â€” priority badge legibility
...
## 2026-06-18 â€” fix: UX polish Â§6.4 â€” disabled button state
...
## 2026-06-18 â€” feat: theme + sidebar toggle commands in the palette

- Added "Toggle light / dark theme" and "Collapse / expand sidebar" to the Cmd+K palette â€” standard
  palette affordances. `toggleTheme` flips `prefs.Theme` (non-light â†’ dark) then Set/Persist/ApplyPrefs
  ...
  import); no en.go, no screen files â€” zero collision.
...
## 2026-06-18 â€” harden custom pages + workflows (acted on an adversarial test critique)
...
- Ran a critic subagent against the e2e suite; it found real gaps. Acted on them:
  - **M1 (real bug):** the txn-added trigger was wired into quick-add only. Centralized it in
    ...
    fire) â€” now every add path (inline editor, transfer, duplicate, CSV + image import) honors it. Added a
  ...
  - **M2:** `applyEffect` createTask is now idempotent â€” skips when an open task with the same title
...
## 2026-06-18 â€” fix: UX polish Â§6.3 â€” upcoming bill dates use preferences
...
## 2026-06-18 â€” feat: workspace management commands in the palette + HEAD health check

- Health check (repo is churning fast under the parallel session): `go test ./...` on disk showed 3
  appstate FAILs, but those are the parallel session's UNCOMMITTED WIP â€” committed HEAD passes appstate in
...
- Feature: added New / Export current / Import workspace commands to the Cmd+K palette (alongside the
  ...
  importWorkspace / pickFile / promptName) â€” so full workspace management is keyboard-reachable. Entirely
...
## 2026-06-18 â€” fix: UX polish Â§6.2 â€” rail section label legibility

- Closed the non-custom rail label part of Â§6.2 in `internal/app/shell.go`: rail section labels moved from
...
## 2026-06-18 â€” feat: switch workspaces from the command palette

- Enriched the Cmd/Ctrl+K palette with "Switch to workspace: <name>" entries (one per non-active
  workspace) that call `switchWorkspace(id)` â€” integrating two features I own, entirely within
...
- Build note: a full wasm build currently fails ONLY on the parallel session's `internal/screens/
  custompage.go` (unused imports â€” their mid-edit WIP); my `shortcuts.go` change has zero errors. Verified
  ...
  blocks `gwc dev`'s rebuild) â€” confirm in-browser once their build is green.

## 2026-06-18 â€” feat: Â§6.6 command palette (Cmd/Ctrl+K) â€” the capstone keyboard item

- The remaining [H] Â§6.6 item. Self-contained DOM overlay in `shortcuts.go` (same pattern as the help
  overlay): a search input + a filtered result list, built once and toggled via `display`.
- Commands = every screen from `primaryNav()`/`toolsNav()`/`systemNav()` (â†’ `router.Navigate(path)`) plus
  two actions (Add a transaction â†’ `UseQuickAdd().Set(true)`; Keyboard shortcuts â†’ `toggleHelpOverlay`).
- Keyboard model: type to substring-filter; â†‘/â†“ wrap-move the highlight (re-styling rows + scrollIntoView);
...
- gofmt clean, wasm build exit 0, probe ok/200. Can't drive Cmd+K + typing headlessly â€” logic-verified;
  confirm in-browser. Â§6.6 keyboard set complete: Alt+1..9, Alt+N, Enter, ?, Cmd/Ctrl+K.

## 2026-06-18 â€” feat: Â§6.6 quick-add hotkey (Alt+N)

- Alt+N opens the quick-add transaction panel by setting `uistate.UseQuickAdd().Set(true)` from the global
  keydown handler. Confirmed safe: `UseAtom` â†’ `runtime.GoUseAtomGlobal` is a global id-keyed atom (not
...
- Chord choice: the audit suggested Ctrl/Cmd+Shift+A, but that's reserved (Chrome tab-search, Firefox
  add-ons). Used Alt+N instead â€” conflict-free and consistent with the Alt+1..9 family. Added to the `?`
  overlay. Lives entirely in `shortcuts.go` (+uistate import); low collision while the parallel session
  works other Â§6 items.
- gofmt clean, wasm build exit 0, probe ok/200. Keystroke can't be sent headlessly â€” logic-verified.

## 2026-06-18 â€” fix: UX polish Â§6.1 â€” rail nav hit areas

- Closed the Â§6.1 rail-nav touch-target item in `internal/app/shell.go`: all `navItem` variants now carry
  `min-w-10 min-h-10`, so expanded and collapsed rail items keep a stable 40Ã—40px minimum hit area.
...
## 2026-06-18 â€” fix: Â§6.9 toast â€” error notices linger + labelled dismiss

- `toast.go` already had the role/aria-live work (status/polite for info, alert/assertive for errors, plus
  a persistent live region). Closed the remaining Â§6.9 item: split the auto-dismiss timeout â€”
  `toastErrTimeoutMS` 7500ms for errors vs 4500ms for ordinary notices â€” so error text isn't whisked away
  before it's read. Added `aria-label="Dismiss"` to the close button (Â§6.9 [L], to go with its title).
...
- Process: committing via `git commit -- <paths>` now (not `git add` then commit) so the parallel session's
  pre-staged files don't get swept into my commit â€” last time my "?" overlay commit absorbed their
  en.go/allocate.go because `git commit` writes the whole shared index.
- Â§6 progress (mine): 6.1, 6.2, 6.3, 6.4(switcher), 6.6(Alt+1..9/Enter/?-overlay), 6.9(toast), 6.11.

## 2026-06-18 â€” test: 10 user stories e2e for custom pages + workflows
...
- Wrote `docs/CUSTOM_PAGES_STORIES.md` (10 stories + acceptance + edge cases) and
  ...
  artifacts (CSV parse â†’ table rows, image data URL), page organize (reorder/hide/unique re-slug),
  ...
  exportâ†’import round trip). All green.
- Browser e2e: seeded a multi-page workspace and screenshotted a stress page with all six widget types
  ...
  the hidden-pages section, and inter-page navigation â€” all correct.
- Bug found + fixed: currency KPIs truncated `valueÃ—100` to int, so float error could drop a cent
  ($15,343.50 â†’ $15,343.49). Now `math.Round`. No other defects surfaced.

## 2026-06-18 â€” fix: UX polish Â§6.10 â€” custom-field key validation
...
## 2026-06-18 â€” feat: Â§6.6 "?" keyboard help overlay (self-contained DOM)

- Added `?`-to-toggle (and Esc/âœ•/backdrop-to-close) a shortcuts cheat sheet, documenting the Alt+1..9,
  Enter, Esc, and Shift bindings. Built as a pure-DOM overlay created/toggled entirely in `shortcuts.go`
  (createElement + innerHTML + display toggle) â€” deliberately NOT a framework component, so it needs no
  shell mount point and stays zero-collision while the parallel session churns Â§6 (it just shipped Â§6.10).
- Wiring: extended the existing global keydown handler â€” Esc calls `closeHelpOverlay` (no-op if closed;
...
- English strings hardcoded for now (consistent with FlipPanel's hardcoded Save/Cancel); noted an i18n
  ...
  exercised headlessly â€” logic-verified; confirm in-browser.
- Â§6.6 keyboard set now: Alt+1..9 jump, Enter-save, and this help overlay. Remaining 6.6: quick-add hotkey
  (touches quick-add state) and Cmd/Ctrl+K palette (bigger). Next I'll likely step to other low-collision
  Â§6 items or a quick-add hotkey.

## 2026-06-18 â€” fix: UX polish Â§6.10 â€” Allocate score bar labelling

- Picked a narrow clean-screen item from Â§6.10: allocation score bars were visual-only fills. `AllocRow`
...
## 2026-06-18 â€” feat: Â§6.6 FlipPanel Enter-to-submit
...
- Added an Enter case to the FlipPanel's existing document keydown handler (the one that already does
  ...
  saveâ†’close; skipped when `activeElement` is TEXTAREA (multi-line), BUTTON (let it click natively), or
  SELECT, and when CloseOnly (nothing to save). One file, `internal/ui/flippanel.go`.
- Picked an `internal/ui` item deliberately: the parallel session is also in Â§6 now, so I'm taking the
...
- Verified: gofmt clean (ran `gofmt -w` after the insert), wasm build exit 0, probe ok/200. Keystroke can't
  be sent headlessly â€” Enter-save is logic-verified; confirm in-browser.
- Next Â§6.6: a "?" help overlay (needs a shell mount + small overlay component) and a quick-add hotkey.

## 2026-06-18 â€” feat: Â§6.6 keyboard shortcut â€” Alt+1..9 section jump

- First of the Â§6.6 keyboard-shortcut items. New file `internal/app/shortcuts.go` +
...
- Robustness: keys off `KeyboardEvent.code` ("Digit1".."Digit9") â€” layout-independent and excludes the
...
- Chose canonical `primaryNav()` order (not the user's reordered/hidden-filtered visible list) â€” that
...
- Coordination note: the parallel session is ALSO picking Â§6 items now (it shipped a Â§6.4 add-menu radius
...
- Verified: gofmt clean, wasm build exit 0, probe ok/200. Can't send Alt+1 headlessly, so the nav itself
  is logic-verified â€” confirm in-browser.
- Next Â§6.6: a "?" help overlay documenting shortcuts (needs a shell mount point + a small overlay
  component), and a quick-add hotkey.

## 2026-06-18 â€” fix: UX polish Â§6.4 â€” add-menu radius utility

- Completed another low-collision Â§6.4 item in `internal/app/addmenu.go`: the top-bar **+ Add** button
...
## 2026-06-18 â€” fix: UX polish Â§6.3/Â§6.4 â€” progress bar height + switcher separator

- Two tiny low-collision Â§6 items in non-screen files: `ui/progress.go` track `h-1.5`â†’`h-2`, and the
  workspace-switcher dropdown separator `my-1`â†’`my-2` (`wsswitcher.go`). Go changes, so wasm rebuilt;
  gofmt clean, build exit 0. Atomic commit.
- Remaining Â§6 is mostly in the parallel session's hot screen files (transactions/accounts/dashboard/
  shell) and the bigger 6.6 keyboard-shortcut features (command palette, ? overlay, Alt+1..9) which need
  shell/router wiring â€” those next, checking each target file is clean before editing.

## 2026-06-18 â€” fix: UX polish Â§6.11 â€” light-theme contrast + toggle size (CSS)
...
- Continued section 6, again pure `web/index.html` CSS (single shared file, no rebuild). Light-theme idle
  ...
  (~2.7:1) â†’ `#6a6a72` (verified ~5:1, clears the 3:1 UI / 4.5:1 text thresholds). Note `.set-close`
  wasn't in the existing light-theme override group, so I added it.
- Settings `.switch` 36Ã—21 â†’ 40Ã—24 (24px clears the touch-target minimum); knob 17â†’18px, travel recomputed
  (left 3â†’19). Both `.switch::after` and `.switch.on::after` updated together.
- Verified probe ok/200. Atomic commit. Next Â§6: 6.4 [L] switcher separator spacing (my own wsswitcher.go,
  low collision), then the 6.6 keyboard-shortcut features (new files).

## 2026-06-18 â€” fix: UX polish Â§6.1â€“6.2 â€” WCAG contrast + touch targets (CSS)
...
- 6.2 contrast: `faint` #6c6c72â†’#7d7d85 (verified ~4.7:1 on #0e0e0f, was ~3.1:1) and `dim`
  #a6a6acâ†’#ababb3, in BOTH the Tailwind config block and the `:root` vars (two color systems coexist).
  `.insight-dot` 1.05remâ†’1rem.
- 6.1 touch targets: `.field` padding up + 38px min-height (36px floored under compact density); to-do
  `.check` made a centered 24Ã—24 inline-grid with a right margin; `.btn-del` padding bumped within its
  existing 24Ã—24; color picker 46Ã—34â†’44Ã—44. (`.btn-del`/`.toast-x`/`.set-close` already had the 24Ã—24 hit
  area from a prior fix.) Skipped the screen-file parts of 6.1 (transactions.go checkbox markup,
  custompagesnav â‹¯, shell rail min-size) â€” those are the parallel session's hot files; deferred.
- Did NOT touch TODOS.md to check the items off â€” it's perpetually dirty from the parallel session; the
...
- Next in Â§6: 6.11 light-theme contrast (index.html only, low collision), then the keyboard-shortcut
  features in 6.6 (new files, e.g. Alt+1..9 section jump / Cmd+K palette).

## 2026-06-18 â€” feat: reorder workspaces (arrange the switcher)

- `Registry.Move(id, toIndex)` â€” clamps toIndex into range, order-preserving, no-ops on unknown id /
  single-element list / same-position. Crucially leaves ActiveID and StartupID alone (they're id-tracked,
  not index-tracked) â€” covered explicitly in the table test alongside firstâ†”last, middle, clamp, and
...
- UI: up/down arrows in each `wsManageRow` (new Index/Total props; workspacesSection passes them). Both
  arrows always render â€” Move clamps, so a keyboard activation of a dimmed boundary arrow is a harmless
...
## 2026-06-18 â€” feat: workspace export / import (move a whole context as a file)

- Another isolated workspace-layer feature (the parallel session is churning store/domain/screens, so I
  stayed out). Reused the existing `downloadBytes`/`pickFile` DOM helpers â€” no new file plumbing.
- `wsExport` envelope `{version, name, color, bundle}` where bundle is the workspace's perWorkspaceKeys
  ...
  (new `slugify` helper). `importWorkspace` parses, rejects a nil/!bundle file (returns false â†’ caller
...
- Process: committed atomically (single `git add <files>; git commit` shell call) per the new
  ...
  session's commit. en.go required three Readâ†’Edit retries â€” the other session was writing it live.
- Verified: i18n + workspace tests pass, gofmt clean, `GOOS=js GOARCH=wasm` build exit 0, probe healthy.
  ...
  logic-verified â€” confirm in-browser.

## 2026-06-18 â€” feat: per-workspace color (tell contexts apart at a glance)

- Followed the user's "add other useful settings like this" invite with a self-contained, isolated
  workspace feature (deliberately avoided the obvious "open to screen X" startup setting â€” its boot-time
...
- UI (`wsswitcher.go`): a `wsColorDot` helper; the expanded switcher trigger and each dropdown row show
  ...
  panel's bump). No new i18n key (the swatch is inline). No `en.go`/`settings.go`/`app.go` changes â€”
...
- Verified: workspace tests pass, gofmt clean, full `GOOS=js GOARCH=wasm` build exit 0 (the parallel
  ...
  so the dot/picker are build-verified â€” confirm visually after a hard reload.

## 2026-06-18 â€” feat: custom pages â€” widget engine + grid rendering (Phase B)
...
- `internal/screens/custompage.go` now renders a page's `[]PageWidget` as a bento grid: `dashlayout.Pack`
  ...
  (its own delete hook) wraps each body. Did NOT reuse `ui.Widget` â€” it's wired to the dashboard's global
...
- `addWidgetBar`: a single stable component (form hooks not in a loop) â€” pick type, title, and one binding
...
- Name clash: `kpiBody` already existed in dashboard.go â†’ renamed mine `cpKPIBody`.
- Verified end-to-end with a Go-generated seed dataset (a page with all four widget types) loaded into
  ...
  transactions, the trend chart drew (D3 SVG â€” works because of the earlier createElementNS fix), and the
...
## 2026-06-18 â€” feat: startup workspace preference (pin a workspace to open with)
...
- Boot (`internal/app`): new `applyStartupWorkspace()` runs in `Run()` between `ensureWorkspaceRegistry`
  ...
  it bundles the last-active workspace out and the pinned one in â€” no reload needed because nothing has
...
- UI: `workspacesSection` gained a `wsStartupSelect` component (its own component for a stable OnChange
  hook) â€” an "On launch, open" dropdown listing *Last used workspace* + every workspace; mirrors the
...
- Concurrency note: the parallel custom-pages/widgets session is mid-refactor and currently has the
  ...
  but it's entirely their uncommitted WIP â€” my changes have zero wasm build errors. Verified my commit
...
## 2026-06-18 â€” feat: collapsed-rail variant for the workspace switcher
...
- Re-applied (cleanly, mine-only) the previously-reverted polish: `WorkspaceSwitcher` now reads
  ...
  active workspace's initial (`workspaceInitial`) instead of the full-width name + â–¾ button, which was
...
## 2026-06-18 â€” fix: new workspace starts empty (was cloning the current sample)
...
- Root cause: `createWorkspace` cleared the per-workspace UI keys and the `cashflux:dataset` key, then
  ...
  sample â€” so the "new" workspace came up as the Michael Brooks sample, indistinguishable at a glance
...
- Fix: added `store.EmptyDataset()` (one default "You" member, USD base, no financial data) and made
  ...
  it empty. Now a new workspace boots to a genuine clean slate. `duplicateWorkspace` is untouched â€” that
...
- Tests: `TestEmptyDataset` asserts no accounts/txns/budgets/goals, exactly one member, USD base, and a
  lossless exportâ†’import round-trip (the same path boot takes). `go test ./internal/store/...` +
  `./internal/workspace/...` green; gofmt clean; wasm rebuilt + probe shows the app healthy (Default â–¾,
...
## 2026-06-18 â€” feat: custom pages â€” "My pages" rail group (Phase A cont.)
...
- Added `internal/app/custompagesnav.go` (`CustomPagesNav`): renders the "My pages" rail section from
  ...
  reuse the built-in nav), plus a "New page" action (prompt â†’ `pages.UniqueSlug` + `id.New` + `NextOrder` â†’
  `PutCustomPage` â†’ navigate). Drag-reorder persists via `pages.Reorder` + `PutCustomPage`. Wired into
...
- Gotcha: a component rendered via `CreateElement` that returns a **bare `Fragment`** did NOT keep its
  sibling position among the `MapKeyed` rail groups â€” "My pages" jumped to the top of the rail. Returning a
...
## 2026-06-18 â€” feat: workspaces â€” Phases 2+3 (engine + UI), wired & browser-verified
...
- Phase 2 (engine, `internal/app/workspace.go`): the active workspace keeps using the canonical `cashflux:*`
  ...
  everything â€” no per-atom re-seeding and zero changes to the 12 uistate stores. `createWorkspace` clears
...
- Race guard: added `suspendAutosave` in persist.go â€” set before a switch's reload so the dying page's
  pagehide/ticker save can't clobber the swapped-in `cashflux:dataset`.
- Phase 3 (UI): `WorkspaceSwitcher` at the top of the rail (active name + â–¾ â†’ menu: switch rows + New +
  Duplicate; per-row `wsMenuItem` for stable hooks) and a Settings â†’ Workspaces section (`wsManageRow`:
...
- Verified via `gwc probe` on the live dev server: fresh session auto-creates "Default", the rail shows
  "Default â–¾", the dashboard renders the seed, zero console errors. Couldn't headlessly click the menu
  (probe loads+captures only), so the switch/new/duplicate and rename/delete click-throughs rest on the
  unit-tested registry + a healthy render â€” flagged for manual confirmation. Full suite 43 green.

## 2026-06-18 â€” feat: workspaces â€” Phase 1 (pure registry)
...
- Plan (bottom-up, commit per layer): (1) pure registry â€” this commit; (2) workspace-scoped localStorage
...
- Phase 1: new pure `internal/workspace` package â€” `Workspace{ID,Name}` + `Registry{Workspaces, ActiveID}`
...
## 2026-06-18 â€” feat: custom pages â€” persistence (Phase A cont.)
...
- Wired `CustomPage` through the store with the existing JSON-in-SQLite seam: `custompages` table,
  ...
  pattern â€” no new mechanism.
- Extended `sampleDataset()` with a page (layout + bound KPI widget) so both round-trip tests
  (`Exportâ†’Import` byte-equality and SQLite `Loadâ†’Snapshot`) prove pages are lossless, including the nested
...
## 2026-06-18 â€” feat: custom pages â€” Phase A starts (data model + pure logic)
...
- Bottom-up, started at the data model. Added `domain.CustomPage` (id/slug/name/icon/order/hidden + a
  ...
  custom page is user content that must export/import â€” keeps it consistent with "persist artifacts."
...
## 2026-06-18 â€” fix: invisible icons (SVG namespace bug in the framework)
...
- Diagnosis: the collapse toggle (`app.TopBar`, `.menu-btn`) is icon-only, so when its icon doesn't
  paint the whole control is invisible â€” same root cause as the rail icons. Traced to the framework: the
...
- Decision (asked the user): fix the root cause in the framework AND re-pin, rather than work around it in
  CashFlux. The CSS-mask data-URI workaround was therefore unnecessary â€” adding it would have been dead,
...
- Framework fix: routed SVG-only tags through a cached `createElementNS`, and made
  ...
  No raw-HTML sink introduced â€” attrs/text still set via `setAttribute`/`textContent`. Committed +
...
- Gotcha that caused a false "still broken": the served wasm is `web/bin/main.wasm`, but the CLAUDE.md
  "build wasm directly" snippet targets `static/bin/` â€” rebuilding there left the live app on the old
...
- Next: the build-output path mismatch (`static/bin` vs `web/bin`) is a footgun â€” worth reconciling the
  CLAUDE.md snippet / dev flow so there's one canonical wasm output location.

## 2026-06-18 â€” seed data: a middle-aged single homeowner persona
...
- Rewrote `store.SampleDataset()` as Michael Brooks, 46, single homeowner. Balance sheet: Everyday Checking,
  High-Yield Savings, Brokerage/401(k), Home (TypeOther asset), Mortgage, Auto Loan, Credit Card â€” with
  liquidity/stability/expected-return scores set so the Allocate screen is meaningful. Three months
  (Aprâ€“Jun 2026) of recurring activity generated in a loop: salary in, ~14 categorized bills/expenses from
...
- Net worth lands ~\$354k (â‰ˆ\$640k assets âˆ’ \$286k debt) â€” plausible for the persona. All ids are stable so
...
## 2026-06-18 â€” fix stray nested wasm re-tracking (.gitignore mid-slash anchoring)
...
- A clean-state check caught `internal/screens/static/bin/main.wasm` tracked *again* after I thought I'd
  ...
  the repo root**, so it never matched the nested path â€” and the untracking commit's `git add -A` re-added
...
## 2026-06-18 â€” untrack stray bin/ screenshots; ignore bin/ wholesale

- Follow-on to the wasm untracking. The four `bin/*.png` review screenshots (dash/dash2/mobile/mobile2)
  were the only remaining tracked things under `bin/`. Grepped the repo â€” nothing references them (only my
...
## 2026-06-18 â€” stop committing wasm build artifacts (stale .gitignore)

- The full-suite check's `git diff --stat` kept showing `static/bin/main.wasm | Bin 27MB` change every
  commit â€” I'd been re-committing a 27 MB binary each cycle via `git add -A`. Root cause: `.gitignore`
...
- Verified deployment safety before touching it: `.github/workflows/deploy-pages.yml` builds the wasm fresh
  in CI (`go build -o web/bin/main.wasm`), copies wasm_exec.js, and uploads `web/` as the Pages artifact â€”
...
- Found four tracked wasm artifacts: `static/bin/main.wasm`, `bin/main.wasm`,
  ...
  `/bin/*.wasm`, `/bin/*.hotreload-manifest.json`. Left `bin/*.png` (screenshots â€” possibly referenced).
- Net: no tracked `.wasm` remains; future commits won't carry the binary. Full suite still 41 green, gofmt
  clean, wasm builds. (Process note to self: gofmt every hand-edited .go file before committing â€” two prior
  commits slipped through dirty.)

## 2026-06-18 â€” extract spend-breakdown ranking into ledger.RankSpending (+ i18n the labels)

- The broader scan found one more real chunk of view-embedded logic: the dashboard breakdown widget's
  rank-categories-by-spend â†’ top-N â†’ collapse-the-rest-into-"Other". Extracted the pure part to
...
- Bonus: the breakdown's "Other" and "Uncategorized" were hardcoded English â€” added `dashboard.other` /
...
- The other view helpers scanned (uistate's loadItems/resolveTheme/loadRailCollapsed, the dashboard
  bills/segment *rendering*) are platform glue (localStorage/JS) or display â€” correctly staying put.

## 2026-06-18 â€” consolidate account-by-id lookup into domain.AccountByID

- `accByIDFrom` (documents, 3 uses) and `accountName` (goals) each had their own linear "find account by id"
  scan. Added `domain.AccountByID(accounts, id) (Account, bool)` â€” pure, table-tested; chose `domain` as the
...
- Gotcha: the test first compared the not-found result with `!= Account{}`, which doesn't compile â€”
  `Account` has a `map[string]any` (Custom) field and so isn't comparable. Switched to checking `a.ID == ""`.
  domain tests + wasm build green.

## 2026-06-18 â€” move firstNonEmpty to textutil; view-logic extraction wrapping up
...
- This effectively wraps the view-logic extraction vein. The genuinely-pure, logic-bearing helpers that were
  buried in `screens` are now extracted and tested: `recentTransactions`â†’`ledger.Recent`,
  `parseTags`/`parseOptions`â†’`textutil.CommaFields`, `parseFloatOrZero`/`parseIntOrZero`/`parseWeight`â†’
  `textutil.ParseFloat`/`ParseInt`, and `firstNonEmpty`â†’`textutil.FirstNonEmpty`. What remains in the view
  (`fmtMoney`, `figTone`, `amountClass`, `accentFor`, `humanizeType`, `*Label`, `catColor`, `indentLabel`,
  the insights `highlight*` mappers, `toDocumentRows`' 1:1 field copy) is legitimately presentation/glue â€”
  extracting it would add coupling for no real logic, so it stays.

## 2026-06-18 â€” consolidate numeric form parsing into textutil
...
- (`trimWeight` left in allocate â€” it's floatâ†’string display formatting, a view concern, not logic.)

## 2026-06-18 â€” unify comma-list parsing into tested textutil.CommaFields
...
- Created `internal/textutil` with `CommaFields(s) []string` (pure, table-tested: empties, all-separators,
  ...
  textutil test green.
- (`validateRuleInput` is also pure but returns i18n keys â€” more view-coupled â€” so left for now.)

## 2026-06-18 â€” extract dashboard "recent transactions" into tested ledger.Recent
...
- New vein of work the user's "keep going" surfaced: pure logic still embedded in the wasm-only view
  ...
  for pure helpers; `recentTransactions` (copy â†’ sort newest-first â†’ take N) was the most logic-bearing.
- Moved it to `ledger.Recent(txns, n)` (`recent.go`) â€” ledger is pure and transaction-focused, and `screens`
  already imports it (no cycle). Added a negative-n guard (the inline version would panic on n<0). Table
  tests in `recent_test.go`: ordering, top-N limit, n>len returns all, nâ‰¤0 empty, and input not mutated.
  Rewired `dashboard.go` to call it; removed the local copy. ledger tests + wasm build green.
- More such helpers remain in `screens` (parseTags, parseWeight/trimWeight, validateRuleInput, plural, â€¦);
  I'll extract the logic-bearing ones over subsequent commits.

## 2026-06-18 â€” widgetcfg IDs() to 100%
...
- Last pure package under 90%. `widgetcfg` was 88.1% with one untested function, `IDs()` (the sorted
  ...
  (consistent with Has/SchemaFor), and includes the known widgets. `widgetcfg` â†’ **100%**.
- With this, every pure logic package is â‰¥90% (the great majority 95â€“100%); only `store` sits lower (84%),
...
## 2026-06-18 â€” custom-field validation enforced on every write (91% â†’ 92%)
...
- Verified the data-integrity guarantee that a required custom field is enforced on every entity write that
  ...
  rejected â€” the `validateCustom`-rejection branch each of those `Put*` methods had left uncovered (they were
  at 75%; PutAccount was already covered by `TestPutAccountValidatesCustomFields`). Those methods â†’ 87.5%;
  `appstate` â†’ **91.8%**.
- **Coverage work concluded.** Every native package now sits at 84â€“100% (domain/validate ~100; insights 98;
  budgeting/goals/payoff 95â€“96; ledger 94; formula 93; id 91; appstate 92; store 84), error paths included
  (via the closed-DB/store technique). The remaining slivers are duplicative base-validation branches,
  construction-error injection, and unreachable safety backstops â€” not worth further churn. wasm/UI packages
  still need the browser lane (unavailable here).

## 2026-06-18 â€” appstate resolver + freshness-override paths (90% â†’ 91%)

- Two more reachable feature paths: `idResolver` (the C27 nameâ†’id CSV resolver, callable directly since the
...
- Added `resolver_test.go`: `idResolver` across all three branches (exact-id passthrough, case-insensitive
  name match, unresolved passthrough) and a `FreshnessWindows` test that layers a household "checking" â†’ 5
  override over the defaults. Both functions â†’ **100%**; `appstate` â†’ **90.7%**.

## 2026-06-18 â€” appstate business-logic branches (87% â†’ 90%)

- The reachable business-logic gaps after the error-path pass: `PutCustomFieldDef`'s rejection of an
  invalid definition (55.6%) and `ReassignOwner`'s per-entity move loops + individual-target path (62.5% â€”
...
- Added `logic_test.go`: rejecting an incomplete custom-field def (and asserting it isn't stored), and a
  ...
  ownership/scope flipped). `appstate` â†’ **89.7%** (`PutCustomFieldDef` 100%, `ReassignOwner` 87.5%). The
  residual is mid-loop store-error returns (error injection partway through a reassign) â€” marginal.

## 2026-06-18 â€” appstate error-path coverage via a closed store (82% â†’ 87%)
...
- Added `errors_test.go`: a table of `Put*`/`Delete*`/`PutSettings` calls plus `ImportJSON` (fed valid JSON
  ...
  the nil-returning accessors. `appstate` â†’ **86.6%**. The residual is reassign/validateCustom partial
  branches and per-loop error arms â€” genuinely marginal now.

## 2026-06-18 â€” store error-path coverage via a closed DB (81% â†’ 84%)

- Revisited the store error arms I'd earlier written off as needing a fault-injecting mock. They don't: a
  **closed** `*sql.DB` makes every Exec/Query/QueryRow fail, and `Close()` is already public â€” so the error
...
- Added `errors_test.go` running the helpers against a closed store: `putJSON`/`getJSON`/`deleteRow`/
  ...
  `GetSettings`/`PutSettings`/`Wipe`, and `Snapshot`/`Load`. `store` â†’ **83.7%** (`deleteRow` 100%, the JSON
  helpers ~85â€“91%). The residual is per-table error branches in Load/Snapshot (only the first table's arm
  fires before returning) and unreachable `json.Marshal` failures.

## 2026-06-18 â€” payoff negative-APR branch (92% â†’ 96%); coverage sweep complete
...
- Last reachable-gap package. `payoff` was 91.7%; the untested branch was the interest floor (a negative
  ...
  Project(100000, -12, 10000) â†’ 10 months, no interest. `payoff` â†’ **95.8%**; the only residual is the
...
- **Coverage sweep summary.** Pure logic packages after the sweep: domain 100, validate 99, insights 98,
  ...
  that would need a fault-injecting store mock â€” deliberately not added just for coverage. The wasm/UI
  packages (ui/screens/app/uistate) can't be measured natively and need the browser lane (unavailable here).

## 2026-06-18 â€” insights down-anomaly coverage (92% â†’ 98%)

- Coverage sweep, anomaly detection. `insights` was 92.0%; the untested branches were the *down* path of
  `Detect` (a category whose spend fell) and `abs64`'s negative arm â€” the latter only runs inside the sort
  comparator, which needs â‰¥2 anomalies to fire.
- Added `insights_edge_test.go` with a three-series case: a +400% rise (Up), an âˆ’80% drop (Down), and a
  âˆ’10% drop that's below the 50% threshold (skipped). Two flagged anomalies exercise the sort/`abs64` path
  and the equal-magnitude tie-break by name. `insights` â†’ **98.0%** (`abs64` 100%, `Detect` 96.7%).

## 2026-06-18 â€” validate to 99% (edge rules)
...
- Added `validate_edge_test.go` for exactly those. `validate` â†’ **98.9%** (Error/validCode/ValidateTask now
...
## 2026-06-18 â€” Cover appstate accessors, deletes, settings, CSV (64% â†’ 82%)
...
- Added `appstate_more_test.go`: accessor/handle checks on an empty app; a Task putâ†’delete round-trip plus a
  validation-failure; a delete-each-entity sweep (mirroring the existing tests' valid literals â€” accounts
  need owner/scope/type/class, tasks need a valid priority, etc.); a settings round-trip asserting the
  manual export keeps the OpenAI key while the redacted export strips it; and a CSV exportâ†’import round-trip
  that exercises the nameâ†’id resolver. `appstate` â†’ **81.7%**.
- The residual is store-error and ImportJSON/validateCustom error arms that need fault injection, plus parts
  of ReassignOwner/ReassignCategory â€” diminishing returns for now.

## 2026-06-18 â€” Cover the untested store CRUD wrappers (76% â†’ 81%)

- Coverage sweep on persistence. `store` was 76.2%; the per-function profile showed the Get/Delete/List (and
  some Put) wrappers for **Member, Category, Transaction, Budget, Goal, Task** at 0% â€” the existing CRUD
...
- Added `crud_more_test.go`: a Put â†’ Get â†’ List â†’ Delete â†’ Get-absent round-trip per entity, following the
  existing `TestAccountCRUD` shape (and `newStore(t)` helper). `store` â†’ **81.4%**.
- The residual is `sqlitestore.go` infra (NewMemory/Load/Snapshot/replaceRows) and the generic helpers'
  marshal/scan error arms â€” DB-failure paths that need fault injection to hit; deferred as low-value.

## 2026-06-18 â€” Raise id test coverage (73% â†’ 91%)

- Coverage sweep. `id` was 72.7%; the gaps were the **package-level** `NewWithPrefix` (0% â€” the existing
...
- Added `id_edge_test.go` (reusing the existing `errReader`/`errBoom` helpers): the crypto/rand-backed
  `NewWithPrefix("acc")` happy path and the generator's `NewWithPrefix` error propagation. `id` â†’ **90.9%**.
...
## 2026-06-18 â€” domain enums to 100% coverage
...
- `TestEnumInvalid` already covered the invalid branch for AccountClass/AccountType/CategoryKind/TaskPriority;
  added `enums_edge_test.go` for the rest â€” invalid Scope/Period/TaskStatus/RelatedType/TaskSource â€” plus
  `TestPeriodLabel` over all three periods and the unknownâ†’Monthly default. `domain` â†’ **100.0%**.

## 2026-06-18 â€” Raise formula test coverage (89% â†’ 93%)
...
- Added `eval_edge_test.go` driving each through `Eval`: boolâ†’number coercion via `(2>1)+10`, unary `+`,
  `if("x",â€¦)`/`if("",â€¦)` for the string truthiness, `"a" != "b"`, and the error set (`"a" == 1`, `avg()`,
  `min()`, `max()`, `round(1,2)`, `sum("x")`). `formula` â†’ **92.7%** (`asNumber` 100%). The residual is
...
## 2026-06-18 â€” Raise ledger test coverage (87% â†’ 94%)

- Next package in the coverage sweep. `ledger` was 86.5%; the misses were the error branches of the
  money-aggregation functions â€” opening-balance currency mismatch, a transaction currency that differs from
...
- Added `ledger_edge_test.go`: opening-mismatch + Add-mismatch errors for `ClearedBalance`/`RunningBalances`;
  ...
  archived-account skip. `ledger` â†’ **93.6%** (`ClearedBalance` 100%). The residual uncovered lines are
  defensive `Add`/`Sub` mismatch branches that can't trigger â€” conversions always return base currency, so
  the accumulator and operand always share it.

## 2026-06-18 â€” Raise budgeting test coverage (83% â†’ 95%)
...
- `matches` (the exact-category helper) turned out to be **dead** â€” superseded by the inline cover
...
- Added `budgeting_edge_test.go`: `EvaluateAll` happy + error path, the empty-limit-currency default
  (`normalizedLimit` â†’ base), and the currency-conversion error path threaded through `Spent`, `Evaluate`,
  `EvaluateAll`, and `EnvelopeAvailable` (a covered expense in a currency the rate table can't resolve).
  `budgeting` â†’ **95.1%** (`EvaluateAll`/`normalizedLimit` now 100%). The remainder is the defensive
  `limit.Sub(spent)` mismatch branch, unreachable since spend is always computed in the limit's currency.

## 2026-06-18 â€” Repo health check + raise goals test coverage
...
- Added `goals_edge_test.go`: currency-mismatch error paths for all four, the `Project`â†’`Remaining` error
  propagation, both `Evaluate` error branches (Remaining-first and Project-via-mismatched-monthly), the
  partial-final-month month-count round-up, and `MonthlyNeeded`'s mismatch path. `goals` â†’ **94.6%**. The
...
## 2026-06-18 â€” Remove dead `stub` screen placeholder

- Swept the Go source for `TODO`/`FIXME`/`HACK`/`XXX`/`BUG` markers â€” none (clean). The one real find was
  `screens.stub(...)`, the "Planned Â· Phase N" placeholder for not-yet-built screens. Grepped its call
...
- Deleted it. `stat(...)` next to it is still used widely, so it stays. The dot-imported shorthand helpers
  ...
  `go vet ./internal/screens` green. (Left the now-orphaned `.badge-soon` CSS class in place â€” harmless and
  cheap to keep for any future placeholder.)

## 2026-06-18 â€” Move dashboard span math into pure dashlayout (with tests)

- The tile resize arithmetic (`cycleSpan` grow/shrink/wrap, `clampSpan` bound) lived as unexported helpers
  in `internal/ui/widget.go` â€” a wasm-only view package, so they had no native unit tests. That's
  computation in view code, which CLAUDE.md explicitly forbids ("never put computation in view code";
  "logic packages â€¦ unit-tested").
...
## 2026-06-18 â€” i18n: route the last hardcoded user-facing messages through the catalog

- Audited the screens for user-facing strings that bypass `uistate.T`. Grepped the error/notice setters
  (`errMsg.Set`, `notifyErr`, `noticeAtomâ€¦With`, `promptText`). Almost all were already i18n'd or just
  `errMsg.Set("")` clears; three real offenders remained:
  - `accounts.go` validation `"Enter a valid opening balance."` â†’ new key `accounts.invalidOpening`.
  - `dashboard.go` reminder failure toast `"Couldn't create the reminder: "+err` â†’ `dashboard.reminderErr`
  ...
  - the two dashboard-tile resize-handle tooltips I'd added in the #1032 commit (kept hardcoded then for
    consistency with the surrounding tooltips) â†’ `widget.resizeWidth` / `widget.resizeHeight`.
...
## 2026-06-18 â€” B15: announce account balance updates (reconcile was silent)

- Last open live-region sub-item: announce inline balance updates. Found that `accounts.go`'s `setBalance`
  (the reconcile / Update-balance flow) succeeded *silently* â€” it posted the adjustment txn, set
  `BalanceAsOf`, cleared the error, and bumped, but gave no confirmation. Mark-updated already posted a
  notice; reconcile didn't.
- Reused the existing `noticeAtom` toast (already a persistent polite live region) â€” on success it now sets
  `â€¦With(uistate.T("accounts.balanceUpdated", ac.Name, fmtMoney(money.New(target, ac.Currency))), false)`
  (the `false` = not-an-error â†’ polite, not assertive). New i18n key `accounts.balanceUpdated` = "Updated
...
## 2026-06-18 â€” C28 (blank icons): root-caused to the framework, not app code
...
- Framework side is the defect: grepped the entire GoWebComponents module â€” **there is no `createElementNS`
  ...
  still paint â€” which explains why chart *axis labels* looked fine (C16) while icon glyphs are blank: same
...
- Marked C28 **framework-blocked** (alongside B1/B3 SPA routing). Couldn't live-verify the current state
  either â€” the gwc browser oracle isn't available in the headless loop environment. No code change made;
...
## 2026-06-18 â€” #1032: explicit mouse shrink on the dashboard resize handle
...
- Added Shift+click to shrink one step. Checked the framework first: `OnClick`/`OnKeyDown` both take
  ...
  So the handle's `OnClick(func(e uic.MouseEvent))` reads `e.JSValue().Get("shiftKey").Bool()` â€” same
  technique the keyboard handler already uses for `shiftKey`.
- Factored the span math into a `cycleSpan(cur, max, shrink)` helper: shrink â†’ `cur-1` clamped at 1;
  otherwise grow â†’ `cur+1` wrapping to 1 past max. Both edge handles (width/height) use it. Tooltips now
...
## 2026-06-18 â€” Closed the Members/Accounts "Add button no-op" finding (not a defect)

- Browser-oracle findings #4â€“#8 reported that the Members and Accounts **Add buttons** were silent no-ops
...
- Audited the actual code: in both `members.go` and `accounts.go` the Add button is `Type("submit")`
  *inside* `Form(Class("form-grid"), OnSubmit(add), â€¦)`, and `add` is `ui.UseEvent(Prevent(func(){ â€¦ name.Get() â€¦ }))`.
  A submit-button click and an Enter keypress both dispatch the form's `submit` event â†’ the same `add`,
  reading the same live `name` atom. There is no separate click handler and no stale-state path.
- The structure is **uniform across all six add forms** â€” Budgets and Goals have the identical
  `MapKeyed(defs, â€¦)` custom-field block and were reported *working*, so there's nothing structurally
...
- Action: closed the #4 and #8 checkboxes with the analysis. No code change (the code is already correct),
  ...
  key events, not value-set â€” tracked there.

## 2026-06-18 â€” B7: derive the rail nav from the screen registry

- B7's last item: the rail's three groups (primaryNav/toolsNav + an inline System block) were
  hand-maintained lists in shell.go, separate from `screens.All()` â€” so a newly routed screen could be
  reachable by URL yet silently missing from the menu (exactly the bug B7 was about).
- Added a `Group` field to `screens.Route` (`GroupPrimary`/`GroupTools`/`GroupSystem`) â€” pure data, no
...
- Kept the design data (icons + i18n label keys) in shell.go as a `railMeta` pathâ†’{Key,Icon} map, honoring
  ...
  vanishing â€” fail-safe, which is the whole point of B7.
- Replaced the hardcoded System block (members/categories/rules `If(!hiddenâ€¦)`) with a `MapKeyed` over
...
## 2026-06-18 â€” B15 a11y: announce filtered transaction count via live region

- The Transactions list already rendered a visible count+net summary, but it wasn't a live region and it
  unmounted at zero results, so a screen-reader user changing filters heard nothing â€” and never learned a
...
- Added an always-mounted `P(Class("sr-only"), role=status, aria-live=polite, aria-atomic=true)` carrying a
  ...
  text at zero, empty when there are no transactions at all. Staying mounted is the key â€” `aria-live`
...
- Marked the existing visible summary `aria-hidden="true"` so SR users get the live region's announcement
  ...
  â€” no new strings. Scoped to Transactions (the only screen with a real filter bar). wasm build + vet green.
- Remaining live-region item: announcing inline balance updates after an edit.

## 2026-06-18 â€” B15 a11y: per-field aria-describedby for form errors
...
- The architecture has **one form-level error string per add-form** (an `errMsg`/`rErr`/`plErr` atom), not
  per-field errors, so the honest association is error â†” the form's **primary input** (the name/first field).
- Built a shared pair in `internal/screens/aria.go` (package `screens`, not `internal/ui` â€” the screens
  import the *framework's* `ui` package under that name, so a helper there would collide): `errAttrs(id,msg)`
  returns the `aria-describedby`+`aria-invalid` options (nil when no error â€” so spreading it after the input's
  `OnInput` keeps the hook count stable), and `errText(id,msg)` is the drop-in for the old
  `If(msg!="", P(Class("err"), role=alert, â€¦))` but adds the matching `id`.
- Wired into all 11 add-forms: accounts, budgets, categories, custom-fields, goals, members, rules, to-do,
  transactions, and planning's recurring + plan forms, each with a stable error id (acct-err, budget-err, â€¦).
  Spread pattern: `Input(append([]any{â€¦, OnInput(on)}, errAttrs(id, msg)...)...)`.
...
## 2026-06-18 â€” B15 a11y: a light-theme-safe default accent

- The last open contrast item: the default accent `#54b884` (a light mint) only cleared ~2.1:1 against the
  light surface, failing WCAG AA for UI/large elements (3:1). The accent isn't decorative â€” it paints the
  focus ring and large strokes â€” so the default has to pass on whichever theme the user runs.
...
## 2026-06-18 â€” B15 a11y: route the last hardcoded aria-labels through i18n
...
- Grepped for hardcoded `aria-label`/`aria-roledescription` literals; only two remained (the widget gear
  ...
  keys); controls.go gained the uistate import (no cycle â€” widget.go already imports it). The gear's
  title attr now uses the same key. Closes the B15 i18n-aria item. Build + i18n catalog test green.

## 2026-06-18 â€” B15 a11y: keyboard resize (completes bento keyboard control)

- Extended the tile `OnKeyDown` with Shift+Arrow resize: â†/â†’ adjust width, â†‘/â†“ adjust height via
...
## 2026-06-18 â€” B15 a11y: keyboard reorder for the bento
...
- Closed the biggest remaining keyboard gap: the bento was drag-only (pointer). Made each draggable tile
  ...
  slot earlier (Left/Up) or later (Right/Down) via `dashlayout.Move` on the arranged order â€” persisting
...
## 2026-06-18 â€” B15 a11y: icon-button labels + closing satisfied items

- Added explicit `aria-label`s to the icon-only buttons the spike flagged: the widget gear ("Widget
  settings", glyph wrapped in `aria-hidden`), the accounts "â‹¯" overflow ("More actions"); made the
...
## 2026-06-18 â€” feature D10: forecast scenario comparison + dollar axis

- The Planning 12-month forecast used the axis-less sparkline and showed the trim scenario as text only.
  Switched it to the D3 `ui.Chart`: plots major units with a compact-currency Y axis ($0/$10k/â€¦, the C16
  fix), and overlays a second series (the trimmed scenario, gold) beside the baseline when a trim amount
  is entered â€” so the two net-worth curves compare directly (the Â§2.6 comparison gap).
- Added legend rendering to the D3 shim (`web/chart.js`): when `spec.legend` and >1 series, a small
  top-right colored-dot + name list, so the baseline/scenario lines are labeled. SW cache v13â†’v14.
- Unit: `forecast.TestProjectSpendingDeltaShiftsEndBalance` â€” trimming by `delta`/month pulls the curve
  ahead by delta each month (end = deltaÃ—months higher), backing the what-if.
- Verified live on /planning: D3 svg with `$0/$10k/$20k/$30k` ticks; entering a trim adds the scenario
  line (stroked paths 3â†’4).

## 2026-06-18 â€” feature B2: live drag-over preview
...
- The reorder previously only happened on drop; added a live preview during the drag. Clean,
  ...
  so the grid reflows live. Crucially this never touches the persisted `items` â€” so the drop path is
...
## 2026-06-18 â€” B10 closed: resolution control verified + responsive
...
- Verified live on the dashboard: the control shows a single label "Jun 2026" (not the old
  "Jun 2026 â€“ Jun 2026"); the "Last period" preset shifts the window to May 2026; there's one stepper
...
## 2026-06-18 â€” feature B2: FLIP animations for the dashboard bento
...
- Grid placement changes don't transition, so animating tile reorder/resize needs FLIP. Did it with a
  ...
  zero â€” the classic FLIP. State lives in JS, so there are no per-move Go `js.FuncOf` callbacks to leak.
...
- Go side: a `ui.UseEffect` in `Dashboard` invokes the shim, keyed on a layout signature (mode + each
  item's id/spans/importance) so it fires exactly when the arrangement could change â€” not on every data
  tick. Loaded flip.js in index.html + SW CORE (cache v12â†’v13).
...
- Verified the mechanism in the oracle: `cashfluxFlipBento` is loaded; after forcing a tile to a new
  ...
  FLIP invert) â€” the rAF then plays it to zero. (The time-based glide itself isn't headless-observable.)
- Still open under B2: a live drag-over *preview* (reflow lands on drop) and pointer-events for touch.

## 2026-06-18 â€” C9: accounts-row overflow menu

- Account rows exposed six actions each â€” visually busy. Kept the primary three inline (Transactions /
  Edit / âœ•) and moved Update balance / Mark updated / Archive into a "â‹¯" overflow menu, reusing the C23
  popover CSS (.add-wrap/.add-menu/.add-item/.add-backdrop/.hidden-menu) â€” no new styles.
...
- Verified live (in-app nav to /accounts): rows show a â‹¯ that opens with [Update balance, Mark updated,
  Archive]; hiddenâ†’shown on click.

## 2026-06-18 â€” feature D6 (envelope): carry-forward budgeting view

- User said to grind until every TODO is done and stop pausing for direction (recorded in
  [[dont-ask-which-ticket-next]]). Took the envelope view â€” the decision-gated item I'd flagged â€” and
...
- Pure model: `budgeting.EnvelopeAvailable` accumulates `limit âˆ’ spent` per period from that first
...
## 2026-06-18 â€” tests D7: month/week/quarter boundary membership
...
- Hardened the period-boundary correctness that C1 was about, at the `ledger.PeriodTotals` (totals)
  ...
  must land in exactly one period across consecutive windows â€” no drop, no double-count.
  - Month: May 31 / Jun 1 / Jun 30 / Jul 1 â†’ May=100, June=600, July=800, and the three windows sum to
...
## 2026-06-18 â€” feature D5: parent-category budgets roll up sub-category spend
...
- Budgeting: refactored the matcher around a `covers func(string) bool` predicate (`matchesCovered` /
  ...
  `EvaluateRollup(..., covers map[string]bool)` â€” counts spend in the budget's category OR any covered
...
- For a budget whose category has no sub-categories, `Descendants` = {id} and `EvaluateRollup` is
  identical to `Evaluate` â€” so no behavior change on flat category sets (verified the dashboard still
...
## 2026-06-18 â€” tests D2: budgeting threshold boundaries

- With the tractable feature backlog thin (remaining items are the B10 redesign, B2 FLIP animations, and
  the envelope carry-forward view â€” all large or hard to verify headlessly), spent this iteration
  hardening the package I just extended. Added `internal/budgeting/threshold_test.go`: white-box boundary
  tests for `classify` (`==limit` â†’ Over, `==near%` â†’ Near, one cent below â†’ OK; two thresholds) and
  `percent` (including the zero-limit guard that returns 100/0 instead of dividing by zero). 12 cases.
- Test-only, native â€” guards the exact-boundary behavior so a future tweak to the `>=` comparisons can't
  silently shift a budget's state off by a cent. Closes the D2 "unit" item.

## 2026-06-18 â€” feature D6: budgeting method (Simple / Zero-based)

- Built the budget-methodology selector. Model: `budgeting.Methodology` (simple/zero-based/envelope) +
  `Valid`/`ParseMethodology` (unknown/empty â†’ simple) + `ToAssign(income, totalBudgeted)`, all pure and
...
- UI: a Settings â†’ household "Budgeting method" selector (Simple Â· Zero-based; Envelope reserved in the
  type but not offered yet â€” it needs the carry-forward view). On the Budgets screen, when zero-based,
...
- Verified live end-to-end: set Zero-based in Settings, navigated to /budgets (client-side), and the
  banner read "$3,600.00 left to assign this month" (sample income âˆ’ total budgeted).
- Deferred: the Envelope view (carry-forward) and the per-member config-layering â€” noted in TODOS.

## 2026-06-18 â€” cleanup C7: one period control for Budgets

- The Budgets card carried its own `â€¹ January 2006 â€º` month stepper (a `monthOffset` UseState +
  prev/next handlers) that competed with the global top-bar resolution control â€” two controls, two
...
- Removed the stepper UI + `monthOffset`/`prevMonth`/`nextMonth`, and pointed `viewMonth` at the shared
  window (`uistate.UsePeriod().Get().From`) â€” the same atom the top-bar writes â€” so stepping the period
...
## 2026-06-18 â€” feature B8: drag-reorder the sidebar
...
- **Design call:** implemented *always-draggable* rather than the TODO's Shift-gating. Shift-gating the
  ...
  that re-renders the whole rail on every Shift press. Always-draggable is simpler and fully functional â€”
...
- Verified live by dispatching real DragEvents (dragstart/dragover/drop with a DataTransfer): dragging
  Accounts onto Dashboard reordered the rail to `[Accounts, Dashboard, â€¦]` and persisted the path order.
  Apply-on-boot is reasoned (tested Apply + the atom seeds from localStorage like the other prefs).

## 2026-06-18 â€” feature: per-widget "add" affordances on empty dashboard tiles (C23)

- Completed the open part of C23: empty Accounts/Goals/Budgets/To-do dashboard tiles now show an
  in-context "Add a â€¦" button. Added a reusable `emptyAddCTA` component (its own component so the
  `router.UseNavigate` hook stays stable) that renders the message + a primary button routing to the
  screen. The Budgets widget distinguishes genuinely-empty (`len(app.Budgets()) == 0` â†’ CTA) from the
  at-risk filter being empty (budgets exist â†’ plain "Nothing near or over budget.").
- Verified: i18n + wasm build green; the sample dataset populates all four tiles so the CTA isn't visible
  by default, but I confirmed the navigation mechanism live (clicking a nav item routes client-side â€”
  /goals renders the Goals screen), which is exactly what the CTA does.

## 2026-06-18 â€” feature: opt-in "remember my key on this device" (C27 closed)
...
- `prefs.RememberAIKey` (new bool, off by default). `uistate/aikey.go`: `PersistAIKey`/`ClearAIKey`/
  ...
  hydrateDataset). Settings â†’ AI gained a ToggleRow + a plain-English unencrypted-storage note; the key
  input's onKey re-persists when the toggle is on so edits stay in step.
- Secure-by-default: off â†’ nothing stored; on â†’ user has explicitly opted in with a clear notice.
- Verified live by driving the UI: open settings â†’ type a key â†’ toggle Remember on writes
...
## 2026-06-18 â€” feature: local dataset persistence (data survives reload)

- Re-armed loop ("implement all features"). Picked the highest-value gap I'd surfaced while investigating
  the C27 AI-key bug: the dataset wasn't persisted locally at all â€” `appstate` seeded the *sample* on
  boot and only manual Export/Import saved anything, so a reload lost the user's data.
- Design choice: rather than instrument all ~25 mutation methods with an OnChange hook (error-prone â€”
...
- Security: added `appstate.ExportJSONRedacted()` â€” snapshots and zeroes `Settings.OpenAIKey` before
...
## 2026-06-17 â€” bugfix C27: vision category near-name matching
...
- Chose the contained post-processing fix over constraining the vision prompt to the category list (a
  bigger AI-call change) â€” noted prompt-constraint and a per-row picker as further hardening. Route-gated
  screen + AI call, so verified by reasoning + build.

## 2026-06-17 â€” bugfix C27: "Save as task" title
...
## 2026-06-17 â€” bugfix C27: document-review amounts in accounting style

- The receipt-import review list showed the AI's raw amount string ("âˆ’4.50"), out of step with the
  app's accounting format. Passed the chosen import account's currency (falling back to base) into
  `DraftRow` and formatted the display amount via `fmtMoney` (parse â†’ money â†’ format), with a raw-string
...
## 2026-06-17 â€” bugfix C27: CSV import accepts its documented format

- The starred C27 bug: pasting the on-screen `date,payee,amount,account` example failed with a leaked
  `store: csv line 2: amount and currency are required` â€” the parser hard-required a `currency` column the
...
## 2026-06-17 â€” C26: text/display size to 200% for accessibility

- The Display scale (B6) topped out at 130% and the TODO flagged that `zoom` would overflow the
  non-responsive layout at large values â€” so C26 was blocked on C10. C10/C19 are now fixed, which
  unblocks the TODO's option (b): keep the `zoom`-on-`#app` mechanism and just raise the ceiling.
- Raised `prefs.ScaleMax` 130 â†’ 200 (WCAG 2.1 SC 1.4.4), updated the clamp test (200 valid, 250 clamps),
  and relabelled the setting "Text & display size" so it reads as an accessibility control. The
  `scaleOptions` builder derives from the constants, so the dropdown now offers 70â€“200% automatically.
- **Empirically validated the reflow** (the part the TODO was unsure about): set `--ui-scale: 2` on a
  1280px window â€” Chromium's `zoom` makes the content lay out at the effective ~640px width, so the
...
- Composes with C25: density rebalances the base tokens; this is a zoom multiplier on top â€” independent.
- Wasm-only change (no index.html), so no SW cache bump (main.wasm is network-first).

## 2026-06-17 â€” C25: rebalance the default density down

- Picked C25 next ("UI too fat/chunky"). The TODO said to confirm the approach first, but the user told
  me to stop asking and just pick â€” so I made the call: rebalance the *default* density down rather than
  introduce new Cozy/Compact presets. Simpler, lower-risk, and the existing Compact toggle + Display
  scale still layer on top.
- Trimmed the shared tokens in index.html: body 16px/1.5 â†’ 14.5px/1.45 (Fraunces display figures keep
  their explicit sizes, so the data stays prominent); `.field` 0.5/0.6â†’0.4/0.55rem padding + radius 8â†’6;
  `.btn` 0.55/0.9â†’0.4/0.8rem + radius 8â†’6; `.wbody` 0.85â†’0.7rem.
- Checked the a11y angle: the B15 touch-target rule enforces a 24px minimum on small controls; fields are
  now ~34px and buttons ~30px, both above it â€” no regression. (The app's standard is 24px, not the 44px
...
- Verified live on the dashboard + the quick-add form: body computes to 14.5px, `.field` is 34px with no
  ...
  oracle but share these tokens, so the effect is uniform. SW cache v11â†’v12.

## 2026-06-17 â€” feature C23: "+ Add" multi-entity add menu

- Picked the next ticket myself (the user said to stop asking which to do â€” see the new memory). Chose
...
- "+ Add" opened only the quick-add transaction panel; every other entity could be added only from its
  ...
  quick-add) Â· New account Â· New budget Â· New goal Â· Scan a document (the entity items route via the
...
- Verified live: the menu is hidden initially, opens on click with 5 items, "New transaction" opens the
  ...
  nav rail uses. SW cache v10â†’v11.
- Left open (enhancement): per-widget in-context "add" affordances (e.g. an empty Budgets tile offering
  "Add a budget").

## 2026-06-17 â€” feature C24 (importance UI): rank tiles from the gear â€” C24 done

- Final piece: a per-tile Importance control. Added pure `dashlayout.SetImportance`/`ImportanceOf`
  (+ tests), then an `importanceRow` component in the gear settings panel (Highest/High/Normal/Low â†’
  2/1/0/âˆ’1) that writes the layout items atom and persists on change.
- Resolved the C21 tension cleanly: importance is a *universal* per-tile setting, so the settings panel
  ...
  â€” in the other modes the gear still only shows on schema'd tiles (C21 preserved).
...
- **End-to-end live verification** (the satisfying one): switched the header selector to Auto-importance
  â†’ the no-schema freshness tile now shows a gear â†’ opened it â†’ set Importance to Highest â†’ the freshness
...
- C24 is complete (model + state + selector + importance editing). Remaining decision-gated C-items:
  C17 (â†’ B10 redesign), C23 (add-menu feature), C25 (density â€” confirm tokens), C26 (text-resize).

## 2026-06-17 â€” feature C24 (wiring): layout-mode state + selector
...
- Two ordering subtleties handled: (1) a manual drag in an auto mode bakes the current *arranged* order
  into the stored sequence and flips to Custom â€” otherwise the drop index (computed against the visual
...
- Verified live: the selector shows 3 options, defaults to Custom, persists `cashflux:layout-mode` on
  ...
  the model's table tests; it'll be visually demonstrable once importance is settable (next commit) â€”
...
## 2026-06-17 â€” feature C24 (model): dashboard auto-layout engine

- After clearing the whole C-series bug backlog (C1â€“C22 this session), asked the user which decision-
  ...
  only reorders). Noted the C21 tension (KPI tiles have no gear) â€” the resolution is to make importance a
...
- Built the **model first** (bottom-up): pure `dashlayout.Arrange(items, mode) []Item` that reorders the
  ...
  never touches spans â€” sizes remain user-set per the decision.
...
## 2026-06-17 â€” bugfix C19 (list rows): action buttons wrap instead of overlapping
...
- The last C19 sub-item: at narrow widths the transaction row's buttons overlapped the description/date
  ...
  whenever the row still fits on one line. Shared by every list screen (transactions/accounts/budgets/â€¦),
...
- /transactions can't be loaded in the static oracle (no SPA fallback), so I verified the *mechanism* the
  ...
  Duplicate/âœ•) at 360px and measured â€” the row wrapped (height ~204px) with 0 of 5 buttons overlapping
  the text rect. SW cache v9â†’v10.
...
## 2026-06-17 â€” bugfix C19 (KPI clip): 2-column tablet bento
...
- Measured the reported KPI clip live: at 900px the bento was still the desktop 4-column grid, tiles
  ...
  didn't reach this range, so 768â€“1024px fell through to the cramped desktop grid.
...
- Verified live at 900px: 0 clipped figures, KPI tiles ~315px (was 153), header ~640px (full content
  width = both columns), no horizontal page scroll. SW cache v8â†’v9.
- C19 now done except transaction-row action-button wrapping at narrow widths â€” that's route-gated
  (can't drive /transactions in the static oracle) and a list-row concern, left open under C19.

## 2026-06-17 â€” bugfix C20: persist the sidebar collapsed state
...
- C20 ("collapsible panel reads as missing") had three parts. Two are now resolved: C15 fixed the
  ...
  across reloads â€” it was a transient `state.UseAtom` in app/shell.go, lost on refresh.
...
- Verified live: clicking the toggle writes `cashflux:rail-collapsed` = `1` then `0` and the rail width
  goes 58â†”240. The oracle uses a fresh browser profile per launch (confirmed: localStorage set in one
  launch reads null in the next), so the load-on-reload leg can't be exercised end-to-end there â€” but it
  is structurally identical to the resolution-pref load path that's already in use.
- Remaining C20: an on-panel collapse affordance (chevron) â€” a placement/design call; the working
  top-bar toggle stands in the meantime, so I left it for a spec decision rather than guessing.

## 2026-06-17 â€” bugfix C21: widget gear only where there are settings

- The gear opened real, persisted settings for 8 widgets but also showed on the 4 KPI tiles and
  cashflow/bills/freshness, which have no schema â€” there it opened the C11 close-only "no settings yet"
  ...
  (visibility:hidden, aria-hidden) so the grip Â· title Â· gear header stays balanced.
...
- Verified live: 16 tiles â†’ 8 real `button.gear-inline` (exactly the schema'd widgets) and 8 hidden
  `span.gear-inline`; the net-worth KPI's gear slot is a span, not a button. SW cache v7â†’v8.
- C22 ("layout doesn't reflow on move/resize") is the same root cause as B2/C14 and is already resolved
  by the Pack migration (widget.go uses Pack/Move/ResizeItem, no Swap) â€” only the live drag *preview*
  remains, tracked under B2.

## 2026-06-17 â€” bugfix C19 (top bar): controls were unreachable on tablet/phone
...
- First attempt â€” `flex-wrap: wrap` + `width: 100%` on the control cluster â€” *didn't* work, and the
  ...
  shared the row.
- Fix that worked: `.topbar-controls { flex: 1 0 100% }` (no shrink, full-basis â†’ forced onto its own
...
- Verified live with the oracle's viewport flags: 768px â†’ bar 175px, breadcrumb readable (96px), every
  control's right edge â‰¤ 768, no page h-scroll; 390px â†’ all controls reachable, no h-scroll. SW v6â†’v7.
- Remaining C19 (separate concerns, left open): transaction-row action buttons overlapping at narrow
  widths (route-gated â€” can't drive /transactions in the static oracle) and KPI figure clipping when the
  desktop bento is squeezed between 768â€“1024px.

## 2026-06-17 â€” bugfix C18: inline-edit stacked fields vertically
...
- Editing a Transactions or Accounts row rendered its fields in a tall single column with dead space to
  ...
  `Form(Class("form-grid"))` â€” the difference was the *wrapper*. Transactions/Accounts wrapped the form
...
- Verified the mechanism in the oracle (the routes don't load there, but the CSS does): injected a
  `form-grid` with four fields into a 600px `.row-edit` vs `.row` â€” 3 columns vs 1. SW cache v5â†’v6.

## 2026-06-17 â€” bugfix C15: collapsed rail hid every nav icon

- Collapsing the sidebar left only the brand mark and the active-item highlight â€” no nav icons â€” so you
  ...
  each `uic.CreateElement(navItem, â€¦)` in its own `<div>`, so the selector matched every item too.
- Fix: gave `railHeader` a `rail-section` class and retargeted the rule to `nav .rail-section`. The
  `<768px` mobile rail had a copy of the same `nav > div` rule (C10) â€” fixed it there as well, so the
  phone rail won't lose its icons either.
- Bumped SW cache v4â†’v5 (index.html is a precached CORE asset and changed).
...
## 2026-06-17 â€” bugfix C1: period totals dropped first-of-period UTC-dated transactions
...
- The Dashboard Income KPI read `$0.00` for June even though a $4,200 salary was dated 2026-06-01. Root
  ...
  machine behind UTC, the local month-start (e.g. `Jun 1 00:00 âˆ’05:00` = `Jun 1 05:00Z`) sorts *after* a
  `Jun 1 00:00Z` transaction, so `InRange` (`!Before(start) && Before(end)`) excluded it. The Jun 2â€“5
  expenses survived because they're a day clear of the boundary.
- **Fix â€” one canonical convention: UTC-midnight calendar dates.** `midnight`, `MonthStart`,
  `FiscalMonthRange`, and `NextMonthlyDue` now take the calendar date from the reference instant (in its
  own location â€” the user's wall calendar) but emit the boundary at `time.UTC` midnight; `WeekStart`
...
- Why take the date in local but emit in UTC: "what month is it" should follow the user's wall calendar,
  ...
  1st in a far-behind zone, far-ahead zones) â€” a same-day-1 UTC transaction is now always counted.
...
## 2026-06-17 â€” bugfix C16: net-worth trend chart plotted cents

- The net-worth trend D3 chart fed `Y: float64(m.Amount)` â€” raw minor units (cents) â€” so the Y axis
...
- **Fix, two parts.** (1) `dashboard.go` now divides by the currency's decimal factor (same `div` loop
  ...
  `format` hint (`chartspec.Axis.Format`) â€” it builds a `d3.format` tick formatter for X and/or Y when a
...
- Bumped the service-worker cache (`cashflux-v3` â†’ `v4`) because `chart.js` is a cached CORE asset â€”
...
- **Audited the other chart feeds** as the TODO asked: `customize.go` already converts to major units;
  the planning `AreaChart` is the pure-Go sparkline (`internal/ui/chart.go`) â€” a normalized path with no
...
- Note: there were still-open C-series items beyond C2â€“C14 (C1, C15â€“C19). C16 done here; C1 (Income $0
  correctness) and C15 (collapsed rail loses nav) are the next high-value bugs.

## 2026-06-17 â€” bugfix C2 (negatives): one money formatter, parentheses everywhere
...
- **Approach â€” collapse to one formatter rather than migrate 38 call sites.** The two functions differed
...
- **The risk the TODO flagged â€” parentheses must never reach an input value â€” does not apply.** Grepped:
...
- Verified live in the headless oracle: the Dashboard (which already used parentheses, now via the
  collapsed `fmtMoney`) still renders correctly â€” `$20,749.25 | $1,800.75 | ($60.20) | ($240.55) |
  ($1,500.00)` â€” confirming the collapse caused no regression. The other screens route through the same
  tested formatter, so their rows now match.
- With this, every substantial C-series bug (C2â€“C14) is fixed. What remains is B2 *enhancement* polish
  (live drag-over preview, FLIP animations) â€” beyond the reported bug, which is already fixed.

## 2026-06-17 â€” bugfix C13: quick-add panel height fits its content
...
- The "+ Add" quick-add flip panel used the default FlipPanel height (470px), leaving the compact
  ...
  scrolls â€” there's no risk of clipping content with a fixed reduction.
- Verified live: drove the headless browser oracle to click "+ Add" (async re-render handled with a
  Playwright-awaited async IIFE + 500ms settle), then measured `.flip-wrap` â€” `panelH=420 fields=5`.
...
- Next: the remaining C-series items are minor/debatable â€” C2-negatives (parentheses-vs-minus
...
## 2026-06-17 â€” bugfix C9: Accounts asset-input placeholders clipped

- The asset inputs on the Accounts add/edit form had long placeholders ("Expected return APR %",
  "Liquidity 0â€“100", "Stability 0â€“100") that clipped in the ~150px grid columns. Shortened the
  placeholders to "Return %"/"Liquidity"/"Stability" and added `title` attrs with the full label + range
  (e.g. "Liquidity score (0â€“100)") on both the add form and the edit row. Route-gated (the static serve
...
## 2026-06-17 â€” C14/B2: migrate dashboard onto the Pack model

- Discovered `dashlayout/pack.go` (the ordered-sequence + bin-packing model: `Item`, `DefaultItems`,
  `Pack`, `Move`, `ResizeItem`) already existed and is table-tested â€” the remaining B2 work was wiring
...
- Migrated the UI: `uistate` layout atom is now `[]dashlayout.Item` (`UseLayoutItems`/`PersistItems`;
  ...
  `Move(items, src, targetIndex)` (reorder â†’ reflow, not pairwise Swap), and resize calls `ResizeItem`.
...
- Fixes C14's core: a widened tile no longer overlaps its neighbor (Pack reflows), so the resize handle
  ...
  1â†’4/1â†’3"). **Verified in a headless browser**: the packed default arrangement is pixel-identical to
...
## 2026-06-17 â€” browser oracle online; verified D3 + fixed C10 (responsive)
...
- **Verified B14 D3 charts**: `eval` on the live dashboard reports `d3=object`, `.cf-chart svg` present
  â†’ the net-worth trend renders via D3 with axes. The boot overlay seen in a screenshot is just the
...
- **Fixed C10** (was flagged blocked-on-browser): a `@media (max-width:767px)` block forces the rail to
  ...
  drawer (icon nav is tappable) â€” minor follow-ups.

## 2026-06-17 â€” bugfix C9: Insights bare without a key

- Verified: the offline Spending-highlights card already renders unconditionally (line 205), so that
  half of C9 was done. The "Ask about your money" card, though, was `If(key != "", â€¦)` â€” invisible to
  keyless users. Restructured so the Ask card always renders: the working `Form` stays under
  `If(key != "")` (identical OnSubmit/OnInput hook conditionality â€” no hook-order change), and a
...
## 2026-06-17 â€” bugfix C12: last settings row clipped by footer
...
- The global settings flip panel is a flex column (header / `.set-body` flex:1 overflow-auto / sticky
  ...
  TODO's prescribed fix, bumped `.set-body` bottom padding (1rem â†’ 1.5rem) so the final row clears the
...
## 2026-06-17 â€” bugfix C3: household card (verified resolved + tidy)

- Re-checked C3 against current code: the reported "GWC avatar overlapping/clipping its own text" is
  gone â€” `HouseholdCard` is now a clean flex Button (gear icon + a two-line text span), and the only
  `.hh` CSS is the collapsed-rail rule (no absolute positioning, no avatar). So the layout bug is
  resolved by the redesign since the review. Minor tidy: dropped the redundant trailing "Â· Settings"
  from the visible summary (the gear icon + tooltip already signal it), keeping it in the Title tooltip.
  wasm + vet green.

## 2026-06-16 â€” bugfix C2 (part): grouped fmtMoney

- `fmtMoney` rendered ungrouped amounts ("$20749.25"), so every screen using it (Accounts/Budgets/
  Goals/Allocate/Documents/Plansâ€¦) looked locale-naive next to the Dashboard's grouped accounting
  figures. Wrapped its `FormatMinor` in the existing `money.Group`, so all `fmtMoney` output is now
  comma-grouped â€” fixed in one place. Kept the **minus-sign** negative style (not parentheses) so
  there's zero risk to any input pre-filled via `fmtMoney` (parentheses wouldn't re-parse) and no
  jarring inline parentheses. The remaining C2 sub-point â€” unifying negatives to parentheses (Transactions
  rows minus vs Dashboard parentheses) â€” is a visual-standard call best verified in a browser and is
  left as a follow-up (the row-level `fmtAccounting` migration). wasm + money tests green.

## 2026-06-16 â€” bugfix C4: resolution control on non-period screens
...
- The TopBar rendered `ResolutionControl` on every route, so a Week/Month/Quarter stepper showed on
  ...
  `router.InspectCurrentRoute().Path`. Left `+ Add` everywhere â€” logging a transaction is a valid action
  on any screen, so it has an obvious target (C4's second bullet is a no-op by that reasoning). wasm +
  vet green.

## 2026-06-16 â€” bugfix C9: category color never surfaced
...
## 2026-06-16 â€” bugfix C8: member color picker was a bare line
...
- The Add/Edit member forms used `<input type="color">` with the `.field` class (tuned for text), so
  ...
  (46Ã—34, padding, swatch-wrapper resets for webkit) so it renders as a proper clickable color swatch,
  plus a `title`/`aria-label` "Member color". Chose styling the native input over swapping to the fixed
  `SwatchPicker` palette â€” it keeps full color choice, avoids an off-palette-selection edge case, and is
  a smaller, reliable change (native color inputs honor explicit width/height). New `members.color` key.

## 2026-06-16 â€” bugfix C5: two "Net worth" tiles read as duplicates

- The dashboard has both the `kpi-networth` KPI (figure + â–²/â–¼% this month) and the `trend` tile (figure
...
## 2026-06-16 â€” bugfix C6: Allocate zero-score noise (+ labels verified)

- C6 part 1 (unlabeled weight inputs) was already fixed â€” the five weight inputs carry
  `Title`+`Placeholder` ("Returns weight", â€¦ "Goal-progress weight"). Part 2 (zero-score candidates):
  `RankWith` returned accounts with all-zero criteria, which `AllocRow` rendered as
  "0% Â· returns 0 Â· stability 0 Â· liquidity 0" noise. Now filter `ranked` to `Score > 0` before both
...
## 2026-06-16 â€” bugfix C11: empty widget panel showed Save
...
- Added a `CloseOnly` option to `ui.FlipPanel`: when set, the footer is a single "Close" button instead
  ...
  Also checked C9's "goals unlabeled current-amount field" â€” already fixed (it has a "Saved so far"
  placeholder), so that one was stale. wasm + vet green.

## 2026-06-16 â€” bugfix C7: budget row "Food Â· Food"

- Shifted to the C-series UI bugs. `BudgetRow` built its label as `Name + " Â· " + Category`
  unconditionally, so a budget named after its category read "Food Â· Food", and an unnamed budget read
  "Â· Food". Replaced with a switch: empty name â†’ show the category alone; name == category
  (case-insensitive) â†’ show one; otherwise "name Â· category". Code-logic fix verified by inspection
  (no browser needed). C7's second part (the Budgets card's own month stepper duplicating the global
  resolution control) is a separate UX concern tied to C4/B10 â€” left for its own slice. wasm + vet green.

## 2026-06-16 â€” B14: migrate net-worth trend widget to ui.Chart (proof)

- Migrated `netWorthTrendWidget` from the pure-SVG `uiw.AreaChart` to `uiw.Chart`: build a
  `chartspec.Spec{Kind: Area, Series: [{Points: (i, netWorth[i])}]}` (empty Color â†’ theme accent) and
  render it at 120px. This is the B14 proof that the Go â†’ JSON â†’ D3 pipeline works end to end. Kept the
  blast radius to one widget â€” `AreaChart` still renders the planning forecast and the plan-row
  sparklines â€” so if D3 misbehaves only this widget is affected. wasm + vet green. **Needs an
  in-browser smoke test** (D3 render correctness, theme colors, resize) before declaring parity and
  migrating the rest.

## 2026-06-16 â€” B14: ui.Chart Go component (drives the D3 shim)

- Added `internal/ui/chartd3.go` â€” `ui.Chart(ChartProps{Spec, Height, Class, Label})`. It renders a
  ...
  box's innerHTML on unmount / before a redraw, so no stale SVG lingers â€” the ref/portal pattern for
...
## 2026-06-16 â€” B14: D3 chart shim + offline caching (renderer foundation)

- User chose D3 for B14 (via AskUserQuestion). Started the integration bottom-up: JSON-tagged the
  `chartspec` types (lowercase wire keys), added `web/chart.js` â€” a self-contained `cashfluxRenderChart(el, specJSON)`
...
## 2026-06-16 â€” a11y: aria-required on key form fields
...
## 2026-06-16 â€” a11y: announce form validation errors (role=alert)

- Added `role="alert"` to all 17 inline error-message `P(Class("err"), â€¦)` elements across the screens
  (accounts, allocate, budgets, categories, customfields, customize, documents, goals, insights,
  members, planning Ã—4, rules, todo, transactions) via a safe mechanical replace. `role="alert"` is an
...
## 2026-06-16 â€” planning: projected-balance sparkline on each plan row

- `PlanRow` now renders a compact `uiw.AreaChart` of `planning.Project(p)` (int64 curve â†’ float64),
  ...
  engine â€” no new logic. i18n + wasm green.

## 2026-06-16 â€” settings: accent-contrast indicator (uses contrast pkg)
...
- Added `accentContrastNote` under the accent SwatchPicker in `globalSettingsForm`: computes the
  ...
  muted "Contrast X.X:1 â€” passes AA" line, or a danger-colored "low; may be hard to see" warning when
  it fails AA for UI/large elements (3:1 â€” accent is used for fills/active states/focus ring). For the
...
## 2026-06-16 â€” planning: one-time item in the plan create form

- Extended the Plans create form with an optional one-time amount + "in month #" pair. When both are
  given (and the month is within the horizon â€” else a validation error), `addPlan` appends a
...
## 2026-06-16 â€” a11y: keyboard operability for div-based switch/swatch

- Verified the framework exposes a declarative `OnKeyDown` (shorthand â†’ `html.OnKeyDown`) whose callback
  gets a `ui.KeyboardEvent` with `GetKey()`/`PreventDefault()` â€” so no per-instance js wiring is needed.
  ...
  they get a focus ring for free. (The Segmented control is real `<button>`s â€” already operable.)
  This resolves the earlier "needs a framework key handler" blocker. wasm + vet green.

## 2026-06-16 â€” a11y: theme-token contrast audit + text-faint fix

- Audited the suspect tokens with the new `contrast` package (throwaway test, not committed). Findings:
  `text-faint` failed AA-normal on both themes â€” dark #6c6c72 = 3.70/3.33 (bg/elev), light #8a8a90 =
  3.18/2.93 (the light-on-elev even failed AA-large). text-dim, danger, and accent-on-dark all passed.
  The shared **accent** #54b884 fails on light (2.27 on bg / 2.45 on card) â€” but it's mostly fills/large
...
- Fix (data-driven): lightened dark `--text-faint` to **#888890** (5.49/4.94) and darkened light
  `--text-faint` to **#686870** (5.11/4.72) â€” both clear AA-normal (4.5) on base and elevated surfaces,
...
## 2026-06-16 â€” contrast: WCAG luminance/ratio utility (pure)
...
- New pure package `internal/contrast`: `ParseHex` (3/6-digit, optional #), `RelativeLuminance`, `Ratio`
  ...
  black/white=21:1 + symmetry, identical=1, the canonical #767676-on-white â‰ˆ4.54 boundary, and the
...
## 2026-06-16 â€” a11y: FlipPanel focus trap + restore
...
## 2026-06-16 â€” a11y: minimum touch targets (24px, WCAG 2.5.8)
...
- Gave the small icon-only buttons (`.btn-del`, `.toast-x`, `.rstep`, `.set-close`) a `min-width`/
  ...
  tappable box â€” meeting WCAG 2.5.8 (AA, 24Ã—24) without inflating the dense desktop layout. The
  `.menu-btn` already gets â‰¥24px from its `w-7 h-7` utility classes. 44Ã—44 (AAA) noted as aspirational.
  Pure CSS; no Go change.

## 2026-06-16 â€” a11y: prefers-reduced-motion for interaction animations
...
- Extended the existing `@media (prefers-reduced-motion: reduce)` block to neutralize the remaining
  ...
  rail flyout were already handled (flyout is gated behind `no-preference`). Pure CSS in index.html â€”
  no Go/wasm change. Minor residual transitions (toggle knob, button filter) left as-is (low motion).

## 2026-06-16 â€” B14: pure chartspec package (decision-independent half)

- New pure package `internal/chartspec`: `Kind` (line/area/bar/donut) + `Point`/`Series`/`Axis`/`Spec`,
  with `Kind.Valid`, `Spec.Validate` (unknown kind, no series, empty series, multi-series donut â€” all
  ...
  (incl. empty). This is B14's framework-agnostic foundation â€” a renderer (pure-Go SVG or D3) consumes
  a Spec.
...
## 2026-06-16 â€” B13: rewire ui.Icon to icon.Name (call sites migrated)

- `ui.Icon`/`iconBody` now take `icon.Name` and switch on the `icon.*` constants; migrated every call
  site (railItem.Icon and navItemProps.Icon fields â†’ `icon.Name`; nav literals, members/categories/
  rules, the household `Settings` and top-bar `Menu` icons). Checked the framework for a raw-SVG inject
  primitive (dangerouslySetInnerHTML/innerHTML) to consume `icon.Inner()` directly â€” none exists, so
...
## 2026-06-16 â€” B13: pure icon registry (type-safe Name)
...
## 2026-06-16 â€” allocate: max-per-destination input (Â§2.7)
...
- Surfaced the `Distribute` engine's already-tested `MaxPer` cap in the Allocate amount-split UI: a new
  ...
  display logic was needed. Pure wiring of an existing capability â€” no logic/test changes. Closes the
  "amount-split UI next" item under Â§2.7 constraints. i18n + wasm green.

## 2026-06-16 â€” B10: presets dropdown

- Added a "Jump toâ€¦" quick-pick to `ResolutionControl`: a native `<select>` (keyboard-accessible, no
  custom menu state) with This period / Last period / This quarter / Year to date, wired to
  `period.NewWindow`/`Previous`/`YearToDate`. It's an action menu â€” the placeholder option is always
...
## 2026-06-16 â€” B10: ResolutionControl rebuilt (single stepper + reset + custom range)

- Rebuilt the top bar's `ResolutionControl`. Default is now a single-period stepper using
  `Window.Label()`/`Shift(Â±1)` (pages the whole window, reads as one label). A local `uic.UseState`
  `rangeMode` toggles to the old dual From/To steppers (`Custom range` â†” `Single period`); leaving
  range mode calls `Window.Single()` to collapse cleanly. A `This period` button appears only when
  `!w.IsCurrent(now)` â€” the off-now cue + one-tap reset to `NewWindow(res, now)`. Granularity segmented
...
## 2026-06-16 â€” B10: single-period Window helpers (pure logic)

- Added to `period.Window`: `IsSinglePeriod()` (From == To), `Single()` (collapse To := From), and a
  combined `Label()` that returns one unit label when single ("Jun 2026") or "from â€“ to" for a range.
  This is the pure-logic prerequisite for the redesigned single-stepper resolution control â€” the label
  collapses correctly so the common "this month" case stops reading as "Jun 2026 â€“ Jun 2026". Three
...
## 2026-06-16 â€” B12: goals widget config schema (sweep complete)

- Registered a `goals` widget schema: "byProgress" Toggle (default false) and "showDate" Toggle
  (default true). Wired `goalsWidget` â€” byProgress features the goal with the highest `goals.Percent`
  (first wins ties) instead of `list[0]`; showDate gates the "Â· by <date>" caption. A dynamic
...
- With this, every dashboard widget that has feasible settings now exposes them (savings, recent,
  trend, breakdown, to-do, accounts, budgets, goals) â€” B12's incremental-schema item is effectively
  complete. wasm + tests green.

## 2026-06-16 â€” B12: budgets widget config schema
...
## 2026-06-16 â€” B12: accounts widget config schema

- Registered an `accounts` widget schema: a "count" Number (default 6, clamped [3,12]) and a "cleared"
  Toggle (default false). Wired `accountsWidget` to read both â€” the limit caps the grid, and when
...
## 2026-06-16 â€” B12: to-do widget config schema
...
## 2026-06-16 â€” a11y: FlipPanel is a real modal dialog
...
- Gave `ui.FlipPanel` `role="dialog"` + `aria-modal="true"` + `aria-label` (its title) on the flip-wrap,
  ...
  on unmount â€” matches the panel's open/close lifetime since it mounts fresh each open). One change
...
## 2026-06-16 â€” B11: quick-add transaction flip panel
...
- The top bar "+ Add" no longer navigates to /transactions; it opens a quick-add flip panel via a new
  ...
  today). Save builds the transaction (expense â†’ negative) and calls `app.PutTransaction`, bumps the
  data-revision atom so screens refresh, and toasts the outcome.
- Hook-ordering care: all hooks (atoms, 6Ã—UseState, 5Ã—UseEvent) run unconditionally *before* the
  open/closed guard so hook order is stable across opens. Avoided setting state during render â€” used
...
## 2026-06-16 â€” Planning UI: plans card (Â§2.6)
...
- Added a "Savings & spending plans" card to the Planning screen: create (name, horizon, start
  ...
  keys. Closes Â§2.6's Plan/PlanItem feature across model â†’ logic â†’ persistence â†’ UI. i18n + wasm green.

## 2026-06-16 â€” Plan persistence (Â§2.6)
...
- Wired `domain.Plan` through the store exactly like the other JSON-blob entities: a `plans` table
  ...
  positive horizon) /`DeletePlan`. No SchemaVersion bump â€” additive, the blob carries everything.
...
## 2026-06-16 â€” Plan model + planning engine (Â§2.6)
...
- Added `domain.Plan{ID, Name, HorizonMonths, StartBalance, Items}` and `domain.PlanItem{ID, Label,
  ...
  "BaseScenario" as `StartBalance` (the base starting point) and "Assumptions[]" as `Items` â€” plain
...
## 2026-06-16 â€” documents UI: monthly-spend summary view (Â§2.2)
...
- The Documents screen renders `spendsummary.Summarize` over the draft (awaiting-import) rows as a
  ...
  Undated/OutIn. Closes Â§2.2's monthly-spend summary across logic + UI. i18n catalog + wasm green.

## 2026-06-16 â€” spendsummary: monthly-spend summary logic (Â§2.2)
...
- New pure package `internal/spendsummary`: `Summarize([]extract.Row, decimals) []MonthSpend` buckets
  ...
  commas) â€” no float rounding; undated rows collect under an empty Month sorted last (surfaced, not
...
## 2026-06-16 â€” allocate UI: goal-progress weight wired end-to-end
...
- Final (UI-last) slice of the goal-progress criterion. The Allocate screen now: populates each goal
  ...
  (saved profiles) / the live `weights` struct / saveProfile; and the breakdown line appends a "Â· goal
  ...
  catalog + wasm green. Completes Â§2.7's goal-progress criterion across all layers.

## 2026-06-16 â€” persistence: goal-progress weight on saved profiles
...
- Added `GoalProgress float64` (`json:"goalProgress,omitempty"`) to `domain.AllocationProfile`. Because
  ...
  needed â€” the field round-trips for free, and `omitempty` keeps old rows clean. Extended the CRUD and
  ...
  persistence layer beneath the next slice â€” the Allocate screen's goal-progress weight input/UI.

## 2026-06-16 â€” allocate: goal-progress criterion (Â§2.7)
...
- Bottom-up split: this is the tested logic only. The Allocate screen still needs a weight input, a
  profile-preset entry, populating `GoalProgress` from each goal's pace, and a breakdown line â€” that
  UI wiring is the next slice (UI last, per the SDLC rule).

## 2026-06-16 â€” B15 (slice): single h1 per screen
...
- Promoted the top bar's current-page breadcrumb title from a `Span` to an `<h1>`, so every screen
  ...
  dashboard's in-canvas header `<h1>` â†’ `<h2>` so it no longer double-h1s. Other screens' section
...
## 2026-06-16 â€” B15 (slice): color-not-only-cue audit
...
- Audited the UI for status conveyed by color alone. Most spots already pair color with text/shape:
  ...
  use â–²/â–¼ arrows, stale accounts show a "Stale" badge. The one offender was the dashboard To-do
  widget, where high and medium priority were both `â—` differing only by tone (and the dot was a
  silent glyph). Fixed: high/medium/low now use distinct shapes (â–²/â—/â—‹) plus a `title`/`aria-label`
  naming the priority. Closes the B15 color-only-cue item.

## 2026-06-16 â€” B15 (slice): persistent live region for notices
...
- Reworked `Toast` so the live region is always mounted: when idle it renders an empty `.sr-only`
  ...
  Added a `.sr-only` utility to `index.html`. Covers the B15 "live regions for async results" item â€”
  every notice flows through this one surface (saves, imports, AI, failures).

## 2026-06-16 â€” document.title per screen

- Set `document.title` to "<Screen> Â· CashFlux" on each route change (and initial load) from the same
  `Shell` route-change effect, via a new `setDocumentTitle` helper. Title is set unconditionally
  (including first render) â€” unlike the focus move, which still skips first render. Completes the
  document-title half of the SPA route-change a11y item; the focus half shipped in the prior slice.

## 2026-06-16 â€” B15 (slice): route-change focus
...
## 2026-06-16 â€” B15 (slice): skip link + focus rings

- Added a `.skip-link` as the Shell's first child (`href="#main"`), off-screen until focused, and made
  `<main id="main" tabindex="-1">` a focus target â€” standard skip-to-content. Plus a global
...
- Closes the "Semantics & landmarks â†’ skip link" and "Focus visibility" B15 items. Remaining big B15
...
## 2026-06-16 â€” B15 (slice): stepper + swatch labels

- `StepperPill` gained `PrevLabel`/`NextLabel` props (default "Previous"/"Next"), wired to the
  resolution control's from/to steppers with localized labels ("Move start earlier", â€¦) so the bare
  â€¹/â€º buttons announce. `SwatchPicker` is now a `role="radiogroup"` (aria-label "Accent color") of
...
- Same as the Toggle decision: didn't add tabindex to the div-based swatches (focusable-but-not-operable
  trap) â€” keyboard ops is the separate item. Closes the icon-button-label part of B15's "Custom
  controls". New `resolution.*` keys; catalog + wasm green.

## 2026-06-16 â€” B15 (slice): central control ARIA

- Roles/state on the shared controls (one edit â†’ every usage): `Toggle` gets `aria-checked` + an
  ...
  operable â€” this just adds the semantics AT needs.
...
## 2026-06-16 â€” B15 (slice): chart + landmark labels
...
- These are the "Charts: role=img + aria-label" and part of the "Semantics & landmarks" B15 items.
  ...
  â€” each its own small commit, no big-bang. New i18n keys; catalog + wasm green.

## 2026-06-16 â€” B8: menu visibility covers all nav items

- Extended `hideableScreens` (Settings â†’ Screens toggles) to the Tools group (Planning/Allocate/
  Insights/Documents/Customize) and Rules â€” they already respected `hidden.IsHidden` in the sidebar
...
## 2026-06-16 â€” B8 (partial): drop the "My pages" segment
...
- The other B8 sub-items: menu **visibility settings** already exist (Settings â†’ Screens module
  toggles, `hideableScreens`); **shift+drag reorder** of nav items remains (the heavy part â€” needs a
...
## 2026-06-16 â€” B4: consolidate the duplicate Settings

- There were two settings entry points: the household-card â†’ global panel (the real editor) and a
...
## 2026-06-16 â€” B9: top-bar breadcrumb

- Replaced the plain title in `TopBar` with a `<nav aria-label="Breadcrumb">`: a Dashboard crumb
  (a real `<button>`, so it's keyboard-operable) + a `â€º` separator + the current screen title marked
...
- The app's routes are flat (one screen per route), so the trail is at most two deep â€” no nested-route
  derivation needed. Reused existing utility classes (no new CSS); works in both themes. wasm green.

## 2026-06-16 â€” B5: collapsed-rail hover labels

- Added `Title(props.Label)` to `navItem` (and a title on the household card) â€” a native tooltip that
...
## 2026-06-16 â€” B6: display-scale setting

- Now green-lit ("do all the todos"), built B6 â€” the font/UI scale the user had earlier asked me to
  hold. Pure `prefs.Scale` (int percent, `ScaleMin/Max/Default` = 70/130/100, clamped in `Normalize`,
  `ScaleFraction()` â†’ CSS multiplier; table-tested). `uistate.ApplyPrefs` sets `--ui-scale`;
  `index.html` adds `#app { zoom: var(--ui-scale, 1) }`; Settings â†’ Appearance gets a "Display scale"
  `<select>` (70â€“130% in 10s, 100% marked default) that persists + applies live.
...
## 2026-06-16 â€” saved formulas: Customize UI
...
- Â§2.5's remaining piece is now just surfacing enabled formulas' results on the dashboard (a widget) â€”
  the harder design call. The Customize save/list loop is complete. New `customize.*` keys; catalog +
  wasm green.

## 2026-06-16 â€” saved formulas: model + persistence

- User said "do all the todos" â€” scope opened up to the full backlog (gated UX items included). Picked
  up Â§2.5's `Formula{...}` + CRUD first: `domain.Formula{ID, Name, Expr, Enabled}` persisted like the
  other entities (formulas table, store CRUD, dataset round-trip, validated appstate accessors;
  table-tested). Kept it minimal (Target/ResultType/Format from the spec deferred â€” ResultType is
  computed at eval, the rest are display extras).
- Next on Â§2.5: a save/list/delete UI on the Customize screen and surfacing enabled formulas' results
  on the dashboard. all layers + wasm green.

## 2026-06-16 â€” nav: Tools group for the missing main-line screens (B7)

- Planning, Allocate, Insights, Documents, Customize were routed but absent from the sidebar â€” only
...
- Scoping note: this is the uncontroversial reachability half of the menu work (B7) â€” making routed
...
## 2026-06-16 â€” allocation profile picker + weight editor
...
- Allocate now drives the ranking from four editable weight inputs (returns/stability/liquidity/debt)
  ...
  saved selections.
- Helpers `parseWeight` (blank/invalid/negative â†’ 0) and `trimWeight` (no trailing zeros). New
  `allocate.weights*`/`saveProfile`/etc. i18n keys. This completes Â§2.7's AllocationProfile picker UI
  on top of the persisted model. Catalog + wasm green.

## 2026-06-16 â€” saved allocation profiles: model + persistence
...
- Kept domain a leaf: the weights are inline floats, not `allocate.Weights`, so domain doesn't import
  the scoring engine; the Allocate UI maps profile â†’ `allocate.Weights` at the call site (it already
  builds Weights from its presets). Fits the "highly configurable" project value â€” custom profiles
  beyond the four presets.
- This is Â§2.7's `AllocationProfile + CRUD`. Next: a profile picker/manager on the Allocate screen
  (merge saved profiles with the presets). all layers + wasm green.

## 2026-06-16 â€” recurring autopost
...
- Added `appstate.PostDueRecurring(asOf)`: for each autopost recurring with an account, post a
  ...
  is past asOf â€” a bounded catch-up loop (guard 600) handling multiple missed periods. Skips
...
- This makes recurring genuinely functional â€” schedules turn into real ledger entries on demand. (A
...
## 2026-06-16 â€” recurring monthly-equivalent total

- Added pure `domain.Recurring.MonthlyEquivalent()` (weekly Ã—52/12, quarterly Ã·3, yearly Ã·12, monthly
...
- Chose a display-only summary over wiring recurring into the 12-month forecast: the forecast's
  ...
  salary/bill that already shows in actuals â€” a semantic decision worth leaving for explicit design.
  The monthly-equivalent total is unambiguous and useful on its own. domain + i18n + wasm green.

## 2026-06-16 â€” Recurring management UI on Planning

- Added a "Recurring cash flows" card to the Planning screen (no new route â€” avoids the gated nav
...
## 2026-06-16 â€” Recurring cash-flow model + persistence
...
- Added `domain.Recurring` (label, signed `money.Money` amount, `RecurringCadence`, NextDue,
  ...
  `dateutil.AddMonths`, unknownâ†’monthly) and `Recurring.Advance()` (value receiver, returns a copy).
...
- Modeling call: followed Transaction's convention â€” signed amount + currency carried in
  `money.Money`, no separate Kind/Currency enum from the spec's field list â€” so income/expense is the
...
- This is Â§2.6's `Recurring{...}` + CRUD item. Next: a management UI and (later) autoposting due ones
  into real transactions; the forecast engine can also read these. wasm + all layers green.

## 2026-06-16 â€” forecast + payoff: edge-case tests

- forecast: added tests for one-times outside the horizon (month 0, beyond, negative â†’ ignored),
  multiple one-times in the same month summing, negative horizon â†’ nil, and balances going negative
  (reported truthfully, no flooring).
- payoff: added single-month clear (final payment capped â†’ TotalPaid = principal + one month
...
- Completes Â§2.6 "Tests: forecast projection, payoff math"; both already had happy-path coverage, this
  pins the boundaries. No engine changes; both suites green.

## 2026-06-16 â€” allocate: determinism + clamping tests

- Filled the Â§2.7 "extensive tests" gap that mattered most: an explicit determinism test (Rank +
  Distribute identical across 25 runs â€” guards against map-iteration or unstable-sort creeping in),
  a tie-stability test (equal scores keep input order, since the UI shows a ranked list), and a
  breakdown-clamping test (negative APR â†’ 0, stability > 100 â†’ 1, negative liquidity â†’ 0).
...
## 2026-06-16 â€” formula engine: security + edge-case tests

- Added `eval_security_test.go` to nail down the Â§2.5 "extensive tests incl. security (no escape)"
...
- No engine changes needed â€” the existing design already enforces the allow-list and scalar-only
  values; this pins those guarantees with tests so a future change can't regress the sandbox. Whole
  formula suite green.

## 2026-06-16 â€” rules: shadowed-rule detection

- Added pure `rules.Conflicts(rs)`: a rule is shadowed when an earlier rule's match phrase is a
  substring of its own (case-insensitive) â€” first-match-wins means the later one can never fire.
...
- Surfaced it on the Rules screen: each row carries a `Warning` (computed via `Conflicts` keyed by
  rule ID) shown as a `text-warn` meta line â€” "Never runs â€” an earlier rule (â€¦) already matches it."
  This is the "conflict handling beyond first-match" piece of Â§2.4. Catalog + rules tests + wasm green.

## 2026-06-16 â€” pin/save insights: UI
...
- Completes Â§2.3's pin/save-insights item (storage + UI). New `insights.pin*`/`unpinTitle` keys;
  catalog + wasm green.

## 2026-06-16 â€” pin/save insights: model + persistence
...
- Kept it a separate entity from `Task` deliberately: "Save as task" already exists for actionable
  items, but pinning an explanation to revisit shouldn't pollute the to-do list â€” different intent,
  different store. The UI (a Pin button on the Insights answer + a pinned-insights card) is next.

## 2026-06-16 â€” AI request cancellation
...
- One AbortController is created once and reused across retry attempts, so a single abort kills the
  ...
  safely. Existing callers that ignore the return value (allocate, documents) compile unchanged â€” Go
  discards an unused return value.
- Completes Â§2.1 "retry/backoff; request cancellation". Can't browser-test the abort here, but it's the
  standard AbortController shape; ai + i18n tests + wasm green.

## 2026-06-16 â€” B1 triage: deep-link 404 is now framework-side only
...
- Worked the Section-B bug backlog starting at B1 (deep-link refresh 404). The CashFlux-side fixes
  ...
  generates a `404.html` shell â€” so the **deployed/offline PWA already routes deep links correctly**.
- Verified the current build: `GOOS=js GOARCH=wasm go build` is green and `go test ./...` passes.
- Empirically probed the dev server (`gwc dev -port 8099`): `/` â†’ **404**, `/index.html` â†’ **404**,
  `/accounts` â†’ **404**, but `/bin/main.wasm` â†’ **200**. So `gwc dev` isn't serving the HTML shell at
  *any* route â€” this is the Â§0 "`gwc dev -html` resolution" bug, and on top of it there's no SPA
...
- **Decision needed before B1 can fully close:** the only remaining buildable piece is the framework
  ...
  in the GoWebComponents sibling repo + a `.tools/gwc.exe` rebuild â€” a separate repo, so flagging it
  rather than assuming scope. Until then, browser end-to-end verification also waits on Playwright (Â§0,
  not installed). Next: B2 (dashboard reflow) is fully in-repo and unblocked.

## 2026-06-16 â€” model picker + AI-off-until-key hint

- Expanded the Settings â†’ AI model `<select>` from two options to the six the cost table knows
...
- Added an inline hint under the key input shown only when no key is set: "AI features stay off until
  you add a keyâ€¦", restating the bring-your-own-key/local-first promise right where it matters. The
  enable toggle + empty-key gating already existed; this makes the off state explicit.
- Completes Â§2.1's model/cost item (cost surfacing + model picker + AI-off-until-key). i18n + wasm
  green.

## 2026-06-16 â€” surface AI token usage + cost in Insights
...
- Threaded `Usage` through the transport: `SendChat`/`SendVisionChat`/`postCompletions` now call
  ...
  four callers â€” allocate + documents ignore it with `func(c string, _ ai.Usage)`, Insights captures
  it.
- Insights renders a faint "Used N tokens Â· about $X" line under the answer, using `EstimateCostUSD`
...
- This completes the "token + cost surfacing" half of Â§2.1's model/cost item; the model picker and the
  explicit "AI off until key set" state remain. ai + i18n tests + wasm green.

## 2026-06-16 â€” docs: static-hosting SPA rewrite
...
## 2026-06-16 â€” service-worker SPA navigation fallback

- The SW was network-first with `.catch(() => caches.match(req))` â€” but `.catch` only fires on a
...
## 2026-06-16 â€” AI guardrails: explicit FinancialContext
...
- Added pure `ai.FinancialContext{NetWorth, Income, Spending, Accounts}` + `Line()`, and routed both
  ...
  into the prompt's context is those four aggregate fields â€” there's no field for payees, account
...
- Dropped the now-unused `fmt` import from insights.go. ai test (incl. a no-leak assertion) + wasm
  green. This is the Â§2.3 "guardrails + pure prompt-assembly tests" item.

## 2026-06-16 â€” documents import-history list UI

- Added an "Import history" card to the Documents screen: `app.Documents()` sorted newest-first,
  rendered as `DocHistoryRow` components (own delete handler) showing kind Â· date Â· status Â· row count
  Â· account, each removable via `app.DeleteDocument`. Empty state when there's nothing yet.
...
- This closes the Â§2.2 Document lifecycle end-to-end: model â†’ persistence â†’ recorded on import â†’
  visible/auditable history. Catalog + wasm green.

## 2026-06-16 â€” record Documents on import
...
- Wired the Document model into the Documents screen: both import paths now call a `recordDocument`
  ...
  extract.Row â†’ domain.DocumentRow via `toDocumentRows`); CSV records the metadata (the store parses
...
- Now the Â§2.2 lifecycle has a real producer. Remaining: a documents history/audit list UI to view
  them (the records persist + export already). wasm green.

## 2026-06-16 â€” Document lifecycle model + persistence
...
- Layering call: gave Document its own `DocumentRow` rather than importing `extract.Row`, keeping
  ...
  extract.Row â†” domain.DocumentRow at the edge (same four string fields). Avoids a domainâ†’extract
  dependency and the can't-define-methods-on-another-package's-type problem with `extract.Row`.
- This is the model/persistence half of Â§2.2's lifecycle item; recording a Document when the user
  imports (CSV or image) and a documents history list are the UI follow-ups. wasm green.

## 2026-06-16 â€” vision extraction via structured outputs
...
- Gave the structured-outputs codec a real consumer: `BuildStructuredVisionRequest` (extracted shared
  ...
  JSON schema. The system prompt no longer has to beg for "ONLY a JSON array, no code fence" â€” the
  schema enforces shape â€” though `extract.ParseRows` still parses the `{"transactions":[â€¦]}` result
...
## 2026-06-16 â€” AI structured-outputs codec
...
- Decision: pure codec only, consistent with how the rest of `ai` separates request/response shaping
  ...
  (replace the free-form-JSON-then-`extract.ParseRows` path with a strict schema) â€” left as a
...
## 2026-06-16 â€” suggested-rules review UI
...
- Wired `rulesuggest.Suggest(transactions, existingRules, 3)` into the Rules screen as a "Suggested
  ...
  (owns its own Add handler, per the no-hooks-in-loops rule) showing "Categorize X as Y Â· Seen in N
...
- After accepting, `bump()` re-renders: the new rule appears in the list and that suggestion drops off
  (Suggest skips keys an existing rule already covers) â€” a satisfying review-and-accept loop with no
  extra bookkeeping. New `rules.suggested*`/`rules.accept*` i18n keys. Catalog + wasm green.
- Completes the review-and-accept half of Â§2.4's "rules from history".

## 2026-06-16 â€” rulesuggest: deterministic rule suggestions
...
- Design call: did this as a deterministic heuristic rather than the backlog's literal "AI-proposed
  ...
  determinism/explainability rule â€” strictly better than an AI round-trip for this. AI proposals could
...
## 2026-06-16 â€” AI retry with backoff
...
- Care points: each attempt allocates its own onResp/onText/onCatch and releases them before
  ...
  errors (400/401/404) short-circuit to onError â€” no point retrying a bad key.
- Request cancellation (the other half of the Â§2.1 line) is still open; it needs an AbortController
  wired to a caller-held cancel handle, which is a bigger surface. Retry/backoff stands alone. Pure
  tests + wasm green.

## 2026-06-16 â€” AI cost estimation (pure)

- Added `ai.EstimateCostUSD(model, usage)` over a small per-1M-token price table, plus `pricingFor`
  with longest-prefix matching so `gpt-4o-mini-2024-07-18` resolves to gpt-4o-mini (not gpt-4o â€” that
...
- Scope: pure logic only this commit. Surfacing it in the UI ("this used ~N tokens, ~$0.000x") needs
  ...
  passes the content string) â€” a transport-signature change touching Insights + Documents callers, so
  it's a separate follow-up rather than bundled here. The codec already has `ParseUsage`.

## 2026-06-16 â€” AI error handling: plain-English messages

- Added pure `ai.ErrorMessage(status, body)`: maps an OpenAI HTTP failure to an actionable message â€”
...
- Wired the fetch transport to capture `response.status` in the first `.then` and route any `>= 400`
  ...
  connectionâ€¦" â€” friendlier than echoing the raw JS error.
...
## 2026-06-16 â€” retroactive Apply-to-existing for rules
...
- Added `appstate.ApplyRules() (int, error)`: walks transactions, and for each uncategorized,
  ...
  the user's saved rules (not the implicit category-name rules) â€” this is an explicit user action, so
...
- Table-tested at the appstate layer (matchâ†’update, already-categorized untouched, no-match left
  alone, tags applied). New `rules.applyExisting*`/`rules.applied*` i18n keys. wasm green.

## 2026-06-16 â€” apply rules on image import
...
- Scope: the vision/AI image-import path (where UI code turns rows into transactions). The CSV-paste
  path goes through `app.ImportTransactionsCSV` â†’ the pure `store` CSV parser, which has no rules
  dependency; routing that through rules would mean a store-layer or post-import pass â€” left as a
  separate follow-up. wasm green.

## 2026-06-16 â€” apply saved rules on transaction entry
...
- Scope: manual entry only this commit. Import paths (CSV paste + vision reviewâ†’import) match
  categories by name today; routing those through `autoRules` is the remaining Â§2.4 "apply on import"
  piece â€” kept separate to stay feature-granular. wasm green.

## 2026-06-16 â€” Rules management screen
...
- Validation: client-side `validateRuleInput` shows localized messages (match + category required)
  so the raw `appstate:` error never reaches the UI â€” same approach as Categories' `nameRequired`.
- Reused the existing `parseTags` helper from transactions.go (caught a redeclare at build â€” they
  were identical) rather than duplicating it. Added `rules.*` + `nav.rules` i18n keys.
- This is the management-UI half of Â§2.4. Remaining: apply rules automatically on entry/import (the
  engine + store are ready; just needs to be invoked at those write paths).

## 2026-06-16 â€” store/state: persist auto-categorization rules
...
- Added persistence for the existing `rules` engine (which had no storage): a `rules` table, store
  ...
  foundation for Â§2.4 â€” the management UI and apply-on-entry come next, on top of this.
- Architecture decision: persisted `rules.Rule` directly rather than introducing a `domain.Rule` +
  engine refactor. Precedent already exists â€” the store persists `customfields.Def` (also a
  non-domain type) the same way â€” so this stays consistent and fully additive (no churn to the
...
- Schema decision: added `Rules []rules.Rule json:"rules,omitempty"` to the Dataset without bumping
  SchemaVersion, matching how `CustomFields` was added â€” an additive optional field is backward
...
## 2026-06-16 â€” dashboard: top Spending highlight widget

- Added `topHighlightWidget` to the bento: shows the #1 spending anomaly this month as a one-line
  highlight (green/red â†‘/â†“ + plain-English sentence), or a calm "no big changes" message. Placed at
  a fresh grid row (1 / span 4, row 9) so its new widget ID can't collide with users' persisted
  layouts â€” and since the dashboard is reconfigurable, they can drag it wherever they like.
- Refactored to keep it DRY: extracted `detectSpendingAnomalies` (build series â†’ Detect) and
...
- New i18n keys `dashboard.highlight`/`dashboard.noHighlights`. Catalog test, full native `go test`,
  and wasm build all green. Closes the Â§2.3 "show top insight on dashboard" piece.

## 2026-06-16 â€” insights UI: offline Spending highlights card

- Wired the anomaly engine into the Insights screen: `spendingHighlights` builds the last four
  monthly boundaries, runs `ledger.CategorySpendSeries` â†’ maps category IDs to names (uncategorized
  grouped under a localized label) â†’ `insights.Detect` with `DefaultOptions`, and renders each
  anomaly as a plain-English row with a green/red â†‘/â†“ marker. Rendered first in the screen and
...
- This completes the Â§2.3 "trend/anomaly highlights" line end-to-end (engine + feeder + UI). The
  remaining Â§2.3 items (pin/save insights, top insight on dashboard, advice from AI) stay open.

## 2026-06-16 â€” ledger: per-category spend-series feeder for anomalies
...
- Added `ledger.CategorySpendSeries(all, bounds, rates)`: buckets non-transfer expense into the
  ...
  raw transactions and `insights.Detect` â€” the UI maps the result to `[]insights.CategorySeries` with
  display names.
- Decoupling decision: the feeder lives in `ledger` (which already owns FX-aware aggregation) and
  ...
  with zero domain/currency dependencies. The trivial mapâ†’CategorySeries mapping happens at the UI
...
## 2026-06-16 â€” insights: pure spending anomaly/trend engine

- New `internal/insights` package: `Detect(series, opts)` takes per-category spend histories
  (oldestâ†’newest, positive minor-unit magnitudes), computes a baseline = mean of the prior periods,
  ...
  surfaced â€” no black box, per the determinism rule.
- Design choices: baseline excludes the current period (compares "this month" to "how it normally
  behaves"); a `MinBaseline` noise floor avoids the $0.50â†’$1.00 = "+100%" trap; zero/sub-floor
...
## 2026-06-16 â€” README: badges, live-demo link, License section

- The README already covered what/stack/build/architecture/docs; this fills the Â§0 gaps: a badge row
...
- Skipped screenshots/GIF for now â€” capturing them needs a browser session and image assets; the live
  demo link is the better first-impression substitute until those are produced deliberately.

## 2026-06-16 â€” MIT licensing: LICENSE file + SPDX convention

- Added a top-level `LICENSE` (standard MIT text, 2026, copyright holder `monstercameron` â€” the
  ...
  idiomatic Go and the TODOS Â§0 note.
- Placement decision: in `main.go` the SPDX line goes *above* the `//go:build js && wasm` constraint
  ...
  the native toolchain still excludes the file â€” so the constraint survived the insertion.
- Scope decision: did **not** sweep SPDX across all ~150 files in this commit. Many carry build tags
  ...
  it now does). TODOS Â§0 MIT item left open, narrowed to those follow-ups.

## 2026-06-16 â€” i18n: CI catalog-quality guard
...
- Added `TestDefaultCatalogQuality` to `internal/i18n`: asserts the seeded English catalog is
  ...
  would silently render the raw key in the UI â€” this catches that at `go test` time (ci.yml runs
  `go test ./...`).
- Scoped deliberately: values are NOT checked for trimming or format-verb validity. Several catalog
  strings legitimately carry leading/trailing spaces (suffix fragments like " Â· by %s") or a literal
  `%` ("APR %", "...cut spending 10%?"), and format strings are only Sprintf'd when args are passed â€”
...
## 2026-06-16 â€” Backlog: MIT licensing setup (Â§0)
...
- Decided on light SPDX references (`// SPDX-License-Identifier: MIT`) over a full per-file header â€”
  idiomatic for Go and keeps source files clean. Placed alongside the README item in Â§0 since it's
  foundational repo setup. Docs-only change; no code/tests affected.

## 2026-06-16 â€” i18n: language selector â€” central language store COMPLETE

- Added a **Display language** `<select>` to Settings â†’ Languages, listing `uistate.Languages()`
...
- **Milestone:** the central-language-store loop is closed â€” pick, export, import. What's left is
  optional polish: a CI catalog-completeness test (via `MissingKeys`) and localized language names.

## 2026-06-16 â€” i18n: TransactionRow â€” verbiage migration COMPLETE
...
- **Milestone:** every screen and shared component now renders user-facing text through `uistate.T`
  against the English catalog â€” the "hook up all the verbiage" request is fulfilled. Intentional
...
- **What's left for full localization** (not English-text changes): a language **selector** in
  Settings so imported languages actually display (TODOS Â§1.19), then optionally a CI catalog-
  completeness test. `i18n` tests + wasm green.

## 2026-06-16 â€” i18n: Transactions screen (main function)
...
## 2026-06-16 â€” i18n: AccountRow (Accounts screen complete)
...
- Migrated the `AccountRow` component: inline edit form (reusing the accounts.* field keys + common.name/
  ...
  cleared-balance suffix, and all row action buttons + titles (viewâ†’nav.transactions, update balance,
  mark updated, edit, archive/restore, delete). Accounts is now fully localized.
- Left `humanizeType` (a generic enum title-caser) as-is â€” localizing account-type display names is a
  separate domain-enum task. `i18n` tests + wasm green.
- **Last remaining UI verbiage: the Transactions screen** (the other giant).

## 2026-06-16 â€” i18n: Accounts screen (main function)
...
- Account *type* labels still come from the `humanizeType` helper (shared, used elsewhere) â€” left for a
...
## 2026-06-16 â€” i18n: settings panel right column (panel complete)
...
- Migrated the right column of `app/settings.go`'s global panel: AI section (title/enable/key
  ...
  seg Sunday/Monday), Date format title, Data action buttons, and the Languages buttons â€” all via
...
## 2026-06-16 â€” i18n: settings panel (left column + chrome)
...
- Kept the currency option display names ("USD â€” US Dollar") literal (registry territory). Split the
  panel across two cycles to keep edits bounded â€” the right column (AI/appearance/prefs/data/languages)
  is next.
- `i18n` tests + wasm green.

## 2026-06-16 â€” i18n: custom-fields components migrated
...
## 2026-06-16 â€” i18n: Settings screen migrated

- Twelfth screen onto i18n: `screens/settings.go` â€” household summary (reusing `nav.members/accounts/
...
## 2026-06-16 â€” i18n: Dashboard chrome migrated

- Eleventh screen onto i18n: `dashboard.go` â€” every widget title (reusing `nav.*` for Accounts/
...
- Left a couple of dynamic KPI sublines (`periodLabel + plural(...)`) literal â€” they concat a
  date-label with the English `plural()` helper, so cleanly localizing them is its own task (plural
  rules). The cashflow bar heights and the freshness "Â· %dd" chip stay `fmt` (numeric). `fmt` remains.
...
## 2026-06-16 â€” i18n: Allocate screen migrated

- Tenth screen onto i18n: `allocate.go` â€” profile picker + amount/reserve inputs, ranked rows (the
  breakdown via `T` args, exclude/restore), candidate name prefixes ("Pay down %s", "Goal Â· %s"),
...
## 2026-06-16 â€” i18n: Documents screen migrated

- Ninth screen onto i18n: `documents.go` â€” vision-import + CSV-import cards, the review/edit draft
...
- All `fmt.Sprintf`s became `T(key, argsâ€¦)`, so the `fmt` import was dropped. `i18n` tests + wasm green.
...
## 2026-06-16 â€” i18n: Customize screen migrated

- Eighth screen onto i18n: `customize.go` â€” formula-calculator title/desc, expression placeholder, the
  example chips ("Savings rate %", etc. â€” `%` is literal since T only Sprintf's with args), and the
  ...
  `%v` value fallback.
- `CustomFieldsManager` (rendered here, defined elsewhere) still has its own strings â€” a later screen.
...
## 2026-06-16 â€” i18n: Planning screen migrated

- Seventh screen onto i18n: `planning.go` â€” debt-payoff calculator (inputs, stat labels, hint/invalid/
...
## 2026-06-16 â€” i18n: Insights screen migrated
...
- **Decision:** left the OpenAI system/user prompt strings in English â€” they're instructions sent to
...
## 2026-06-16 â€” i18n: Budgets screen migrated (+ shared owner picker)

- Fifth screen onto i18n: `budgets.go` â€” add form (incl. `Limit (%s)` via T args), period picker, month
  stepper titles, spent/budgeted/left stats, the over/near summary, and budget rows (on-track/near/over
  labels, the `%s Â· %s Â· %d%% Â· %s left` sub via T args, edit/delete).
- Also localized the **shared** `ownerSelectOptions` helper ("Group (shared)" â†’ `owner.group`), which
  the budgets add-form now uses (replaced its inline duplicate) and which goals' edit row also calls â€”
...
## 2026-06-16 â€” i18n: Goals screen migrated

- Fourth screen onto i18n: `goals.go` â€” add form (incl. `Target (%s)` placeholder via T args), owner +
...
## 2026-06-16 â€” i18n: Categories screen migrated

- Third screen onto i18n: `categories.go` fully migrated â€” add form, kind + parent pickers (incl. the
...
- Introduced shared keys to curb sprawl: `common.name`, `common.reassignTitle`, `common.moveAndDelete`,
  and `category.expense`/`category.income`. (Members still has its own reassign/name keys â€” a tiny
...
## 2026-06-16 â€” i18n: Members screen migrated

- Second screen onto i18n: `members.go` fully migrated â€” add form, the reassign-before-delete panel
  (incl. the `%q owns %dâ€¦` description via `T(key, argsâ€¦)`), member rows (make-default, view
...
## 2026-06-16 â€” i18n: To-do screen migrated (first full screen)

- First full screen onto the language store: every user-facing string in `todo.go` now resolves via
  `uistate.T` â€” add-form title/placeholders, priority options (both the add form and inline edit),
...
- Added shared keys (`priority.high/medium/low`, `common.notReady`) so the other screens reuse them
  rather than re-adding. Catalog grew accordingly; English values match the old literals â†’ no visible
  change.
...
## 2026-06-16 â€” Extract credit-utilization into ledger
...
## 2026-06-16 â€” Top-bar verbiage migrated to i18n (chrome complete)
...
- The page title itself still comes from `screens.All()` data (per-route Title/Subtitle); i18n-ing
  those needs per-route title keys â€” left for the screen-verbiage migration pass.
- `i18n` tests + wasm green. **Next:** begin per-screen verbiage migration, or pivot to another area.

## 2026-06-16 â€” Language bundle export/import in Settings
...
- Made the language store's round-trip user-accessible (the user's "easy to export/import all langs"
  ...
  Settings â†’ Languages "Export languages" / "Import languages" buttons, wired through the toast.
- Note: with T still non-reactive (English-only display), an imported language is stored/ready but not
  shown until the language selector lands â€” the notice says "reload to apply" and the selector is the
  remaining Â§1.19 piece. Export gives translators the English source + any existing langs.
...
## 2026-06-16 â€” i18n live wiring + sidebar verbiage migrated

- Wired the language store into the UI: `uistate` holds a shared `i18n.DefaultBundle()`, a `UseLang`
  atom (default English), and a hook-free `T(key, argsâ€¦)` helper (resolves against the bundle default).
...
- **Design note:** kept T non-reactive for now (English-only). When a language selector lands (TODOS
  Â§1.19), the active lang gets threaded at render edges (read `UseLang` at a component top) rather than
  inside T â€” preserving the no-hooks-in-loops rule.
- wasm build green. **Next:** migrate more screens' verbiage onto T incrementally, or the next feature.

## 2026-06-16 â€” Extract bill next-due date into dateutil

- Moved `nextDue` out of the js-only dashboard into pure `dateutil.NextMonthlyDue(now, day)` â€” the
  next occurrence of a monthly due-day on/after today, day clamped to 1â€“28 so it's valid every month
  (incl. February). The upcoming-bills widget calls it.
- Table-tested the fiddly cases: later-this-month, on-the-day, already-passedâ†’next-month, >28 clamp,
  February clamp, non-positiveâ†’1. `internal/dateutil` + wasm green.
- (Fixed a wrong expected value while writing the test â€” day=0 clamps to the 1st, which is already
  past the 10th, so it rolls to next month.)

## 2026-06-16 â€” Extract savings-rate calc into ledger
...
## 2026-06-16 â€” Default category scheme (Â§1.10, pure)

- Added `internal/catscheme.Default()` â€” a starter set of income/expense categories plus a few
  sub-categories (Housing â†’ Rent/Utilities, Transportation â†’ Fuel/Public transit). Returns ID-less
...
- Bottom-up piece of Â§1.10 "Default scheme + reset": the pure scheme now exists; the "reset
  categories" action (apply it via appstate, replacing/merging) and methodology presets (envelope/
  zero-based) remain.

## 2026-06-16 â€” Backlog: Electron desktop app as a post-core item (Â§5.1)

- Added a new `TODOS.md` Â§5 "Future / nice-to-have (post-core)" tier and logged **standalone
  ...
  lower-priority and post-core â€” not part of the spec.
- Scoped it as a thin wrapper that reuses the *exact* production `web/` build (wasm bundle +
  ...
  wasm stays the single source of truth. Sub-tasks cover wrapper choice (Electron vs. Tauri/Wails â€”
...
## 2026-06-16 â€” Currency display helper (Â§1.2)

- Closed the Â§1.2 "format a Money in a target/base currency" checklist item: added
  `Rates.FormatAccounting(m, toCurrency)` and `Rates.FormatInBase(m)` to `internal/currency` â€”
...
- Table-tested: same-currency, negative-parenthesized, cross-currency conversion (â‚¬â†’$ and $â†’â‚¬),
...
## 2026-06-16 â€” B10 foundation: pure period presets
...
- Shifted off the widget-settings sweep to the B10 resolution-control redesign, starting with the
  ...
  whole window as a unit â€” distinct from the edge-only StepFrom/StepTo) and `Window.IsCurrent` (flag
...
- Dropped `LastNDays` from the plan: the Window model is unit-based (week/month/quarter), so an
  arbitrary N-day range doesn't fit it cleanly â€” noted for the UI step (those would need a different
...
## 2026-06-16 â€” B12: Spending breakdown top-N setting

- Fourth widget on the settings API: "breakdown" schema (`topN`, 2â€“6, default 3); the widget reads it
...
## 2026-06-16 â€” B12: Net worth trend months setting

- Third widget on the settings API: "trend" schema (`months`, 3â€“12, default 6); `netWorthTrendWidget`
...
## 2026-06-16 â€” B12: Recent transactions row-count setting

- Second widget on the settings API: registered a "recent" schema (`count`, number 3â€“20, default 6)
  and had `recentWidget` read it (`widgetCfgs.For("recent")`) to size the list instead of the
  hardcoded 6. Same pattern as savings â€” schema in `widgetcfg`, consumption in the screen, persisted
  via the `WidgetConfigs` atom.
- widgetcfg tests + wasm build green.
- **Next:** keep extending â€” net-worth-trend range (months), spending-breakdown top-N, etc.

## 2026-06-16 â€” B12: savings widget consumes its settings
...
- Closed the loop on the per-widget settings example: `savingsRateWidget` now reads its persisted
  ...
  hook position) and passes `widgetCfgs.For("savings")` down â€” cleaner than a hook inside the helper.
- Behavior: tone now reflects performance vs target â€” at/above target green, positive-but-short amber
  (`text-warn`), negative red; the subline shows the target; the bar hides when `showBar` is off.
- End-to-end demoable: gear â†’ set target/hide bar â†’ persists across reload â†’ widget reflects it.
...
## 2026-06-16 â€” B12 wiring: schema-driven, persisted widget settings panel
...
- Re-added `uistate.WidgetConfigs` (localStorage-backed atom; `For` + copy-on-write `WithField`), and
  ...
  (toggleâ†’ToggleRow, numberâ†’numeric input, selectâ†’Select) bound to the persisted config; placeholder
...
## 2026-06-16 â€” Pages deploy workflow (built); E2E test stories logged (B16)
...
- **Built `deploy-pages.yml`:** on push to main, set up Go, build `web/bin/main.wasm` (GOOS=js), copy
  ...
  - **Chose Actions-deploy over committing a `/docs` folder.** Same live URL
    ...
    Updated the Â§0 hosting item to reflect this; the only manual step is setting Pages Source =
    "GitHub Actions" once (will try via `gh api`).
- **B16 â€” E2E stories:** logged a trustworthy-app testing program â€” dozens of scripted user-journey
  ...
  Needs the Playwright/Chromium browser lane (Â§0, not installed) to run; authored/queued until then.
- Planning for B16; the deploy workflow is the one build action this turn (explicitly requested).

## 2026-06-16 â€” App-wide accessibility spike + program (B15)

- User: think deeply about app-wide accessibility and log it as a spike (it's extensive). Added B15.
- Framed it as a **spike first** (axe/keyboard/SR audit â†’ catalogue framework a11y primitives â†’ decide
  reusable patterns â†’ output a prioritized plan), then a deep area checklist that becomes the tasks:
  ...
  targets, i18n'd labels, and CI axe once Playwright is in (Â§0).
- Pointed the Â§1.20 one-line a11y item at B15 (subsumed). Planning-only.

## 2026-06-16 â€” Designed Lucide (B13) + D3 (B14) integrations (planning)
...
- **B13 Lucide:** the existing `ui.Icon(name string)` already emits 24Ã—24 stroked currentColor SVGs â€”
  Lucide's exact format â€” so it's a clean swap. Proposed a type-safe `internal/icon` (`Name` constants
  â†’ embedded Lucide path data, compile-checked), with a build-time generator to pull the curated set.
...
- **B14 D3:** proposed a pure, typed `chartspec.Spec` (Kind/Series/Axis + Validate + extent helpers)
  ...
  the heavy D3 dep vs. keep growing the working pure-Go SVG charts â€” needs the user's call before build.
- Both interfaces sketched in TODOS so the "strong Go interfaces" ask is answered at design level.

## 2026-06-16 â€” Switch pushes to gh; pause widget-settings wiring â†’ B12

- User: use the `gh` CLI for pushing (more reliable), and go back to TODO-planning mode.
- **Push fix:** background `git push` was failing with "could not read Username for github.com" â€” the
...
- **Back to planning:** the per-widget settings API foundation (`internal/widgetcfg`) is committed and
  ...
  package stays â€” it's a standalone tested foundation with no consumers yet, which is fine.

## 2026-06-16 â€” Per-widget settings API (widgetcfg) â€” step 1

- User wants the per-widget flip panel wired to each widget's own settings (savings rate â†’ savings
  settings), persisted. Building it bottom-up.
- Step 1: pure `internal/widgetcfg` â€” `Field` (toggle/number/select + default/bounds/options),
  `Schema` per widget, `Config` (keyâ†’string values) with typed accessors (`Str`/`Bool`/`Int` with
...
## 2026-06-16 â€” README + Pages-hosting TODOs; background pushes

- Logged two Â§0 items: a proper **README.md**, and **hosting the app on GitHub Pages from `/docs`**
  (production build committed to `docs/`, relative paths for the `/CashFlux/` subpath, a `404.html`
  shell for deep-link routing â€” the static-host side of B1 â€” and a build script so `/docs` is
  regenerated, not hand-copied). Ticked the now-stale "create repo + push" Â§0 item.
...
## 2026-06-16 â€” Logged "+ Add" flip-panel of add actions (B11)
...
## 2026-06-16 â€” Deep-analyzed the time-resolution control (B10)
...
- Core finding: the dual From/To stepper makes a *range* the default when ~90% of users want a single
  period â€” and it reads "Jun 2026 â€“ Jun 2026" (looks broken) in that common case. Also no presets, no
...
- Proposed: a single period stepper as the primary, a presets dropdown (This/Last month, This quarter,
  YTD, Last 30 days, Custom rangeâ€¦), a "this period"/Today reset, and the existing From/To range tucked
...
## 2026-06-16 â€” Logged top-bar breadcrumb (B9)
...
- User wants a clickable breadcrumb on the right of the top-level panel for stepping backwards.
  ...
  defined meaning â€” home-rooted `Dashboard / {page}` (recommended, no history needed), a visited-
...
## 2026-06-16 â€” Logged sidebar menu management (B8)
...
## 2026-06-16 â€” Polished boot loader + app settle-in animation

- Replaced the plain "Loading CashFluxâ€¦" boot text with an on-brand animated loader: a spinning
...
- **Next.** (User has queued more menu requests â€” logging those separately.)

## 2026-06-16 â€” Logged UI-scale (B6) and missing-menu-items (B7); reverted a premature impl
...
- User asked for a font/UI-scale setting and to update the menu with all main-line features, then
  ...
  scale feature (prefs field + ApplyPrefs + CSS + settings control) â€” reverted those uncommitted
  changes back to HEAD and captured both as backlog items instead.
- **B6 (UI scale):** design is px-heavy, so a rem scale won't catch buttons â€” logged a `--ui-scale`
  zoom on `#app` driven by a `prefs.Scale` percent (70â€“130). Analysis kept for when it's picked up.
...
## 2026-06-16 â€” Localization foundation: central language store (i18n)
...
- User wants all page verbiage localizable via a central store with easy export/import of all langs
  ...
  (`nav.accounts`) over English-source-as-key â€” stable keys, copy edits don't orphan translations,
  cleaner export files.
- Built the bottom-up foundation: pure `internal/i18n` â€” `Bundle`/`Catalog`, `T(lang, key, argsâ€¦)`
  with an enâ†’key fallback chain and `fmt.Sprintf` formatting, `Set`, `Languages` (default first),
...
- This is the store only â€” no UI wiring yet. "Hook up all verbiage" is a large per-screen migration,
  so it's tracked as a multi-commit task (see TODOS Â§1.19): active-language atom + localStorage of
...
## 2026-06-16 â€” Logged settings-duplication (B4) and collapsed-rail-hover (B5)
...
- **B4 settings duplication:** confirmed two settings surfaces. The `/settings` nav screen
  ...
  emptier duplicate. Logged fix: make the household-card panel the single primary surface â€” move the
...
## 2026-06-16 â€” B2 step 1: pure dashboard packing engine

- Started the B2 dashboard-grid rewrite bottom-up: added `internal/dashlayout/pack.go` â€” an
...
- Kept the legacy `Placement`/`Swap`/`Resize` API intact â€” this commit is the engine only, fully
...
- **Design note:** chose first-fit *with* gap backfill (CSS auto-flow "dense" semantics) â€” deterministic
...
## 2026-06-16 â€” Dashboard: Shift-to-reveal resize handles (+ animation reqs â†’ B2)
...
- The two animation requirements are entangled with the B2 reflow-engine rewrite (CSS-grid placement
  changes don't transition natively â€” they need a FLIP technique over the packed layout), so I folded
...
## 2026-06-16 â€” Diagnosed the page-duplication-on-route bug (B3)
...
- User reported the page sometimes duplicating on navigation and asked me to scan the live DOM with
  ...
  server isn't connected this session. Diagnosed from the framework router source instead â€” which is
  definitive for this bug.
- **Cause:** GoWebComponents' router is a *nested-layout* router. `expandPathPrefixes("/accounts")` â†’
  ...
  registers every route â€” including `/` â€” as a full `Shell`, and no Shell calls `GetOutlet()`. Result:
  non-root navigation renders the `/` Dashboard Shell **and** the target Shell = duplicated page. The
  `*` route is innocent â€” `Register("*", â€¦)` is the router's not-found factory, not a stacking pattern.
- **Fix (logged as B3):** adopt the framework's layout+outlet structure â€” `/` becomes a chrome-only
...
## 2026-06-16 â€” Logged two bugs (deep-link 404, dashboard drag) to the backlog

- User reported two bugs; analyzed both and added a high-priority **Â§B Bug fixes** section to TODOS.
- **B1 â€” deep-link 404:** not a router bug. `NewHistoryRouter` gives clean pushState URLs and its `*`
  ...
  + a server SPA history rewrite (ties into the existing `gwc dev -html` item). Keep clean paths â€” no
  hash router.
- **B2 â€” dashboard drag:** `ui.Widget`'s drop calls `dashlayout.Swap`, a pairwise exchange of absolute
  ...
  reflow in the UI â€” iOS-home-screen behavior that respects multi-cell tiles.
...
- **Next.** Implement when picked up â€” B1 is the smaller/safer (SW + serve config); B2 is a real
  layout-engine rewrite (pure model + tests first).

## 2026-06-16 â€” Extract to-do ordering into a tested package

- Knocked off part of TODOS Â§1.14 "Tests: ordering, status transitions": the list ordering was a
...
- Mirrors the earlier `txnfilter` extraction â€” same pattern of pulling a core behavior out from
...
## 2026-06-16 â€” Extract (and fix) the net-worth percent-change calc

- The dashboard KPI computed `(net - prev) * 100 / prev` inline â€” both a "no computation in view
  code" violation and a latent bug: dividing by the *signed* baseline flips the arrow when net worth
  is negative (âˆ’1000 â†’ âˆ’500 is a +50% improvement but rendered as â–¼50%). Liability-heavy households
  hit this.
...
- **Why ledger and not a UI helper?** It's a money-derivation, same family as `NetWorth`/`PeriodTotals`
  â€” keeping it there means it's covered by the package's table tests and reusable by other KPIs.
...
## 2026-06-16 â€” Toast confirmations for data actions

- Made the toast dual-purpose (it already supported a non-error/info style via `Notice.Err=false`):
  the Settings data actions â€” Export JSON/CSV, Import, Load sample, Wipe â€” now post a success message,
...
- **Plumbing note:** these are package-level funcs (`exportJSON`, â€¦), not components, so they can't
...
- Scope check: this is the Â§1.4 error-surface item, not feature-inference â€” export/import/wipe
...
## 2026-06-16 â€” Route remaining swallowed writes through the toast

- Followed up the toast surface by sweeping the last `_ = app.Putâ€¦` sites in the screens
  (`grep _ = app\.(Put|Delete|â€¦)`): Accounts' "Mark all updated" bulk balance refresh, and the
  dashboard freshness nudge's "Remind me" task creation. Both now surface a friendly toast on failure.
- The nudge additionally **gates navigation on success** â€” previously it jumped to /todo regardless,
...
## 2026-06-16 â€” App-wide toast surface for silent failures

- Picked up TODOS Â§1.4 "Error/toast surface for failed persistence." Several bulk paths in the
  ledger screen swallowed errors with `_ = app.Putâ€¦` (bulk recategorize, bulk mark cleared/uncleared,
  and the paired-transfer delete) â€” so a failed write left the UI looking successful.
- Added `uistate.Notice` (a tiny `{Seq, Text, Err}` atom; `Seq` bumps per post so identical text
  ...
  timer and releases the `js.Func` exactly once â€” `Cleared()` preserves `Seq` so the fire doesn't
...
- Verified the wasm build and native suite (toast/notice are js-only, so no native target â€” expected).
- **Next.** Route more swallowed `_ = app.Putâ€¦` sites (other screens' bulk/delete) through the toast,
  or further small polish.

## 2026-06-16 â€” Dashboard week boundaries honor the week-start preference
...
- Added a pure, tested `period.Window.WithWeekStart(weekday)` mutator (re-snaps both anchors under
  ...
  â€” consistent with how the date-format pref already updates lists live.
- **Why a mutator in the pure package rather than rebuilding the window in view code?** Date/anchor
  ...
  behavior table tests (week re-snap Monâ†’Sun, no-op, month-untouched).
...
- **Next.** Further small polish; the local-first feature set is comprehensive (sync remains the only
  out-of-scope major item â€” needs a hosted backend).

## 2026-06-16 â€” Dashboard resolution persists across reloads
...
- **Why persist only the resolution, not the whole window?** The From/To anchors are transient
  navigation â€” restoring last session's anchored week/month would dump the user on a stale period.
...
## 2026-06-16 â€” Refactor: transaction filtering â†’ pure tested package
...
## 2026-06-16 â€” Goals: combined progress header

- Added a stat-grid above the goals list (when there are goals): total saved, total target, and
  overall progress % â€” the at-a-glance "how am I doing across everything" the per-goal bars don't
...
## 2026-06-16 â€” Transactions: filtered summary line

- Added a "N shown Â· net $X" line above the ledger list: the count of the filtered set plus its net
  ...
  date, cleared) live.
- **Net, not income/expense.** It's the raw sum of shown amounts â€” for an account or category filter
  that's the meaningful figure; transfers (rare in a filtered view) net out within an account anyway.
- Pairs naturally with the existing "Export CSV" of the same filtered set â€” see the total, then
  export it.
- **Next.** Genuine small polish only; the local feature set is comprehensive (sync needs a backend).

## 2026-06-16 â€” Docs: status refresh

- The CLAUDE.md "Status" bullet had drifted badly â€” it still listed custom-field defs, document
  vision AI, and reload-persistent prefs as *remaining*, all of which shipped many commits ago.
  Rewrote it to reflect reality: Phases 1â€“2 essentially complete, every entity add/edit/delete, the
...
## 2026-06-16 â€” Members â†’ ledger drill-down

- Added a "Transactions" button to each member row that sets the persisted `TxFilter.Member` and
  navigates to `/transactions` â€” same parent-owned-closure + `OnView` pattern as the account
...
## 2026-06-16 â€” Accounts: update-balance reconcile

- Added "Update balance" to account rows: `promptText` asks for the real balance, then `setBalance`
  posts a cleared "Balance adjustment" transaction for `target âˆ’ currentBalance` and stamps
  `BalanceAsOf`. So the computed balance matches a statement in one step, with the gap recorded as a
  real (cleared) transaction rather than silently overwritten â€” keeps the ledger honest.
...
- Satisfies the Â§1.9 "Update balance â†’ adjustment txn + set BalanceAsOf" backlog item.
- **Next.** Genuine small polish; sync is the only large remaining item and needs a backend.

## 2026-06-16 â€” Freshness nudge â†’ to-do (create-from-nudge)
...
- The dashboard freshness widget's stale state now has a "Remind me" button that creates a
  ...
  widget as a `ui.Handler` param â€” keeping the widget a plain function while keeping the hook order
  safe.
- **Gotcha:** `ui.UseEvent` returns `ui.Handler`, not `func()`; the widget param had to be typed
  `ui.Handler` for `OnClick` to accept it.
- Both create-from hooks are now done: AI insight â†’ task (Insights) and freshness nudge â†’ task
...
## 2026-06-16 â€” Insights â†’ to-do (create-from-insight)
...
- Wires the Â§1.14 "create-from-insight" backlog hook â€” AI advice becomes an actionable, tracked
...
## 2026-06-16 â€” Accounts â†’ ledger drill-down

- Each account row gets a "Transactions" button that sets the persisted `TxFilter` to that account
  (normalized, other filters cleared) and navigates to `/transactions` â€” a quick way to see one
...
## 2026-06-16 â€” Accounts: Mark all updated

- Added a bulk "Mark all updated (N stale)" button (shown only when something's stale) that stamps
  `BalanceAsOf = now` on every stale, non-archived account via `PutAccount` â€” clearing all stale
...
## 2026-06-16 â€” Accounts: edit lock-until inline (lock-until complete)
...
## 2026-06-16 â€” Allocation: honor account lock-until

- Put the long-unused `Account.LockUntil` to work. The Allocate screen now skips an asset account
  whose `LockUntil` is in the future when building candidates â€” you can't put new money into a locked
  account (a CD, a vesting lot), so it shouldn't be suggested. Added a "Locked until" date to the
  account add form's asset section.
- Clear semantics (locked = no new money before the date), so no spec ambiguity â€” unlike a member
...
## 2026-06-16 â€” Spending breakdown: roll up to parent categories
...
- Reused the existing top-3-plus-Other rendering â€” only the bucketing key changed (root ancestor
...
## 2026-06-16 â€” Sub-categories: re-parent on the inline editor
...
- **Self excluded; deeper cycles tolerated.** The picker drops the category itself (the obvious
  ...
  guard drops cyclic nodes from display rather than looping â€” so the worst case is a temporarily
...
## 2026-06-16 â€” Sub-categories: add-form picker + indented lists
...
- **Kind/parent consistency.** Changing the kind clears the parent choice (`onKind` resets
  `parentID`), since a parent must share the child's kind â€” avoids creating a cross-kind nesting.
...
## 2026-06-16 â€” Sub-categories: the tree engine

- `Category.ParentID` has existed in the schema but was unused. Started sub-categories bottom-up with
  a pure `internal/categorytree`: `Build` â†’ forest of `Node`s (siblings name-sorted), `Flatten` â†’
...
- **Defensive by construction.** Bad parent data shouldn't break the UI: an orphan (parent missing)
  or self-reference becomes a root, and a mutual cycle (aâ†”b) yields no roots rather than recursing
...
## 2026-06-16 â€” Ledger: cleared balance

- Added pure `ledger.ClearedBalance` (opening balance + only `Cleared` transactions) â€” the
  reconciliation figure â€” mirroring `Balance` but skipping uncleared rows. Tested against a mix of
...
- Surfaced on the account row: when the cleared balance differs from the live balance, the meta line
  shows "Â· cleared $X", so the gap (uncleared activity) is visible at a glance and the cleared figure
...
## 2026-06-16 â€” Transactions: bulk mark cleared
...
- Reconciliation is now fully ergonomic: filter to "not cleared", select a run of statement-matched
  rows, "Mark cleared" â€” repeat. Per-row toggle remains for one-offs.
- **Next.** Continue with genuine small polish where it helps; the major remaining backlog (sync)
  needs a backend.

## 2026-06-16 â€” Transactions: cleared-status filter
...
- Completed the reconciliation loop: a tri-state cleared filter (both / not cleared / cleared) added
  ...
  items off it.
- Reused the existing persisted-filter + `applyTxFilter` plumbing â€” the new field flows through
...
## 2026-06-16 â€” Transactions: cleared/reconciled toggle

- Surfaced the long-defined-but-unused `Transaction.Cleared` flag. Each row gets a toggle
  ("Mark cleared" â†” "Cleared âœ“") that flips it via `PutTransaction`, and the meta line shows
  "Â· cleared" â€” the start of statement reconciliation.
...
## 2026-06-16 â€” To-do: inline edit (CRUD-edit complete across all entities)
...
- Added inline edit to `TaskRow` (title, priority, due, notes), the last entity without it.
  ...
  persists via `PutTask` â€” mirroring the goal date-clearing behavior.
- Now genuinely every entity â€” accounts, transactions, budgets, goals, members, categories, tasks â€”
...
## 2026-06-16 â€” Categories: inline edit (CRUD-edit fully complete)
...
- **Every entity is now fully editable inline** â€” accounts, transactions, budgets, goals, members,
  categories â€” each with the same unconditional-hooks + editing-toggle shape. Combined with the
...
## 2026-06-16 â€” Members: inline edit (name + color)

- Closed a real CRUD gap â€” members had add / delete / set-default but no edit. `MemberRow` now has
...
## 2026-06-16 â€” Transactions: export the filtered view to CSV
...
- Added an "Export CSV" button to the ledger filter bar that downloads exactly what's shown. To
  ...
  used by both the render and the export handler â€” no duplicated predicate logic to drift.
...
- **Caught my own refactor fallout:** removing the inline `fa/fc/fm` locals left the filter-option
  ...
  suite in a clean shell â€” a stray `GOOS=js` from the combined build command had made the first
...
- **Next.** The feature set is comprehensive; I'll keep making small, genuinely-useful additions
  (export ergonomics, empty states) and avoid inventing churn â€” the major remaining item (sync)
  needs a backend.

## 2026-06-16 â€” Accounts: editable owner (ownership editing uniform)
...
- Added the owner picker to the account inline editor. Since `AccountRow` already builds a `cp` copy
  ...
  `ownerSelectOptions`).
- Ownership is now editable inline everywhere it can be owned â€” accounts, budgets, goals â€” closing
...
- **Next.** The local, single-device feature set is now effectively complete (every entity: create /
  ...
  backend and per-entity version metadata â€” out of scope for this local-first build to implement
  meaningfully. Will keep doing contained polish where it adds real value.

## 2026-06-16 â€” Goals: editable owner
...
## 2026-06-16 â€” Budgets: editable owner
...
- Added a reusable `ownerSelectOptions(members, selected)` helper (group + members) â€” the first step
...
## 2026-06-16 â€” Budget periods: the UI (feature complete)
...
- **Note on the month stepper.** It still navigates a reference date by month; a weekly/quarterly
  ...
  nicer but means a per-period stepper â€” deferred; the current behavior is correct and clear with the
  period label visible.
- Budget periods are now end-to-end: enum â†’ `PeriodRange` engine (tested) â†’ selector + per-budget
  evaluation.
...
## 2026-06-16 â€” Budget periods: enum + range engine
...
- Lifting budgets beyond monthly, bottom-up. `domain.Period` gains `PeriodWeekly`/`PeriodQuarterly`
  ...
  (Aprâ€“Jun â†’ Apr 1..Jul 1), monthly via the existing `MonthRange`; unknown falls back to monthly.
...
## 2026-06-16 â€” Goals: linked account
...
- Put the long-defined-but-unused `Goal.AccountID` to work. Added an optional "linked account"
  ...
  grew a param), and the row now shows "Â· linked to <name>" via an `accountName` lookup.
- **Scope â€” record the link, don't auto-sync the balance (yet).** Linking captures *which* account
...
## 2026-06-16 â€” Goals: pace guidance (save $X/mo)

- Added `goals.MonthlyNeeded(goal, from)`: remaining Ã· whole months until the target date, partial
...
- `GoalRow` shows "Â· save $X/mo" alongside the "by <date>" when the goal has a future target and
  isn't complete â€” turning a static deadline into an actionable pace. Reused `fmtMoney` and the
  prefs date format.
- **Decision â€” round up, minimum one month.** Better to suggest slightly too much than to land short
...
## 2026-06-16 â€” Accounts: inline edit (CRUD-edit fully complete)

- The last CRUD-edit gap: `AccountRow` now edits inline. It mirrors the add form â€” name, opening
  balance, and the `If(isLiab,â€¦)`/`If(!isLiab,â€¦)` split for liability vs asset attributes. `OnSave`
...
## 2026-06-16 â€” Transactions: bulk recategorize (bulk actions complete)
...
- **Transfers skipped.** Transfers aren't categorized (they show "Transfer"), so bulk recategorize
  ignores selected transfer legs rather than stamping a meaningless category on them â€” mirrors how
  the per-row edit/duplicate already exclude transfers.
- The empty option is "No category", so Apply can also *clear* categories â€” a legitimate bulk
  action â€” not just set one.
- Bulk actions are now complete: select â†’ recategorize and/or delete â†’ clear. Closes the Â§
...
## 2026-06-16 â€” Transactions: bulk select + delete

- Added multi-select to the ledger. A `selected` set state in `Transactions`; each `TransactionRow`
  gets a `â˜/â˜‘` toggle button (reusing the to-do `check` style and the per-row-component hook rule),
...
- **Bulk delete reuses `deleteTxn`.** Rather than a separate path, bulk delete calls the existing
  per-row `deleteTxn` for each selected id â€” so transfer pairs are still removed together, and a
...
- Used a glyph toggle button instead of a real `<input type=checkbox>` to dodge the checked-attribute
  binding question and match the existing to-do check control â€” consistent and simple.
...
## 2026-06-16 â€” Document import: dedupe vs existing (review TODO closed)
...
- **Same Signature for both sides.** The screen builds the seen-set by rendering each existing
  ...
  `Signature()` â€” so the row-side and txn-side normalize identically. That sidesteps the earlier
...
- Scoped to the chosen account and to date+amount (not description), which is the right
  conservativeness â€” it won't suppress two genuinely-different same-day same-amount entries across
  accounts, and the user still sees and can re-add anything via the review list. Skips are reported.
- Closes the Â§2.2 review TODO (list, edit, remove, import, dedupe all done). Tests: signature
...
## 2026-06-16 â€” Document review: per-row edit (review complete)
...
- The review screen is now full-featured: list â†’ edit any row â†’ remove any row â†’ pick account â†’
...
## 2026-06-16 â€” Document review: per-row remove

- Small polish on the just-shipped import: the review list rows are now `DraftRow` components with a
  âœ• that removes that row from the draft slice (`removeDraft(i)` rebuilds the slice without index i).
...
- The row owns only an event hook (no state), so reusing instances across a removal is harmless; a
  plain index loop building `CreateElement` nodes is enough â€” no MapKeyed needed.
...
## 2026-06-16 â€” Document vision AI: the Documents UI (feature complete)

- Wired the three pieces into a working flow: Choose image â†’ `pickImageDataURL` (a small js helper
  that creates a hidden file input, reads the chosen file via `FileReader.readAsDataURL`, and calls
  back with the data URL) â†’ "Read with AI" â†’ `SendVisionChat` with a strict JSON system prompt â†’
  `extract.ParseRows` â†’ a review list â†’ pick account â†’ `importDraft` maps rows to transactions and
...
- **Mapping decisions at import:** amounts parse to minor units with the chosen account's currency
  ...
  read-only for v1 â€” per-row editing can come later, but the user already controls the account and
...
- Document vision import is now end-to-end (codec â†’ transport â†’ parser â†’ UI). The CSV paste path is
...
## 2026-06-16 â€” Document vision AI: the extraction parser
...
- **Decision â€” strings out, not domain.Transaction.** `Row` is all strings and the package has no
  domain dependency. The user reviews/edits before import, and the screen maps rows â†’ real
...
- Fixed a first-draft bug where `amountString` returned early on a missing key instead of trying the
  next synonym â€” caught by the string-amount test. Six table tests cover array, wrapper, fence,
  skip-empty, and two error cases.
- **Next.** The Documents-screen flow: pick an image â†’ base64 data URL â†’ `SendVisionChat` with a
  strict "return JSON" prompt â†’ `ParseRows` â†’ editable draft rows â†’ import against a chosen account.

## 2026-06-16 â€” Document vision AI: the transport
...
- Added `SendVisionChat` and, while there, factored the fetch promise chain out of `SendChat` into a
  ...
  their body (text or vision) and hand it to the same network code â€” no duplicated js.Func juggling
  or release logic.
...
- **Next.** The Documents-screen flow: pick an image, base64 it into a data URL, call
  ...
  the user review and import. The JSONâ†’transactions mapping should be a pure, tested helper.

## 2026-06-16 â€” Document vision AI: the request codec
...
- **Decision â€” a separate `visionRequest` shape, not a looser `Message`.** Rather than change
  ...
  *response* is identical to a text chat, so `ParseResponse` is reused as-is â€” no new parse path.
...
- **Next.** A js/wasm `SendVisionChat` transport (read the picked file â†’ base64 data URL â†’ fetch),
  then a Documents-screen flow: pick an image, parse the model's JSON into draft transactions to
  review and import.

## 2026-06-16 â€” Transactions: inline edit (non-transfers)
...
- **Sign + account preserved, not re-entered.** The amount field shows the absolute value
  ...
  (changing it is rare and affects currency) â€” that stays a delete-and-re-add.
- **Transfers excluded.** Editing one leg of a paired transfer can't keep the pair consistent, so â€”
  like Duplicate â€” Edit is hidden on transfer rows.
...
## 2026-06-16 â€” Goals: inline edit
...
- **Empty date clears the deadline.** A blank date field sets `TargetDate` to the zero time (no
  deadline) rather than erroring â€” matching the add form's optional-date behavior. The date input is
...
## 2026-06-16 â€” Budgets: inline edit
...
- **Hook discipline.** All the row's hooks (`del`, `editing`, two field states, five event hooks)
  ...
  mode never reorders hooks â€” the trap that bites when you wrap hooks in an `if`.
...
## 2026-06-16 â€” Members: reassign-before-delete

- Mirrored the category reassign flow for members. `appstate.ReassignOwner(old, new)` moves owned
  accounts, budgets, and goals â€” and re-attributes the member's transactions â€” to the new owner,
...
- **Decision â€” reuse the existing per-screen reassign shape rather than abstracting it.** Categories
...
- Both delete-guards (members Â§1.13, categories) are now reassign flows, not dead ends.
- **Next.** A larger remaining area â€” document vision-AI parsing, or a Phase-3 sync primitive â€” or an
  empty-state/accessibility polish pass.

## 2026-06-16 â€” Categories: reassign-before-delete
...
- Replaced the hard block on deleting an in-use category with a reassignment flow. Logic first:
  ...
  store directly â€” the records are already valid, just re-categorized) and reports how many moved;
...
- **Decision â€” reassign to any category, not just same-kind.** Simpler and occasionally useful
...
- **Next.** Pick the next backlog item â€” likely the document vision-AI parse path, or a Phase-3 sync
  primitive, or smaller polish (empty-state/accessibility pass).

## 2026-06-16 â€” Freshness overrides: the editor (feature complete)
...
- **Per-row component again.** Each input is a `freshnessRow` (CreateElement) so its `OnInput` hook
  is at a stable position â€” rendering them in a plain loop would break hook order.
- **0 means never.** Kept freshness's existing semantics (`window <= 0` â†’ never stale) rather than
  inventing a separate "off" control; the helper text says "0 = never". To restore a default a user
  re-types it â€” acceptable, and avoids a tri-state.
- Freshness overrides are now end-to-end: stored field â†’ `Merge` in the engine â†’ `FreshnessWindows`
  application â†’ Settings editor.
...
## 2026-06-16 â€” Freshness overrides: apply them

- `Settings.FreshnessOverrides` (a `map[string]int` of account-type â†’ days) has existed and round-
  trips through export/import, but nothing read it â€” the screens always used `DefaultWindows()`.
...
- **Bottom-up first.** No editor UI yet, but the feature is already functional â€” overrides set via
  imported JSON now change staleness â€” and the logic (`Merge`) was already tested. The Settings
...
## 2026-06-16 â€” Transactions: persist the last filter
...
- **Refactor shape â€” one atom, a `setFilter(mutator)` helper.** Rather than seven persisted states,
...
- **Why an atom and not just persisted `UseState`s:** reading-then-persisting the full struct avoids
  the trap of `Set` followed by a stale `Get` in the same handler â€” the mutator operates on a fresh
  `Get()` and persists exactly what it sets.
- **Next.** Another contained backlog item â€” the category-delete reassign flow, or the
  freshness-window overrides editor (the `Settings.FreshnessOverrides` field already exists).

## 2026-06-16 â€” Allocation: amount-split UI

- Added two number inputs to the Allocate profile card â€” amount to allocate and emergency buffer â€”
...
- **Reactive for free.** Everything recomputes from the input states each render, so changing the
  amount, buffer, profile, or excluding a destination instantly re-splits â€” no explicit wiring,
...
- Allocation constraints are now meaningfully complete: rank â†’ exclude/restore â†’ split an amount
  with buffer and (engine-level) per-destination caps.
- **Next.** A different backlog item â€” persist-last-transaction-filter, the category-delete reassign
  flow, or the freshness-window overrides editor.

## 2026-06-16 â€” Allocation: amount-split engine
...
- The ranking told you the *order*; `Distribute` now turns it into *amounts*. Pure function:
  ...
  remainder.
- **Decision â€” don't redistribute the remainder, return it.** Capped overflow and integer-rounding
...
## 2026-06-16 â€” Allocation: exclusion UI

- Wired the engine constraint into the Allocate screen. An `excluded` map state feeds
  `allocate.Constraints{Exclude: â€¦}` into `RankWith`; excluded destinations drop out of the ranked
...
- **Component split for hooks.** The ranked list was a plain `for`-loop of `Div`s; adding an Exclude
  button there would put an `OnClick` hook in a loop â€” the cardinal sin. So the row became its own
...
## 2026-06-16 â€” Allocation: exclusion constraint (engine)
...
## 2026-06-16 â€” Transactions: per-row Duplicate
...
- Small, self-contained quality-of-life feature: a Duplicate button on each transaction row. The
  ...
  `app.PutTransaction` â€” so it re-validates and honors custom fields like any new entry.
- **Decision â€” clear the transfer link on duplicate and only offer it for non-transfers.** A
  ...
  button is hidden on transfer rows (`If(!IsTransfer, â€¦)`).
...
- **Next.** Another contained item â€” persist-last-transaction-filter, or an allocation constraint
  (e.g. exclude destinations / emergency buffer) which would start with a pure engine change.

## 2026-06-16 â€” Appearance prefs: the light theme (feature complete)
...
- Authored the `[data-theme="light"]` skin deferred last commit. Three layers needed overriding,
  ...
  (one block flip covers all the screen components â€” cards, stats, fields, buttons, bars, badges);
  (2) the shell's Tailwind utility classes (`.bg-base`, `.text-fg`, `.border-line`, the
  arbitrary-value `.bg-[#1c1c1e]` active-nav surface â€” escaped as `.bg-\[\#1c1c1e\]`); (3) the
...
- **Verified the one risky override.** `text-base` is also a Tailwind font-size utility, so blindly
  coloring it could turn body text invisible â€” grep showed it's used in exactly one place (the brand
...
- Appearance preferences are now complete end-to-end: engine (week-start/date/theme/accent/density)
  â†’ localStorage atom â†’ Settings UI â†’ `ApplyPrefs` to the DOM â†’ working light/dark skins, all
  reload-persistent. Only fiscal-month start remains from the original preferences line.
- **Next.** A fresh backlog item â€” likely persist-last-transaction-filter or a per-row transaction
  duplicate action (both small, contained), or an allocation constraint.

## 2026-06-16 â€” Appearance prefs: apply to the DOM
...
- `uistate.ApplyPrefs(p)` reflects prefs onto `document.documentElement`: `data-theme` (with
  ...
  mounting, so the first paint is already correct â€” no flash of defaults. `savePrefs` calls it too,
...
- **Why accent works immediately:** the legacy `:root --accent` var is wired through the
  ...
  rows, and fields.
- **Honest scope note â€” theme is half-applied on purpose.** The candidate-C surfaces are authored in
...
## 2026-06-16 â€” Appearance prefs: wire the controls

- Replaced the three local `UseState`s (theme/accent/compact) in `globalSettingsForm` with reads off
  the normalized `pr` and writes through the existing `savePrefs` (normalize â†’ atom set â†’
  `PersistPrefs`). Dropping three hooks is safe â€” they were removed wholesale, so hook order stays
...
- **Note.** This makes the *preference* real and durable, but it does not yet *apply* visually â€” the
...
## 2026-06-16 â€” Appearance prefs: extend the engine
...
- **Decision â€” validate the accent in `Normalize` with a tiny `isHexColor`.** Accent comes from a
...
## 2026-06-16 â€” Module visibility: Settings toggles (feature complete)
...
- Final step: a "Screens" section in the global settings left column. A package-level
  ...
  the per-row toggles render safely in a plain loop â€” no parent hook-ordering worry.
- Each toggle's `OnChange` calls `toggleModule(path)` â†’ `Toggle` (immutable) â†’ atom `Set` â†’
  ...
  survives reload.
- Module visibility is now end-to-end: pure engine (locked + toggle) â†’ localStorage atom â†’ sidebar
  filter â†’ Settings toggles. Closes Â§1.18's show/hide-screens item.
- **Next.** Other Â§1.18 items remain (theme/density, fiscal-month start, budgeting methodology
  selector), or move to a contained Phase-3 sync primitive. Will pick the next granular increment.

## 2026-06-16 â€” Module visibility: sidebar filtering
...
- `Sidebar` now reads `uistate.UseHiddenModules().Get()` and filters: the primary nav is built into a
  ...
  (Members, Categories) are wrapped in `If(!hidden.IsHidden(path), â€¦)`. Settings and Dashboard are
  locked in `internal/modules`, so they are never filtered â€” no special-casing needed here.
...
- **Scope note.** Hiding is a *navigation* concern: the routes stay registered, so a hidden screen
  reached directly by URL (or the unknown-path fallback) still renders. That is deliberate â€” we are
...
- **Next.** The Settings panel show/hide toggles, which write `Toggle` + `PersistHiddenModules` for
  each hideable screen â€” the last step to close this feature.

## 2026-06-16 â€” Module visibility: the persistence atom

- `uistate/modules.go` â€” the third localStorage-backed atom (after layout and prefs), same shape:
...
- Thin plumbing again â€” the `Normalize`/locked-path logic all lives in the tested `internal/modules`
  package; this file is JSON â†” localStorage only.
- **Next.** Filter the sidebar nav by the hidden set (shell.go), then add per-screen show/hide
  toggles to the global Settings panel.

## 2026-06-16 â€” Module visibility: the pure engine

- New backlog item (Â§1.18 module-visibility toggles). Same reload-persistent shape as preferences,
...
- **Decision â€” lock the home and settings screens.** Hiding the dashboard or the settings screen
...
- **Decision â€” immutable Toggle returning a minimal set.** `Toggle` clones rather than mutating
...
## 2026-06-16 â€” Reload-persistent preferences: wiring dates through (feature complete)
...
- Final step: the three user-facing date displays (TransactionRow, GoalRow, TaskRow) now format via
  ...
  the top of its component â€” unconditionally, because GoalRow's date sits inside an
...
- Left `dateutil.FormatDate` in place for machine/edit contexts (date `<input>` values, parsing)
  where ISO is required â€” preferences only change *display*, not the canonical storage/parse format.
- Â§1.18 week-start + date-format preference is now end-to-end: pure engine â†’ localStorage atom â†’
  Settings UI â†’ live rendering, all surviving reload. Theme/density and fiscal-month start remain as
  separate future prefs.
- **Next.** Move to the next backlog area â€” likely module-visibility toggles (show/hide screens,
  also a reload-persistent preference) or a contained Phase-3 sync primitive.

## 2026-06-16 â€” Reload-persistent preferences: the Settings UI
...
- Step 5 (UI): a "Preferences" block in the global settings back-face. Week start is a `Segmented`
  ...
  `OnChange` *does* register a parent hook â€” so `onDateStyle` is declared unconditionally at the top
...
- Both controls funnel through one `savePrefs` closure that normalizes, sets the atom, and calls
  ...
  reflects a valid value. Date options show a live example (2026-06-05, 06/05/2026, â€¦) so the choice
  is self-explanatory â€” plain-English-UI rule.
...
## 2026-06-16 â€” Reload-persistent preferences: the persistence atom
...
- Step 4 (state): `uistate/prefs.go`, a near-mirror of `layout.go`. `UsePrefs` is a `state.Atom`
  ...
  error); `PersistPrefs` marshals the normalized prefs back. No store involvement â€” by design,
...
- This keeps the wasm/persistence layer thin: all the meaning (formatting, week math, normalization)
  is in the tested `internal/prefs` package; this file is just JSON â†” localStorage plumbing.
...
## 2026-06-16 â€” Reload-persistent preferences: the pure engine

- New backlog area: preferences that survive a reload (week start, date format). Established first
  that store-backed `Settings` do *not* survive reload â€” `app.Run` calls `appstate.Init(nil, true)`,
  ...
  localStorage-backed atom, seeded on boot, written on change â€” separate from the dataset.
...
- **Decision â€” `Normalize` everywhere a value is read.** Persisted prefs may be partial or from an
...
## 2026-06-16 â€” Custom fields: Goals, Budgets, Members (rollout complete)
...
- **Grouped as one feature deliberately.** The three integrations are byte-for-byte the same shape
  ...
  maps to one checklist line in Â§1.16.
- Â§1.16 form rendering is now closed for all five entity types (accounts, transactions, budgets,
  goals, members). The whole custom-fields feature â€” model, validate, persist, manage UI, render on
  forms, export/import â€” is complete.
...
## 2026-06-16 â€” Custom fields: Transactions form
...
- Second entity wired up, and the reusable pieces paid off: the Transactions add-form now renders
  ...
  pattern, and `appstate.PutTransaction` gained the `validateCustom("transaction", â€¦)` guard.
- **Decision â€” custom fields apply to income/expense, not transfer legs.** A transfer is two paired
...
- Confirmed the empty-slice-flattens trick again: `MapKeyed(nil, â€¦)` renders nothing, so no `If`
  guard is needed around the custom inputs.
- **Next.** Budgets, Goals, Members forms â€” same mechanical integration â€” then Â§1.16 is fully closed.

## 2026-06-16 â€” Custom fields: rendering on entity forms (Accounts first)
...
- The defs now drive real inputs. `CustomFieldInput` is a reusable component that picks the control
  ...
  declared unconditionally at the top so hook order is stable whatever the field type â€” the
  component is then safe to render from a `MapKeyed` list.
- **Decision â€” push values up, don't pull them down.** Each input is controlled and emits changes to
  a single parent map rather than holding its own state, so the submit handler can read every value
  at once and build the typed `custom{}` map (`customValuesToMap`: numbersâ†’float64, yes/noâ†’bool,
...
- **Validation lives in `appstate.PutAccount`, not the view.** Added `validateCustom`, which loads
  the account defs and runs `customfields.Validate`, returning `validate.Issues` â€” so any save path
...
- **Framework gotcha hit:** `If(cond, MapKeyed(...))` doesn't compile â€” `MapKeyed` returns
...
## 2026-06-16 â€” Custom fields: management UI

- Step 5 (UI last) for Â§1.16: `CustomFieldsManager`, a thin shell over the now-tested persistence.
  ...
  stable render position â€” the cardinal framework rule.
- **Decision â€” host it on the existing Customize screen, not a new route.** That screen is already
...
- **UI choices.** The choice-field options input only appears when the type is "Choice"
  (`If(isChoice, â€¦)`); required is a plain Optional/Required select rather than a checkbox to match
...
- **Next.** The defs exist and persist; the remaining step is rendering these fields as inputs on
  the actual entity forms (accounts/transactions/â€¦) and validating `custom{}` on save â€” a per-form
  integration I'll do entity by entity.

## 2026-06-16 â€” Custom fields: persistence layer

- Step 3 of the SDLC for Â§1.16: persist `CustomFieldDef`s. Added a `customfielddefs` table to the
...
- **Decision â€” keep `Def` in `internal/customfields`, not `internal/domain`.** The type and its
...
- **No schema-version bump.** The new `customFieldDefs` array is additive and `omitempty`; old
  exports decode fine (nil slice), so `SchemaVersion` stays at 1 â€” the migration guard is reserved
...
## 2026-06-16 â€” Custom fields: the validation core first

- Started SPEC Â§1.16 (user-defined custom fields) bottom-up, per the SDLC rule: model + validate
...
- **Trade-offs.** Custom values arrive from JSON, so numbers are `float64` â€” `isNumber` accepts the
  ...
  existed (or after one is removed) never hard-fails â€” forward/backward compatible by default.
- **Next.** Persist `CustomFieldDef`s (store + export/import round-trip), expose them via appstate,
  then a thin Settings UI to manage defs and render the inputs on entity forms â€” strictly in that
  order.

## 2026-06-15 â€” Porting candidate C: design-system foundation

- Resumed the `/loop`, now executing Â§1.7c (port the chosen design into Go components), one feature
  per commit. First feature is the foundation everything else references.
- **Decision â€” adopt Tailwind (CDN) + the candidate-C custom CSS** rather than re-authoring every
...
- Added the **accounting money formatter** as pure logic before any UI uses it (SDLC bottom-up):
  ...
  style â€” symbol-prefixed, always two decimals, negatives in parentheses (`($240.55)`). Kept in
...
- Confirmed the framework's `html/shorthand` exposes SVG element constructors (`Svg`/`Path`/`Rect`/
  ...
  full pointer/drag event set (`OnPointerDown/Move/Up`, `OnDragStart/Over/Drop/End`) â€” so the SVG
...
- Started the shared design-system package `internal/ui` (js/wasm-tagged) with the first reusable
  primitive: **`Icon`** â€” the candidate-C stroked SVG set as a single props-driven component
...
- Kept design data in the design layer: the routeâ†’icon mapping lives in `primaryNav()` (not the
...
- Full `GOOS=js GOARCH=wasm` build is green (~22 MB). Top bar's menu toggle, time-resolution control,
  and the Add action are present but static for now â€” wired in upcoming features.
...
- Completed the rail: **My pages** (example custom pages with colored page icons + a muted "New page"
  ...
  navigate later). Generalized `navItem` into the one reusable rail primitive â€” optional `Path`
...
- **Collapsible rail**: the framework's `state.UseAtom` is global-by-id and re-renders every
  ...
  button (toggles) and the `Sidebar` (adds the `collapsed` class â†’ the CSS does the 58px icon-only
...
- Modeled the time-resolution control **bottom-up first**: new pure `internal/period` package wraps
  `dateutil` with a `Resolution` (week/month/quarter) and anchor math â€” `Truncate` (snap to unit
  start), `Step` (move by whole units), `Label` ("Jun 2026" / "Q3 2026" / "Jun 15 â€“ Jun 21"), and
  `Range` (from/to anchors â†’ half-open reporting range, with a to<from clamp). Table-driven tests
...
- Wired the **time-resolution control**: new `internal/uistate` package holds the shared dashboard
  ...
  `Dashboard` now reads the same atom for its period range and re-renders on change â€” first proof the
...
- Built the keystone **`Widget` shell** in `internal/ui`: the candidate-C bento cell (square outlined
  `.w`, unified `.wh` header = grip Â· centered title Â· gear, padded `.wbody`) as one generic
...
- Browser/DOM check: the gwc dev server is up at :8080 serving the current wasm, but the gwc MCP
  ...
  loop context, and the playwright lane isn't set up â€” so automated DOM assertions aren't available
  here. Staying on compile-green + review as the gate; the owner can eyeball the live server.
- Built the **`FlipPanel`** primitive (`internal/ui`): the candidate-C settings overlay â€” dimmed/
  ...
  **both** per-widget and global settings (the reusability directive). The open animation runs once
  ...
  re-run), so the CSS transition animates from frontâ†’back rather than appearing pre-flipped.
...
- Re-sequenced: built a **real bento dashboard** before the settings wiring, so the gear has a live
  widget to open. `Dashboard` now renders the `.bento` grid â€” a full-width header cell + four KPI
...
- Added the **Recent transactions** widget (2Ã—2): newest six as a compact table (short "Jan 2" dates,
...
- Added the `ProgressBar` primitive (`internal/ui`): a display-only helper (no hooks â†’ plain
...
- Added the **Budgets** widget (1Ã—2): current-month spend per budget via `budgeting.EvaluateAll`,
...
- Added the `AreaChart` ui helper (feeds `chart` paths into an `<svg>` with a gradient fill, built
  ...
  (1Ã—2): current figure + a six-month end-of-month area chart from `ledger.NetWorthSeries`. Cutoffs
  are first-of-month from M-5 to M (AddMonths(start, i-4)).

- Added the **Cash flow** widget (2Ã—1): income/expense bars for the last four months (div bars,
...
- Added **Savings rate** (period income saved %, big figure + bar) and **Spending breakdown**
  (segmented bar of period expenses by category â€” top three + "Other" â€” with a color-keyed legend,
...
- Added the **Upcoming bills** widget (2Ã—1), completing the 12-widget catalog: next due date + min
...
- Wired **per-widget settings**: new `settings:target` atom in `uistate` (`SettingsTarget{Kind,ID,
  Title}` â€” closed/widget/global). The `Widget` gear defaults to opening its own panel (computes the
  ...
  `FlipPanel` for the active target and nothing (`Fragment()`) when closed â€” so each open is a fresh
...
- Built the **global settings** panel body: a two-column form inside the `FlipPanel` (760Ã—560) with
...
- Wired the **Export JSON** data action: a tiny `downloadBytes` helper (the one DOM-touching spot for
  file egress â€” Blob + transient anchor via `syscall/js`) downloads `appstate.ExportJSON()` as
...
- Added the data-action seams to `appstate`: `ExportCSV` (via `store.TransactionsToCSV`), `LoadSample`
  (replace with `store.SampleDataset` â€” `store.Load` replaces, as the import path proves), and `Wipe`.
  Native test loads sample â†’ asserts populated â†’ wipes â†’ asserts empty, plus a CSV smoke test.

- Wired all global-settings **Data actions**: Export CSV (download), Import (`.json` file picker â†’
  `ImportJSON`), Load sample, and Wipe (guarded by a native confirm). Added `pickFile` (file input +
  `FileReader` â†’ bytes, releasing the js callbacks after read) and `confirmAction`. Refresh uses a
...
- Started bento drag/resize **bottom-up**: new pure `internal/dashlayout` holds the grid model â€”
...
- Wired **drag-to-swap**: the framework's `Prevent` wrapper calls `PreventDefault` before the handler,
  ...
  `.drag`. No `DataTransfer` needed â€” the atom carries the source id.

- Made the **resize handles functional**: the right/bottom edge handles cycle the widget's col/row
  span via `dashlayout.Resize` (clamped to the 4Ã—3 grid), re-placing it live through the layout atom.
  Chose click-to-cycle over pointer-drag for now â€” it's reliable without browser testing and the math
...
- Enabled drag+resize on **all** widgets (normalized the net-worth widget then `Resizable`-stamped
  ...
  in-memory and re-seeded on boot â€” only browser storage actually survives a reload. Missing widgets
...
- Restyled all non-dashboard screens in one move by **retargeting the legacy CSS variables** to the
  ...
  match the flat neutral-dark shell without rewriting any Go â€” and they already inherit Inter from the
...
- Synced `TODOS.md` (Â§1.7c mostly done) and implemented **account transfers** (Â§1.11 â˜…). The model is
  ...
  other â€” a paired delete is the follow-up.
...
- **Paired transfer delete**: deleting a transfer leg now finds and removes its reciprocal (accounts
  ...
  migration); a shared transfer-group id would be more robust if duplicate transfers collide â€” noted.
...
- Added a **Freshness nudge** widget (full-width row 8 â€” grew the bento to 8 rows + a `dashlayout`
  placement, test count 14â†’15): friendly "N balances could use a refresh" with per-account days-since
...
- Added a one-tap **"Mark updated"** action per active account (`AccountRow` grows a third hook,
  ...
  staleness the freshness nudge reports. A full "update balance" (enter a new figure â†’ adjustment txn)
...
- Added **contribute-to-goal**: a per-goal Contribute button prompts for an amount (`promptText`
  ...
  validated put â€” advancing the progress bar. Auto-progress from a linked account is a follow-up.

- Wired the top-bar **"+ Add"** button to navigate to Transactions (was a no-op).

- Added a **budget month stepper**: a `monthOffset` state + â€¹/â€º pills drive `dateutil.AddMonths` so
...
- Added **transaction sort** (newest first / largest amount via `absAmount` / payee Aâ€“Z), applied to
  the filtered list before render.

- Replaced the Net worth KPI's static "Assets X" subline with a real **month-over-month delta** (â–²/â–¼
...
- Added a **Hide done / Show all** toggle to the To-do list (filters completed tasks; distinct
  "All done ðŸŽ‰" state when everything's hidden).
...
- Income/Spending KPI sublines now show the period plus the real deposit/transaction **count** for it
  (a `plural` helper), replacing the bare period label and matching the mockup's "June Â· 1 deposit".
...
- Added a **budget health summary** line ("N over budget Â· M near the limit") from the evaluated
...
## 2026-06-15 â€” Phase 2 begins (bottom-up): debt payoff
...
- Surfaced payoff in the **Planning screen** (replaced the stub): a live debt-payoff calculator
  (balance / APR / monthly payment â†’ months, total interest, total paid) wired to `payoff.Project`,
...
- Built the **Allocate screen** (replaced the stub): assembles candidates from non-archived asset
  accounts (return/stability/liquidity) and interest-bearing liabilities ("Pay down â€¦", guaranteed
...
- Added the formula **parser** (`Parse` â†’ AST): recursive descent with a precedence ladder
...
- Surfaced the engine in the **Customize screen** (replaced the stub): a live formula calculator over
  real figures â€” net worth/assets/liabilities + current-month income/expense (converted to major
  units by base-currency decimals) and account/transaction/member counts â€” with the result (or the
...
- Surfaced the forecast on **Planning**: a 12-month net-worth projection chart seeded from current
  `ledger.NetWorth` and this month's net cash flow (income âˆ’ expense) as the recurring monthly figure,
...
- Built the **Documents** screen (replaced the stub): paste-and-import transactions from CSV via a new
  ...
  (Reminder logged: don't run native `go test` with `GOOS=js` still set â€” it silently "fails" fast.)
...
- Wired AI end to end: a js `fetch` **transport** (`ai.SendChat`) that POSTs a chat request with the
  user's key and resolves the promise chain (`then(text) â†’ ParseResponse`), releasing its `js.Func`s
  on both success and catch paths; and an **Insights** screen with "Explain my month" that builds a
  system+user prompt from live figures (`fmtMoney(net/income/expense)`), shows a Thinkingâ€¦/error/
...
- Applied auto-categorization on **transaction entry** with zero new storage: built implicit
  `rules.Rule`s from the existing categories (Match = name â†’ SetCategoryID = id) and, on each
  description keystroke, suggest a category via `rules.Category` only when none is chosen â€” so it
...
- Added **GitHub Actions CI** (`.github/workflows/ci.yml`): on push/PR it sets up Go from `go.mod`,
  ...
  pushed to GitHub (the create-repo step is still pending â€” needs the owner's `gh` auth).
...
- Added an **extra-payment scenario** to the debt-payoff calculator: an optional extra-monthly input
  ...
  saved in plain English â€” the engine's first what-if surfaced.
...
- Added the **liability sub-form** to the Accounts add form: when the selected type is a liability
  ...
  lender inputs (each a conditional `If(isLiab, â€¦)` so hooks stay stable) and the add handler parses
...
- Refreshed the **`CLAUDE.md` status** line (was stuck at "Phase 0 â€¦ Phase 1 not yet started") to
...
- Added the **allocation-attributes sub-form** for asset accounts (expected return APR, liquidity,
  stability), mirroring the liability sub-form (conditional `If(!isLiab, â€¦)` inputs, parsed in the
...
- Wrote a project **`README.md`** (the repo had none): tagline, feature highlights, stack, build/run
  commands, the pure-logic-vs-thin-wasm-UI architecture, and links to the other docs â€” the GitHub
...
**Next:** per-row duplicate, persist-last-filter, then more Phase 2 polish â€” as the loop continues.

## 2026-06-15 â€” Dashboard design direction chosen (candidate C)
...
- **Selected candidate C**: flat neutral-dark palette, Fraunces serif headings + accounting figures
  ...
  unified per-widget header (grip Â· title Â· gear), drag-to-reorder + edge resize handles, a
  gearâ†’center+flip per-widget settings panel, a collapsible icon-only sidebar with a "My pages"
...
- Decomposed the mockup into a granular component backlog in `TODOS.md` Â§1.7c, each item referencing
...
**Next:** resume porting â€” apply the candidate-C shell (sidebar + top bar + bento) and start with the
design tokens + app shell, then the widget shell and first widgets, per Â§1.7c.

## 2026-06-15 â€” Phase 1 begins: data model (money)

- Started executing the backlog at Â§1.1, SDLC bottom-up. First service: `internal/money` â€” a
...
- Added `internal/currency`: registry + manual `Rates` table + `Convert`/`ToBase` (cross-currency
  via base, mixed decimals, nearest-minor rounding). A rounding test surfaced a good lesson â€”
  `1.005` as float64 is `1.00499â€¦`, so exact half-cents aren't representable; tests now use
...
- Added `internal/id`: 128-bit hex IDs via crypto/rand, optional prefix, seedable source for
  deterministic tests. (Test helper lesson: a single-byte counter wraps at 256 and collides â€” the
...
- Added `internal/ledger`: `Balance`, `RunningBalances`, `PeriodTotals` (income/expense, transfers
  excluded, base-converted), `NetWorth` (assets âˆ’ liabilities, liabilities reported positive), and
...
- Added `internal/validate`: `Validate{Member,Account,Category,Transaction,Budget,Goal,Task}`
  ...
  related-ref requirements. Tested. **Â§1.3 pure-logic services layer is complete** â€” 10 packages,
  all green on native `go test`.

## 2026-06-15 â€” Persistence: pure-Go SQLite (corrected course)
...
- Added `internal/money` `FormatMinor`/`ParseMinor` (plain decimal â†” minor units, strict, validated,
...
- Added `Get/PutSettings`, atomic `Wipe`, and `SampleDataset` (a valid starter seed â€” checked by
  running `internal/validate` over every entity in tests). **Â§1.4 persistence is complete.**

**Â§1.3 + Â§1.4 done â€” 11 packages green** (money, currency, id, dateutil, domain, ledger, budgeting,
goals, freshness, validate, store).

- Added `internal/logging` (Â§1.5): a `log/slog` `Handler` (writes lines to an `io.Writer` + records
  into a bounded `Ring`), with level filtering and `With`/`WithGroup`. Kept pure â€” the wasm app will
  pass a console-backed writer. Ring eviction, attr capture, grouping, and filtering are tested.

- Added `internal/appstate` â€” the UIâ†”logic seam. Kept it **pure Go** (no syscall/js): it owns the
  ...
  browser console â€” so no platform code needed.
...
- **Note:** embedding SQLite (ncruces wasm) pushed the raw wasm to ~20 MB (was ~6.5 MB). It
  compresses well (gzip/brotli) but is a real first-load cost â€” track it; consider lazy-loading or
...
- Built the **Accounts add form** â€” the first mutating feature. Reactivity pattern that works with
...
## 2026-06-15 â€” Project kickoff & spec
...
- **Framework study:** analyzed the local `GoWebComponents` checkout â€” confirmed the public API
...
- **Standards:** wrote `CLAUDE.md` â€” pure idiomatic Go, clean architecture (logic packages with no
...
- **Dependency cleanup:** replaced the local `../GoWebComponents` `replace` with a real `go get`
  ...
  clean â€” `agenthub` is pruned (core packages don't import it); only `cbor`/`float16`/`goldmark`
...
- **Layout cleanup:** moved web/build assets under `web/` so the project root holds only Go source,
  config, and docs â€” clean and standard.
- **Framework bug found (parked):** `gwc dev` resolves `-html` relative to the build/module root,
  ...
  fix is in GoWebComponents `tools/gwc/dev.go` â€” to be done, then rebuild + recopy `gwc`.
- **Planning:** wrote `TODO.md`, the priority-ordered master backlog, and made bottom-up SDLC
  (model â†’ services â†’ store â†’ UI) an explicit rule in `CLAUDE.md`.

**Next (per SDLC + TODO Â§1.1):** start the data model â€” `internal/domain` types + `internal/money`
and `internal/currency` services with table-driven tests â€” before any feature UI.
...
â‰¥2-member households, captured-atom setter so it's panic-safe from callbacks). Transactions overlays
...
wide (account-based). Reports by-member section gate relaxed to â‰¥2 members + â‰¥1 spend so the seeded
sample (only Daniel has spend) shows it. Fixed the agent's `If(cond, Attr("selected",...))` (invalid
â€” Attr is a PropOption, not a Node) â†’ `SelectedIf(cond)`. e2e member_view_toggle_check + crosscurrency
+ year-view all green.

## 2026-06-22 â€” L7 a11y: roving tabindex + committed gate
...
only a faint border tint â€” added `.field:focus-visible { outline:2px solid var(--accent) }`. Segmented
auto-merged from the worktree; settings/segmented regressions still green.

## 2026-06-22 â€” L30 guided statement reconciliation
New pure `internal/reconcile.Diff(clearedMinor, statementMinor) -> {DifferenceMinor, Reconciled}`
(integer minor units, table-tested) backs a new "Reconcile to statement" mode on each account (â‹¯
menu, `accounts.go`). Enter the statement ending balance, tick cleared txns (reusing the existing
per-txn clear flag + `ledger.ClearedBalance`), and the live difference closes to 0 â†’ "Reconciled âœ“"
...
## 2026-06-22 â€” Quick-hits batch 1: guided empty states
Extended EmptyStateCTA with an optional Href (route-based CTA) for derived screens with no on-page
add form. Wired Bills (â†’/accounts), Subscriptions (â†’/transactions), Reports breakdown
(â†’/transactions), and Planning Recurring/Plans (focus their add form via new ids recurring-add /
plan-add). Covers quick-hit tasks #49â€“#52. Also reconciled stale TODOS: Bills annualCost
...
## 2026-06-22 â€” Quick-hits batch 2: formula fix + UX/a11y polish
Fixed the real bug (#43): Customize minted a new formula id on every Save, so loadâ†’save duplicated.
Editor now tracks editID (set on load, cleared on save/delete) and reuses it. Added gate
formula_save_inplace_check (3â†’3, no dup). Plus: budgets over/near badges (#40), reports "Covering
Xâ€“Y compared with â€¦" caption (#32), rules match/category/tags aria-labels (#45), members name
...
keyâ†’Settings link, #37 documents select label, #38 todo labels, #39 todo overdue cue, #41 subs price
...
## 2026-06-22 â€” C73/C79 add-modal foundation (Goal/Account/Budget)
...
the right add form in a CloseOnly FlipPanel â€” the form owns its submit + errText, calling OnDone (close)
...
## 2026-06-22 â€” C73/C79 + C72A complete: all entity adds are modal-in-place
...
quick-add (now the only manual path) called PutTransaction directly â€” quick-add now runs
...
## 2026-06-22 â€” L-series lane D (dashboard/todo/documents/artifacts) + 4-agent sweep close
...
â†’ inline style). The 4 new e2e gates initially failed on a shared root cause â€” injecting into
...
pre-existing todo_nesting gate (port 8080â†’8099 + open the +Add task modal + nav-away/back to refresh
the list after a modal add). All lane-D gates + a11y green; en.go merged by disjoint key prefix.

## 2026-06-22 â€” L-series 6-agent parallel sweep (single main tree)
Ran 6 sonnet agents in parallel on the single main branch (no worktrees â€” the CC update broke worktree
...
Jump-To, added nav-alt-hint CSS, and repaired the agents' e2e gates â€” many shared root causes (row â‹¯-menu
not opened, goal add now in the +Add modal, money field serialized as .Amount not .amount, FlipPanel
.set-btn.save vs type=submit, localStorage cold-reads needing a flush, and a fleet-wide port 8080â†’8099
...
## 2026-06-23 â€” L-series 6-agent sweep round 2
...
tax-deductible reporting (Category.Deductible + reports.DeductibleTotals + Reports section); incomeâ†’
Allocate nudge + custom-fields in Insights context; and tooling â€” FIXED the internal/icon test (added
...
## 2026-06-23 â€” L-series 6-agent sweep round 3 (long-tail close)
...
gates â€” fixed each to expand "Show advanced fields" first. All round-3 gates + regressions green; go test
./... green.

## 2026-06-23 â€” C-series 6-agent audit+build sweep
Audited the whole C backlog with 6 agents (disjoint files). The vast majority was already done â€” much of
it via the recent L-series per-screen work â€” so most lanes returned "already-done with file:line proof".
...
build (undefined wb* symbols). Fixed the artifacts CSV-preview []ui.Nodeâ†’[]any table type. Three items
...
## 2026-06-23 â€” C-series epic + tail closeout
...
## 2026-06-23 â€” C73/C74/C78 epic build-out
