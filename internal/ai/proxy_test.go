...
func TestBuildProxyChatRequest(t *testing.T) {
	raw, err := BuildProxyChatRequest("gpt-5.4-mini", []Message{{Role: RoleUser, Content: "hello"}}, 0.2)
	if err != nil {
	...
	if req.Model != "gpt-5.4-mini" || len(req.Messages) != 1 || req.Messages[0].Content != "hello" || req.Temperature != 0.2 {
		t.Fatalf("proxy chat = %+v", req)
	}
}

func TestBuildProxyStructuredVisionRequest(t *testing.T) {
	raw, err := BuildProxyStructuredVisionRequest(" gpt-5.5 ", "sys", "extract", "data:image/png;base64,AAAA", 0.1, " rows ", []byte(`{"type":"object"}`))
	if err != nil {
	...
	if req.Model != "gpt-5.5" || req.SchemaName != "rows" || req.ImageURL != "data:image/png;base64,AAAA" {
