	...
	return Div(css.Class("card", "password-auth-card", tw.Mt1, tw.Flex, tw.FlexCol, tw.Gap2), Attr("data-testid", "password-auth-card"),
		...
		If(!signedIn.Get() && code == "", Fragment(
			...
			Input(css.Class("set-input"), Type("text"), Attr("autocomplete", "username"),
				...
				Placeholder(uistate.T("authCards.usernamePlaceholder")), OnInput(onUsernameInput), ui.FieldValue(username.Get())),
			Input(css.Class("set-input"), Type("password"),
				...
				Placeholder(IfElseValue(registering || recovering, uistate.T("authCards.passwordPlaceholderRegister"), uistate.T("authCards.passwordPlaceholderLogin"))), OnInput(onPasswordInput), ui.FieldValue(password.Get())),
			If(recovering, Input(css.Class("set-input"), Type("text"), Attr("autocomplete", "one-time-code"),
				...
				Placeholder(uistate.T("authCards.recoveryCodePlaceholder")), OnInput(onRecoveryInput), ui.FieldValue(recoveryInput.Get()))),
	...
	return Div(css.Class("card", "device-link-card", tw.Mt1, tw.Flex, tw.FlexCol, tw.Gap2), Attr("data-testid", "device-link-card"),
		...
		If(!linked.Get(), Fragment(
			Input(css.Class("set-input"), Type("text"), Attr("inputmode", "numeric"), Attr("autocomplete", "one-time-code"),
				...
				Placeholder(uistate.T("authCards.pairingCodePlaceholder")), OnInput(onCodeInput), ui.FieldValue(code.Get())),
...
func SetPasswordCard() uic.Node {
	...
	return Div(css.Class("card", tw.Mt1, tw.Flex, tw.FlexCol, tw.Gap2), Attr("data-testid", "set-password-card"),
		...
		Input(css.Class("set-input"), Type("text"), Attr("autocomplete", "username"),
			...
			Placeholder(uistate.T("authCards.usernamePlaceholder")), OnInput(onUser), ui.FieldValue(username.Get())),
		Input(css.Class("set-input"), Type("password"), Attr("autocomplete", "new-password"),
			...
			Placeholder(uistate.T("authCards.passwordPlaceholderRegister")), OnInput(onPass), ui.FieldValue(password.Get())),
