...
func PasswordAuthCard(props PasswordAuthCardProps) uic.Node {
	...
	return Div(css.Class("card", "password-auth-card", tw.Mt1, tw.Flex, tw.FlexCol, tw.Gap2), Attr("data-testid", "password-auth-card"),
		...
		If(!signedIn.Get() && code == "", Fragment(
			...
			ui.Field(username.Get(), css.Class("set-input"), Type("text"), Attr("autocomplete", "username"),
				Attr("aria-label", uistate.T("authCards.usernameLabel")), Attr("data-testid", "password-auth-username"),
				Placeholder(uistate.T("authCards.usernamePlaceholder")), OnInput(onUsernameInput)),
			ui.Field(password.Get(), css.Class("set-input"), Type("password"),
				...
				Placeholder(IfElseValue(registering || recovering, uistate.T("authCards.passwordPlaceholderRegister"), uistate.T("authCards.passwordPlaceholderLogin"))), OnInput(onPasswordInput)),
			If(recovering, ui.Field(recoveryInput.Get(), css.Class("set-input"), Type("text"), Attr("autocomplete", "one-time-code"),
				Attr("aria-label", uistate.T("authCards.recoveryCodeLabel")), Attr("data-testid", "password-auth-recovery-input"),
				Placeholder(uistate.T("authCards.recoveryCodePlaceholder")), OnInput(onRecoveryInput))),
...
func DeviceLinkCard(props DeviceLinkCardProps) uic.Node {
	...
	return Div(css.Class("card", "device-link-card", tw.Mt1, tw.Flex, tw.FlexCol, tw.Gap2), Attr("data-testid", "device-link-card"),
		...
		If(!linked.Get(), Fragment(
			ui.Field(code.Get(), css.Class("set-input"), Type("text"), Attr("inputmode", "numeric"), Attr("autocomplete", "one-time-code"),
				Attr("aria-label", uistate.T("authCards.pairingCodeLabel")), Attr("data-testid", "device-link-code"),
				Placeholder(uistate.T("authCards.pairingCodePlaceholder")), OnInput(onCodeInput)),
...
func SetPasswordCard() uic.Node {
	...
	return Div(css.Class("card", tw.Mt1, tw.Flex, tw.FlexCol, tw.Gap2), Attr("data-testid", "set-password-card"),
		...
		ui.Field(username.Get(), css.Class("set-input"), Type("text"), Attr("autocomplete", "username"),
			...
			Placeholder(uistate.T("authCards.usernamePlaceholder")), OnInput(onUser)),
		ui.Field(password.Get(), css.Class("set-input"), Type("password"), Attr("autocomplete", "new-password"),
			Attr("aria-label", uistate.T("authCards.passwordLabel")), Attr("data-testid", "set-password-password"),
			Placeholder(uistate.T("authCards.passwordPlaceholderRegister")), OnInput(onPass)),
