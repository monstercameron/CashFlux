...
func credentialForm(props credentialFormProps) uic.Node {
	...
	return Div(css.Class("acct-edit-form"),
		...
		credField(uistate.T("creds.username"),
			uiw.Field(userS.Get(), css.Class("field"), Attr("autofocus", ""), Attr("data-testid", "cred-username"), Type("text"),
				Attr("autocomplete", "off"), Attr("spellcheck", "false"),
				Placeholder(uistate.T("creds.usernamePh")), OnInput(onUser))),
		credField(uistate.T("creds.password"),
			Div(css.Class(tw.Flex, tw.ItemsCenter, tw.Gap2),
				...
				uiw.Field(newPassS.Get(), css.Class("field"), Attr("data-testid", "cred-password"), Type("password"),
					Attr("autocomplete", "new-password"), Attr("spellcheck", "false"), Style(map[string]string{"flex": "1"}),
					Placeholder(passPh), OnInput(onPass)),
		...
		credField(uistate.T("creds.loginUrl"),
			uiw.Field(urlS.Get(), css.Class("field"), Attr("data-testid", "cred-url"), Type("url"),
				Placeholder(uistate.T("creds.loginUrlPh")), OnInput(onURL))),
