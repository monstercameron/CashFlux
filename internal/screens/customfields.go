	...
	composer := Div(css.Class("fld-composer"), Attr("data-testid", "fields-composer"),
		...
		Form(css.Class("fld-form"), OnSubmit(add),
			...
			Label(css.Class("fld-field"),
				...
				Input(append(append([]any{css.Class("field"), Attr("id", "fld-key"), Type("text"), Placeholder(uistate.T("cf.keyPlaceholder")), Attr("pattern", "[A-Za-z0-9_]+"), OnInput(onKey)}, errAttrs("cf-err", errMsg.Get())...), uiw.FieldValue(key.Get()))...),
				Span(css.Class("fld-hint"), uistate.T("cf.keyTitle")),
			),
			fldField(uistate.T("cf.labelLabel"),
				Input(css.Class("field"), Type("text"), Placeholder(uistate.T("cf.labelPlaceholder")), OnInput(onLabel), uiw.FieldValue(label.Get()))),
			...
			If(isChoice, fldField(uistate.T("fld.choicesLabel"),
				Input(css.Class("field"), Type("text"), Placeholder(uistate.T("cf.optionsPlaceholder")), OnInput(onOptions), uiw.FieldValue(options.Get())))),
