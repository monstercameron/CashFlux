...
func BillsSmartForm(props BillsSmartFormProps) ui.Node {
	...
	explain := ui.UseEvent(func() {
		...
		if useBackend {
			ai.SendProxyChat(pr.ServerURL, uistate.EffectiveServerToken(pr.ServerToken), model, messages, 0.5, done, fail)
		} else {
	...
	setup := Div(css.Class("bills-smart-setup"),
		labeledField(uistate.T("bills.smartAnchorLabel"),
			Input(css.Class("field"), Type("date"), Attr("data-testid", "bills-smart-anchor"),
				Attr("aria-label", uistate.T("bills.smartAnchorLabel")), OnInput(onAnchor), OnChange(onAnchorCommit), uiw.FieldValue(anchorVal))),
		...
		If(advOpen.Get(), Fragment(
			labeledField(uistate.T("bills.smartKeep", base),
				Input(css.Class("field bills-smart-keep"), Type("number"), Attr("min", "0"), Step("0.01"),
					Attr("aria-label", uistate.T("bills.smartKeep", base)), Attr("data-testid", "bills-smart-keep"), OnInput(onKeep), OnChange(onKeepCommit), uiw.FieldValue(minorInput(plan.Cfg.MinKeepMinor, dec)))),
