...
// NetWorthPanel is the canonical net-worth tile shared between the standalone
// /networth screen and the Reports "networth" tab (FEATURE_MAP §5.7b). It
// renders the assets/liabilities/net-worth stat grid, the assets-vs-liabilities
// composition bar (R52), and the 6-month net-worth trend area chart (C217),
// plus a "View accounts" drill-through link (R52b).
//
...
// NetWorth is the dedicated /networth screen (FEATURE_MAP §5.7a). When no
// accounts exist it shows a friendly empty-state CTA; otherwise it renders
// the canonical NetWorthPanel — the stat grid, composition bar, and 6-month
// trend — as a self-contained scoped page. It is a thin shell: all computation
// lives inside the registered NetWorthPanel component.
func NetWorth() ui.Node {
	...
	return ui.CreateElement(NetWorthPanel, NetWorthPanelProps{})
}
