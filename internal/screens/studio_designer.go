...
func studioTextField(p studioTextFieldProps) ui.Node {
	...
	input := Input(css.Class(inputCls), Type("text"), Placeholder(p.Placeholder), Attr("aria-label", p.Label), OnInput(h), uiw.FieldValue(p.Value))
	if p.Compact {
...
// studioGoals returns the household's goals for the metric pickers (so each goal's
// variables are offerable), or nil when the app isn't ready.
func studioGoals() []domain.Goal {
	if appstate.Default == nil {
		return nil
	}
