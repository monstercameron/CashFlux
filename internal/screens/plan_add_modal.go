...
func PlanAddForm(props PlanAddFormProps) ui.Node {
	...
	form := Form(css.Class("plan-add-form"), OnSubmit(save),
		...
		Input(append(append([]any{css.Class("field"), Attr("id", "plan-add"), Type("text"), Attr("aria-required", "true"),
			Placeholder(uistate.T("plans.namePlaceholder")), OnInput(onName)}, errAttrs("plan-err", errS.Get())...), uiw.FieldValue(name.Get()))...),
		Div(css.Class("form-grid"),
			labeledField(uistate.T("plans.horizonPlaceholder"),
				Input(css.Class("field"), Type("number"), Attr("min", "1"), Attr("aria-required", "true"), Step("1"), OnInput(onHorizon), uiw.FieldValue(horizon.Get()))),
			...
			labeledField(uistate.T("plans.startPlaceholder", base),
				Input(css.Class("field"), Type("number"), Step("0.01"), OnInput(onStart), uiw.FieldValue(start.Get()))),
			labeledField(uistate.T("plans.monthlyPlaceholder", base),
				Input(css.Class("field"), Type("number"), Step("0.01"), OnInput(onMonthly), uiw.FieldValue(monthly.Get()))),
