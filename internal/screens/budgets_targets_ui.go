	...
	return Div(css.Class("budget-target-section"),
		...
		If(needsAmount, Div(css.Class("budget-edit-row"),
			labeledField(uistate.T("budgets.targetAmountLabel"),
				uiw.NumField(d.Amount, css.Class("field"), Type("number"), Attr("data-testid", "budget-target-amount"),
					Placeholder(uistate.T("budgets.targetAmountLabel")), Step("0.01"), OnInput(onAmount))),
			If(isByDate, labeledField(uistate.T("budgets.targetDateLabel"),
				uiw.Field(d.Date, css.Class("field"), Type("date"), Attr("data-testid", "budget-target-date"),
					Attr("aria-label", uistate.T("budgets.targetDateLabel")), OnInput(onDate)))),
		)),
