		...
		uiw.EntityListSection(uiw.EntityListSectionProps{
			...
			Body: Fragment(
				...
				Form(css.Class("form-grid"),
					labeledField(uistate.T("planning.balancePlaceholder", base), Input(css.Class("field"), Type("number"), Attr("min", "0"), Step("0.01"), OnInput(onBal), uiw.FieldValue(balStr.Get()))),
					labeledField(uistate.T("planning.aprPlaceholder"), Input(css.Class("field"), Type("number"), Attr("min", "0"), Step("0.01"), OnInput(onApr), uiw.FieldValue(aprStr.Get()))),
					labeledField(uistate.T("planning.paymentPlaceholder", base), Input(css.Class("field"), Type("number"), Attr("min", "0"), Step("0.01"), OnInput(onPay), uiw.FieldValue(payStr.Get()))),
					labeledField(uistate.T("planning.extraPlaceholder", base), Input(css.Class("field"), Type("number"), Attr("min", "0"), Step("0.01"), OnInput(onExtra), uiw.FieldValue(extraStr.Get()))),
				),
			),
		}),
		uiw.EntityListSection(uiw.EntityListSectionProps{
