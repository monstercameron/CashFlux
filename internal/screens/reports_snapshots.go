...
func reportSnapshotControl(props reportSnapshotProps) ui.Node {
	...
	// The frozen state opens READ-ONLY in the app-standard flip modal (#46):
	// picking a snapshot must not expand content inline under the toolbar.
	// FlipPanel carries a hook, so it is constructed unconditionally and gated
	// with If — the same pattern the insights prompt modal uses.
	snapOpen := false
	title := ""
	body := []any{css.Class("t-caption"), Attr("data-testid", "report-snap-panel"),
		Style(map[string]string{"display": "grid", "gap": "0.35rem"})}
	if s, ok := reportsnap.ByID(list, selS.Get()); ok {
		snapOpen = true
		title = uistate.T("reports.snapModalTitle", s.PeriodLabel)
		...
		for _, c := range s.Categories {
			body = append(body, Div(Style(map[string]string{"display": "flex", "gap": "1rem"}),
				Span(Style(map[string]string{"flex": "1 1 auto"}), c.Label), Span(m(c.Amount))))
		}
	}
	panel := uiw.FlipPanel(uiw.FlipPanelProps{
		Title:     title,
		Width:     uiw.FlipSmallW,
		Height:    "min(80vh, 480px)",
		CloseOnly: true,
		OnClose:   func() { selS.Set("") },
		Back:      Div(body...),
	})

	// The cluster itself is INLINE — it renders inside the toolbar row beside
	// the saved-views / actions / export controls (#46), not on its own line.
