		...
		Div(
			labeledField("Statement balance",
				Input(css.Class("field"), Attr("id", "acct-reconcile-stmt-"+a.ID), Attr("autofocus", ""),
					...
					Placeholder("Enter statement balance"), Value(stmtBalS.Get()), OnInput(onStmtBal))),
		),
		Div(Style(map[string]string{"margin": "0.5rem 0"}),
			Span(Style(map[string]string{"margin-right": "1rem"}), "Cleared balance: ", fmtMoney(curCleared)),
			Span(Attr("data-testid", "reconcile-difference"), "Difference: ", diffLabel),
