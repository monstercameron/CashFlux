...
func Apply(txns []domain.Transaction, c Criteria) []domain.Transaction {
	...
	switch c.Sort {
	case "amount":
		sort.Slice(out, func(i, j int) bool { return AbsAmount(out[i]) > AbsAmount(out[j]) })
	case "payee":
		sort.Slice(out, func(i, j int) bool { return strings.ToLower(out[i].Desc) < strings.ToLower(out[j].Desc) })
	}
	return out
}
