...
func TestBackendSecurityNotesDocumentProtectedRoutes(t *testing.T) {
	...
	for _, want := range []string{
		...
		"Production operators must not read customer sync snapshots",
		"actor, reason, ticket or incident id",
		"request_id",
...
func TestLegalCompliancePackDocumentsLaunchRequirements(t *testing.T) {
	...
	for _, want := range []string{
		...
		"DELETE /v1/account",
		"does not collect, transmit, process, or store payment-card numbers",
		"Stripe-hosted Checkout and customer-portal sessions",
		"Stripe Radar",
		"Stripe",
		"OpenAI",
		"30 days",
