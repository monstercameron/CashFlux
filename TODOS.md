...
- [ ] **R38 [MAJOR][UX] Redesign the Smart insight system from repeated cards into a prioritized decision layer.** Smart cards are helpful but overused across Dashboard, Accounts, Transactions, Budgets, Goals, Planning, Bills, Subscriptions, Allocate, and Smart. They often push the actual page content below the fold. Define severity, source, confidence, next action, dismiss/snooze, and "why shown" hierarchy; show only the top 1-3 contextual insights inline, with the rest in `/smart`. Acceptance: no page shows a long Smart stack above its core content unless the page itself is `/smart`.
...
- [ ] **R41 [MAJOR][UX] Rework first-run/sample mode framing.** The sample-data banner appears across almost every route, consumes prime space, and competes with page work. Replace the persistent banner with a compact sample-mode chip plus a guided "sample tour / start fresh" affordance. Acceptance: sample-mode status remains visible and reversible without occupying a full-width alert on every page.
...
- [~] **R44 [MAJOR][UX] Build a world-class page scorecard and visual regression gate.** Keep the audit evidence repeatable: screenshot every registered route at desktop (mobile DESCOPED per the 2026-06-27 no-mobile directive), compute density metrics (visible controls, cards, text length, horizontal overflow), and fail when pages regress beyond agreed thresholds. Acceptance: a script produces a score report for all routes, and each R36-R43 fix can be verified against the same evidence. **— LARGELY DONE 2026-06-27:** shipped a 4-dimension desktop UX quality gate (`e2e/ux_quality_gate.mjs` runner over `ux_contrast_audit.mjs` §12, `ux_density_audit.mjs` §11, `ux_overflow_audit.mjs` §5.5.11, `ux_touch_audit.mjs` §5.5.9 coarse-pointer); all four currently green. Contact-sheet/score-report polish remains.
...
- [ ] **R46 [MAJOR][VISUAL] Build a semantic theme-adherence audit.** Separate brand accent, positive/negative money semantics, warning/critical severity, selected navigation, and passive chrome into distinct token roles. Today green/red/yellow compete across money, alerts, buttons, and Smart cards. Acceptance: each theme passes a token usage audit showing that color conveys one consistent meaning per context and that components do not hardcode off-token colors.
- [ ] **R47 [MAJOR][UX] Create a button-density budget and action hierarchy.** Define max visible actions per card, row, toolbar, and first viewport; demote secondary row actions into overflow/details; reserve filled buttons for primary actions. Acceptance: high-density pages (`/transactions`, `/todo`, `/categories`, `/widget-manager`, `/accounts`) reduce visible resting controls by at least 40% without removing functionality.
...
- [ ] **R49 [MAJOR][VISUAL] Standardize headline financial figures.** Define rules for one hero metric per page, supporting deltas, sign treatment, currency precision, trend context, and when secondary figures collapse. Acceptance: Dashboard, Reports, Accounts, Budgets, Goals, Health, Bills, Subscriptions, Allocate, and Planning each have a single obvious headline figure/story and no competing same-weight money figures in the first viewport.
...
- [ ] **R69 [MAJOR][VISUAL] Audit light/dark/theme parity as a product quality dimension.** Existing checks cover contrast fixes, but enterprise-grade theming also requires equivalent hierarchy, semantic color meaning, chart palettes, focus states, warning states, print/export views, and custom accent resilience. Acceptance: every desktop route is captured in dark and light themes and scored for hierarchy parity, not just contrast.
