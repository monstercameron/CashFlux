...
- [~] **R34 [RESEARCH]** Help/support/trust surface — **SPEC delivered 2026-06-25.**
  - **What exists (assessed):** keyboard shortcuts are implemented (`internal/app/shortcuts.go`) but **not discoverable** (no cheatsheet); the logging package keeps an in-app ring buffer (usable for a bug report); the privacy/trust line was specified in CLOUD_UX but only lives in the admin console (C289). No `/help` route, no command palette, no feedback/bug-report form, no what's-new/roadmap, no onboarding tour.
  ...
  - **Suggested tickets:** R34-help (/help route + topics), R34-shortcuts (`?` cheatsheet from the registry), R34-feedback (log-bundling report form), R34-whatsnew (changelog sheet), R34-trust (C289 trust line), R34-onboard (first-run checklist, C21/C31). All local-first; none require the hosted backend.
