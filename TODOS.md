...
- [ ] **[SEC-1][CRITICAL] Mega security review of the institution-credential vault** (`internal/app/credvault.go`, `credentialhost.go`). First-pass encrypted credential storage shipped 2026-06-30: AES-GCM-256 + PBKDF2(600k) keyed by the app passcode, stored in a dedicated LOCAL-ONLY browserstore key (`cashflux:credvault`) that is never in the dataset blob, so never exported/synced/backed-up. Gated behind an app passcode. Known gaps the review MUST close before this is trusted for real bank logins: (1) XSS on-origin can read the derived key / plaintext while unlocked (same exposure as the in-memory dataset) — consider a worker + stricter CSP + short-lived reveal; (2) passcode strength gates everything — consider enforcing strength / WebAuthn-PRF-derived keys; (3) no hardware-backed key / secure enclave; (4) changing the passcode ORPHANS the vault (becomes undecryptable) — needs a re-encrypt-on-passcode-change hook; (5) reveal + clipboard copy leak into screenshots/clipboard managers; (6) no per-field audit / access log; (7) confirm the vault key is excluded from every export/backup/sync path (currently relies on export = dataset-only). Until reviewed, the UI shows an "Experimental — not yet security-reviewed" banner.
