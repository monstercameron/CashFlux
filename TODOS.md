...
- [ ] **C176 [MAJOR]** Owner/linked-account hidden behind advanced. **C177 [MAJOR]** Goal add-form save not reflected (add-form pattern, R2). **C178 [MAJOR]** Pace not contribution-rate aware. ~~**C179 [MAJOR]** "by date" raw ISO.~~ **— DONE (verified 2026-06-27):** dashboard.go `goalsWidget` + `recentWidget` now call `pr.FormatDate()` (see R18-dashboard above).
- [ ] **C180 [MAJOR]** Inline edit/contribute hides actions + no progress context. ~~**C181 [MINOR]** Delete button pointer-events.~~ **— DONE (verified 2026-06-27):** Added `@media (pointer:coarse)` rule + `:focus-visible` fallback in `web/index.html`; MEASURED on mobile viewport: 5 goal `.btn-del-hover` buttons, computed opacity=1, pointer-events=auto; 0 console errors. **C182 [DESIGN]** Overall-progress no tooltip.
...
- [ ] **C223 [MAJOR]** ~~Add-account silently fails to persist — CONFIRMED 3× (F9/F23/F31)~~ **— DONE (verified 2026-06-27):** `accountaddform.go` calls `app.PutAccount(...)` then `uistate.BumpDataRevision()` (and `props.OnDone()`) so the new account persists and the Accounts list (subscribed via `UseDataRevision`) shows it immediately instead of only after reload. Cross-ref R2→C223 (committed). **C224 [MAJOR]** No Property/Vehicle types. **C225 [MAJOR]** No valuation history.
...
- [ ] ~~**C239 [MINOR]** Bar-chart SVG negative-height error.~~ — DONE (verified 2026-06-27): in `internal/store/sample.go` `svgBarChart`, added `if h < 0 { h = 0 }` guard after computing bar height; prevents negative SVG `height` attribute when any data value is negative. Build rc=0, `go test ./internal/store/...` pass. **C240 [MINOR]** Redundant dual export surfaces. **C241 [MINOR]** "Covering" ISO date. **C242 [DESIGN]** Advanced report types hidden. **C243 [DESIGN]** No report-type selector.
