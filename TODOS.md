...
- [x] **R25 [RESEARCH]** Unified anomaly hub - **SPEC delivered 2026-06-27:** docs/DESIGN_UNIFIED_ANOMALY_HUB.md inventories the fragmented anomaly detectors (smartengine a1BalanceAnomaly, insights highlights, notify large-txn, attention) + 5 surfaces and specifies the unification (one Anomaly shape with a stable entity+kind+period ID extending smart.Insight, one detection pass, /smart as canonical hub + Unusual filter, derived surfaces sharing ID+dismissal per 8.6). Presentation half already shipped (R38 decision layer + R38/8.6 severity-sorted notifications); design closes the detection/identity/dismissal loop. Adversarial-verified accurate against the codebase. Implementation = follow-on C-items.
- [x] **R24 [RESEARCH]** No-key AI fallback - **SPEC delivered 2026-06-27:** docs/DESIGN_NO_KEY_AI_FALLBACK.md maps every AI-key-gated feature to its on-device deterministic no-key fallback (smartengine insights, rules categorization, CSV/OFX + statement.ParseAny import, allocate breakdown); the one real gap (image OCR) is isolated and routed to R10, with the CSV-first + labeled-gated-control interim UX already shipped (R55/8.9). Verified accurate against the codebase by an adversarial review.
