...
- [ ] **C157 [MAJOR]** No autopay toggle/flag in UI. **C158 [MAJOR]** ~~Bill-due reminder 7-day horizon too short~~ **— DONE (verified 2026-06-25):** `defaultBillLeadDays` raised 7 → 14 (`internal/notify/defaults.go`), so the default bill-due reminder fires two weeks ahead — enough lead to move money before a large bill (rent/mortgage/insurance) hits, vs a week that often lands mid-cycle. Still user-tunable per rule (Threshold). `go test ./internal/notify` ok (tests reference the const, not a literal). MEASURED live: Settings → Manage alerts → bill-due threshold input now defaults to **14** days; build rc=0, 0 errors. **C159 [MAJOR]** Notification badge desynced from feed. **C160 [DESIGN]** Autopay inference-only.
