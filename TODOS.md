...
- [ ] **C4 [MINOR]** Sample-data banner low prominence. **C5 [MINOR]** "Synced" pill on an empty workspace. **C6 [MINOR]** ~~Missing &lt;meta name=description&gt;~~ **— DONE 2026-06-25:** added `<meta name="description">` + Open Graph (`og:title`/`og:description`/`og:type`/`og:site_name`) + Twitter card (`summary`/`title`/`description`) to `web/index.html`, copy mirrored from `manifest.webmanifest`; descriptive `<title>` ("CashFlux — Local-first household budgeting"; the SPA still sets per-route titles at runtime). No `og:image` (app icons not generated yet — C306; a broken image is worse than none). MEASURED: `curl http://127.0.0.1:8099/` (raw HTML = exactly what share-crawlers read, no JS) returns all 8 new meta tags; app boots clean, 0 JS errors; SW cache bumped v268→v269 (core asset changed).
