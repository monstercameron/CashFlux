...
const run=async theme=>{
  ...
  await p.reload({waitUntil:"domcontentloaded"}); await p.waitForSelector('nav[aria-label="Main navigation"] a[title]',{timeout:30000}); await p.waitForTimeout(800);
  const m=await p.evaluate(()=>{
    const el=[...document.querySelectorAll('.sample-banner-btn')].find(e=>e.textContent.trim()==="Start fresh")||document.querySelector('.sample-banner-btn');
  ...
  if(!m){F(theme+": Start fresh not found"); await p.close(); return;}
  const r=ratio(m.color,m.bg);
  console.log(`[${theme}] Start fresh color=${m.color} bg=${m.bg} WCAG=${r.toFixed(2)}`);
  ...
  await p.screenshot({path:`e2e/screenshots/samplebanner_${theme}.png`}); await p.close();
};
