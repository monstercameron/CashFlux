...
const open = async () => { await p.goto(URL + "/notifications", { waitUntil: "domcontentloaded" }); await p.waitForSelector(".notif", { timeout: 15000 }).catch(()=>{}); await p.waitForTimeout(1200); };
...
// Link to alerting resource.
check("T8 notifications link to their resource", await p.locator('.notif-main.is-linked').count() >= 1);
const billCard = p.locator('.notif').filter({ hasText: "due soon" }).first().locator('.notif-main.is-linked');
await billCard.click();
await p.waitForTimeout(900);
check("T9 clicking a bill notification opens /bills", p.url().endsWith("/bills"), p.url());
await open();
// Inline dismiss removes that specific card.
...
check("T10 inline dismiss removes that card", (await p.locator('[data-testid="'+firstId+'"]').count()) === 0);
...
check("T11 severity filter shows only that tier", warnOnly, `${await p.locator('.notif').count()} cards`);
...
check("T12 clear all empties the feed", await p.locator('.notif').count() === 0);
check("T13 no page errors", errs.length === 0, errs.slice(0, 3).join(" | "));
