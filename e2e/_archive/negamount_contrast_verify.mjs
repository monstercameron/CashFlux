...
const run=async theme=>{
  ...
  await p.evaluate(()=>{const l=[...document.querySelectorAll('nav[aria-label="Main navigation"] a[title]')].find(x=>x.getAttribute("title")==="Accounts");if(l)l.click();});
  await p.waitForTimeout(1500);
  const m=await p.evaluate(()=>{
    const el=[...document.querySelectorAll('*')].find(e=>/this month/i.test(e.textContent)&&/[−-]\$/.test(e.textContent)&&![...e.children].some(c=>/this month/i.test(c.textContent)));
    ...
    return {color:cs.color,bg,txt:el.textContent.trim().slice(0,30)};
  });
  if(!m){F(theme+": neg amount not found"); await p.close(); return;}
  const r=ratio(m.color,m.bg);
  console.log(`[${theme}] "${m.txt}" color=${m.color} bg=${m.bg} WCAG=${r.toFixed(2)}`);
  ...
  await p.screenshot({path:`e2e/screenshots/negamount_${theme}.png`}); await p.close();
};
