...
for(const theme of ["dark","light"]){
  ...
  const r=await p.evaluate(()=>{const el=document.createElement('button');el.className='btn btn-primary';document.querySelector('#cf-page-view,main,body').appendChild(el);const cs=getComputedStyle(el);const o={bg:cs.backgroundColor,color:cs.color};el.remove();return o;});
  const c=ratio(r.color,r.bg);
  console.log(`[${theme}] btn-primary bg=${r.bg} color=${r.color} WCAG=${c.toFixed(2)}`);
  if(r.color==='rgb(255, 255, 255)') P(`${theme}: primary text is white (consistent)`); else F(`${theme}: text=${r.color}`);
  if(c>=4.0) P(`${theme}: contrast ${c.toFixed(2)} >= 4.0`); else F(`${theme}: ${c.toFixed(2)} < 4.0`);
  await p.close();
}
await browser.close();
