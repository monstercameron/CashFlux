...
try {
  ...
  // The by-category card is the one holding the rollup toggle.
  const card = page.locator(".card", { has: page.locator('[data-testid="reports-rollup-toggle"]') }).first();
  const rowCount = () => card.locator(".rows .row").count();
  const rowText = async () => (await card.locator(".rows").innerText()).replace(/\s+/g, " ");
