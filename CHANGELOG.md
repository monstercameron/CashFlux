...
### Added
- **/accounts: an "Assets & liabilities" spot-check tile (2026-07-07):** the accounts page showed asset rows but sent liabilities off to Debt payoff, so there was nowhere to eyeball both sides of net worth at once. A new read-only tile (`acct-glance`) lists every active account by its net-worth-signed balance, sorted low to high — the largest debts on top (parenthesised negatives), the largest holdings at the bottom — reusing the same raw-balance convention as the /networth breakdown (assets positive, liabilities negative; no per-class negation). It only appears when a liability exists (otherwise it would just duplicate the asset list); editing still happens in the asset list above or on /debt. The "Liabilities (N) — managed in Debt payoff →" link is unchanged.
