...
### Added
- **TODOS: new BG series — budgets competitive features (BG1–BG9, later implementation) (2026-07-14):** curated 9 of 10 from the budgets ideation: YNAB-style target types per budget (refill/set-aside/by-date, with a budget↔goal boundary to settle), Monarch-style flex budgeting as a fourth methodology (fixed/non-monthly/flex — one managed number), pacing indicators on the budget meters, quick-budget fill chips (last month/average/underfunded), rollover caps + envelope-debt visibility, seasonal-aware auto-budget true-up as SMART flags, YNAB credit-card payment budgeting (planning-gated), a daily-allowance molecule ("$23/day until the 1st"), and a view-only Quicken-style annual plan-vs-actual grid. Dropped by curation: monthly/seasonal budget overrides (BG9's grid ships view-only as a result). Cross-linked to XC3/XC4/XC6 and TX9.
- **TODOS: TX series batch 2 — TX9–TX17 (2026-07-14):** nine more curated transactions tickets: bill matching as a durable expected↔actual link (enhancing the existing occurrence/missing-txn machinery), **Events as a first-class domain entity** with transaction mapping (Cam's reframe — not a saved filter; engine variables + full CRUD, bottom-up), virtual round-ups sweeping to goals, Quicken-style register mode with a true running-balance column, new-merchant/trial-conversion SMART flags, YNAB-style inline overspend cover at the causing transaction, transaction comments with multi-user stubbed (mentions → member tasks), math-in-amount-fields via the app's own formula engine, and entry-time budget impact in Quick-Add ("leaves $142 in Dining · safe-to-spend $890"). Dropped by curation: planned/ghost future transactions.
- **TODOS: new TX series — transactions competitive features (TX1–TX8, later implementation) (2026-07-14):** curated from the big-name comps ideation (Copilot/Monarch/Mint/YNAB/Simplifi/Rocket Money/Emma/Lunch Money/Empower/Actual), adapted local-first: merchant alias table (do-first — cleans names for everything downstream), natural-language search as a SMART-series feature (Free local parser → filter chips, AI-tier fallback, both compiling to txnfilter.Criteria), saved views/watchlists pinnable as dashboard widgets, Amazon order-history import + enrichment (gated on a planning ticket), receipt attachments via the artifacts system, merchant context panel, "apply to N similar" recategorize preview, and a calendar view of the filtered ledger. Review-inbox and reconciliation-session ideas were considered and dropped by Cam. Cross-linked into the XC series (TX4→XC1/XC11, TX5→XC11, TX6→XC5).
- **TODOS: new XC series — cross-concept workflows (XC0–XC11, later implementation) (2026-07-14):** captured the ideation session on workflows where a real-world money object doesn't map 1:1 onto the app's atomic concepts (the transaction-split spirit), scoped to the first seven pages. Eleven detailed tickets: order grouping (N:1 dual of the split), refund/reimbursement pairing with period-true netting, annual-bill smoothing via system-managed sinking funds, committed-vs-free per budget, subscription price-creep accept-flow, month-close leftover sweep into goals, goal-earmark integrity checks against real account balances, self-resolving tasks (data-conditions on completion), payday pre-flight checklists, member-owned split lines, and receipt→proposed-split via the vision importer — plus two design gates (rituals vs. mechanics; ONE generalized txn-link primitive shared by split/group/refund-pair, to be settled before the split data model freezes). Each ticket names the machinery it reuses (anti-sprawl rule) and its guardrails.

### Added
- **Molecule save/delete validation — self-references, cycles, reserved names, and dependents caught with clear errors (2026-07-14):** `PutMolecule` now rejects a molecule that references itself, one that would close a reference cycle with other molecules (the error shows the path, e.g. `cyc_a → cyc_b → cyc_a`), and a name that shadows a built-in non-molecule variable (atoms, health/credit factors, alloc/planning/recurring/bills vars) — while overriding a *default molecule* by name stays allowed, as the Studio contract intends. `DeleteMolecule` now refuses to remove a custom molecule that other formulas still reference, naming the dependents (deleting an overridden built-in still restores the default and is never blocked). Previously all of these saved/deleted successfully and the fallout was a figure quietly reading as 0 somewhere else.

### Changed
- **The engine surface computes every account balance in ONE pass (2026-07-14):** new `ledger.Balances`/`ClearedBalances` compute all account balances in a single O(A+T) sweep (semantically identical to per-account `Balance`, guarded by equivalence tests), with `NetWorthFromBalances`/`LiquidFromBalances` reusing the map — and `NetWorthExplained` refactored onto the same shared core so the two can never drift. `engineenv.Vars` now builds the map once and threads it through the net-worth, liquid, revolving, per-account, per-debt, pool, composition, health, and credit variables — previously each of those re-scanned the full transaction ledger per account (~8 full O(A×T) walks per `Vars()` call, on every render and chart month). `HealthInputs`/`CreditInputs` keep their public signatures and now do one bulk pass internally for external callers too.

### Changed
- **Render-path engine recomputation cut — custom pages join the memoized surface, chart months cache, LiquidBalance scans deduped (2026-07-14):** `/pages` custom pages now read `liveEngineVars` — the same memoized, FULL engine surface the dashboard/Studio/formula-builder use — instead of hand-rolling a partial `engineenv.Vars` call on every re-render. That fixes a correctness wart too: the old partial call omitted `Molecules` (so KPI tiles evaluated the *default* net_worth/health_score even after a Studio override, disagreeing with a chart of the same figure on the same page) and the cf_*/pool_*/recurring_* var families. The per-month surfaces behind "formula" series charts (dashboard + custom pages) are now memoized per (revision, scope, month window) with the same 60s TTL as the KPI surface — a 12-month chart previously re-ran 12 full engine passes on every render; now they compute once per data change. Inside a single `Vars()` call, `ledger.LiquidBalance` (a full transaction scan) now runs once and is threaded into the bills-smart and reports helpers instead of being recomputed three times.

### Changed
- **Assistant math runs on one engine — legacy `calculator` tool removed, `evaluate_formula` description generated from the real function list (2026-07-14):** the assistant had two overlapping math tools. `calculator` built its own six-variable environment from raw ledger calls over the hardcoded calendar month, ignoring the household's Studio molecule overrides — so "what's net_worth × 4% / 12" could return a different number than `evaluate_formula` or the dashboard for the same question, depending on which tool the model picked. It's gone; `evaluate_formula` (full live engine surface, molecule overrides, derivation notes) is the one math path, and the system prompt now points there. Its tool description is also now **generated from `formula.Functions()`** — the model finally learns about `safediv`, `clamp`, `if`, `and/or`, `contains`, and the rest (the hand-written text claimed only "+ - * / parentheses and comparisons"), and the description can never drift from the evaluator again.

### Fixed
- **Studio formulas: a refused delete now explains itself (2026-07-14):** removing a custom molecule that other formulas still reference is blocked by the save-layer validation — but the Studio row swallowed that error and just did nothing. The row now shows the reason ("`base_fig` is still used by `dep_fig` — update or delete those formulas first") in its alert line and keeps the editor open, matching how save errors are already surfaced.
- **Health score no longer counts unconvertible foreign amounts at face value (2026-07-14):** two health atoms fell back to the RAW minor-unit amount when an FX conversion failed — a liability's minimum payment (a ¥50,000 payment with no JPY rate was added to the obligation sum as if it were $50,000) and a credit card's balance/limit in the aggregate-utilization inputs. Both now follow the same convention as every other atom: **no usable rate → the figure is excluded** (for utilization, the whole card is excluded so a half-converted balance/limit pair can't skew the ratio). Covered by a test with JPY/EUR accounts and no rates, asserting the ratios match the convertible entries only — and rise once rates are supplied.
- **Molecule evaluation no longer depends on declaration order, can't shadow atoms, and never fails silently (2026-07-14):** `engineenv.Vars` used to evaluate molecules in one linear pass — but persisted custom molecules load in *name* order, so a molecule referencing a sibling later in the alphabet (`a_summary = z_base * 2`) deterministically failed and read as a quiet 0 everywhere, with the error swallowed. The molecule pass now runs to a **fixpoint** (unresolved molecules retry while passes make progress), so forward references and chains resolve in any order; reference **cycles terminate cleanly** and stay unset; a molecule named after an atom (possible via dataset import, which skips the Studio form's only collision check) is **ignored instead of overwriting the atom** — an imported molecule called `assets` can no longer corrupt `net_worth` and every other consumer; and every molecule that ultimately fails is **logged via `slog` once per distinct failure** (deduped, since Vars runs per render) instead of vanishing without a trace. Covered by new table-driven tests: order independence, three-link reversed chains, direct/self cycles, atom shadowing, and typo'd references.

### Changed
- **Formula engine hardened — guarded results, lazy logic, safer parsing, parse-once API (2026-07-14):** the sandboxed expression language behind molecules, widget bindings, workflow conditions, and the AI's `evaluate_formula` got a robustness pass. `if()`/`and()`/`or()` now **short-circuit**, so the natural guard idiom `if(count == 0, 0, total / count)` finally works instead of erroring on the division it guards. A formula can **never return NaN or Inf** anymore — the top-level result is checked (intermediates can still be rescued by `clamp()`/`safediv()`), and `safediv` itself now treats a NaN divisor or non-finite quotient as "fall back", honouring its never-garbage contract. `==`/`!=` compare numbers with a tiny relative tolerance so `0.1 + 0.2 == 0.3` is true (real cents still differ). Chained comparisons (`a < b < c`) are **rejected at parse** instead of silently coercing the first result to 0/1. The parser gained a nesting-depth limit and a source-length cap, so a pathological formula (e.g. from a corrupted import) returns an error instead of fatally overflowing the wasm stack. The tokenizer now accepts **scientific notation** (`1e3`, `2.5E-2`), rejects multi-dot numbers with a clear message, and reports non-ASCII characters as themselves. New API: `Compile` → `Program` (parse once, evaluate many; `Eval` also memoizes ASTs so render-path re-evaluation stops re-parsing), `Validate(expr, known)` for save paths, `Functions()` as the single source of truth for the built-in function set, and a structured `*formula.Error` carrying the byte offset so editors can point at the problem. All existing formulas keep evaluating as before; full native suite + js/wasm consumer builds verified.
...
- **Assistant tool loop runs on the OpenAI Responses API so the thinking level works with tools (2026-07-13):** `/v1/chat/completions` rejects `reasoning_effort` alongside function tools for the reasoning models (gpt-5.x / o-series), so the assistant now posts its tool loop to `/v1/responses` (`ai.SendResponsesChatTools` + a codec that converts to/from the same message/tool shapes, so the loop was a one-line swap). The Responses request carries `reasoning.effort` for reasoning models (the Thinking control now works for ALL of them — no more hiding/stripping), sets `store: false`, and **echoes the model's reasoning items** back in follow-up turns (required by the Responses API for reasoning models with tools). Verified end-to-end against a live server for gpt-5.6-sol.
...
- **Budgets page redesign (2026-07-12):** a batch of budgets improvements Cam asked for.
  ...
  - **Row actions decluttered.** **Transactions** moved into the ⋯ kebab (it's navigation), alongside new **Notes** and **Formulas** actions; inline stays just the proactive Cover/Top-up.
  - **Notes** — a kebab modal to add/edit a budget note (new `Budget.Notes`), shown on the card as a readable, click-to-expand line (mirrors /accounts). The notes textarea fills the modal.
  - **Formulas modal** — a read-only kebab modal listing the budget's engine variables (`budget_<slug>_{limit,spent,remaining,over,percent}`) with live values; click a variable name to copy it, or copy a value with the button (`internal/screens/clipboard.go`).
  - **Sort picker** — order the list by Importance (health, default), Over budget (severity = $ over, then % over), **Close to the limit** (`|% used − 100|` ascending — a 99%/101% budget beats a 200% one, and 0%-used budgets rank last), Underused (% used ascending), Largest limit, or Name. Order verified by a regression that reads each card's real spent/limit/% and asserts the on-screen ordering is monotonic per the metric.
  - **Toolbar cleanup** — removed the "Budget metrics" toggle **and** the whole opt-in FormulaBuilder tile it revealed; moved the **50/30/20 template** into the Add-budget modal as a "Start from a template" banner.
- **Accounts toolbar is glyph buttons that flag modal vs navigation (2026-07-12):** the accounts toolbar actions (Transfer money, Mark all updated, Manage exchange rates) became fixed-size **glyph buttons** with hover labels, matching the transactions toolbar. Each carries a persistent **corner badge + a tooltip sub-line** telling you what it does before you click: **⧉ "Opens a dialog"** for the flip-modal opener (Transfer), **↗ "Goes to Settings"** for the navigation button (Exchange rates), and no badge for the in-place bulk action (Mark all updated, amber). Colour conveys importance (accent/amber/neutral); the badge conveys behaviour — two orthogonal signals.

### Fixed
- **Accounts toolbar glyph tooltips no longer clip (2026-07-12):** a toolbar tile gets a `transform` on hover (the lift), which makes it a stacking context and trapped its glyph tooltips behind the tile below — the transactions page had a ledger-only fix, the accounts page had none. Generalized it: any bento tile with a hovered/focused `.tbar-btn` now lifts above its siblings (`z-index: 30`), so the tooltip always paints on top (fixes every FilterToolbar surface). Also right-anchored the last toolbar action's tooltip so it can't spill past the viewport edge.

### Changed
- **Version bumped to v1.0.18 (2026-07-12):** cuts the accounts-page refinements + the transactions search-debounce / pager scroll-to-top into a release.
- **Accounts page refinements — merged editor, transfer modal, kebab quick actions, readable notes (2026-07-12):** a batch of accounts-page cleanups Cam asked for.
  - **Row actions are quick-actions-only, everything else in the ⋯ menu.** Inline are now just **Edit** (always) + **Update value / balance** (only for the accounts you actively maintain — stale ones, emphasized amber, and valuation-type assets you revalue by hand); a fresh cash account shows only Edit. **Transactions** moved *into* the ⋯ menu (`acct-view-txns-<id>`) — it's navigation, not a per-row edit — alongside Reconcile / Transfer / Mark updated / Login & credentials / Archive / Delete. "Update value" stays reachable in the menu for the accounts that don't show it inline, so it's never duplicated. New testids `edit-account-btn-<id>` / `update-value-btn-<id>`.
  - **Update-value and Edit are one merged modal.** The standalone "update balance" form was folded into the top of the full edit form as an `.acct-value-section` (a new-value input with a live delta preview + adjustment category, and a "Currently $X — leave blank to keep it" caption). One save does either or both: a non-empty value routes through `OnSetBalance(cp,…)` (persists edits **and** posts the balance adjustment + freshens in one write), else a plain `OnSave`. "Update value" opens it focused on the number; "Edit" on the name. `setBalForm` was deleted.
  - **Page-level "Transfer money" is now a shell-root flip modal** (`app.AccountTransferHost` + `screens.AccountPageTransferForm`), centered like the row editors, instead of the old inline `acct-transfer` tile (spec/registry retired).
  - **Notes are readable.** An attached note now renders as a legible framed line under the account (clamped to two lines, click to expand) — `.acct-notes` / `acct-notes-<id>` — instead of a hover-only glyph. The row's month-to-date value change + bill-payment metadata are unchanged.
  - **Removed the "Smart" sparkle** shortcut beside the account list's All/Assets/Liabilities filter (`smart-section-action` — kept on other pages).
  - **Fixed:** the editor's Cancel/Save bar scrolled off the taller merged form — all account editor forms now split into a scrolling `.modal-scroll` region + a pinned `.modal-foot` (via `FlushBody`), so the actions stay put. And the update-value modal **title** now matches the row button's wording ("Update value" for estimated-asset types, "Update balance" for cash) instead of being hardcoded to "Update balance". New styles in `internal/styles/rules_accounts.go`; regressions in `e2e/regression/accounts.spec.mjs` (6 tests, incl. the pinned-footer check).
- **Transactions search is debounced longer; paging jumps the ledger back to the top (2026-07-12):** the search field already coalesced input at 250ms, but a deliberate typist who pauses can still fire a re-filter per keystroke, so search now gets its own **400ms** debounce (the numeric min/max filters stay at 250ms). And clicking **Prev/Next** on the pager now scrolls the ledger back to the top of its scroll container (opt-in `DataTable.AnchorID` → the pager smooth-scrolls the `<table>` on page change), so you land on the new page's first rows instead of being stranded at the bottom. Regression test drives scroll-to-bottom → Next → asserts the list snaps to the top.

### Fixed
- **E2E never serves stale wasm (2026-07-12):** the app's `index.html` loads `bin/main.wasm.gz` first (falling back to the raw `main.wasm`), but `e2e/global-setup.mjs` only ever rebuilt `main.wasm` — so a leftover `main.wasm.gz` (from a deploy or `gwc dev`) made the whole suite silently run **old code** against a freshly-built binary. Global-setup now removes `main.wasm.gz` after building, and the test server (`e2e/serve.mjs`) sends `Cache-Control: no-store` (Chromium's compiled-wasm cache was surviving even a fresh origin). Deploys to `web/bin` must likewise refresh **both** files.
...
### Changed
- **Bulk bar no longer scrolls; filter + select-all buttons toggle (2026-07-12):** the bulk-action bar's category/member selects are capped narrow (≤150px) so the whole row fits on screen instead of forcing a horizontal scrollbar. The **Filters** button now toggles its panel open/closed (with `aria-expanded`), and **select-all** toggles too — a second click, when everything shown is already selected, clears the selection.
- **Consolidated the Transactions toolbar + flattened the bulk bar + fixed the tooltip z-index (2026-07-12):** the **select-all** glyph moved up into the single toolbar row with the other actions (retiring the separate below-toolbar controls row and its duplicate-notice text — the badged "Review duplicates" button + the duplicates modal already cover that, so the redundant "select duplicates" action was dropped). The **bulk-action bar** is now one flat row that scrolls horizontally instead of wrapping into an uneven block. And the glyph **hover tooltips** were being trapped behind the tile below (a ledger tile makes a transform stacking context on hover); the hovered tile now lifts its layer above its siblings so the tooltip paints on top.
- **Glyph-ified the Transactions filter, select-all, and bulk-action controls (2026-07-12):** extending the sleek icon-toolbar treatment to the rest of the surface. The **Filters** trigger (reused on Accounts too) is now a funnel-glyph button with the active-filter count as a corner badge, an accent tint when filters are active, and the "Filters" label on hover; its inline panel header gained a filter glyph + a real ✕ Close glyph, and the active-filter chips' remove buttons use the Close glyph instead of a unicode "✕". The **select-all** and **select-duplicates** controls became glyph buttons (check-circle / copy) with hover labels. The **bulk-action bar** that appears on a selection keeps its category/member pickers but turns its actions into glyph buttons — apply (✓), assign member (people), mark cleared (check-circle), mark uncleared (⃠), export (↓), and a red-danger delete (✕) — with "Clear selection" kept as a plain text link. Generalized `toolbarIconBtn` to a `variant` ("" / primary / danger); removed the now-dead `actionBtn`. All 38 interaction + a11y e2e stay green (the FilterToolbar change is exercised on Accounts and budgets too).
- **Sleek icon toolbar on Transactions — glyph buttons with hover labels (2026-07-12):** the transactions toolbar's action buttons were a run of uneven-width text buttons (Add / Export CSV / Import / Review duplicates / Columns / Categorize). They're now fixed-size **glyph buttons** (a reusable `toolbarIconBtn` + `.tbar-btn` styles) that read as an even row, each revealing its text label on **hover/focus** as a small styled tooltip (`.tbar-tip`) — the label doubles as the `aria-label` so the icon-only controls stay accessible. Add keeps the accent (green) as the primary action; the rest are neutral. Glyphs: Add=＋, Export=↓, Import=↑, Duplicates=⧉, Columns=☰, Categorize=✦. All testids preserved (Export gained `txn-export-btn`); import + duplicates e2e stay green.
...
### Changed
- **Import wizard: refined the four source forms + gave both tiers a distinct design (2026-07-12):** the per-source forms (opened by clicking a picker tile) were unpolished — each led with a redundant heading (you'd already picked the type), a wall of description text, and inconsistent layouts. Each form now opens with a compact **tier-branded header** (the source's icon in a tier-tinted square + title + a `Smart` / `Smart+` chip + a one-line description), followed by a tidy input body; long guidance moved into a terse hint + the existing collapsible. Generative-AI actions inside a form (**Extract with AI**, **Read with AI**, **Read statement**) now render as the violet Smart+ `btn-plus` (with the ✦ glyph), so the deterministic-vs-AI split reads at a glance — e.g. the statement form pairs a green **Parse statement** with a violet **✦ Extract with AI**. The picker tiles + branch pills were rerouted through the new tier tokens (Smart now has its own green identity, not just "neutral"), and the redundant corner ✦ badge on Smart+ tiles was dropped (the tile's violet icon + branch chip already mark the tier). `CsvImportCard` / `ImageImportCard` were slimmed to return just their inputs (title/description live in the shared header). All import-wizard e2e stay green.
- **Version bumped to v1.0.17 (2026-07-12):** cuts the merged import wizard + visual Smart/Smart+ picker into a release.
- **Import picker Smart+ tier marker unified with the Smart pill (2026-07-12):** the Smart branch used a text pill (`INSTANT · NO AI`) while the Smart+ branch rendered the ✦ glyph at its default (large) size, ballooning into a mismatched circular badge. The ✦ is now sized to sit inline and the tier pill is an `inline-flex`, so both branches read as matching pills (`INSTANT · NO AI` vs a violet `✦ AI`). Cosmetic only.
- **Import wizard Stage 1 is now a visual document-type picker with Smart / Smart+ branches (2026-07-12):** the merged import modal's first stage used to stack every source form vertically (CSV card, statement textarea, PDF card, receipt card). It's now a **grid of document-type tiles** split into two branches that mirror the app's Smart tiers: **Smart** (deterministic helpers — instant, offline, no key): *CSV / spreadsheet* and *Statement text* (auto-mapped columns); and **Smart+** (generative AI, ✦, needs an OpenAI key): *Statement PDF* and *Receipt photo*. Each tile shows an icon, a title, and a one-line description; Smart+ tiles carry the brand/violet accent + the ✦ AI glyph. Clicking a tile **animates in** just that source's form (a slide/scale entrance, honoring `prefers-reduced-motion`), with a "← Choose a different type" back button; producing a draft still advances to Stage 2 review. New styles live in `internal/styles/rules_import.go` (`.doc-picker`/`.doc-type-tile`/`docFormIn`). Verified: the picker renders both branches with all four tiles, Smart+ tiles carry the AI accent, clicking each tile reveals the right form and back returns to the grid, and a full statement-text → Parse → review → Save flow still imports — zero console errors.
- **The two transaction importers are merged into one two-stage import wizard (2026-07-12):** the Transactions toolbar used to carry two separate import buttons opening two separate flip modals — **Import** (CSV paste/file + statement-text + receipt-image, with an inline import-history list) and **Import statement** (the ✦ AI statement-PDF modal). They're now a **single Import** button opening one double-wide flip modal built as a **two-stage wizard**: **Stage 1 "Add your data"** is a hub for every source — CSV, statement text, **statement PDF** (folded in from the retired standalone modal, including its "keep a copy in Artifacts" toggle), and receipt image; **Stage 2 "Review & import"** is the shared editable draft table + spend summary, reached automatically when any AI/parse source yields rows, with a "← Add more data" back step, the Cancel/Save footer, and (replacing the old inline import-history list) a **"Past imports →" link to the Artifacts vault**. The lossless CSV path still imports **directly** from Stage 1 (it never enters the review table, whose row shape can't carry CSV's account/member/tag/transfer columns — preserving export→import losslessness); only AI/parse extractions, which need human review, advance to Stage 2. Retired: the `txn-statement-import-btn` toolbar button, `StatementImportHost` + `StatementImportBody` (`statement_import.go`, `statementimporthost.go`), the inline `ImportHistoryList`/`DocHistoryRow`, and the orphaned `UseStatementImportOpen` atom; the statement schema/prompt moved into `documents.go`. Verified end-to-end: one toolbar button, all four sources present in Stage 1, a non-AI statement Parse advances to Stage 2 and its rows import via footer Save, Back preserves the draft, the review shortcut returns forward, CSV still imports directly from Stage 1, and the Artifacts link navigates + closes — zero console errors.
- **The import flip modal now has a standard pinned Cancel + Save footer (2026-07-11):** the double-wide import modal previously had no footer (dismiss only via ✕/Escape/backdrop), unlike every other modal. It now carries the app's standard pinned **Cancel + Save** footer. Because the panel is a multi-flow importer (CSV paste commits inline; statement/receipt/wizard flows converge on a reviewed *draft*), footer **Save** commits whatever is ready — a reviewed draft (as split receipt or N transactions), otherwise pasted CSV — then closes; with nothing entered it simply closes (Done); a failed commit keeps the modal open so its error stays visible. **Cancel** dismisses without importing. The per-flow inner buttons (CSV *Import*, *Import draft*) are unchanged. Mechanically, `DocumentsPanel` renders a hidden form (`ImportModalFormID`) that FlipPanel's footer Save submits by id (FormID mode, so no auto-close before the commit runs); the panel dismisses via the shared `transactions:importPanel` atom. Successful CSV import now also clears the paste box. Verified end-to-end: footer shows Cancel + Save; Cancel, empty-Save, and ✕/Escape all close; a pasted CSV committed via footer Save lands in the ledger (searchable) and closes the modal; zero console errors.
- **CSV / receipt import now opens in a double-wide flip modal instead of taking over the Transactions page (2026-07-11):** the toolbar's **Import** button used to swap the whole ledger out for a full-width in-place import panel (the `TxnViewImport` sub-view) — so you lost sight of your transactions while importing. It now opens the same import UI (CSV paste, receipt vision, draft review, import history) as a **shell-root double-wide flip modal** (900×660, ✕/Escape/backdrop to dismiss), matching the existing "Import statement" modal, with the ledger still visible behind it. Mechanically: a new `transactions:importPanel` atom drives a shell-root `ImportPanelHost` (fixed modals must mount at the shell root, clear of the bento tile transform that would clip them); the `DocumentsPanel` content is unchanged and reached via an exported `ImportPanelBody` wrapper. The in-place `TxnViewImport` view-swap and its `txn-import` tile registration were retired (Duplicates remains an in-place sub-view). The `txn-import-btn` testid is preserved. Verified end-to-end: opens on click, ledger stays behind it, both ✕ and Escape close it, zero console errors.
- **All 46 pages now rate A on the perf framework — v1.0.16 (2026-07-11):** every route hit **≥90** on a clean full-suite run (avg **98**, grades A+ ×38 / A ×8; lowest `/smart` 92), up from 8 pages below A at the v1.0.15 baseline. The technique throughout was **below-the-fold deferral** via a reusable `useAfterSettle` hook: render each page's primary above-the-fold content on mount, then hydrate the heavy secondary/detail content ~300ms after first paint (past the route cross-fade). Applied to the chat rail, the investments/custom-page/dashboard bento grids (whole grids deferred), the smart-hub catalog + findings feed, and the subscriptions/artifacts lists. The dashboard additionally deferred its placement-building loop (persistence gated so the mount never writes an incomplete layout), its sparkline series, and its engine-variable surface, and its net-worth computation is now shared across the render (one ledger pass instead of two) — taking it from 46 to 90+. Ratings stored under `e2e/perf/results/v1.0.16.*`; trend v1.0.14 85 → v1.0.15 93 → v1.0.16 98.
- **Reusable "load secondary content after first paint" helper, applied to Smart hub + Subscriptions (2026-07-11):** a shared `useAfterSettle` hook defers a page's heavy below-the-fold content ~300ms past first paint (after the route cross-fade). The Smart hub now paints its findings feed immediately and mounts the full opt-in catalog + digest config (its heaviest DOM) a beat later; the Subscriptions page defers its page-level smart-insight scan (which only feeds secondary badges). Both keep all hooks unconditional and render correctly (validated).
- **AI chat page renders the conversation instantly; the periphery rail loads a beat later (2026-07-11):** the Insights/assistant page's right-hand rail (flagged-activity anomaly detectors + spending-highlights, each a full-transaction scan) ran on the initial mount, blocking the chat from appearing. The rail is now deferred until ~300ms after first paint (past the 160ms route cross-fade), so the chat is interactive immediately and the secondary "what I noticed" rail fills in once the page has settled. Measured mount 315→58ms, blocking 713→80ms — the page's score rose from the 60s to **93 (A)**. A tightly-scoped handler also swallows the benign "Transition was skipped" AbortError the browser emits when a deferred render lands during a View Transition (a non-actionable browser artifact).
- **AI chat no longer rescans every transaction on each keystroke (2026-07-11):** the Insights/assistant chat page ran several full-dataset scans directly in its render body, and the page re-renders on every keystroke, send, and effect — so those scans ran per character typed. Now memoized on the data revision (+ scope/week/month): (1) starter questions (top spend category over every transaction + nearest-limit budget evaluation + soonest goal); (2) the four SMART anomaly detectors behind the "Flagged activity" card; (3) the spend-anomaly detection (four monthly per-category series over every transaction) behind the "What changed" highlights. `spendingHighlights` became a pure renderer of pre-computed anomalies. Verified rendering correct (Flagged activity shows real detector output; chat typing crash-free). Code-reasoned fixes to per-render waste; the automated per-page score can't confirm them under heavy local machine load (a work-removing change was measuring as a regression).
- **Dashboard no longer runs the spend-anomaly scan twice per render (2026-07-11):** the dashboard's top-highlight widget and its "Needs attention" ranking each ran `detectSpendingAnomalies` (four monthly per-category spend series over every transaction) independently, on every dashboard re-render. Both now go through a revision-keyed memo (`detectSpendingAnomaliesMemo`) that computes once per data revision + month and returns a fresh copy (safe for the caller that takes `&anomalies[0]`). Validated: dashboard renders correct (Needs-attention items intact), 0 console errors.
- **Smart insight input is memoized across the ~10 pages that show the strip (2026-07-11):** `buildSmartInput` copied nine dataset slices (including every transaction) on *every* render, and the Smart strip renders on the dashboard plus ~10 pages — so rapid navigation rebuilt the same input repeatedly. It's now memoized on the store revision + week start + a 1-minute time bucket (the rule engines read the input strictly read-only — verified — so one shared instance is safe). Removes redundant per-render allocation/GC across the strip pages, the dashboard, and the anomaly/digest drivers; behaviour is unchanged (all 21 interaction regressions green).
- **Wasm now loads gzip-compressed — ~4× smaller download (2026-07-11):** the deploy already shipped `main.wasm.gz` alongside the raw binary, but the loader never used it (GitHub Pages won't serve it with `Content-Encoding` on its own). The boot loader now fetches `main.wasm.gz` and decompresses it in a stream (`DecompressionStream`) before instantiating, falling back to the raw `main.wasm` if the browser lacks the API — so it can only make boot lighter, never break it. Real transfer drops from **~72 MB to ~15 MB**; measured cold-load score **39 → 50**, with time-to-interactive 7.0s→4.7s and boot blocking 7.8s→5.2s. The build also strips debug info (`-ldflags="-s -w" -trimpath`) and the service worker precaches the smaller `.gz` (offline cache shrinks with it). Note: the remaining cold-load cost is the browser *compiling* the 72 MB code section — bounded by binary size, not the loader.

### Added
- **E2E coverage for the merged import wizard (2026-07-12):** a new `import wizard` block in `interactions.spec.mjs` (replacing the old standalone `statement import` test): asserts Stage 1 is a Smart/Smart+ document-type picker with all four source tiles (Smart+ tiles carry the `.smartplus` accent) and only one Import button; that picking the Statement PDF tile reveals its form and "← Choose a different type" returns to the grid; that a deterministic statement-text Parse advances to review and the footer Save imports the rows (searchable after); and that the CSV tile imports pasted rows directly (lossless path). All four pass; full `interactions` + `smoke` suites stay green.
- **Per-page performance framework + v1.0.14 baseline ratings (2026-07-11):** a Lighthouse-style harness (`e2e/perf/`) that boots the real wasm app in Chromium and rates every route from the browser's own Performance timeline — no source instrumentation. Per page it measures a **warm SPA navigation** (route mount, main-thread blocking/TBT, layout shift/CLS, settle time, DOM weight) and scores each metric on Lighthouse's log-normal curve (p10→90, median→50), combined into a 0–100 page score + letter grade. A separate **cold-load** section scores the one-time wasm boot (FCP/LCP/TBT/transfer weight). Ratings are stored per version under `e2e/perf/results/` (`v<version>.json` machine data, `v<version>.md` detailed analysis, `v<version>.html` a self-contained visual scorecard via `report-html.mjs`, `index.json` history). **v1.0.14 baseline:** average page score **85/100** (46/46 routes rated); best `/plans` 100; worst `/` dashboard 46 (mount 771ms, blocking 970ms). Cold-load **39/100** — dominated by the **78 MB** transfer (wasm 74 MB) and ~7.8s of boot-time main-thread blocking; FCP 68ms and CLS ~0 are excellent. Run: `node perf/perf-audit.mjs` from `e2e/`.
...
### Changed
- **Read accessors are now revision-cached — big render-time speedup (2026-07-11):** every list accessor on `App` (`Accounts`, `Transactions`, `Categories`, `Members`, `Budgets`, `Goals`, `Tasks`, `Holdings`, `Rules`, `Documents`, `Recurring`) used to run a fresh SQL query **and** JSON-unmarshal every row on *each call* — and a single screen render calls them dozens of times (per-tile budget/goal/net-worth computations, per-row lookups). They now memoize their result keyed on the store's monotonic mutation revision, re-querying only when the data actually changes, while still returning a fresh top-level copy each call so existing callers that sort/filter in place are unaffected. No call site changed; it fixes all of them at once.
- **Derived surface views are memoized per render (2026-07-11):** the investments, debt, goals, and live-formula surfaces each aggregate the whole ledger, yet a single render calls that aggregation once *per tile* (investments has ~6). `computeInvestView`, `computeDebtView`, `computeGoalView`, and `liveEngineVars` now memoize on the store revision (plus the inputs that live outside the store — the active member, the debt payoff config, the current month), so a multi-tile render computes each view once instead of 2–6×. A shared `memoByRev` helper keeps the caching uniform.

### Fixed
- **Regression suite realigned with two shipped UI changes (2026-07-11):** the account-add "Other → liability" interaction test clicked a submit button inside the form, but the add modal was standardized onto the FlipPanel FormID footer (the Save button now lives in the pinned footer and submits the form by id); and the budgets last-month toggle test asserted the old label "viewing last month" instead of the shipped "Showing last month's spend". Both tests now target the shipped UI. (The broader coverage-manifest ratchet is intentionally left for a separate regeneration — it also spans app-wide smart-strip/kebab/account-action controls and a rise in untestable controls that deserve testids first.)
- **Smart-bills setup fields no longer lag while typing (2026-07-11):** the "minimum balance to keep" and payday-anchor inputs re-ran the whole bill-payment optimizer and re-rendered the modal on *every keystroke*. Both commits are now debounced (~300ms) and flushed on blur/change — so typing an amount is smooth and the schedule recomputes once you pause (or immediately when you tab away or press the button). Same `debounce` package as the savings and transaction-filter fields.
- **Undo / import / sample-load now invalidate revision-keyed memoization (2026-07-11):** the bulk dataset-replace path (`SQLiteStore.Load`) swapped the entire dataset without advancing the store's mutation revision — unlike `Wipe`, which does. Any `Rev()`-keyed cache (the dashboard selectors, and now the read accessors) could therefore serve stale rows after an undo, import, or sample-load. `Load` now advances the revision on commit, so all derived/memoized views refresh correctly when the whole dataset changes underneath them.
- **All in-app links navigate client-side — no more reloads / passcode re-entry (2026-07-11):** a single document-level anchor interceptor at the router mount turns any same-origin route `<a href>` into a client-side router navigation, so no in-page link triggers a full page reload (which threw away wasm state and dropped the in-memory app-lock passcode, forcing a re-unlock). Fixes every page's route links at once — accounts, allocate, assistant, debt, investments, net worth, planning, etc. — while keeping plain anchors for accessibility. Modified clicks, `target=_blank`, downloads, external/`mailto:` links, `#fragments`, and handlers that already `preventDefault` are left to the browser.
...
### Changed
- **Standardized flip-modal footer buttons + sizes across the remaining modals (2026-07-11):** every remaining modal footer (transaction edit with its Delete, account/member editors, credential vault, invest add/pool, recurring, allocate, bills-smart, subscription prefs, txn-link) now uses the same footer buttons as the standard footer — the muted-green primary, the ghost secondary, a left-aligned danger Delete — and one of the three standard panel sizes (Small 440×440 / Medium 560×680). Recurring and investment-pool moved onto the fully-pinned standard footer (keeping their e2e testids via new `SaveTestID`/`CancelTestID`). (Footers on the simpler forms bottom-align and scroll with tall content; converting the complex multi-mode editors to a truly-pinned footer, plus staging the settings/config panels, is in progress.)
...
### Changed
- **Budgets "Last month" is now a spend overlay for planning (2026-07-10):** the button used to re-window the whole page to last month's budget picture. It now keeps the app on THIS month's budgets, but each budget tile **leads with what you actually spent last period** against this month's budget: a **LAST MONTH** tag over the tile's big progress bar, filled to last month's spend (e.g. "$694.80 / $205.20 · 338%", red when it blew this month's budget, accent when under), the over/under-budget gap beneath it, and this month kept as a small "This month: $X spent" reference line. The **summary spend graph reflects it too** — a LAST MONTH tag over last period's total spend vs this month's budget — so the whole page reads as last month at once. So the tiles visibly become last month's picture — easy to read at a glance — for setting this month's amounts against last month's reality. Renamed the button to **"Last month's spend"** with matching hover text.
