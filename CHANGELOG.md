...
### Changed
- **AI chat no longer rescans every transaction on each keystroke (2026-07-11):** the Insights/assistant chat page derived its starter questions from three full-dataset scans (top spend category over every transaction, nearest-limit budget evaluation, soonest goal) directly in the render body — and the page re-renders on every keystroke, send, and effect, so those scans ran per character typed. They're now memoized on the data revision + scope + month, so they compute once and reuse until the data actually changes. (Code-reasoned fix to per-render waste; the automated per-page score couldn't confirm it under heavy local machine load — a work-removing change was measuring as noise.)
