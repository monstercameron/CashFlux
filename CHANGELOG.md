...
### Added
- **Lock screen: a music mute toggle + a Smart+ AI "quote of the day" (2026-06-30):** Two lock-screen additions. (1) A **mute toggle** so the ambient music can be silenced (or resumed) from the lock screen without unlocking — it reads/sets the live player state (`window.cashfluxMuzak`), persists the choice, and hides itself when no music is configured (a deferred re-check covers the player still loading its tracks when the gate builds). (2) The bottom **verbiage now uses the Smart+ "quote of the day" engine** instead of only static text: when SMART-QUOTE is enabled and has produced a quote, the lock screen shows it, otherwise it falls back to the static day-rotating quote (`lockquotes`). The Smart settings + cached quote live in their own browserstore key (`cashflux:smart-settings`, separate from the encrypted dataset), so the AI quote is readable on the lock screen even while the dataset is locked. Verify: new `e2e/lockscreen_check.mjs` 5/5 — the lock quote shows the seeded Smart+ quote when enabled and the static one when disabled; the mute button is present and toggles the music off. SW cache v291→v292.
