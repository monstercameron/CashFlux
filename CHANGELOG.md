...
### Changed
- **To-do rows redesigned as cards + a first-class linked-goal chip (2026-07-01):** Tasks render as rounded **cards** (scoped to /todo so the shared list-row style elsewhere is untouched) with a **priority-tinted left stripe** (red high / accent medium / muted low), a **custom checkbox** (an empty rounded box that fills accent-green with a check when done), the title, and a wrap of pill **chips** — priority, due (red "overdue" / amber "due today" / neutral date), repeat, notes (truncated), and the **linked entity**. The linked-entity chip is now first-class: it carries a per-type icon, and a **linked goal is accent-tinted** (seagreen) so the goal↔to-do connection reads at a glance — the visible half of "link 1-n todos to a goal". Row actions (Edit + ⋯) fade in on hover. Verify: `e2e/todo_redesign_check.mjs` 9/9 — cards + priority stripe + is-goal chip (accent border rgb(46,139,87)) + checkbox toggles done + due-state chips, no page errors.
