...
- **E2E stories (B16).** Scripted user-journey tests, now that Playwright + Chromium are installed — each
  ...
  **transactions filter persistence** (filters the ledger to a unique transaction, confirms the list narrows
  ...
  and **member reassign-on-delete** (gives a member an account, deletes the member choosing a new owner, and
  confirms the account is reassigned with no orphan).
  The
  ...
  browser, exits non-zero on any failure): currently **19 green**.
