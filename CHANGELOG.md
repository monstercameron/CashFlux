...
### Fixed
- **/investments charts are now theme-aware — they follow the accent color (2026-07-02):** The portfolio-growth chart and every account/custom-chart card drew their line + area fill in a hardcoded seagreen (`#2e8b57`), so on any non-default accent (or a recolored theme) the charts stayed green while the rest of the page recolored. They now stroke in the live theme accent (`uistate.UsePrefs().Accent`, the same value that drives `--accent`), falling back to seagreen when no accent is set — so switching the accent to, say, periwinkle repaints all the growth charts to match. The D3 renderer paints the color into SVG stroke/fill attributes (where CSS `var()` can't resolve), so the accent is threaded through as a resolved value. No visual change on the default accent.
