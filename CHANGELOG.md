...
### Changed
- **Budgets "Last month" is now a spend overlay for planning (2026-07-10):** the button used to re-window the whole page to last month's budget picture. It now keeps you on THIS month and overlays each budget with a **slim filled reference bar** — a slimmer, muted echo of the budget bar above it — showing what you actually spent last period as a share of this month's budget, with the amount and the gap inside (e.g. "Last month $694.80 · $489.60 over"). The bar fills and reads **red when last month exceeded this month's budget** (a nudge to raise it) and **accent when it came in under**, so you can see at a glance whether you're tracking above or below last month. Renamed the button to **"Last month's spend"** with matching hover text. A planning aid — set this month's budgets against last month's reality — instead of a time-machine view.
