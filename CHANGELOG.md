...
### Changed
- **Add-task form redesigned from scratch (2026-07-01):** The old form-grid + dropdown-soup add-task modal was rebuilt as a focused "compose" panel that matches the agenda aesthetic. The **title is the hero** (a large underline-only field that lights seagreen on focus); **priority is a segmented control** whose dots echo the list's check-ring colours (faint low / seagreen medium / red high) instead of a dropdown; the due date gets **quick chips** (Today · In a week · Clear); Repeat and Link-to sit in a tidy two-column row; notes are a proper multi-line textarea; and a **sticky bottom bar** owns Cancel + a prominent **+ Add task**. The modal is now `NoFooter` (the form owns its actions) at 500×620. Verify: `e2e/todo_addform_check.mjs` 12/12 — hero field, 3-segment priority (Medium default, High activates on click), quick-date fill/clear, empty-title guard, add persists + closes, Cancel closes, no errors.
